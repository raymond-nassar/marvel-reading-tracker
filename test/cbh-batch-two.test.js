...
test('batch two preserves the approved queue, exact substitutions, and catalog chronology', async () => {
  ...
  assert.equal(existingEntriesForPacket(manifest.lists).length, 36);
  assert.equal(manifest.lists.length, 46);
  assert.equal(catalog.lists.length, 46);
...
test('batch two has no aggregate identity, source, sequence, or issue overlap', async () => {
  ...
  assert.equal(existingRecords.length, 36);
