...
test('batch three preserves the approved source queue and independently verified shelf chronology', async () => {
  ...
  assert.equal(existingEntriesForPacket(manifest.lists, THIRD_PACKET_IDS).length, 46);
  assert.equal(manifest.lists.length, 56);
  assert.equal(catalog.lists.length, 56);
...
test('batch three has no aggregate identity, source, sequence, or issue overlap', async () => {
  ...
  assert.equal(existingRecords.length, 46);
  ...
  for (const id of THIRD_PACKET_IDS) {
    ...
    assert.equal(report.comparisonCount, 55);
    assert.equal(report.comparisons.length, 55);
