...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 56, `the catalog lists ${orders.length} orders, not 56, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the fifty-six files the catalog names and
...
// the 291 files walked are code and cannot parse, so a file that stops parsing has to be skipped
...
// leaves the population with nothing said. A floor of fifty-six still passes at that point, because
// the catalog's own files alone clear it. An exact fifty-nine does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 59, `${scanned} item-bearing files were found, not 59, so this test's coverage has changed`);
