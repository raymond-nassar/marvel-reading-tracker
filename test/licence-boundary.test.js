...
test('no shipped reading order carries Marvel description prose', async () => {
  ...
  assert.equal(orders.length, 66, `the catalog lists ${orders.length} orders, not 66, so this test's coverage has changed`);
...
// The test above reads the catalog, so it sees exactly the sixty-six files the catalog names and
...
// the 322 files walked are code and cannot parse, so a file that stops parsing has to be skipped
...
// leaves the population with nothing said. A floor of sixty-six still passes at that point, because
// the catalog's own files alone clear it. An exact sixty-nine does not.
//
...
test('no file outside node_modules, .git and .copilot-tracking carries an items[].description string', async () => {
  ...
  assert.equal(scanned, 69, `${scanned} item-bearing files were found, not 69, so this test's coverage has changed`);
