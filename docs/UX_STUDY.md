...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2092-2097` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:142-157`, `src/js/lib/model.js:702-778` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:73-78`, `src/js/main.js:3381-3401`
...
Confidence: Observed
Evidence: `src/js/main.js:2870`, `src/js/main.js:1489-1497`, `src/js/main.js:1516-1532`, against
`src/js/main.js:357-378`
...
Confidence: Observed
Evidence: `src/js/main.js:1516-1532`, `src/js/main.js:3105-3111`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:322`, `src/js/main.js:2123-2146`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:626-633`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:404-410`, `src/js/main.js:2775-2783`.
...
`src/styles.css:21-29`, `src/index.html:322`
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:295-330`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:255-268`, `src/js/main.js:357-378`, `src/index.html:17`,
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:261-268`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:295-330`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:823-825`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/main.js:2027-2037`, `src/js/main.js:2092-2097`
...
`src/styles.css:525`, `src/js/main.js:1388-1391`.
...
Confidence: Observed
Evidence: `src/js/main.js:2123-2146`, `src/index.html:322`
...
src/ returning no matches`, `src/js/main.js:812-829`,
...
Confidence: Observed
Evidence: `src/js/main.js:86`, `src/js/lib/readingFilters.js:25-48`
Source: Step 5 information architecture review, addressability
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2031-2037` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:335-336`, `PRODUCT_BACKLOG.md:7172` | If sync existed and was opt-in, would you turn it on? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2730-2735`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2031-2037`.
* When I have read half of a long order across several sittings, I want to come back and see where
  ...
  `src/js/main.js:2952-2986`.
* When my browser storage is cleared or I move machines, I want my progress back from a file I
  control, so I can keep my history without an account. Traced to `src/js/lib/model.js:860-891`.
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:368-370`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:371-375`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:368-370` |
