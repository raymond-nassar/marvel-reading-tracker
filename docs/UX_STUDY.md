...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:84-89`, `src/js/main.js:4744-4764`
...
Confidence: Observed
Evidence: `src/js/main.js:4033`, `src/js/main.js:2148-2156`, `src/js/main.js:2175-2191`, against
...
Confidence: Observed
Evidence: `src/js/main.js:2175-2191`, `src/js/main.js:4394-4400`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:721-728`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:450-456`, `src/js/main.js:3938-3946`.
...
strikethrough, at `src/styles.css:653-654`. The only opacity left on a read row is on the cover
image at `src/styles.css:656`, which carries no text. Re-measured with six rows actually in the
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:725-726`, `docs/ux-artifacts/live-inspection.json`
...
`src/index.html:136`, `src/index.html:484`
...
`src/styles.css:593`, `src/js/main.js:2047-2050`.
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:439`, `src/styles.css:1119-1121`.
...
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:410-416`, `PRODUCT_BACKLOG.md:12024-12029` | If sync existed and was opt-in, would you turn it on? |
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:414-416`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:417-421`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:414-416` |
