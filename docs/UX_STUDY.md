...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2768-2773` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:82-87`, `src/js/main.js:4617-4637`
...
Confidence: Observed
Evidence: `src/js/main.js:3951`, `src/js/main.js:2075-2083`, `src/js/main.js:2102-2118`, against
...
Confidence: Observed
Evidence: `src/js/main.js:2102-2118`, `src/js/main.js:4267-4273`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:336`, `src/js/main.js:2799-2822`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:668-675`, covering Enter,
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:397`,
  `src/styles.css:973-975`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:418-424`, `src/js/main.js:3856-3864`.
...
`src/styles.css:21-29`, `src/index.html:336`
...
`src/styles.css:495`. The figures in the Evidence artifacts are the pre-fix measurements and are
...
strikethrough, at `src/styles.css:594-595`. The only opacity left on a read row is on the cover
image at `src/styles.css:597`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:309-344`
...
alpha at `src/styles.css:437-444`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:666-667`, `docs/ux-artifacts/live-inspection.json`
...
`src/index.html:136`, `src/index.html:452`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:309-344`
...
Confidence: Measured
Evidence: `src/styles.css:47`, `src/index.html:10`, `docs/ux-artifacts/live-inspection.json`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:1210-1212`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/main.js:2708-2713`, `src/js/main.js:2768-2773`
...
`src/styles.css:542`, `src/js/main.js:1974-1977`.
...
Confidence: Observed
Evidence: `src/js/main.js:2799-2822`, `src/index.html:336`
...
Confidence: Measured
Evidence: `src/styles.css:213-216`, `src/styles.css:220-224`,
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:397`, `src/styles.css:973-975`.
...
src/ returning no matches`, `src/js/main.js:1199-1216`,
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:309-344` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2712-2713` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:397-403`, `PRODUCT_BACKLOG.md:11523-11528` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:309-344` and `src/index.html:336`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:3776-3793`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2712-2713`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:382-384`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:385-389`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:382-384` |
