...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:438`,
  `src/styles.css:1071-1073`.
...
`src/styles.css:536`. The figures in the Evidence artifacts are the pre-fix measurements and are
...
strikethrough, at `src/styles.css:643-644`. The only opacity left on a read row is on the cover
image at `src/styles.css:646`, which carries no text. Re-measured with six rows actually in the
...
alpha at `src/styles.css:478-485`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:715-716`, `docs/ux-artifacts/live-inspection.json`
...
`src/styles.css:583`, `src/js/main.js:2047-2050`.
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:438`, `src/styles.css:1071-1073`.
...
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:393-394`, `PRODUCT_BACKLOG.md:11633` | If sync existed and was opt-in, would you turn it on? |
