...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:1642-1647` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:142-153`, `src/js/lib/model.js:466-492` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:142-153` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:65-70`, `src/js/main.js:2725-2740`
...
Confidence: Observed
Evidence: `src/js/main.js:2397`, `src/js/main.js:1223-1231`, `src/js/main.js:1233-1249`, against
`src/js/main.js:327-348`
...
Confidence: Observed
Evidence: `src/js/main.js:1233-1249`, `src/js/main.js:2609-2615`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:311`, `src/js/main.js:1657-1680`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:595-602`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:393-399`, `src/js/main.js:2302-2310`.
...
`src/styles.css:21-29`, `src/index.html:311`
...
strikethrough, at `src/styles.css:547-548`. The only opacity left on a read row is on the cover
image at `src/styles.css:550`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:284-319`
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:601-602`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:225-238`, `src/js/main.js:327-348`, `src/index.html:17`,
`src/index.html:132`, `src/index.html:427`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:231-238`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:284-319`
...
Confidence: Observed
Evidence: `src/js/main.js:1602-1612`, `src/js/main.js:1642-1647`
...
`src/styles.css:525`, `src/js/main.js:1185-1188`.
...
Confidence: Observed
Evidence: `src/js/main.js:1657-1680`, `src/index.html:311`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:380`, `src/styles.css:849-851`.
...
src/ returning no matches`, `src/js/main.js:670-698`,
...
Confidence: Observed
Evidence: `src/js/main.js:78`, `src/js/lib/readingFilters.js:25-48`
Source: Step 5 information architecture review, addressability
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:284-319` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:1606-1612` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:247-248`, `PRODUCT_BACKLOG.md:2892-2895` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:284-319` and `src/index.html:311`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2257-2262`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:1606-1612`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:357-359`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:360-364`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:357-359` |
