...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2871-2876` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:154-169`, `src/js/lib/model.js:888-964` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:154-169` |
| 10 | Help and documentation | 2 | An About view and a thorough README, but no in-app reference for shortcuts or filters. `src/index.html:42-61` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:84-89`, `src/js/main.js:4792-4812`
...
Confidence: Observed
Evidence: `src/js/main.js:4067`, `src/js/main.js:2161-2169`, `src/js/main.js:2188-2204`, against
`src/js/main.js:497-518`
...
Confidence: Observed
Evidence: `src/js/main.js:2188-2204`, `src/js/main.js:4442-4448`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:386`, `src/js/main.js:2902-2925`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:768-779`, covering Enter,
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:482`,
  `src/styles.css:1278-1280`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:469-475`, `src/js/main.js:3972-3980`.
...
`src/styles.css:104-110`, `src/index.html:386`
...
`src/styles.css:585`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:701-702`. The only opacity left on a read row is on the cover
image at `src/styles.css:704`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:359-394`
...
alpha at `src/styles.css:527-534`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:773-774`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:346-359`, `src/js/main.js:497-518`, `src/index.html:21`,
`src/index.html:144`, `src/index.html:503`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:352-359`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:359-394`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:1225-1227`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/main.js:2811-2816`, `src/js/main.js:2871-2876`
...
Confidence: Observed
Evidence: `design/mockups/5-longbox-focus.html:169-172`, `src/index.html:42-46`
...
`src/styles.css:641`, `src/js/main.js:2060-2063`.
...
Confidence: Observed
Evidence: `src/js/main.js:2902-2925`, `src/index.html:386`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:482`, `src/styles.css:1278-1280`.
...
src/ returning no matches`, `src/js/main.js:1214-1231`,
...
Confidence: Observed
Evidence: `src/js/main.js:115`, `src/js/lib/readingFilters.js:25-50`
Source: Step 5 information architecture review, addressability
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:359-394` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2815-2816` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:423-427`, `PRODUCT_BACKLOG.md:12409` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:359-394` and `src/index.html:386`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:3888-3905`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2815-2816`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:433-435`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:436-440`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:433-435` |
