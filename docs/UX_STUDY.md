...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2841-2846` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:146-161`, `src/js/lib/model.js:888-964` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:84-89`, `src/js/main.js:4690-4710`
...
Confidence: Observed
Evidence: `src/js/main.js:4024`, `src/js/main.js:2148-2156`, `src/js/main.js:2175-2191`, against
...
Backlog item: BL-034
Resolved: naming and confirmation moved into one `<dialog>` at `src/js/ask.js:51-93`, and the
...
Confidence: Observed
Evidence: `src/js/main.js:2175-2191`, `src/js/main.js:4340-4346`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:367`, `src/js/main.js:2872-2895`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:699-706`, covering Enter,
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:439`,
  `src/styles.css:1072-1074`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:449-455`, `src/js/main.js:3929-3937`.
...
`src/styles.css:76-82`, `src/index.html:367`
...
`--red-text` for red used as text, at `src/styles.css:76-82`, and the `kbd` tint was removed at
`src/styles.css:537`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:644-645`. The only opacity left on a read row is on the cover
image at `src/styles.css:647`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:340-375`
...
alpha at `src/styles.css:479-486`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:716-717`, `docs/ux-artifacts/live-inspection.json`
...
`src/index.html:136`, `src/index.html:483`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:340-375`
...
Confidence: Measured
Evidence: `src/styles.css:55`, `src/index.html:10`, `docs/ux-artifacts/live-inspection.json`
...
`showView` writes it only to `.ri[data-view]` at `src/js/main.js:1212-1214`, and no rail item declares
...
Confidence: Observed
Evidence: `src/js/main.js:2781-2786`, `src/js/main.js:2841-2846`
...
`src/styles.css:584`, `src/js/main.js:2047-2050`.
...
Confidence: Observed
Evidence: `src/js/main.js:2872-2895`, `src/index.html:367`
...
Confidence: Measured
Evidence: `src/styles.css:229-232`, `src/styles.css:236-240`,
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:439`, `src/styles.css:1072-1074`.
...
src/ returning no matches`, `src/js/main.js:1201-1218`,
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:340-375` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2785-2786` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:394-395`, `PRODUCT_BACKLOG.md:11685-11690` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:340-375` and `src/index.html:367`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:3849-3866`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2785-2786`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:413-415`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:416-420`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:413-415` |
