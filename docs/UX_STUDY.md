...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:395`,
  `src/styles.css:950-952`.
...
`src/styles.css:493`. The figures in the Evidence artifacts are the pre-fix measurements and are
...
strikethrough, at `src/styles.css:592-593`. The only opacity left on a read row is on the cover
image at `src/styles.css:595`, which carries no text. Re-measured with six rows actually in the
...
alpha at `src/styles.css:435-442`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:664-665`, `docs/ux-artifacts/live-inspection.json`
...
`src/styles.css:540`, `src/js/main.js:1735-1738`.
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:395`, `src/styles.css:950-952`.
...
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:376-377`, `PRODUCT_BACKLOG.md:10655` | If sync existed and was opt-in, would you turn it on? |
