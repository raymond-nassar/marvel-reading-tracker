...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:1229-1234` |
| 3 | User control and freedom | 3 | Restoring a backup can be undone, deleting a list cannot. UX-H-003 |
| 4 | Consistency and standards | 3 | Two different error and prompt systems run side by side. UX-H-002 |
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:122-133`, `src/js/lib/model.js:376-402` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:122-133` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:61-66`, `src/js/main.js:2199-2212`
...
Confidence: Observed
Evidence: `src/js/main.js:1981`, `src/js/main.js:927-935`, `src/js/main.js:937-949`, against
`src/js/main.js:188-197`
...
Confidence: Observed
Evidence: `src/js/main.js:937-949`, `src/js/main.js:2088-2091`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:291`, `src/js/main.js:1256-1274`
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:351-357`, `src/js/main.js:1891-1899`.
...
`src/styles.css:20-29`, `src/index.html:291`
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:264-299`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:150-163`, `src/js/main.js:188-197`, `src/index.html:13`,
`src/index.html:120`, `src/index.html:385`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:156-163`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:264-299`
...
Confidence: Observed
Evidence: `src/js/main.js:1197-1207`, `src/js/main.js:1229-1234`
...
`src/styles.css:393`, `src/index.html:324-328`.
...
Confidence: Observed
Evidence: `src/js/main.js:1256-1274`, `src/index.html:291`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:248`, `src/styles.css:695-697`.
...
src/ returning no matches`, `src/js/main.js:463-490`,
...
Confidence: Observed
Evidence: `src/js/main.js:69`, `src/index.html:324-328`
Source: Step 5 information architecture review, addressability
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:264-299` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:1201-1207` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:213-214`, `PRODUCT_BACKLOG.md:1168-1171` | If sync existed and was opt-in, would you turn it on? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:1846-1851`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:1201-1207`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:338-340`.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:338-340` |
