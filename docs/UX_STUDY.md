...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:1654-1659` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:138-149`, `src/js/lib/model.js:519-545` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:138-149` |
| 10 | Help and documentation | 2 | An About view and a thorough README, but no in-app reference for shortcuts or filters. `src/index.html:34-52` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:64-69`, `src/js/main.js:2743-2758`
...
Confidence: Observed
Evidence: `src/js/main.js:2416`, `src/js/main.js:1200-1208`, `src/js/main.js:1210-1226`, against
`src/js/main.js:326-347`
...
Backlog item: BL-034
Resolved: naming and confirmation moved into one `<dialog>` at `src/js/ask.js:48-80`, and the
...
Confidence: Observed
Evidence: `src/js/main.js:1210-1226`, `src/js/main.js:2627-2633`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:307`, `src/js/main.js:1669-1692`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:580-587`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:389-395`, `src/js/main.js:2321-2329`.
...
`src/styles.css:20-29`, `src/index.html:307`
...
`--red-text` for red used as text, at `src/styles.css:20-29`, and the `kbd` tint was removed at
...
strikethrough, at `src/styles.css:428-429`. The only opacity left on a read row is on the cover
image at `src/styles.css:431`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:280-315`
...
alpha at `src/styles.css:288-295`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:482-489`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:224-237`, `src/js/main.js:326-347`, `src/index.html:13`,
`src/index.html:128`, `src/index.html:423`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:230-237`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:280-315`
...
Confidence: Measured
Evidence: `src/styles.css:7`, `src/index.html:6`, `docs/ux-artifacts/live-inspection.json`
...
Confidence: Observed
Evidence: `src/js/main.js:1614-1624`, `src/js/main.js:1654-1659`
...
Confidence: Observed
Evidence: `design/mockups/5-longbox-focus.html:169-172`, `src/index.html:34-37`
...
`src/styles.css:393`, `src/js/main.js:1162-1165`.
...
Confidence: Observed
Evidence: `src/js/main.js:1669-1692`, `src/index.html:307`
...
Confidence: Measured
Evidence: `src/styles.css:87-90`, `src/styles.css:94-98`,
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:248`, `src/styles.css:733-735`.
...
src/ returning no matches`, `src/js/main.js:632-660`,
...
Confidence: Observed
Evidence: `src/js/main.js:77`, `src/js/lib/readingFilters.js:25-48`
Source: Step 5 information architecture review, addressability
...
half is still open and belongs to UX-I-001.
...
| Reading style | Follows a long curated order end to end rather than dipping in | Strong. The product is built around order, resume and next-unread rather than around browsing. `src/index.html:280-315` | When a list is abandoned partway, what caused it? |
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:1618-1624` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:247-248`, `PRODUCT_BACKLOG.md:2854-2857` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:280-315` and `src/index.html:307`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2276-2281`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:1618-1624`.
* When I have read half of a long order across several sittings, I want to come back and see where
  ...
  `src/js/main.js:2498-2532`.
* When my browser storage is cleared or I move machines, I want my progress back from a file I
  control, so I can keep my history without an account. Traced to `src/js/lib/model.js:602-626`.
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:353-355`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:356-360`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:353-355` |
