...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:606-611` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:66-77`, `src/js/lib/model.js:358-384` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:66-77` |
| 10 | Help and documentation | 2 | An About view and a thorough README, but no in-app reference for shortcuts or filters. `src/index.html:36-50` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:41-46`, `src/js/main.js:1551-1566`
...
Confidence: Observed
Evidence: `src/js/main.js:1345`, `src/js/main.js:328`, `src/js/main.js:334-341`, against
`src/js/main.js:138-143`
...
Confidence: Observed
Evidence: `src/js/main.js:334-341`, `src/js/main.js:1448-1451`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:147`, `src/js/main.js:634-650`
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:207-213`, `src/js/main.js:1258-1262`.
...
`src/styles.css:20-29`, `src/index.html:147`
...
`src/styles.css:247`. The figures in the Evidence artifacts are the pre-fix measurements and are
...
strikethrough, at `src/styles.css:316-317`. The only opacity left on a read row is on the cover
image at `src/styles.css:319`, which carries no text. Re-measured with six rows actually in the
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:125-155`
...
alpha at `src/styles.css:189-196`. Sampling the rendered background across all eight catalog
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:138-143`, `src/index.html:13`, `src/index.html:64`,
`src/index.html:235`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:125-155`
...
Confidence: Observed
Evidence: `src/js/main.js:579`, `src/js/main.js:606-611`
...
Confidence: Observed
Evidence: `design/mockups/5-longbox-focus.html:169-172`, `src/index.html:36-38`
...
Confidence: Observed
Evidence: `src/js/main.js:634-650`, `src/index.html:147`
...
src/ returning no matches`, `src/js/main.js:266-286`,
...
Confidence: Observed
Evidence: `src/js/main.js:50`, `src/index.html:180-184`
Source: Step 5 information architecture review, addressability
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:581-584` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:683-686` | If sync existed and was opt-in, would you turn it on? |
...
* When I finish an issue in the Marvel Unlimited reader, I want to mark it read and be shown the
  ...
  `src/index.html:125-155` and `src/index.html:147`.
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:1213-1218`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:581-584`.
...
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:194-196`.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:194-196` |
