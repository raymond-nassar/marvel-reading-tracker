...
strikethrough, at `src/styles.css:649-650`. The only opacity left on a read row is on the cover
image at `src/styles.css:652`, which carries no text. Re-measured with six rows actually in the
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:721-722`, `docs/ux-artifacts/live-inspection.json`
...
`src/styles.css:589`, `src/js/main.js:2047-2050`.
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:439`, `src/styles.css:1077-1079`.
...
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:395-396`, `PRODUCT_BACKLOG.md:11736` | If sync existed and was opt-in, would you turn it on? |
