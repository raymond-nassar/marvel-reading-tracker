...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:2867-2872` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:84-89`, `src/js/main.js:4788-4808`
...
Confidence: Observed
Evidence: `src/js/main.js:4063`, `src/js/main.js:2161-2169`, `src/js/main.js:2188-2204`, against
...
Confidence: Observed
Evidence: `src/js/main.js:2188-2204`, `src/js/main.js:4438-4444`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:386`, `src/js/main.js:2898-2921`
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:473`,
  `src/styles.css:1269-1271`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:469-475`, `src/js/main.js:3968-3976`.
...
`src/styles.css:576`. BL-166 has since renamed both to `--accent` and `--accent-text` and repainted
...
strikethrough, at `src/styles.css:692-693`. The only opacity left on a read row is on the cover
image at `src/styles.css:695`, which carries no text. Re-measured with six rows actually in the
...
alpha at `src/styles.css:518-525`. Sampling the rendered background across all eight catalog
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:764-765`, `docs/ux-artifacts/live-inspection.json`
...
Confidence: Observed
Evidence: `src/js/main.js:2807-2812`, `src/js/main.js:2867-2872`
...
`src/styles.css:632`, `src/js/main.js:2060-2063`.
...
Confidence: Observed
Evidence: `src/js/main.js:2898-2921`, `src/index.html:386`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:473`, `src/styles.css:1269-1271`.
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:2811-2812` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:422-426`, `PRODUCT_BACKLOG.md:12365` | If sync existed and was opt-in, would you turn it on? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:3884-3901`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:2811-2812`.
