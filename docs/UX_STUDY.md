...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:615-620` |
...
* 2.2.2 Pause, Stop, Hide, not applicable, because the only moving element is a progress ring
  transition that already respects reduced motion. Evidence: `src/styles.css:149`,
  `src/styles.css:448-450`.
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:207-213`, `src/js/main.js:1267-1275`.
...
life of the product
Confidence: Measured
Evidence: `src/styles.css:369-376`, `docs/ux-artifacts/live-inspection.json`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:137-149`, `src/index.html:13`, `src/index.html:64`,
`src/index.html:235`
...
Confidence: Observed
Evidence: `src/js/main.js:588`, `src/js/main.js:615-620`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:149`, `src/styles.css:448-450`.
...
src/ returning no matches`, `src/js/main.js:275-295`,
...
Confidence: Observed
Evidence: `src/js/main.js:51`, `src/index.html:180-184`
Source: Step 5 information architecture review, addressability
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:590-593` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:213-214`, `PRODUCT_BACKLOG.md:1080-1083` | If sync existed and was opt-in, would you turn it on? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:1222-1227`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:590-593`.
