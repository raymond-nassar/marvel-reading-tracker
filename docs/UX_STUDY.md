...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:1604-1609` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:138-149`, `src/js/lib/model.js:466-492` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:64-69`, `src/js/main.js:2686-2701`
...
Confidence: Observed
Evidence: `src/js/main.js:2359`, `src/js/main.js:1185-1193`, `src/js/main.js:1195-1211`, against
...
Confidence: Observed
Evidence: `src/js/main.js:1195-1211`, `src/js/main.js:2570-2576`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:307`, `src/js/main.js:1619-1642`
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:389-395`, `src/js/main.js:2264-2272`.
...
strikethrough, at `src/styles.css:415-416`. The only opacity left on a read row is on the cover
image at `src/styles.css:418`, which carries no text. Re-measured with six rows actually in the
...
Confidence: Observed
Evidence: `src/js/main.js:1564-1574`, `src/js/main.js:1604-1609`
...
`src/styles.css:393`, `src/js/main.js:1147-1150`.
...
Confidence: Observed
Evidence: `src/js/main.js:1619-1642`, `src/index.html:307`
...
`docs/ux-artifacts/live-inspection.json`, `src/styles.css:248`, `src/styles.css:717-719`.
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:1568-1574` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:246-247`, `PRODUCT_BACKLOG.md:2776-2779` | If sync existed and was opt-in, would you turn it on? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2219-2224`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:1568-1574`.
* When I have read half of a long order across several sittings, I want to come back and see where
  ...
  `src/js/main.js:2441-2475`.
* When my browser storage is cleared or I move machines, I want my progress back from a file I
  control, so I can keep my history without an account. Traced to `src/js/lib/model.js:543-567`.
