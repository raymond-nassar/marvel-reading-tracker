...
| 2 | Match between the system and the real world | 1 | Strong. Labels are written in reader language, and availability wording hedges rather than promises. `src/js/main.js:1537-1542` |
...
| 5 | Error prevention | 1 | Strong. Unreadable saved data pauses writing rather than overwriting, and a future schema is refused. `src/index.html:130-141`, `src/js/lib/model.js:409-435` |
...
| 9 | Help users recognize, diagnose, and recover from errors | 1 | Strong where it matters most. The unreadable-data banner explains the cause, preserves the original, and offers a salvage download. `src/index.html:130-141` |
...
Confidence: Measured
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:63-68`, `src/js/main.js:2569-2582`
...
Confidence: Observed
Evidence: `src/js/main.js:2292`, `src/js/main.js:1118-1126`, `src/js/main.js:1128-1144`, against
`src/js/main.js:303-324`
...
Confidence: Observed
Evidence: `src/js/main.js:1128-1144`, `src/js/main.js:2453-2459`
...
blocked one
Confidence: Observed
Evidence: `src/index.html:299`, `src/js/main.js:1552-1575`
...
Backlog item: BL-026
Resolved: BL-026 added the reference to the About view at `src/index.html:554-561`, covering Enter,
...
* 3.2.2 On Input passes, overturning a tool result. HTML_CodeSniffer flagged `#form-catalog-search`
  ...
  so the criterion is met. Evidence: `src/index.html:363-369`, `src/js/main.js:2197-2205`.
...
`src/styles.css:20-29`, `src/index.html:299`
...
`docs/ux-artifacts/pa11y-reading-seeded.json`, `src/index.html:272-307`
...
blocking
Confidence: Observed
Evidence: `src/js/main.js:201-214`, `src/js/main.js:303-324`, `src/index.html:13`,
`src/index.html:120`, `src/index.html:397`
...
Backlog item: BL-027
Resolved: which channel a message uses is now read off the container at `src/js/main.js:207-214`
...
imported
Confidence: Measured
Evidence: `docs/ux-artifacts/pa11y-landing.json`, `src/index.html:272-307`
...
Confidence: Observed
Evidence: `src/js/main.js:1497-1507`, `src/js/main.js:1537-1542`
...
`src/styles.css:393`, `src/js/lib/readingFilters.js:25-48`.
...
Confidence: Observed
Evidence: `src/js/main.js:1552-1575`, `src/index.html:299`
...
src/ returning no matches`, `src/js/main.js:604-632`,
...
Confidence: Observed
Evidence: `src/js/main.js:76`, `src/js/lib/readingFilters.js:25-48`
Source: Step 5 information architecture review, addressability
...
| Tolerance for missing metadata | High, provided the app admits what it does not know | Strong. Pending and by-hand states are surfaced rather than hidden, and this was a deliberate decision. `src/js/main.js:1501-1507` | Would you rather see a guess or a clearly marked gap? |
| Attitude to cloud services | Actively prefers local-only and treats that as the point | Strong. Recorded as a product constraint and stated in the backlog's own out-of-scope list. `PRODUCT_BACKLOG.md:239-240`, `PRODUCT_BACKLOG.md:2109-2112` | If sync existed and was opt-in, would you turn it on? |
...
* When I open a crossover I have never read, I want to know how much reading I am committing to
  ...
  `src/js/main.js:2152-2157`.
* When an issue has no metadata yet, I want the app to say so plainly, so I can tell a pending
  lookup apart from a comic that does not exist. Traced to `src/js/main.js:1501-1507`.
* When I have read half of a long order across several sittings, I want to come back and see where
  ...
  `src/js/main.js:2374-2408`.
* When my browser storage is cleared or I move machines, I want my progress back from a file I
  control, so I can keep my history without an account. Traced to `src/js/lib/model.js:486-510`.
* When I follow one crossover, I want its progress counted for that list alone, so I can see how
  ...
  Hypothesis, and the gap behind existing story 4.2. Traced to `src/index.html:345-347`.
  Resolved: BL-014 scoped the count to the active list and put the choice in the view at
  `src/index.html:348-352`, with the subtitle naming whichever of the two is being counted.
...
| Checking progress | Opens Progress by series to see how far through the crossover they are | Confused. Counts include every other list they ever imported | Existing story 4.2, `src/index.html:345-347` |
