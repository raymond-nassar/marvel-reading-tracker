...
without a browser runtime dependency. The first production batch publishes ten normal catalog
orders containing 238 distinct issues. The inventory, mappings, reports, and preparation scripts do
not reach the browser; only the authored checklists and vendored order data do.

Twelve candidates were blocked on missing source data, missing issue metadata, an open range, or
unapproved overlap before the ten authorable guides were reached in source order. `sourceLicense`
stays `null` for every published order, and each card records the permission boundary in
`sourceOrigin` as `Compiled for this project from Comic Book Herald's guide` and links to the exact
guide followed.
...
Thirty-six files under [`src/data/`](../src/data), one per curated list, holding 2,889 issue records
covering 2,281 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the thirty-six, 2,820 records
carry a cover URL and 2,762 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 22 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The thirty-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Eighteen are generated by committed scripts: eight from series metadata and ten from
independently reviewed source mappings. Sixteen are compiled by hand. Every file carries its own
derivation trail. Twenty-five reference a Comic Book Herald guide, and twenty-two name it on their
catalog card and link to the exact page followed.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all thirty-six today |
...
The consequence is already committed. 63 of the 2,889 curated items across the thirty-six orders hold
...
BL-141 and the ten added in BL-181 added none, because every issue in them was on sale before the
portal closed.
...
Nothing already saved is affected. The tree holds 2,241 distinct cover URLs across 2,820 records; 60
...
when the selection was not made here. Twenty-two of the thirty-six shipped lists name Comic Book Herald
...
Marvel origin and its actual route at `src/index.html:279`. Redistributing Content outside an App, and sublicensing it onward, are the
...
interface in exactly one place, at `src/js/main.js:2446`, and the function behind it already
answers for the absence at `src/js/main.js:2630-2639`, with a test asserting the sentence it
