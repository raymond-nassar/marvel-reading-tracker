...
without a browser runtime dependency. Its first three production batches publish thirty normal catalog
orders containing 616 distinct issues. The inventory, mappings, reports, and preparation scripts do
...
Fifty-six files under [`src/data/`](../src/data), one per curated list, holding 3,267 issue records
covering 2,659 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the fifty-six, 3,198 records
carry a cover URL and 3,133 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 42 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The fifty-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Thirty-eight are generated by committed scripts: eight from series metadata and thirty
from reviewed source mappings. Sixteen are compiled by hand. Every file carries its own derivation
trail. Forty-five reference a Comic Book Herald guide, and forty-two name it on their catalog card
and link to the exact page or section followed.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all fifty-six today |
...
The consequence is already committed. 63 of the 3,267 curated items across the fifty-six orders hold
...
BL-141 and the ten each added in BL-181, BL-182 and BL-185 added none, because every issue in them
was on sale before the portal closed.
...
Nothing already saved is affected. The tree holds 2,619 distinct cover URLs across 3,198 records; 60
...
when the selection was not made here. Forty-two of the fifty-six shipped lists name Comic Book Herald
