...
Sixty-six files under [`src/data/`](../src/data), one per curated list, holding 3,395 issue records
covering 2,787 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the sixty-six, 3,326 records
carry a cover URL and 3,261 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 52 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The sixty-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Forty-eight are generated by committed scripts: eight from series metadata and forty
from reviewed source mappings. Sixteen are compiled by hand. Every file carries its own derivation
trail. Fifty-five reference a Comic Book Herald guide, and fifty-two name it on their catalog card
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all sixty-six today |
...
The consequence is already committed. 63 of the 3,395 curated items across the sixty-six orders hold
...
BL-141 and the ten each added in BL-181, BL-182, BL-185 and BL-186 added none, because every issue in them
...
when the selection was not made here. Fifty-two of the sixty-six shipped lists name Comic Book Herald
...
Marvel origin and its actual route at `src/index.html:287`. Redistributing Content outside an App, and sublicensing it onward, are the
...
interface in exactly one place, at `src/js/main.js:2468`, and the function behind it already
answers for the absence at `src/js/main.js:2656-2665`, with a test asserting the sentence it
