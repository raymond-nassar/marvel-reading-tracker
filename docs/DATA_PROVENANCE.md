...
Twenty-six files under [`src/data/`](../src/data), one per curated list, holding 2,651 issue records
covering 2,043 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the twenty-six, 2,582 records
carry a cover URL and 2,532 carry creator credits.
...
| Compiled for this project | 16 | The whole sequence, by hand. See the trail at the top of each file in [`src/data/orders/`](../src/data/orders) |
...
The twenty-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Eight are generated by a committed script from series metadata, and each says so in
its own first paragraph. Sixteen are compiled by hand, and since BL-099 each carries a trail
recording how it was derived, including the cases where an outside guide was used as a
reference: fifteen of the sixteen follow a Comic Book Herald guide, and twelve of those name it
on their catalog card and link to the page they followed.
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all twenty-six today |
...
The consequence is already committed. 63 of the 2,651 curated items across the twenty-six orders hold
...
BL-141 added none, because every issue in them was on sale before the portal closed.
...
Nothing already saved is affected. The tree holds 2,003 distinct cover URLs across 2,582 records; 60
