...
without a browser runtime dependency. Its first two production batches publish twenty normal catalog
orders containing 416 distinct issues. The inventory, mappings, reports, and preparation scripts do
...
Forty-six files under [`src/data/`](../src/data), one per curated list, holding 3,067 issue records
covering 2,459 distinct issues. Each record copies from the upstream API: `issueId`, `title`,
`number`, `url`, `seriesId`, `seriesName`, `onSale`, `mu`, `digitalId`, `pageCount`, a `cover`
object of `path` and `ext`, and `creators` of `name` and `role`. Across the forty-six, 2,998 records
carry a cover URL and 2,938 carry creator credits.
...
| Compiled for this project from Comic Book Herald's guide | 32 | The guide's issue selection and sequence, re-expressed as local checklists. Every card links to the exact guide followed |
...
The forty-four Markdown files in [`src/data/orders/`](../src/data/orders) are authored in this
repository. Twenty-eight are generated by committed scripts: eight from series metadata and twenty
from reviewed source mappings. Sixteen are compiled by hand. Every file carries its own derivation
trail. Thirty-five reference a Comic Book Herald guide, and thirty-two name it on their catalog card
...
| `sourceLicense` | An SPDX expression, or `null`. Only a licence actually conveyed with the vendored order. `null` on all forty-six today |
...
The consequence is already committed. 63 of the 3,067 curated items across the forty-six orders hold
...
BL-141 and the ten each added in BL-181 and BL-182 added none, because every issue in them was on
sale before the portal closed.
...
Nothing already saved is affected. The tree holds 2,419 distinct cover URLs across 2,998 records; 60
...
when the selection was not made here. Thirty-two of the forty-six shipped lists name Comic Book Herald
