...
Those five are built together at `src/js/main.js:66-80`. Read that block and you have read the
...
`src/js/main.js:3355-3357`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2151-2154` hands the store a function; the function itself, at
`src/js/lib/model.js:628-630`, returns a new state and touches nothing. Everything that decides
...
announcement can be gated on the outcome: `src/js/main.js:297-299` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:3669-3689`, but the reading
...
by identity afterwards, at `src/js/main.js:2050`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:1956-1964`.
...
`src/js/main.js:38-39`, and the response cache is not in `localStorage` at all.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:506` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:663` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
