...
itself is loaded at `src/index.html:764`. The launch page, which is the tab a reader's issue opens
...
Those five are built together at `src/js/main.js:67-81`. Read that block and you have read the
...
`src/js/main.js:3714-3716`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2385-2388` hands the store a function; the function itself, at
`src/js/lib/model.js:652-654`, returns a new state and touches nothing. Everything that decides
...
announcement can be gated on the outcome: `src/js/main.js:303-305` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4041-4061`, but the reading
...
by identity afterwards, at `src/js/main.js:2284`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2172-2180`.
...
`src/js/main.js:39-40`, and the response cache is not in `localStorage` at all.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:539` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:729` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
