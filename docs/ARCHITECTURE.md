...
itself is loaded at `src/index.html:802`. The launch page, which is the tab a reader's issue opens
...
`src/js/main.js:4290-4292`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2694-2697` hands the store a function; the function itself, at
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4617-4637`, but the reading
...
by identity afterwards, at `src/js/main.js:2593`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2481-2489`.
