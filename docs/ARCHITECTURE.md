...
`src/js/main.js:3128-3130`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/lib/model.js:436-438`, returns a new state and touches nothing. Everything that decides
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:3381-3401`, but the reading
...
by identity afterwards, at `src/js/main.js:1912`, which is what keeps the keyboard where the reader
