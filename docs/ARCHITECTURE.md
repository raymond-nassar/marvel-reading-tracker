...
itself is loaded at `src/index.html:774`. The launch page, which is the tab a reader's issue opens
...
Those five are built together at `src/js/main.js:68-82`. Read that block and you have read the
...
`src/js/main.js:3834-3836`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2430-2433` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:304-306` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4161-4181`, but the reading
...
by identity afterwards, at `src/js/main.js:2329`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2217-2225`.
...
`src/js/main.js:40-41`, and the response cache is not in `localStorage` at all.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:540` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:730` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
