...
`src/js/main.js:4465-4467`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2797-2800` hands the store a function; the function itself, at
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4792-4812`, but the reading
...
by identity afterwards, at `src/js/main.js:2696`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2584-2592`.
