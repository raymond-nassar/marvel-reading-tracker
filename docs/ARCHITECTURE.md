...
`src/js/main.js:3092-3094`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:1986-1989` hands the store a function; the function itself, at
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:3336-3356`, but the reading
...
by identity afterwards, at `src/js/main.js:1885`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:1859-1867`.
...
BL-042 proposes breaking the 3,442 line view file into per-view modules. A diagram drawn at the
