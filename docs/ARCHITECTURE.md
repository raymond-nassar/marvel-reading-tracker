...
itself is loaded at `src/index.html:909`. The launch page, which is the tab a reader's issue opens
...
`src/js/main.js:4461-4463`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2793-2796` hands the store a function; the function itself, at
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4788-4808`, but the reading
...
by identity afterwards, at `src/js/main.js:2692`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2580-2588`.
