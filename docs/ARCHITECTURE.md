...
`src/js/main.js:3099-3101`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:1993-1996` hands the store a function; the function itself, at
...
by identity afterwards, at `src/js/main.js:1892`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:1866-1874`.
...
next section. Restoring is the one that writes the key directly, at `src/js/storage.js:351-408`,
which also puts it past the latch a failed read sets; the comment above the step that adopts a
restored state, at `src/js/storage.js:466-474`, says that is deliberate, because a restore is a
chosen overwrite.
