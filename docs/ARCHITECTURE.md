...
`src/js/main.js:3758-3760`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2425-2428` hands the store a function; the function itself, at
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4085-4105`, but the reading
...
by identity afterwards, at `src/js/main.js:2324`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2212-2220`.
