...
`src/js/main.js:3204-3206`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2074-2077` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:265-267` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:3468-3488`, but the reading
...
by identity afterwards, at `src/js/main.js:1973`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:1921-1929`.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:474` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:631` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
