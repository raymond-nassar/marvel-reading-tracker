...
itself is loaded at `src/index.html:855`. The launch page, which is the tab a reader's issue opens
...
Those five are built together at `src/js/main.js:76-90`. Read that block and you have read the
...
`src/js/main.js:4421-4423`. The hydrator itself is not rebuilt; only its reference to the client is
...
`src/js/main.js:2767-2770` hands the store a function; the function itself, at
...
announcement can be gated on the outcome: `src/js/main.js:314-316` speaks only if the write
...
surfaces, the six screens plus the blocked banner, at `src/js/main.js:4748-4768`, but the reading
...
by identity afterwards, at `src/js/main.js:2666`, which is what keeps the keyboard where the reader
left it. The row list is committed by moving nodes rather than replacing the container, at
`src/js/main.js:2554-2562`.
...
`src/js/main.js:44-45`, and the response cache is not in `localStorage` at all.
...
| `mrt.settings` | the settings form, the cover art switch, the theme control and the reading filter, at `src/js/main.js:625` | nothing | Preferences, not data. Deliberately outside the state so a settings write can never fail a progress write. |
| `sidebar.collapsed` | the sidebar toggle, at `src/js/main.js:965` | nothing | Whether the rail is collapsed. Wrapped in its own try, because losing it is not worth an error. |
