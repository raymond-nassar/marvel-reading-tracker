...
| C8 | Current Earth-616-relevant manifest entries range from 5 to 230 expected issues. | src/data/curated-lists.json:552-980 | manifest analysis | high | Review cost, not file-format capacity, controls batch size. |
| C9 | Comic Book Herald explicitly permitted continued reading-order work under the existing pattern: credit Comic Book Herald and link to the exact followed guide page; the reply does not grant a broader license. | docs/DATA_PROVENANCE.md:287-310 | read | high | New local orders use sourceLicense null and exact page attribution. |
| C10 | Repository validation exposes focused vendor and schema tests plus lint, full tests, anchors, counts, sizes, publication, contract, and browser commands; the live contract and browser checks are intentionally outside CI. | package.json:8-29 | read | high | Each batch needs offline gates plus manual live and browser checks at defined boundaries. |
...
| C12 | The app already uses issue search to resolve pasted plain-text order rows, but the result and any manual choice live only in browser state. | src/js/main.js:3299-3355 | read | high | A repeatable build-time program needs a persisted mapping artifact. |
...
| C15 | The metadata source ends on 2025-10-29, and waiting does not extend it. | docs/DATA_PROVENANCE.md:163-177 | read | high | Current and future guides need a separate deferred or explicitly sparse workflow. |
