...
All six run in CI on every pull request. `npm test` runs the unit tests. `npm run lint` runs
...
ranked table. `npm run palette` checks colour contrast. `npm run publication` checks that nothing
in the history or the tracked files would be a problem if this repository were made public.
