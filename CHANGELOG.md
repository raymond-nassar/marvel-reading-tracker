...
- **The project now says plainly where its comic data came from, and no longer claims a licence
  over parts of it that were never the project's to license.** Nothing about the app changes and
  nothing you have saved is affected. The comic details shown in the app were never written here:
  ...
  file at all, and that the one place it does declare MIT covers its program code and specifically
  excludes the folder this data came from. So the honest answer is that permission was given for the
  ...
  separate, the licence field will only accept a real licence name or an explicit "not established",
  and every list the project compiles itself has to record how it was put together. That last rule is
  written so that a new list added later is covered automatically, rather than only the ones anybody
  thought to name.
