...
Fifty-three items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-075, BL-077, BL-078 and BL-079.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2653-2687` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2715-2730` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2661` and `src/js/main.js:2670-2675` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2693-2713` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2354-2410`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2418-2487` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2316-2334`, manual issue add at `src/js/main.js:2489-2533` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1514-1534`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:198` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:848-888` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2892-2926`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:2906-2908` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1875` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1971-1977`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2032-2037` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2512` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3023-3029` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2063-2086` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3139-3159`. The work shipped since has
  taken it to 3,244; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1600-1627 |
...
| BL-076 | Stop a reload during a second incident writing another dated salvage copy | Defect | EP-06 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Ready | src/js/storage.js:81 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3139-3153 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3139-3153 |
...
that function is at `src/js/main.js:2567` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2665`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2906-2908` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1967-1977`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/index.html:710`, so resolving the theme in JavaScript would paint dark and then flip, and
...
Reuse is keyed on the whole item, at `src/js/main.js:1836-1845`, rather than on a list of the
...
`src/js/main.js:1826-1834`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:2852`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1567-1570` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1590-1598` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2852`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2820-2831`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1613-1621`. The usual reason
...
`src/js/main.js:3016`, `src/js/main.js:3016` and `src/js/main.js:3023-3029`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1351-1405`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2933-2951`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2499`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3033-3034`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1967-1977`, drawing on the labels at
`src/js/main.js:2032-2037`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1368-1371`, and `renderRows` asks it at `src/js/main.js:1875`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1363-1367` exists because
...
`src/js/main.js:2065`, and the reasoning is recorded at `src/js/main.js:187-191` rather than only
...
`src/js/main.js:1223` carries a comment at `src/js/main.js:1233` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2799`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1076` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2869` puts
...
`renderRail` at `src/js/main.js:848` is the other half, and it is the easier one: its mechanism is
...
`src/js/main.js:3139` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:856`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:999` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2065`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1012`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:872` and `src/js/main.js:1191`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:3172`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:872` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1191`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2365`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1175`. That also retired a latent bug rather than fixing it. The old label said
...
`claim` field, and the bless path at `scripts/check-anchors.mjs:843` then writes only the anchor,
...
change. `reportNearMisses` at `scripts/check-anchors.mjs:689` is the precedent for the script
...
line, at `scripts/check-anchors.mjs:702-718`, and it prints a notice rather than failing, because
...
The check runs before the bless path rather than beside it, at `scripts/check-anchors.mjs:747-781`,
...
The evidence is the bless path itself at `scripts/check-anchors.mjs:843`, which writes the anchor,
...
`scripts/check-anchors.mjs:713`. A YAML comment opens with a hash, an HTML comment with an angle
...
that boot report, and the comment now standing in its place at `src/js/main.js:3216-3220` records
...
file of 3,244 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3046-3048`, because the budget belongs to
...
instead, at `src/js/storage.js:81`, so an old copy cannot be clobbered by a new one. That is right.
...
incident, at `test/storage.test.js:140-166`, which reaches the dated branch by salvaging one, but it
...
glyph that no one reads, since the button's label at `src/js/main.js:1951` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:4846-4847` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:4867-4869`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:4680-4682` describes
...
`CHANGELOG.md:724-727` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2892-2926` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2636` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3139-3159` (renderAll rebuilds
every region), `src/js/main.js:72-77` (store.onChange is wired straight to renderAll),
`src/js/main.js:1847-2030` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2032-2037`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1600-1627` (undo), `src/js/main.js:3023-3029` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3139-3159`, `src/js/main.js:811-828` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,244 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3031-3051`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2693-2713` (attribution rendered in the UI before import). |
...
### Case 2: BL-007 is labelled P1 but ranks fifty-first

- Stated: P1 Core product value, event order variants.
- Calculated: WSJF 1.4, rank 51 of 57, below forty-seven unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2636` and `src/js/main.js:2670-2675`. What is missing is data, in
...
- BL-014, P1, rank 25 of 57. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 19 of 57. Mid-table.
- BL-017, P2, rank 56 of 57. The lowest-ranked scored story other than the one that cannot be
