...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1716-1746` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:348-350`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:341-347`, search-as-you-type at `src/js/main.js:1774-1789` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1720` and `src/js/main.js:1729-1734` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:1752-1772` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1699` and `src/js/main.js:1716-1721`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1429-1482`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1490-1552` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1391-1409`, manual issue add at `src/js/main.js:1554-1598` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:857-877`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:410-435` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:1912-1930`, and the view states it counts across every list at `src/index.html:328-330` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:314-318`, applied without touching stored order at `src/js/main.js:1129-1139` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1097-1100`, pending filter at `src/js/main.js:1133` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1122-1127` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1577` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:1964-1967` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1149-1167` | Partial |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2070-2083`.
...
is written out in words inside the pill at `src/js/main.js:1093-1100`, drawing on the labels at
`src/js/main.js:1122-1127`, and the full sentence is repeated in the `title` attribute from
...
citations were stale the moment they were committed. `src/js/main.js:1113-1118` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:1912-1930` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1699` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2070-2083` (renderAll rebuilds
every region), `src/js/main.js:53-58` (store.onChange is wired straight to renderAll),
`src/js/main.js:1041-1120` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1122-1127`.
...
- User error protection: gap. Deleting a list is guarded only by a native `confirm()` and cannot be
  ...
  `src/js/main.js:849-855` (delete, confirm only), `src/js/main.js:1964-1967` (undoRestore exists).
...
- Self-descriptiveness: gap. The full availability description is carried only in a `title`
  attribute, which never reaches a keyboard or touch user. Evidence: `src/js/main.js:1095`.
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2070-2083`, `src/js/main.js:381-405` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:1977-1990`, `src/js/lib/cachePolicy.js:16-20`,
...
| Error handling and recovery | Gap. Error surfaces are inconsistent: curated import reports failure through native `alert()`, while every other path uses the in-page notice system. Evidence: `src/js/main.js:1860`, `src/js/main.js:1871`, `src/js/main.js:1896` against `src/js/main.js:130-142`. |
| Offline behavior | No gap, and no proposal. Probed as required rather than treated as a caching problem. With the local server running and no internet, the app starts, reads saved state, imports any bundled curated list and marks issues read, because those paths touch only same-origin files. Only cover images, metadata hydration and search degrade, and hydration failure is already surfaced as a pending state rather than as silence. Evidence: `src/data/house_of_m.json`, `src/js/main.js:1097-1100` (pending and by-hand badges), `absent: serviceWorker|navigator.onLine|manifest.json, case-insensitive grep across src/`. Repository Constraint 1 forbids caching cover bytes, so no cover-caching improvement is proposed. |
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:453-477`, `src/js/main.js:1964-1967`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:1752-1772` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1699` and `src/js/main.js:1716-1721`. What is missing is data, in
