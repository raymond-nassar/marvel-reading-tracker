...
A hundred and fifteen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147, BL-149 and BL-150.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3447-3503` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3531-3546` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3470` and `src/js/main.js:3488-3501` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3509-3529` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2971-3027`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3035-3104` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2933-2951`, manual issue add at `src/js/main.js:3220-3340` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1881-1901`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:433` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1009-1049` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3713-3747`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3727-3729` | Done, shipped as BL-014 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3297` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3940-3946` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4290-4310`. The work shipped since has
  taken it further: the file is 4,398 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,152; 224 is the figure as audited.
...
| BL-030 | Stop dimming read rows with a blanket opacity | Defect | EP-08 | Leaves alone | 5 | 3 | 2 | 1 | 10.0 | none | Measured | Shipped | src/styles.css:571-580 |
...
| BL-091 | Let catalog descriptions survive the WCAG text-spacing override | Defect | EP-07 | Leaves BL-028 alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Dropped | src/styles.css:825-828 |
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3989-4026 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3901-3929 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3941-3944 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1433-1434 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4263-4266 |
...
| BL-047 | Split the two meanings of the row class | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Observed | Shipped | src/styles.css:676-692 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4210-4273 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3670 |
...
| BL-150 | Fill in what the metadata snapshot cannot, from the one catalogue a browser can reach | Feature | EP-03 | Follows BL-149 | 5 | 5 | 5 | 5 | 3.0 | none | Measured | Shipped | `src/js/lib/wikitext.js:38-42`, `src/js/main.js:3262-3274`, `absent: any record newer than 2025-10-29 upstream, measured by asking the metadata service for 2026 and being told the total is zero` |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3713-3747 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:978 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3461-3465 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4290-4304 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4290-4304 |
...
that function is at `src/js/main.js:3374` today, with the rename-list handler occupying the lines the
...
Nothing in the app changed. `groupCatalog` at `src/js/lib/catalog.js:398` was already a general
...
real title is `.result-title`, written at `src/js/main.js:3494`, and with the selector corrected the
...
`src/js/main.js:3727-3729` instead of asserting "every list" whatever is being counted.
...
strikethrough, at `src/styles.css:577-578`; the strikethrough is the non-colour indicator. The
only opacity left on a read row is the cover image at `src/styles.css:580`, which carries no
...
`src/js/main.js:3673`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3673`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3642-3653`, leaving nothing behind and the offer still
...
`src/js/main.js:4000`, `src/js/main.js:3924` and `src/js/main.js:3940-3946`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3754-3772`.
...
synthetic id from the clock, per `src/js/main.js:3231`, so the two kinds cannot be ordered against
...
The settings form keeps its own check and its own message at `src/js/main.js:3951-3952`, unchanged.
...
Shipped. The form row is now `.field-row` at `src/styles.css:676-692` and the reading row keeps
`.row`, so the thirteen rules that describe a reading row, from `src/styles.css:565-568` down to the
...
`.card-static > *` at `src/styles.css:669`, which the old rule's 0,2,0 had been masking. Measured
...
The reasoning is recorded at `src/styles.css:618-642`, directly above the rules it governs, along
...
now three, at `README.md:639-650`, and the four sentences over 40 words are now none: the longest
...
`src/js/lib/catalog.js:297-307` takes the opposite decision for the catalog, and the difference is
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3615`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3690` puts
...
`src/js/main.js:4290` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4323`, and `src/js/app.js` is the entry
...
`src/styles.css:663` sets `content` to a right-pointing angle glyph on the card summary marker. It
...
`content` in the stylesheets is the one at `src/styles.css:663`. Closing it would need a tokenizer
...
at `src/js/main.js:2982`. A reader who imports one of this repository's own checklists therefore saw
...
that boot report, and the comment now standing in its place at `src/js/main.js:4370-4374` records
...
file of 4,398 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3963-3965`, because the budget belongs to
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:954`. The choice
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10953-10955` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10972-10976`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10787-10789` describes
...
`CHANGELOG.md:2723-2732` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3901-3929`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3461-3465`,
...
attack. Evidence: `src/js/main.js:3901-3929`, `src/js/lib/model.js:968-1077`.
...
requests, from `src/js/main.js:2936` and `src/js/main.js:2952`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3045` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:4210-4273`.
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2921` and then starts
the hydrator at `src/js/main.js:2928`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2529`, and one of the four tests added to the counts suite pins the
...
stylesheet's own. `src/styles.css:649` puts the per-row actions out of reach with exactly
...
are valuable history and are not a concise public policy. Evidence: `README.md:388-705`,
...
button at `src/js/main.js:3989-4026` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3996-4025` repaints the banner
...
it on. Evidence: `src/js/main.js:642-643`, `src/js/main.js:639`, `src/styles.css:436`.
...
button. Evidence: `src/js/main.js:3632-3638`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3664-3670`,
...
here. Evidence: `src/js/main.js:3679`.
...
in place of that button while a copy is live at `src/js/main.js:4113-4119`, which is the state a
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4764-4772`.
...
`forgetDeleted()`, at `src/js/main.js:4000`, `src/js/main.js:3924` and `src/js/main.js:3945`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1433-1434`.
...
**Delivered.** The first citation now names `src/js/main.js:4000`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3989-4026`. The other two were already right and
are unchanged: `src/js/main.js:3924` is the call in the successful branch of the restore handler,
and `src/js/main.js:3940-3946` is the whole undo-restore handler, whose call is at
`src/js/main.js:3945`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3673` is its one call, `src/js/main.js:3642-3653` is the
...
at `PRODUCT_BACKLOG.md:4769-4770`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4764-4767` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4263-4266`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4245-4259` keeps
...
`src/js/lib/catalog.js:335-341`. Uncanny #129-138 is a story arc, not a book anyone can buy, so
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3713-3747` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3495` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4290-4310` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1969-1996` (undo), `src/js/main.js:3940-3946` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4290-4310`, `src/js/main.js:966-983` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,398 lines now, so nearly every item shipped
...
- Minor analysability gap: the `.row` class carries two unrelated meanings, a reading row and a form
  row, and a leftover empty rule sits between them. Evidence: `src/styles.css:565-568`,
  `src/styles.css:676-692`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3948-3981`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1046-1077`, `src/js/main.js:3940-3946`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3509-3529` (attribution rendered in the UI before import). |
...
### Case 1: BL-026 is labelled P0 but ranks sixty-first

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 61 of 126.
...
- Calculated: WSJF 1.4, rank 120 of 126, below a hundred and sixteen unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3495` and `src/js/main.js:3488-3501`. What is missing is data, in
...
- BL-014, P1, rank 79 of 126. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 63 of 126. Mid-table.
- BL-017, P2, rank 125 of 126. The lowest-ranked scored story other than the one that cannot be
