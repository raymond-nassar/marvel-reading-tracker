...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1756-1786` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:358-360`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:351-357`, search-as-you-type at `src/js/main.js:1814-1829` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1760` and `src/js/main.js:1769-1774` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:1792-1812` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1739` and `src/js/main.js:1756-1761`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1469-1522`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1530-1592` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1431-1449`, manual issue add at `src/js/main.js:1594-1638` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:884-904`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:432-457` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:1952-1970`, and the view states it counts across every list at `src/index.html:338-340` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:324-328`, applied without touching stored order at `src/js/main.js:1169-1179` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1134-1140`, pending filter at `src/js/main.js:1173` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1162-1167` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1617` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2004-2007` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1189-1207` | Partial |
| 7.2 | BL-027 | P1 | announcements exist at `src/js/main.js:137-164`, but they fire into two live regions at once and the first-run heading is empty per `docs/ux-artifacts/pa11y-landing.json` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2110-2123`.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Ready | src/js/main.js:875-882 |
...
| BL-026 | Make every action reachable and repeatable from the keyboard | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P0 | Measured | Ready | src/js/main.js:1189-1207 |
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:144-164 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:1952-1970 |
| BL-034 | Replace the native dialogs with the app's own notice system | Debt | EP-11 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Observed | Ready | src/js/main.js:869-879 |
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Ready | src/js/main.js:68 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2110-2122 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2110-2122 |
...
Which channel to use is now decided by reading the container at `src/js/main.js:150-157` rather
...
`src/js/main.js:1130-1140`, along with the pending badge's. A `title` is unreachable by touch and
...
`.badge-expected` at 1.58:1, but that is unconditional design at `src/styles.css:463` rather than
...
unconditional design at `src/styles.css:463`, where the border is 25 percent alpha green.
...
is written out in words inside the pill at `src/js/main.js:1130-1140`, drawing on the labels at
`src/js/main.js:1162-1167`, and the full sentence is repeated in the `title` attribute from
...
The reasoning is recorded at `src/styles.css:438-461`, directly above the rules it governs, along
...
citations were stale the moment they were committed. `src/js/main.js:1153-1158` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:1952-1970` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1739` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2110-2123` (renderAll rebuilds
every region), `src/js/main.js:60-65` (store.onChange is wired straight to renderAll),
`src/js/main.js:1074-1160` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1162-1167`.
- Operability: gap. Row actions sit at `opacity: 0` until hover or focus-within, so on a touch
  device they are invisible until tapped. Evidence: `src/styles.css:468-475`.
- User error protection: gap. Deleting a list is guarded only by a native `confirm()` and cannot be
  ...
  `src/js/main.js:876-882` (delete, confirm only), `src/js/main.js:2004-2007` (undoRestore exists).
...
- Self-descriptiveness: gap, closed by BL-027. The full availability description was carried only
  ...
  badge, read as part of the badge's accessible name. Evidence: `src/js/main.js:1130-1140`.
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2110-2123`, `src/js/main.js:403-427` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2017-2030`, `src/js/lib/cachePolicy.js:16-20`,
...
| Error handling and recovery | Gap. Error surfaces are inconsistent: curated import reports failure through native `alert()`, while every other path uses the in-page notice system. Evidence: `src/js/main.js:1900`, `src/js/main.js:1911`, `src/js/main.js:1936` against `src/js/main.js:137-164`. |
| Offline behavior | No gap, and no proposal. Probed as required rather than treated as a caching problem. With the local server running and no internet, the app starts, reads saved state, imports any bundled curated list and marks issues read, because those paths touch only same-origin files. Only cover images, metadata hydration and search degrade, and hydration failure is already surfaced as a pending state rather than as silence. Evidence: `src/data/house_of_m.json`, `src/js/main.js:1134-1140` (pending and by-hand badges), `absent: serviceWorker|navigator.onLine|manifest.json, case-insensitive grep across src/`. Repository Constraint 1 forbids caching cover bytes, so no cover-caching improvement is proposed. |
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:453-477`, `src/js/main.js:2004-2007`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:1792-1812` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1739` and `src/js/main.js:1756-1761`. What is missing is data, in
...
- What a human should settle: whether a measured 93 pixel horizontal overflow at 320 pixels, plus
  ...
  `src/styles.css:468-475`, `src/styles.css:87-90`. The original document's own framing supports
