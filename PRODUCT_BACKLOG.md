...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1715-1745` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:348-350`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:341-347`, search-as-you-type at `src/js/main.js:1773-1788` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1719` and `src/js/main.js:1728-1733` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:1751-1771` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1698` and `src/js/main.js:1715-1720`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1428-1481`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1489-1551` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1390-1408`, manual issue add at `src/js/main.js:1553-1597` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:856-876`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:410-435` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:1911-1929`, and the view states it counts across every list at `src/index.html:328-330` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:314-318`, applied without touching stored order at `src/js/main.js:1128-1138` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:259-289` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1096-1099`, pending filter at `src/js/main.js:1132` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1121-1126` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1576` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:1963-1966` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1148-1166` | Partial |
| 7.2 | BL-027 | P1 | announcements exist at `src/js/main.js:130-142`, but they fire into two live regions at once and the first-run heading is empty per `docs/ux-artifacts/pa11y-landing.json` | Partial |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2069-2082`.
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2069-2082 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2069-2082 |
...
is written out in words inside the pill at `src/js/main.js:1092-1099`, drawing on the labels at
`src/js/main.js:1121-1126`, and the full sentence is repeated in the `title` attribute from
...
citations were stale the moment they were committed. `src/js/main.js:1112-1117` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:1911-1929` (renderProgress calls seriesProgress on the whole state, with
  no list filter), `src/index.html:328-330` (the view's own subtitle states it counts across every
  list).
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1698` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2069-2082` (renderAll rebuilds
every region), `src/js/main.js:53-58` (store.onChange is wired straight to renderAll),
`src/js/main.js:1040-1119` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1121-1126`.
...
- User error protection: gap. Deleting a list is guarded only by a native `confirm()` and cannot be
  ...
  `src/js/main.js:848-854` (delete, confirm only), `src/js/main.js:1963-1966` (undoRestore exists).
...
- Self-descriptiveness: gap. The full availability description is carried only in a `title`
  attribute, which never reaches a keyboard or touch user. Evidence: `src/js/main.js:1094`.
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2069-2082`, `src/js/main.js:381-405` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:1976-1989`, `src/js/lib/cachePolicy.js:16-20`,
...
| Category | Verdict |
|----------|---------|
| First-run experience | Gap. The first-run DOM ships an empty `<h2 id="hero-title">`, so the first heading a screen reader meets on an unseeded install is blank. Evidence: `src/index.html:259-289`, `docs/ux-artifacts/pa11y-landing.json`. |
...
| Error handling and recovery | Gap. Error surfaces are inconsistent: curated import reports failure through native `alert()`, while every other path uses the in-page notice system. Evidence: `src/js/main.js:1859`, `src/js/main.js:1870`, `src/js/main.js:1895` against `src/js/main.js:130-142`. |
| Offline behavior | No gap, and no proposal. Probed as required rather than treated as a caching problem. With the local server running and no internet, the app starts, reads saved state, imports any bundled curated list and marks issues read, because those paths touch only same-origin files. Only cover images, metadata hydration and search degrade, and hydration failure is already surfaced as a pending state rather than as silence. Evidence: `src/data/house_of_m.json`, `src/js/main.js:1096-1099` (pending and by-hand badges), `absent: serviceWorker|navigator.onLine|manifest.json, case-insensitive grep across src/`. Repository Constraint 1 forbids caching cover bytes, so no cover-caching improvement is proposed. |
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:453-477`, `src/js/main.js:1963-1966`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:1751-1771` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1698` and `src/js/main.js:1715-1720`. What is missing is data, in
