...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3042-3076` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:421-423`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:414-420`, search-as-you-type at `src/js/main.js:3104-3119` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3050` and `src/js/main.js:3059-3064` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3082-3102` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2743-2799`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2807-2876` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2705-2723`, manual issue add at `src/js/main.js:2878-2922` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3286-3320`, with a scope control at `src/index.html:381-385` and a subtitle that stops claiming every list at `src/js/main.js:3300-3302` | Done, shipped as BL-014 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2901` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3513-3519` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:3863-3883`. The work shipped since has
  taken it further: the file is 3,971 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,018; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3562-3599 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3474-3502 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3836-3839 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3783-3846 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3243 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3286-3320 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3045-3049 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3863-3877 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3863-3877 |
...
that function is at `src/js/main.js:2956` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:3054`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:3300-3302` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3246`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3246`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3215-3226`, leaving nothing behind and the offer still
...
`src/js/main.js:3573`, `src/js/main.js:3497` and `src/js/main.js:3513-3519`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3327-3345`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2888`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3524-3525`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3188`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3263` puts
...
`src/js/main.js:3863` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:3896`, and `src/js/app.js` is the entry
...
at `src/js/main.js:2754`. A reader who imports one of this repository's own checklists therefore saw
...
that boot report, and the comment now standing in its place at `src/js/main.js:3943-3947` records
...
file of 3,971 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3536-3538`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10111-10113` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10130-10134`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:9945-9947` describes
...
`CHANGELOG.md:2375-2384` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3474-3502`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3045-3049`,
...
attack. Evidence: `src/js/main.js:3474-3502`, `src/js/lib/model.js:965-1074`.
...
requests, from `src/js/main.js:2708` and `src/js/main.js:2724`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2817` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:3783-3846`.
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2693` and then starts
the hydrator at `src/js/main.js:2700`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2481`, and one of the four tests added to the counts suite pins the
...
button at `src/js/main.js:3562-3599` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3569-3598` repaints the banner
...
button. Evidence: `src/js/main.js:3205-3211`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3237-3243`,
...
here. Evidence: `src/js/main.js:3252`.
...
in place of that button while a copy is live at `src/js/main.js:3686-3692`, which is the state a
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4716-4724`.
...
`forgetDeleted()`, at `src/js/main.js:3573`, `src/js/main.js:3497` and `src/js/main.js:3518`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1388-1389`.
...
**Delivered.** The first citation now names `src/js/main.js:3573`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3562-3599`. The other two were already right and
are unchanged: `src/js/main.js:3497` is the call in the successful branch of the restore handler,
and `src/js/main.js:3513-3519` is the whole undo-restore handler, whose call is at
`src/js/main.js:3518`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3246` is its one call, `src/js/main.js:3215-3226` is the
...
at `PRODUCT_BACKLOG.md:4721-4722`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4716-4719` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3836-3839`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3818-3832` keeps
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3286-3320` (renderProgress, which now takes a scope), `src/index.html:379`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3025` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3863-3883` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1777-1804` (undo), `src/js/main.js:3513-3519` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:3863-3883`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,971 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3521-3554`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1043-1074`, `src/js/main.js:3513-3519`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3082-3102` (attribution rendered in the UI before import). |
...
### Case 1: BL-026 is labelled P0 but ranks fifty-seventh

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 57 of 116.
- Driver: job size, not value. Its Cost of Delay of 11 is the ninth highest figure in the backlog.
  It is outranked by fifty-four items, thirty-one of them sized 1, 2 or 3 whose Cost of Delay
...
- Calculated: WSJF 1.4, rank 110 of 116, below a hundred unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3025` and `src/js/main.js:3059-3064`. What is missing is data, in
...
- BL-014, P1, rank 71 of 116. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 59 of 116. Mid-table.
- BL-017, P2, rank 115 of 116. The lowest-ranked scored story other than the one that cannot be
