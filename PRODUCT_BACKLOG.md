...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2206-2236` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:396-398`, filterByFacet imported at `src/js/main.js:17` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:389-395`, search-as-you-type at `src/js/main.js:2264-2279` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2210` and `src/js/main.js:2219-2224` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2242-2262` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2189` and `src/js/main.js:2219-2224`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1910-1963`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1971-2040` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1872-1890`, manual issue add at `src/js/main.js:2042-2086` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1213-1233`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:664-704` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2441-2475`, with a scope control at `src/index.html:356-360` and a subtitle that stops claiming every list at `src/js/main.js:2455-2457` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1516` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1568-1574`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1604-1609` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2065` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:538-562` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2570-2576` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1619-1642` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2686-2701`. The work shipped since has
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 333; 224 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1299-1326 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2441-2475 |
...
that function is at `src/js/main.js:2120` today, with the rename-list handler occupying the lines the
...
`src/js/main.js:2455-2457` instead of asserting "every list" whatever is being counted.
...
a stale id at `src/js/lib/model.js:528`, so the two cannot come apart. That invariant also rules out
...
`src/js/main.js:1564-1574`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2344`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1266-1269` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1289-1297` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2401`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2369-2380`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1312-1320`. The usual reason
...
`src/js/main.js:2563`, `src/js/main.js:2563` and `src/js/main.js:2570-2576`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1130-1183`, before the
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:381-431`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2482-2500`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2052`, so the two kinds cannot be
...
`test/library.test.js:164-192` reads both files off disk and fails on any disagreement. That is the
difference from `READING_FILTERS`, which can make its drift impossible to express: this makes its
drift impossible to ship.

Fourteen tests were added, and each was proved able to fail before being trusted. Seven mutations
...
dropped, and list ids reported where names were promised.

One risk was gone looking for and found closed upstream. Everything read prints a date, so what a
restore can put in the read map is this view's problem too, and a hand-edited backup carrying a
string or a zero would otherwise put "Invalid Date" on screen. It cannot: `store.load` runs every
saved state through `migrate` at `src/js/storage.js:36`, and `migrate` replaces any value that is
not a usable number at `src/js/lib/model.js:500`. That is what makes the row renderer safe in
dropping the date when `readAt` is falsy, and it is now pinned by a test rather than left as
something the next person has to re-derive.
...
The settings form keeps its own check and its own message at `src/js/main.js:2580-2581`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1564-1574`, drawing on the labels at
`src/js/main.js:1604-1609`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1147-1150`, and `renderRows` asks it at `src/js/main.js:1516`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1142-1146` exists because
...
`src/js/main.js:1595`, and the reasoning is recorded at `src/js/main.js:160-164` rather than only
...
`src/js/main.js:1038` carries a comment at `src/js/main.js:1048` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2348`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2418` puts
...
`src/js/main.js:2686` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1621`,
...
`src/js/main.js:688` and `src/js/main.js:1006`. Both predate this change and neither is part of its
...
on every catalog card, at `src/js/main.js:1006`, joined an issue count to an invitation to open the
...
at `src/js/main.js:1921`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:990`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:2756-2757` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:2776-2778`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2602-2604` describes
...
`CHANGELOG.md:261-264` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2441-2475` (renderProgress, which now takes a scope), `src/index.html:354`
  (the view's subtitle).
...
- Correctness is well defended: 224 unit tests pass, 235 when this pass shipped and 333 now, and
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2686-2701` (renderAll rebuilds
every region), `src/js/main.js:64-69` (store.onChange is wired straight to renderAll),
`src/js/main.js:1505-1602` (renderRows builds every row with no virtualisation and no early exit when
...
`src/js/lib/model.js:538-562` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1604-1609`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1299-1326` (undo), `src/js/main.js:2570-2576` (undoRestore exists).
...
`src/js/lib/model.js:461-487` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2686-2701`, `src/js/main.js:632-660` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2578-2598`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:461-487`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2242-2262` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2189` and `src/js/main.js:2219-2224`. What is missing is data, in
