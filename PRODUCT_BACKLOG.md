...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3202-3254` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:421-423`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:414-420`, search-as-you-type at `src/js/main.js:3282-3297` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3225` and `src/js/main.js:3243-3252` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3260-3280` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2916-2972`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2980-3049` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2878-2896`, manual issue add at `src/js/main.js:3051-3095` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1836-1856`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:432` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1008-1048` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3464-3498`, with a scope control at `src/index.html:381-385` and a subtitle that stops claiming every list at `src/js/main.js:3478-3480` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2307` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2403-2404`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2459-2464` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3074` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3691-3697` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2490-2513` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4041-4061`. The work shipped since has
  taken it further: the file is 4,149 lines today. 1,566 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2248-2257 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3740-3777 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3652-3680 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4014-4017 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1924-1951 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3961-4024 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3421 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3464-3498 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:966 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3216-3220 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4041-4055 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4041-4055 |
...
that function is at `src/js/main.js:3129` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:3245`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:3478-3480` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:2399-2404`, along with the pending badge's. A `title` is unreachable by touch and
...
Reuse is keyed on the whole item, at `src/js/main.js:2182-2197`, rather than on a list of the
...
`src/js/main.js:2172-2180`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3424`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1891-1894` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1914-1922` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3424`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3393-3404`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1937-1945`. The usual reason
...
`src/js/main.js:3751`, `src/js/main.js:3675` and `src/js/main.js:3691-3697`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1673-1727`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3505-3523`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:3061`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3702-3703`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2399-2404`, drawing on the labels at
`src/js/main.js:2459-2464`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1690-1693`, and `renderRows` asks it at `src/js/main.js:2307`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1685-1689` exists because
...
`src/js/main.js:2147` and the reading rows at `src/js/main.js:2450`, and the reasoning is recorded at
...
`src/js/main.js:1510` carries a comment at `src/js/main.js:1520` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3366`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3441` puts
...
`src/js/main.js:4041` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2492`,
...
`src/js/main.js:1032` and `src/js/main.js:1470`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4074`, and `src/js/app.js` is the entry
...
on every catalog card, at `src/js/main.js:1470`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2927`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1417`. That also retired a latent bug rather than fixing it. The old label said
...
that boot report, and the comment now standing in its place at `src/js/main.js:4121-4125` records
...
file of 4,149 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3714-3716`, because the budget belongs to
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:942`. The choice
...
glyph that no one reads, since the button's label at `src/js/main.js:2383` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10421-10423` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10440-10444`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10255-10257` describes
...
`CHANGELOG.md:2482-2491` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3652-3680`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3216-3220`,
...
attack. Evidence: `src/js/main.js:3652-3680`, `src/js/lib/model.js:965-1074`.
...
`src/js/main.js:2045` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2881` and `src/js/main.js:2897`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2990` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:3961-4024`.
...
The message policy is a pure function at `src/js/main.js:2546-2555` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2866` and then starts
the hydrator at `src/js/main.js:2873`, whose `start()` reports its running phase at
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1698-1699`
...
button at `src/js/main.js:3740-3777` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3747-3776` repaints the banner
...
over, for the hero backdrop alone: `src/js/main.js:2045` reads the setting before building the URL,
...
`src/js/main.js:2196`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3383-3389`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3415-3421`,
...
here. Evidence: `src/js/main.js:3430`.
...
in place of that button while a copy is live at `src/js/main.js:3864-3870`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:3751`, `src/js/main.js:3675` and `src/js/main.js:3696`
...
**Delivered.** The first citation now names `src/js/main.js:3751`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3740-3777`. The other two were already right and
are unchanged: `src/js/main.js:3675` is the call in the successful branch of the restore handler,
and `src/js/main.js:3691-3697` is the whole undo-restore handler, whose call is at
`src/js/main.js:3696`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1891-1894` is `forgetDeleted` itself, `src/js/main.js:1914-1922` is
`forgetDeletedFor`, `src/js/main.js:3424` is its one call, `src/js/main.js:3393-3404` is the
half-import rollback, `src/js/main.js:437` is `notify` with its `action` parameter,
`src/js/main.js:460-464` is `spoken`, `src/js/main.js:1937-1945` is the failed undo keeping its
...
`src/js/main.js:1917`, `src/js/main.js:1933` and `src/js/main.js:1948`. The sentence is right to
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4014-4017`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3996-4010` keeps
...
consumer at `src/js/main.js:2064`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2248-2257` returns a sentence saying no synopsis is
...
The disclaimer at `src/js/main.js:2595` is shown every time a run starts rather than once and
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3464-3498` (renderProgress, which now takes a scope), `src/index.html:379`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3246` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4041-4061` (renderAll rebuilds
every region), `src/js/main.js:75-80` (store.onChange is wired straight to renderAll),
`src/js/main.js:2279-2457` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2459-2464`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1924-1951` (undo), `src/js/main.js:3691-3697` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4041-4061`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,149 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3699-3732`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3260-3280` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3246` and `src/js/main.js:3243-3252`. What is missing is data, in
