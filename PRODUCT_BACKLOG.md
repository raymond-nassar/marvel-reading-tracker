...
Thirty-seven items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-052, BL-053, BL-054, BL-055, BL-056, BL-057, BL-058, BL-059, BL-061, BL-062, BL-063 and BL-066.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2340-2374` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:400-402`, filterByFacet imported at `src/js/main.js:17` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:393-399`, search-as-you-type at `src/js/main.js:2402-2417` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2348` and `src/js/main.js:2357-2362` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2380-2400` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2041-2097`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2105-2174` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2003-2021`, manual issue add at `src/js/main.js:2176-2220` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1309-1329`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:169` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:745-785` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2579-2613`, with a scope control at `src/index.html:360-364` and a subtitle that stops claiming every list at `src/js/main.js:2593-2595` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1612` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1699-1705`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1735-1740` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2199` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2709-2715` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1750-1773` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2825-2845`. The work shipped since has
  taken it to 2,919; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1395-1422 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2579-2613 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2825-2839 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2825-2839 |
...
that function is at `src/js/main.js:2254` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2352`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2593-2595` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1695-1705`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2539`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1362-1365` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1385-1393` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2539`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2507-2518`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1408-1416`. The usual reason
...
`src/js/main.js:2702`, `src/js/main.js:2702` and `src/js/main.js:2709-2715`. Each replaces the whole
...
the id is unknown, at `src/js/lib/model.js:255-257`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1226-1279`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2620-2638`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2186`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2719-2720`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1695-1705`, drawing on the labels at
`src/js/main.js:1735-1740`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1243-1246`, and `renderRows` asks it at `src/js/main.js:1612`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1238-1242` exists because
...
`src/js/main.js:1726`, and the reasoning is recorded at `src/js/main.js:162-166` rather than only
...
`src/js/main.js:1120` carries a comment at `src/js/main.js:1130` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2486`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2556` puts
...
`src/js/main.js:2825` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1752`,
...
`src/js/main.js:769` and `src/js/main.js:1088`. Both predate this change and neither is part of its
...
on every catalog card, at `src/js/main.js:1088`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2052`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1072`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3168-3169` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3188-3190`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:3010-3012` describes
...
`CHANGELOG.md:360-363` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2579-2613` (renderProgress, which now takes a scope), `src/index.html:358`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2323` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2825-2845` (renderAll rebuilds
every region), `src/js/main.js:66-71` (store.onChange is wired straight to renderAll),
`src/js/main.js:1601-1733` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1735-1740`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1395-1422` (undo), `src/js/main.js:2709-2715` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2825-2845`, `src/js/main.js:710-741` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2717-2737`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2380-2400` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2323` and `src/js/main.js:2357-2362`. What is missing is data, in
