...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2615-2649` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2677-2692` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2623` and `src/js/main.js:2632-2637` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2655-2675` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2316-2372`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2380-2449` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2278-2296`, manual issue add at `src/js/main.js:2451-2495` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2854-2888`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2868-2870` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1834` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1933-1939`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1994-1999` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2474` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2985-2991` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2025-2048` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3101-3121`. The work shipped since has
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2854-2888 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3101-3115 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3101-3115 |
...
that function is at `src/js/main.js:2529` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2627`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2868-2870` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1929-1939`, along with the pending badge's. A `title` is unreachable by touch and
...
Reuse is keyed on the whole item, at `src/js/main.js:1888-1890`, rather than on a list of the
...
this risks buying. `currentId` is the one input that is not part of the item, because the up next
marker moves on its own, so it is named in the key separately.
...
against the new tests and all six were caught: removing the toggle listener, removing the guard,
...
`src/js/main.js:2814`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:2814`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2782-2793`, leaving nothing behind and the offer still
...
`src/js/main.js:2978`, `src/js/main.js:2978` and `src/js/main.js:2985-2991`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2895-2913`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2461`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2995-2996`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1929-1939`, drawing on the labels at
`src/js/main.js:1994-1999`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1341-1344`, and `renderRows` asks it at `src/js/main.js:1834`. "In the markup but
...
`src/js/main.js:2027`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2761`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2831` puts
...
`src/js/main.js:3101` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2027`,
...
at `src/js/main.js:2327`. A reader who imports one of this repository's own checklists therefore saw
...
label at `src/js/main.js:1913` reads "Mark X as unread" exactly when it is checked. That is the same
...
`CHANGELOG.md:472-475` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2854-2888` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2598` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3101-3121` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1809-1992` (renderRows, which then built every row with no virtualisation and no
...
way on the same order, a read toggle went from 14.1 ms to 2.8 ms with the order open, and from
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1994-1999`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1573-1600` (undo), `src/js/main.js:2985-2991` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3101-3121`, `src/js/main.js:784-801` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2993-3013`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2655-2675` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2598` and `src/js/main.js:2632-2637`. What is missing is data, in
