...
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1534-1554`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:272` | Done |
...
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:514-543`, quoted into the Markdown export at `src/js/lib/markdown.js:177-179` | Done, shipped as BL-017 |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:831-861` | Done |
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 715; 224 is the figure as audited.
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:781 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Ready | src/js/lib/model.js:116-126 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:523-531 |
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:562-573`; omitting it
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:328` and load normalises
a stale id at `src/js/lib/model.js:826`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:748-749`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:523-531`, for the same reason
...
Shipped. `restoreList` at `src/js/lib/model.js:347-359` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:290-293`: "in library" and "Continue reading" would both resolve to
...
the id is unknown, at `src/js/lib/model.js:361-363`, so an address naming a deleted list would
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:593-643`,
...
`deleteList` at `src/js/lib/model.js:329-330`: issue metadata and read state survive the deletion of
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:483-490`, written the same way
`coerce` writes it at `src/js/lib/model.js:762`, so the two paths cannot disagree and every future
caller inherits it.
...
`lists[k] = {...}` at `src/js/lib/model.js:781`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:809`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:826`.
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:7134-7136` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:7153-7157`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:6968-6970` describes
...
`CHANGELOG.md:1449-1458` that says so and points at the backlog clause instead. That clause was
...
attack. Evidence: `src/js/main.js:3066-3094`, `src/js/lib/model.js:753-861`.
...
characters, and the fixed route puts about 2,000 ids inside one stamp, which leaves 244 million
same-stamp pairs drawing against 36^6. Measured over 60 restores at the ceiling, 4 collided, each
...
The seven defects above were held to the same standard as the first pass: fourteen mutations, each
...
`src/js/lib/model.js:116-126`, `server.mjs:25-48`, `src/index.html:524-531`.
...
button. Evidence: `src/js/main.js:2876`, `src/js/lib/model.js:110`,
`src/js/lib/model.js:645`.
...
`src/js/lib/model.js:831-861` (validated backup shape).
...
`src/js/lib/model.js:673-749` (migrate refuses an unsupported schema version),
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  `src/js/lib/model.js:11` and `src/js/lib/model.js:673-749`.
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:831-861`, `src/js/main.js:3105-3111`. |
| Schema migration | No gap. Stored state carries `SCHEMA_VERSION`, migrations run forward, and a future schema is refused rather than silently coerced, with a test pinning that behaviour. Evidence: `src/js/lib/model.js:11`, `src/js/lib/model.js:673-749`, `test/model.test.js:559-561`. |
