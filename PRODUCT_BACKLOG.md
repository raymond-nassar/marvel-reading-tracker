...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2400-2434` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2462-2477` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2408` and `src/js/main.js:2417-2422` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2440-2460` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2101-2157`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2165-2234` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2063-2081`, manual issue add at `src/js/main.js:2236-2280` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1327-1347`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:179` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:746-786` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2639-2673`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2653-2655` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1634` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1721-1727`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1779-1784` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2259` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2770-2776` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1810-1833` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2886-2906`. The work shipped since has
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2639-2673 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2886-2900 |
...
that function is at `src/js/main.js:2314` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2412`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2653-2655` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1717-1727`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2599`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1380-1383` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1403-1411` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2599`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2567-2578`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1426-1434`. The usual reason
...
`src/js/main.js:2763`, `src/js/main.js:2763` and `src/js/main.js:2770-2776`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1227-1280`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2680-2698`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2246`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2780-2781`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1717-1727`, drawing on the labels at
`src/js/main.js:1779-1784`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1244-1247`, and `renderRows` asks it at `src/js/main.js:1634`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1239-1243` exists because
...
`src/js/main.js:1770`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
`src/js/main.js:1121` carries a comment at `src/js/main.js:1131` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2546`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2616` puts
...
`src/js/main.js:2886` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1812`,
...
`src/js/main.js:770` and `src/js/main.js:1089`. Both predate this change and neither is part of its
...
on every catalog card, at `src/js/main.js:1089`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2112`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1073`. That also retired a latent bug rather than fixing it. The old label said
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2639-2673` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2383` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2886-2906` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1623-1777` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1779-1784`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1413-1440` (undo), `src/js/main.js:2770-2776` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2886-2906`, `src/js/main.js:711-742` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2778-2798`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2440-2460` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2383` and `src/js/main.js:2417-2422`. What is missing is data, in
