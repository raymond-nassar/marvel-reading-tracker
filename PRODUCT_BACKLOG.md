...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2546-2580` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2608-2623` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2554` and `src/js/main.js:2563-2568` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2586-2606` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2247-2303`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2311-2380` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2209-2227`, manual issue add at `src/js/main.js:2382-2426` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1473-1493`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:179` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:814-854` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2785-2819`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2799-2801` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1780` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1867-1873`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1925-1930` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2405` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2916-2922` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1956-1979` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3032-3052`. The work shipped since has
  taken it to 3,126; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1559-1586 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2785-2819 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:3032-3046 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3032-3046 |
...
that function is at `src/js/main.js:2460` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2558`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2799-2801` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1863-1873`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2745`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1526-1529` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1549-1557` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2745`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2713-2724`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1572-1580`. The usual reason
...
`src/js/main.js:2909`, `src/js/main.js:2909` and `src/js/main.js:2916-2922`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1317-1364`, before the
...
passive sync from the live filter, and leaving the group without committing. A browser check of the
first-round fixes passes 25 of 25 and a second of the deferred write passes 31 of 31, over the five
prototype members and thirteen history sequences. Each was measured failing on this tree first: the
deferred-write check reports 5 failures against the design it replaced, and the read toggle it drives
is asserted to have actually moved the store, because the first draft named a control that does not
exist and two of its checks passed without a passive write ever happening.
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2826-2844`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2392`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2926-2927`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1863-1873`, drawing on the labels at
`src/js/main.js:1925-1930`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1334-1337`, and `renderRows` asks it at `src/js/main.js:1780`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1329-1333` exists because
...
`src/js/main.js:1958`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
`src/js/main.js:1189` carries a comment at `src/js/main.js:1199` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2692`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1042` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2762` puts
...
`renderRail` at `src/js/main.js:814` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:792` and reaches `renderYours` through `renderHome` at
`src/js/main.js:794`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:3032` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:822`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:965` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1958`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:978`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:838` and `src/js/main.js:1157`. Both predate this change and neither is part of its
...
built at `src/js/main.js:838` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1157`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2258`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1141`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3507-3508` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3527-3529`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:3349-3351` describes
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2785-2819` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2529` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3032-3052` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1769-1923` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1925-1930`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1559-1586` (undo), `src/js/main.js:2916-2922` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3032-3052`, `src/js/main.js:777-794` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,126 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2924-2944`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2586-2606` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2529` and `src/js/main.js:2563-2568`. What is missing is data, in
