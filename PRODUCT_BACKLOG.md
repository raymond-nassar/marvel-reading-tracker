...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2862-2896` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2924-2939` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2870` and `src/js/main.js:2879-2884` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2902-2922` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2563-2619`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2627-2696` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2525-2543`, manual issue add at `src/js/main.js:2698-2742` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1623-1643`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:408` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:942-982` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3105-3139`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:3119-3121` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2073` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:295-330` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:650-679`, quoted into the Markdown export at `src/js/lib/markdown.js:180-182` | Done, shipped as BL-017 |
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2169-2170`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2225-2230` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2721` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:972-1003` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3332-3338` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2256-2279` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3669-3689`. The work shipped since has
  taken it to 3,732; 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 860; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3368-3405 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3293-3321 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3879-3882 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1377-1378 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3642-3645 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:922 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3589-3652 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3062 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/lib/model.js:201-216 |
...
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Shipped | src/js/lib/model.js:190 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3105-3139 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:2865-2869 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3669-3683 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:659-667 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3669-3683 |
...
that function is at `src/js/main.js:2776` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2874`, and with the selector corrected the same mutation fails it. It is
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:698-709`; omitting it
...
`src/js/main.js:3119-3121` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:464` and load normalises
a stale id at `src/js/lib/model.js:967`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:889-890`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:659-667`, for the same reason
...
`src/js/main.js:2165-2170`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:3065`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:483-495` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:426-429`: "in library" and "Continue reading" would both resolve to
...
about to make at `src/js/main.js:3065`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3034-3045`, leaving nothing behind and the offer still
...
`src/js/main.js:3379`, `src/js/main.js:3316` and `src/js/main.js:3332-3338`. Each replaces the whole
...
the id is unknown, at `src/js/lib/model.js:497-499`, so an address naming a deleted list would
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:729-779`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3146-3164`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:465-466`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:2708`, so the two kinds cannot be
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:619-626`, written the same way
`coerce` writes it at `src/js/lib/model.js:903`, so the two paths cannot disagree and every future
caller inherits it.
...
The settings form keeps its own check and its own message at `src/js/main.js:3342-3343`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2165-2170`, drawing on the labels at
`src/js/main.js:2225-2230`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1477-1480`, and `renderRows` asks it at `src/js/main.js:2073`. "In the markup but
...
`src/js/main.js:1931` and the reading rows at `src/js/main.js:2216`, and the reasoning is recorded at
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3008`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3082` puts
...
`src/js/main.js:3669` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2258`,
...
sequence moved into an exported `boot()` at `src/js/main.js:3702`, and `src/js/app.js` is the entry
...
at `src/js/main.js:2574`. A reader who imports one of this repository's own checklists therefore saw
...
`lists[k] = {...}` at `src/js/lib/model.js:922`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:950`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:967`.
...
that boot report, and the comment now standing in its place at `src/js/main.js:3747-3751` records
...
the client and leaves the limiter alone, at `src/js/main.js:3355-3357`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2149` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:8833-8835` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:8852-8856`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:8667-8669` describes
...
`src/js/storage.js:495-571`, `src/js/main.js:3293-3321`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:2865-2869`,
`src/data/catalog.json`, `src/js/lib/model.js:118-129`.
...
attack. Evidence: `src/js/main.js:3293-3321`, `src/js/lib/model.js:894-1003`.
...
`src/js/lib/model.js:201-216`, `server.mjs:53-59`, `src/index.html:526-531`.
...
requests, from `src/js/main.js:2528` and `src/js/main.js:2544`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2637` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3589-3652`.
...
The message policy is a pure function at `src/js/main.js:2312-2321` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2513` and then starts
the hydrator at `src/js/main.js:2520`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2468`, and one of the four tests added to the counts suite pins the
...
button at `src/js/main.js:3368-3405` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3375-3404` repaints the banner
...
button. Evidence: `src/js/main.js:3025-3030`, `src/js/lib/model.js:187`,
`src/js/lib/model.js:781`.
...
run, so its emptiness is an answer rather than a silence. A refusal met at runtime is recorded
...
Verified with 19 new tests, each watched failing first. Ten targeted reverts, each the smallest edit
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3056-3062`,
...
are green. Evidence: `src/js/lib/model.js:118-129`.
...
`src/js/lib/model.js:125-126`, `src/js/lib/model.js:135-145`.
...
here. Evidence: `src/js/main.js:3071`.
...
in place of that button while a copy is live at `src/js/main.js:3492-3498`, which is the state a
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4702-4710`.
...
`forgetDeleted()`, at `src/js/main.js:3379`, `src/js/main.js:3316` and `src/js/main.js:3337`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1377-1378`.
...
**Delivered.** The first citation now names `src/js/main.js:3379`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3368-3405`. The other two were already right and
are unchanged: `src/js/main.js:3316` is the call in the successful branch of the restore handler,
and `src/js/main.js:3332-3338` is the whole undo-restore handler, whose call is at
`src/js/main.js:3337`. The range was left as the handler rather than narrowed to the call, because
...
the code they name, and the twelve besides this one are sound: `src/js/lib/model.js:483-495` is
`restoreList`, `src/js/lib/model.js:426-429` is the comment on a duplicate not inheriting the
catalog claim, `src/js/main.js:1676-1679` is `forgetDeleted` itself, `src/js/main.js:1699-1707` is
`forgetDeletedFor`, `src/js/main.js:3065` is its one call, `src/js/main.js:3034-3045` is the
...
at `PRODUCT_BACKLOG.md:4707-4708`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4702-4705` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3642-3645`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3624-3638` keeps
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3105-3139` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2845` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3669-3689` (renderAll rebuilds
every region), `src/js/main.js:74-79` (store.onChange is wired straight to renderAll),
`src/js/main.js:2045-2223` (renderRows, which then built every row with no virtualisation and no
...
`src/js/lib/model.js:972-1003` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2225-2230`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1709-1736` (undo), `src/js/main.js:3332-3338` (undoRestore exists).
...
`src/js/lib/model.js:814-890` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3669-3689`, `src/js/main.js:899-916` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3340-3360`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:814-890`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2902-2922` (attribution rendered in the UI before import). |
...
- Calculated: WSJF 3.67, rank 47 of 101.
...
- Calculated: WSJF 1.4, rank 95 of 101, below ninety-one unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2845` and `src/js/main.js:2879-2884`. What is missing is data, in
...
- BL-014, P1, rank 60 of 101. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 49 of 101. Mid-table.
- BL-017, P2, rank 100 of 101. The lowest-ranked scored story other than the one that cannot be
