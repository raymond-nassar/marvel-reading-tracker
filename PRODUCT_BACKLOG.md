...
Seventy-three items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-088, BL-089, BL-095, BL-096, BL-099, BL-100, BL-101, BL-104, BL-106, BL-112, BL-115 and BL-116.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2726-2760` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2788-2803` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2734` and `src/js/main.js:2743-2748` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2766-2786` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2427-2483`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2491-2560` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2389-2407`, manual issue add at `src/js/main.js:2562-2606` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1547-1567`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:301` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:866-906` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2965-2999`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:2979-2981` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1948` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:295-330` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:543-572`, quoted into the Markdown export at `src/js/lib/markdown.js:180-182` | Done, shipped as BL-017 |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2136-2159` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:268-281` and `src/js/main.js:370-391` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3394-3414`. The work shipped since has
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3154-3176 |
...
| BL-105 | Derive the roadmap paragraph's status split in the counts gate | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Ready | scripts/check-counts.mjs:232-265 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Ready | PRODUCT_BACKLOG.md:3856-3859 |
...
| BL-055 | Record the drift in the audited figures instead of letting them go stale | Debt | EP-12 | Leaves alone | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:188-200 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1633-1660 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:810 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Ready | src/js/main.js:3333-3381 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Ready | src/js/lib/model.js:122-132 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Ready | src/js/main.js:499-502 |
...
| BL-062 | Delete the paragraph that BL-054's block states twice over | Debt | EP-12 | Leaves alone | 1 | 1 | 1 | 1 | 3.0 | none | Measured | Shipped | scripts/check-counts.mjs:331-361 |
...
| BL-070 | Print each citation's claim beside its line at bless time | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:539 |
...
| BL-071 | Bring the citations in code comments under the anchors gate | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 3 | 2.0 | none | Measured | Shipped | scripts/check-anchors.mjs:224 |
| BL-077 | Bring relative citations under the anchors gate, or stop writing them | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:314-329 |
| BL-078 | Print a first-time citation at bless time, since it has nothing to be compared against | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:1001 |
| BL-079 | Teach the gate the comment syntax of every file it already reads | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:303 |
| BL-080 | Pair a citation whose scope alone was renamed, rather than report a loss and an addition | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Observed | Shipped | scripts/check-anchors.mjs:1102 |
| BL-081 | Let the repetition check see a copy that is not next to its original | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-counts.mjs:388-426 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:246 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:1025 |
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:99 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3394-3408 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:552-560 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3394-3408 |
...
that function is at `src/js/main.js:2640` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2738`, and with the selector corrected the same mutation fails it. It is
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:591-602`; omitting it
...
`src/js/main.js:2979-2981` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:357` and load normalises
a stale id at `src/js/lib/model.js:855`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:777-778`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:552-560`, for the same reason
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:586` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:274-281` rather
...
`src/js/main.js:2040-2050`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:512` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:524` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:1889-1898`, rather than on a list of the
...
`src/js/main.js:1879-1887`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:305` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:314-352` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:286`, and both success paths clear that
...
`src/js/main.js:2925`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:376-388` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:319-322`: "in library" and "Continue reading" would both resolve to
...
making sense: `forgetDeleted()` at `src/js/main.js:1600-1603` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1623-1631` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2925`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2893-2904`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:370`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:393-397` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1646-1654`. The usual reason
...
`src/js/main.js:3102`, `src/js/main.js:3102` and `src/js/main.js:3118-3124`. Each replaces the whole
...
`src/js/main.js:322-333`. Moving a message into the modal makes it readable where the reader is
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:73-78`. Syncing anywhere else would have left the address naming
...
the id is unknown, at `src/js/lib/model.js:390-392`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1384-1438`, before the
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:622-672`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3006-3024`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:358-359`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:2572`, so the two kinds cannot be
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:512-519`, written the same way
`coerce` writes it at `src/js/lib/model.js:791`, so the two paths cannot disagree and every future
caller inherits it.
...
`src/js/main.js:404-428` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:429-442` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:3128-3129`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2040-2050`, drawing on the labels at
`src/js/main.js:2105-2110`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1401-1404`, and `renderRows` asks it at `src/js/main.js:1948`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1396-1400` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:246` reads the pair back out of
...
`src/js/main.js:1854` and the reading rows at `src/js/main.js:2096`, and the reasoning is recorded at
`src/js/main.js:201-205` rather than only
...
`src/js/main.js:1256` carries a comment at `src/js/main.js:1266` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2872`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1099` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2942` puts
...
`renderRail` at `src/js/main.js:866` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:840` and reaches `renderYours` through `renderHome` at
`src/js/main.js:842`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:3394` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:874`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1019` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2138`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1035`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:890` and `src/js/main.js:1216`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:3427`, and `src/js/app.js` is the entry
...
argument `scripts/check-anchors.mjs:192-195` makes about the anchors gate. The `design/mockups/`
...
`scripts/check-counts.mjs:331-361`, and `npm run counts` fails on a repeat. That remains true of this
...
already refuses a detail block that appears twice, at `scripts/check-counts.mjs:300-310`, which is
...
enumeration of where to look is the thing `scripts/check-anchors.mjs:192-195` argues against, having
...
built at `src/js/main.js:890` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1216`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2438`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1198`. That also retired a latent bug rather than fixing it. The old label said
...
`lists[k] = {...}` at `src/js/lib/model.js:810`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:838`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:855`.
...
`scripts/check-anchors.mjs:539` already slices the prose immediately preceding each citation into a
`claim` field, and the bless path at `scripts/check-anchors.mjs:1001` then writes only the anchor,
...
change. `reportNearMisses` at `scripts/check-anchors.mjs:867` is the precedent for the script
...
So outside Markdown only the backticked form is collected, at `scripts/check-anchors.mjs:347-359`.
...
argues in its own comment at `scripts/check-anchors.mjs:192-195` that an enumeration is exactly the
defect it exists to catch, and "skip the lock file" is an enumeration of one. It is excluded
structurally at `scripts/check-anchors.mjs:224`, by the same `LOCK` constant that says where the
output is written, so the rule is that the gate does not read its own output and nothing has to be
kept in step. Binary files are dropped the same way, by a NUL byte at `scripts/check-anchors.mjs:119`
...
comment, when it comes from the series name at `src/js/main.js:475-476`. Both are re-aimed, and the
...
`claimBefore` at `scripts/check-anchors.mjs:402-428` now strips the marker outside prose, stops at
...
line, at `scripts/check-anchors.mjs:848-863`, and it prints a notice rather than failing, because
...
The check runs before the bless path rather than beside it, at `scripts/check-anchors.mjs:905-939`,
...
`scripts/check-anchors.mjs:314-329`. That is not a nicety. This gate's own test builds its fixtures
...
definition, at `scripts/check-anchors.mjs:338-341`, and two tests hold the two halves apart, because
...
The evidence is the bless path itself at `scripts/check-anchors.mjs:1001`, which writes the anchor,
...
no pathspec at `scripts/check-anchors.mjs:214-216`, and binary content is dropped by the reader
...
syntax instead, at `scripts/check-anchors.mjs:323`, `scripts/check-anchors.mjs:433` and
`scripts/check-anchors.mjs:858`. A YAML comment opens with a hash, an HTML comment with an angle
...
exactly one of each, at `scripts/check-anchors.mjs:1134-1140`. The bucket carries the scope, at
`scripts/check-anchors.mjs:1102`, so a citation whose scope slug is renamed while its anchor and its
...
Delivered as `scopeRenames` at `scripts/check-anchors.mjs:704-750`, exported so a test can hold its
...
re-aim buckets are built, at `scripts/check-anchors.mjs:1113`, or a renamed pair could be the second
...
`scripts/check-anchors.mjs:1102` is now cited under two headings with the same ordinal and the same
...
`scripts/check-counts.mjs:347-353`, so it catches a block directly followed by its copy and nothing
...
`scripts/check-counts.mjs:388-426`, descending from the longest possible repeat down to a floor and
...
destructive one: `src/js/main.js:148-160` confirms Start fresh replaces the unreadable data with an
...
that boot report, and the comment now standing in its place at `src/js/main.js:3472-3476` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:117-136`, and until BL-075 took its
...
together at `src/js/main.js:66-79`, and behind it sit sixteen library modules, none of which holds
...
the client and leaves the limiter alone, at `src/js/main.js:3141-3143`, because the budget belongs to
...
`src/js/storage.js:9-12`, two more belong to the view file at `src/js/main.js:37-38`, and the
...
the one change callback at `src/js/main.js:73-78`, so nothing left on screen said why saving was
paused.
...
glyph that no one reads, since the button's label at `src/js/main.js:2024` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:7449-7451` uses that size as the argument for
...
figure as audited" at `PRODUCT_BACKLOG.md:198-200`. The clause is quoted only as far as its fixed
half. The live number beside it moves whenever a test is added, and pinning a copy of it into this
record would be the same defect in a second place, which is the rule BL-059 later had to state
outright. Appendix A does the same thing in its own idiom, correcting a miscount inside the
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:7468-7472`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:7283-7285` describes
...
sentence, which is the enumeration anti-pattern `scripts/check-anchors.mjs:192-195` argues against in
...
`CHANGELOG.md:1521-1530` that says so and points at the backlog clause instead. That clause was
checked before the entry was allowed to defer to it: `PRODUCT_BACKLOG.md:188-192` and
`PRODUCT_BACKLOG.md:198-200` do each carry a live value and are marked as needing re-derivation, so
...
`src/js/storage.js:495-571`, `src/js/main.js:3079-3107`.
...
proxy for it.
Evidence: `src/js/storage.js:426-450`, `src/js/main.js:81-92`.
...
attack. Evidence: `src/js/main.js:3079-3107`, `src/js/lib/model.js:782-891`.
...
`src/js/main.js:1025` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/lib/model.js:122-132`, `server.mjs:25-48`, `src/index.html:524-531`.
...
`src/js/main.js:1753` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2392` and `src/js/main.js:2408`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2501` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3333-3381`.
...
bitten by, and the reason is written into `scripts/check-anchors.mjs:192-195` for the same class of
defect.
...
button at `src/js/main.js:3154-3176` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3161-3175` repaints the banner
...
corpus, and not one of them is a citation: they are contrast ratios, the served origin quoted in
prose, and a container tag. Checking the name against the tracked list instead enrolls exactly two,
both of them the ignore file citations this item was raised for, and nothing else. That is the rule
the population itself is drawn under, which is that no list written inside the gate decides who is
in it, applied at the other end of the same regexes.
...
failed it for exactly the reason the collectors did. Scoping the new rule to that shape leaves it
...
there. A name carrying no dot at all stays unwatched, deliberately, because that is the shape all
105 false candidates take and a notice nobody reads is worse than none.

Seven tests, and the reverts that prove them name their tests exactly: removing the tracked-list
consultation fails the collection test and the drift test, removing the dot-named notice rule fails
both notice tests, and removing the notice's knowledge of what the collector already took fails the
one asserting silence. Three of the seven appear in none of those lists and cannot, because they
assert that a ratio, an origin and a citation with no list behind it stay out, so a revert removing
the widening can never fail them. They are guards on the widening rather than evidence for it, and
recording which is which is worth more here than the count of seven.
...
`scripts/check-counts.mjs:232-265`. It does not derive the second paragraph, which makes the same two
...
it on. Evidence: `src/js/main.js:499-502`, `src/js/main.js:505-506`, `src/styles.css:436`.
...
over, for the hero backdrop alone: `src/js/main.js:1753` reads the setting before building the URL,
...
button. Evidence: `src/js/main.js:2889`, `src/js/lib/model.js:116`,
`src/js/lib/model.js:674`.
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:2915-2922`,
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1025`
...
it was ever scored and so never in the table.
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4677-4685`.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2965-2999` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2709` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3394-3414` (renderAll rebuilds
every region), `src/js/main.js:73-78` (store.onChange is wired straight to renderAll),
`src/js/main.js:1920-2103` (renderRows, which then built every row with no virtualisation and no
...
`src/js/lib/model.js:860-891` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2105-2110`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1633-1660` (undo), `src/js/main.js:3118-3124` (undoRestore exists).
...
`src/js/lib/model.js:702-778` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3394-3414`, `src/js/main.js:825-842` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3126-3146`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:702-778`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2766-2786` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2709` and `src/js/main.js:2743-2748`. What is missing is data, in
