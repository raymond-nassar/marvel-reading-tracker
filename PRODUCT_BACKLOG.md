...
BL-053, BL-054, BL-055, BL-056, BL-057, BL-058, BL-059, BL-061, BL-062 and BL-063.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2244-2274` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:400-402`, filterByFacet imported at `src/js/main.js:17` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:393-399`, search-as-you-type at `src/js/main.js:2302-2317` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2248` and `src/js/main.js:2257-2262` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2280-2300` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1948-2001`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2009-2078` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1910-1928`, manual issue add at `src/js/main.js:2080-2124` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1251-1271`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:702-742` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2479-2513`, with a scope control at `src/index.html:360-364` and a subtitle that stops claiming every list at `src/js/main.js:2493-2495` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1554` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1606-1612`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1642-1647` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2103` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2609-2615` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1657-1680` | Done, shipped as BL-026 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1337-1364 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2479-2513 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2725-2739 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2725-2739 |
...
that function is at `src/js/main.js:2158` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2252`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2493-2495` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1602-1612`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2382`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1304-1307` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1327-1335` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2439`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2407-2418`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1350-1358`. The usual reason
...
`src/js/main.js:2602`, `src/js/main.js:2602` and `src/js/main.js:2609-2615`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1168-1221`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2520-2538`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2090`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2619-2620`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1602-1612`, drawing on the labels at
`src/js/main.js:1642-1647`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1185-1188`, and `renderRows` asks it at `src/js/main.js:1554`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1180-1184` exists because
...
`src/js/main.js:1633`, and the reasoning is recorded at `src/js/main.js:161-165` rather than only
...
`src/js/main.js:1076` carries a comment at `src/js/main.js:1086` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2386`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2456` puts
...
`src/js/main.js:2725` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1659`,
...
`src/js/main.js:726` and `src/js/main.js:1044`. Both predate this change and neither is part of its
...
on every catalog card, at `src/js/main.js:1044`, joined an issue count to an invitation to open the
...
at `src/js/main.js:1959`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1028`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3027-3028` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3047-3049`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2869-2871` describes
...
`CHANGELOG.md:330-333` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2479-2513` (renderProgress, which now takes a scope), `src/index.html:358`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2227` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2725-2740` (renderAll rebuilds
every region), `src/js/main.js:65-70` (store.onChange is wired straight to renderAll),
`src/js/main.js:1543-1640` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1642-1647`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1337-1364` (undo), `src/js/main.js:2609-2615` (undoRestore exists).
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2617-2637`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2280-2300` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2227` and `src/js/main.js:2257-2262`. What is missing is data, in
