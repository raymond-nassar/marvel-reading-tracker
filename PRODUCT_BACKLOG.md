...
Twenty-three items have since been delivered and are marked `Shipped` in the table below: BL-014,
BL-026, BL-027, BL-029, BL-030, BL-031, BL-034, BL-035, BL-037, BL-039, BL-040, BL-043, BL-044,
BL-045, BL-046, BL-047, BL-048, BL-049, BL-050, BL-051, BL-052, BL-054 and BL-055. Their detail
blocks record what changed, what was measured, and which tasks were deliberately left open, with
one exception: BL-050's block was never written, which is filed as BL-057. BL-049 is the one
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2131-2161` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:371-373`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:364-370`, search-as-you-type at `src/js/main.js:2189-2204` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2135` and `src/js/main.js:2144-2149` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2167-2187` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2114` and `src/js/main.js:2144-2149`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1835-1888`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1896-1965` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1797-1815`, manual issue add at `src/js/main.js:1967-2011` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1126-1146`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:635-660` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2366-2400`, with a scope control at `src/index.html:349-353` and a subtitle that stops claiming every list at `src/js/main.js:2380-2382` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1524-1534` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1481-1487`, pending filter at `src/js/main.js:1528` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1517-1522` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1990` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2445-2451` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1544-1567` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:200-213` and `src/js/main.js:302-323` | Done, shipped as BL-027 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:302-323 |
...
| BL-053 | Make the reading filters one list rather than two that must agree | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 2 | 2.0 | none | Observed | Ready | src/js/main.js:1524-1534 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2561-2573 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2561-2573 |
...
`src/js/main.js:2380-2382` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:481` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:206-213` rather
...
`src/js/main.js:1477-1487`, along with the pending badge's. A `title` is unreachable by touch and
...
outstanding notice is kept as a record at `src/js/main.js:237` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:246-284` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:218`, and both success paths clear that
...
`src/js/main.js:2269`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1179-1182` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1202-1210` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2326`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2294-2305`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:302`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:325-329` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1225-1233`. The usual reason
...
`src/js/main.js:2492`, `src/js/main.js:2438` and `src/js/main.js:2445-2451`. Each replaces the whole
...
`src/js/main.js:254-265`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1062-1096`, before the
...
the code says so: `matchesFilter` at `src/js/main.js:1524-1534` still enumerates the five and
...
`src/js/main.js:336-360` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:362-375` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:2455-2456`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1477-1487`, drawing on the labels at
`src/js/main.js:1517-1522`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1500-1505` shows it plainly: at
...
honoured only when a radio offers it. `matchesFilter` at `src/js/main.js:1524-1534` still
...
control the two lists build, and `preservingFocus` at `src/js/main.js:159` reads the pair back out of
...
`src/js/main.js:1508`, and the reasoning is recorded at `src/js/main.js:154-158` rather than only
...
for one of the twenty-three ids they govern until this change gave each the exception clause it
carries.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2366-2400` (renderProgress, which now takes a scope), `src/index.html:347`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2114` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2561-2574` (renderAll rebuilds
every region), `src/js/main.js:62-67` (store.onChange is wired straight to renderAll),
`src/js/main.js:1418-1515` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1517-1522`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1212-1239` (undo), `src/js/main.js:2445-2451` (undoRestore exists).
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2453-2473`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2167-2187` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2114` and `src/js/main.js:2144-2149`. What is missing is data, in
