...
BL-145, BL-146, BL-147, BL-149, BL-152, BL-153 and BL-154.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3280-3341` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3402-3417` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3304` and `src/js/main.js:3322-3339` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3380-3400` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2961-3017`, `src/js/lib/markdown.js:61-140` | Done |
...
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2923-2941`, manual issue add at `src/js/main.js:3096-3156` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3584-3618`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3598-3600` | Done, shipped as BL-014 |
...
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:674-703`, quoted into the Markdown export at `src/js/lib/markdown.js:205-207` | Done, shipped as BL-017 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3126` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3811-3817` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4161-4181`. The work shipped since has
  taken it further: the file is 4,269 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,148; 224 is the figure as audited.
...
| BL-151 | Carry a pasted reader address through export and back in again | Defect | EP-03 | Follows BL-149 | 3 | 2 | 2 | 1 | 7.0 | none | Measured | Ready | src/js/lib/markdown.js:131-136 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3860-3897 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3772-3800 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4134-4137 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4081-4144 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3541 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3584-3618 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3294-3299 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3359-3374` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4161-4175 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4161-4175 |
...
that function is at `src/js/main.js:3190` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3328`, and with the selector corrected the
...
`src/js/main.js:3598-3600` instead of asserting "every list" whatever is being counted.
...
Notes export to Markdown quoted with `> `, at `src/js/lib/markdown.js:205-207`, and the prefix is
...
`src/js/main.js:3544`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3544`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3513-3524`, leaving nothing behind and the offer still
...
`src/js/main.js:3871`, `src/js/main.js:3795` and `src/js/main.js:3811-3817`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3625-3643`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:3106`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3822-3823`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3486`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3561` puts
...
`src/js/main.js:4161` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4194`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4241-4245` records
...
file of 4,269 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3834-3836`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11006-11008` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11025-11029`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10840-10842` describes
...
`CHANGELOG.md:2739-2748` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3772-3800`.
...
`test/markdown.test.js:181`, and was measured and left alone: it guards a nested quantifier against
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3294-3299`,
...
attack. Evidence: `src/js/main.js:3772-3800`, `src/js/lib/model.js:965-1074`.
...
`src/index.html:125-126`, `src/js/main.js:4081-4144`.
...
button at `src/js/main.js:3860-3897` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3867-3896` repaints the banner
...
button. Evidence: `src/js/main.js:3503-3509`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3535-3541`,
...
here. Evidence: `src/js/main.js:3550`.
...
in place of that button while a copy is live at `src/js/main.js:3984-3990`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:3871`, `src/js/main.js:3795` and `src/js/main.js:3816`
...
**Delivered.** The first citation now names `src/js/main.js:3871`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3860-3897`. The other two were already right and
are unchanged: `src/js/main.js:3795` is the call in the successful branch of the restore handler,
and `src/js/main.js:3811-3817` is the whole undo-restore handler, whose call is at
`src/js/main.js:3816`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3544` is its one call, `src/js/main.js:3513-3524` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4134-4137`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4116-4130` keeps
...
Checked by removing the pieces rather than by running the whole thing. Withdrawing the parser fails
both new parser tests. Blanking the stored field, one line in the normalizer, fails exactly one
test out of 1,120, and it is the one that walks the three steps the form walks: parse the address,
store the issue, build the launch URL. That is the measurement worth keeping, because it names the
line the test defends.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3584-3618` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3329` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4161-4181` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1969-1996` (undo), `src/js/main.js:3811-3817` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4161-4181`, `src/js/main.js:966-983` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,269 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3819-3852`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1043-1074`, `src/js/main.js:3811-3817`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3380-3400` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3329` and `src/js/main.js:3322-3339`. What is missing is data, in
