...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2065-2095` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:371-373`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:364-370`, search-as-you-type at `src/js/main.js:2123-2138` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2069` and `src/js/main.js:2078-2083` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2101-2121` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2048` and `src/js/main.js:2078-2083`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1769-1822`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1830-1899` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1731-1749`, manual issue add at `src/js/main.js:1901-1945` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1082-1102`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:591-616` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2300-2334`, with a scope control at `src/index.html:349-353` and a subtitle that stops claiming every list at `src/js/main.js:2314-2316` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1458-1468` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1423-1429`, pending filter at `src/js/main.js:1462` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1451-1456` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1924` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2379-2385` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1478-1501` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:156-169` and `src/js/main.js:258-279` | Done, shipped as BL-027 |
...
| BL-055 | Record the drift in the audited figures instead of letting them go stale | Debt | EP-12 | Leaves alone | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:149-151 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:258-279 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2300-2334 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:1331 |
...
| BL-053 | Make the reading filters one list rather than two that must agree | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 2 | 2.0 | none | Observed | Ready | src/js/main.js:1458-1468 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2495-2507 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2495-2507 |
...
`src/index.html:349-353`, matching the reading filter's fieldset pattern rather than inventing a
second idiom, and the subtitle at `src/index.html:347` is now written by the render at
`src/js/main.js:2314-2316` instead of asserting "every list" whatever is being counted.
...
The About view carries the shortcut reference at `src/index.html:555-562`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:437` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:162-169` rather
...
`src/js/main.js:1419-1429`, along with the pending badge's. A `title` is unreachable by touch and
...
outstanding notice is kept as a record at `src/js/main.js:193` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:202-240` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:174`, and both success paths clear that
...
`src/js/main.js:2203`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1135-1138` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1158-1166` runs when the deleted list's own order is imported again. An offer the reader
...
rolls the half-import back at `src/js/main.js:2228-2239`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:258`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:281-285` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1181-1189`. The usual reason
...
`src/js/main.js:2426`, `src/js/main.js:2372` and `src/js/main.js:2379-2385`. Each replaces the whole
...
`src/js/main.js:210-221`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1018-1052`, before the
...
**What counts as a filter is the radios, not a list in the code.** A stored value is honoured only
when a radio offers it, which is why an unrecognised one falls back to All without the restore path
enumerating the five. An enumeration there would be a list someone has to keep in step with
`src/index.html:332-336`, and a filter dropped from that markup could otherwise leave the module
holding a value nothing on screen can select or clear. The coupling is reduced rather than gone, and
the code says so: `matchesFilter` at `src/js/main.js:1458-1468` still enumerates the five and
returns true for anything else, so a radio added to the markup and not to that function would be
honoured, stored and checked while filtering nothing. Single-sourcing both paths was not attempted
here, because it is a change to how filtering works rather than to whether the choice survives a
reload. It is filed as BL-053 rather than left in this block, which is a closed record once the
item ships.
...
`src/js/main.js:292-316` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:318-331` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:2389-2390`, unchanged.
...
two at `src/index.html:469` and `src/index.html:476` are direct children of a `.card card-static`,
...
is written out in words inside the pill at `src/js/main.js:1419-1429`, drawing on the labels at
`src/js/main.js:1451-1456`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1442-1447` shows it plainly: at
...
- [ ] Give `matchesFilter` a way to answer for a filter it was not written with, or derive both paths from one list
- [ ] Make a filter present in one place and missing from the other fail loudly rather than silently pass everything
- [ ] Confirm the five filters that exist today behave identically afterwards
...
Filed out of the BL-037 review rather than fixed there. BL-037 made the radios in
`src/index.html:332-336` the list of valid filters for the restore path, so a stored value is
honoured only when a radio offers it. `matchesFilter` at `src/js/main.js:1458-1468` still
enumerates the same five and returns true for anything else, so the coupling was reduced rather
than removed and the failure simply changed direction: a radio added to the markup and not to that
function is now honoured, stored and checked while filtering nothing, and the reader sees a filter
selected against an unfiltered list with nothing failing.
...
- [ ] Keep the focused control identifiable across a rebuild, or move focus somewhere deliberate
- [ ] Cover the click route and the keyboard route, which reach the same rebuild
- [ ] Confirm a reader is not returned to the top of the document every time an issue is marked read

Constraint gate: checked 1 to 11, none breached.

Filed out of the BL-026 review rather than fixed there. `renderShelf` at `src/js/main.js:1331` and
`renderRows` at `src/js/main.js:1363` both hand `replaceChildren` a freshly built set of nodes, so
every `store.update` discards whatever was focused and focus falls to the body. Measured in Edge by
clicking a checkbox in the full order and reading `document.activeElement` immediately afterwards:
`BODY`. The hero escapes this because its buttons are static markup the re-render leaves in place,
which is why the focus work in BL-026 stopped where it did.

Not folded into BL-026 because the click route already behaved this way and was not touched by that
change; BL-026 only added a keyboard route into the same rebuild. Fixing it means deciding what
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:1990-1992` uses that size as the argument for
the gap, which made the understated figure an understatement of the debt.

The obvious fix would have been to overwrite 1,566 with 2,563 everywhere. That is wrong here,
because this document already has a convention for the case and applies it one bullet below: the
audited figure is preserved and the drift is recorded beside it, "the items shipped in this pass
have since taken it to 285; 224 is the figure as audited" at `PRODUCT_BACKLOG.md:156-158`. Appendix A
does the same thing in its own idiom, correcting a miscount inside the `Resolved:` line rather than
editing the bullet it resolves, at `PRODUCT_BACKLOG.md:2007-2009`. Overwriting would have destroyed
the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:1836-1838` is a
past-tense account of the state that motivated OC-3, and the same sentence says there is no linter
and no changelog, both of which have since shipped; correcting the number alone would leave a
coherent snapshot half-updated and half-stale, which is worse than either. It is left as a snapshot,
which is the treatment the as-is journey map in the UX study already gets.
...
table, rather than by hand. Adding one row moved BL-026 from twelfth to thirteenth and BL-007 from
twenty-seventh to twenty-eighth, and both are stated as facts in Appendix B, so both were corrected.

Not closed by this item: nothing enforces the convention. The anchors gate fingerprints the lines a
citation names, so it will keep reporting these three sentences as sound however wrong their numbers
become, which is exactly how the figure survived twelve shipped items. What this item leaves behind
is a written instruction and a worked example, not a gate. A gate would have to know which numbers in
prose are derived and from what, and that is a larger piece of work than correcting the figures; it
is not filed as its own item here, because filing one shifts every rank in the table that Appendix B
states as a fact, and doing that twice in one change would obscure the corrections this item exists
to make.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2300-2334` (renderProgress, which now takes a scope), `src/index.html:347`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2048` (groupCatalog renders variant rows only where a group exists).
- Correctness is well defended: 224 unit tests pass, 235 when this pass shipped and 285 now, and
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2495-2508` (renderAll rebuilds
every region), `src/js/main.js:62-67` (store.onChange is wired straight to renderAll),
`src/js/main.js:1363-1449` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1451-1456`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1168-1195` (undo), `src/js/main.js:2379-2385` (undoRestore exists).
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2387-2407`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2101-2121` (attribution rendered in the UI before import). |
...
open when the pass ran, so the table was 28 rows then. BL-028 has since been parked and five
further items filed, none of them labelled, which is how it reaches 32 rows now. The ranks below are
positions in it as it stands.

### Case 1: BL-026 is labelled P0 but ranks thirteenth

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 13 of 32.
- Driver: job size, not value. Its Cost of Delay of 11 is the fourth highest figure in the backlog.
  It is outranked by twelve items sized 1, 2 or 3 whose Cost of Delay is lower but whose size is
...
- What a human should settle: whether "Foundation" here means "must be finished before anything
  ...
  regardless of the score. If the latter, the score's ordering is fine, because the twelve items
...
### Case 2: BL-007 is labelled P1 but ranks twenty-eighth

- Stated: P1 Core product value, event order variants.
- Calculated: WSJF 1.4, rank 28 of 32, below twenty-four unlabelled items and three places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2048` and `src/js/main.js:2078-2083`. What is missing is data, in
...
- BL-014, P1, rank 18 of 32. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 14 of 32. Mid-table.
- BL-017, P2, rank 31 of 32. The lowest-ranked scored story other than the one that cannot be
