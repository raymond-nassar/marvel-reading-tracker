...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2139-2169` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:370-372`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:363-369`, search-as-you-type at `src/js/main.js:2197-2212` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2143` and `src/js/main.js:2152-2157` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2175-2195` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2122` and `src/js/main.js:2152-2157`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1843-1896`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1904-1973` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1805-1823`, manual issue add at `src/js/main.js:1975-2019` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1146-1166`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:636-661` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2374-2408`, with a scope control at `src/index.html:348-352` and a subtitle that stops claiming every list at `src/js/main.js:2388-2390` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1449` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1501-1507`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1537-1542` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1998` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2453-2459` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1552-1575` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:201-214` and `src/js/main.js:303-324` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2569-2582`. The work shipped since has
  taken it to 2,637; 1,566 is the figure as audited.
...
| BL-059 | Stop the changelog entry that explains stale figures from carrying two of its own | Debt | EP-12 | Leaves alone | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Ready | CHANGELOG.md:104-105 |
...
| BL-056 | Fail the build when a derived count in the backlog disagrees with the table it is derived from | Enabler | EP-12 | Leaves alone | 3 | 1 | 5 | 2 | 4.5 | none | Measured | Ready | PRODUCT_BACKLOG.md:2486-2488 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:303-324 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2374-2408 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:160 |
| BL-058 | Keep focus on the home grid and the rail when their lists rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:2281 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2569-2581 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2569-2581 |
...
`src/js/main.js:2388-2390` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:482` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:207-214` rather
...
`src/js/main.js:1497-1507`, along with the pending badge's. A `title` is unreachable by touch and
...
outstanding notice is kept as a record at `src/js/main.js:238` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:247-285` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:219`, and both success paths clear that
...
`src/js/main.js:2277`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1199-1202` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1222-1230` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2334`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2302-2313`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:303`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:326-330` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1245-1253`. The usual reason
...
`src/js/main.js:2500`, `src/js/main.js:2446` and `src/js/main.js:2453-2459`. Each replaces the whole
...
`src/js/main.js:255-266`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1063-1116`, before the
...
`src/js/main.js:337-361` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:363-376` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:2463-2464`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1497-1507`, drawing on the labels at
`src/js/main.js:1537-1542`, and the full sentence from `describe` sits in a visually-hidden span
...
which is why the ledger above still counts six open tasks and not seven. Counts stated in prose are
the same problem with no gate behind them, filed as BL-056. Citations inside a preserved historical
document are enrolled when they should be exempt, filed as BL-060. The limits described at the end of
...
`src/js/main.js:1080-1083`, and `renderRows` asks it at `src/js/main.js:1449`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1075-1079` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:160` reads the pair back out of
...
`src/js/main.js:1528`, and the reasoning is recorded at `src/js/main.js:155-159` rather than only
...
`src/js/main.js:980` carries a comment saying adding must not move the reader, and the home grid is
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2281`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:852` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2351` puts
...
`renderRail` at `src/js/main.js:636` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:617` and reaches `renderYours` through `renderHome` at
`src/js/main.js:619`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:2569` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:638`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:780` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1554`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:793`. There is no route that rebuilds it while it holds focus, so it is
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:2343-2345` uses that size as the argument for
...
editing the bullet it resolves, at `PRODUCT_BACKLOG.md:2362-2364`. Overwriting would have destroyed
the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2189-2191` describes
...
to that same run of prose. The commit that added the lock schema guard says in its message that it
...
than carried as open tasks here, so the ledger's count of six open tasks is unchanged by this item.
...
quoting what they are now, at `CHANGELOG.md:104-105`, which puts two live numbers inside a record
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2374-2408` (renderProgress, which now takes a scope), `src/index.html:346`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2122` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2569-2582` (renderAll rebuilds
every region), `src/js/main.js:63-68` (store.onChange is wired straight to renderAll),
`src/js/main.js:1438-1535` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1537-1542`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1232-1259` (undo), `src/js/main.js:2453-2459` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2569-2582`, `src/js/main.js:604-632` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 2,637 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2461-2481`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2175-2195` (attribution rendered in the UI before import). |
...
- Calculated: WSJF 3.67, rank 16 of 36.
- Driver: job size, not value. Its Cost of Delay of 11 is the fourth highest figure in the backlog.
  It is outranked by fifteen items sized 1, 2 or 3 whose Cost of Delay is lower but whose size is
...
- What a human should settle: whether "Foundation" here means "must be finished before anything
  ...
  regardless of the score. If the latter, the score's ordering is fine, because the fifteen items
...
### Case 2: BL-007 is labelled P1 but ranks thirty-second

- Stated: P1 Core product value, event order variants.
- Calculated: WSJF 1.4, rank 32 of 36, below twenty-eight unlabelled items and three places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2122` and `src/js/main.js:2152-2157`. What is missing is data, in
...
- BL-014, P1, rank 21 of 36. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 17 of 36. Mid-table.
- BL-017, P2, rank 35 of 36. The lowest-ranked scored story other than the one that cannot be
