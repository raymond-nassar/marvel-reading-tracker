...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1980-2010` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:366-368`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:359-365`, search-as-you-type at `src/js/main.js:2038-2053` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1984` and `src/js/main.js:1993-1998` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2016-2036` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1963` and `src/js/main.js:1993-1998`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1684-1737`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1745-1814` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1646-1664`, manual issue add at `src/js/main.js:1816-1860` | Done |
...
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:2200-2218`, and the view states it counts across every list at `src/index.html:346-348` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1381-1391` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1346-1352`, pending filter at `src/js/main.js:1385` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1374-1379` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1839` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2263-2269` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2372-2385`.
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2372-2384 |
...
`src/js/main.js:1342-1352`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2118`. The same order can be added from the landing page and imported from the
...
`src/js/main.js:1091-1099` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2175`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2143-2154`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1114-1122`. The usual reason
...
`src/js/main.js:2303`, `src/js/main.js:2256` and `src/js/main.js:2263-2269`. Each replaces the whole
...
is written out in words inside the pill at `src/js/main.js:1342-1352`, drawing on the labels at
`src/js/main.js:1374-1379`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1365-1370` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2200-2218` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1963` (groupCatalog renders variant rows only where a group exists).
...
`src/js/main.js:1286-1372` (renderRows builds every row with no virtualisation and no early exit when
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1101-1128` (undo), `src/js/main.js:2263-2269` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2372-2385`, `src/js/main.js:523-551` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2271-2284`, `src/js/lib/cachePolicy.js:16-20`,
...
| Observability | Partial gap, bounded by Repository Constraint 3. Product analytics are forbidden and are not proposed. What is missing is local and private: there is no way for the reader to see why hydration stalled beyond a queue-depth pill. Evidence: `src/js/main.js:2349-2351`. |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1963` and `src/js/main.js:1993-1998`. What is missing is data, in
