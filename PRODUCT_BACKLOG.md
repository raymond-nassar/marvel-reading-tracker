...
A hundred and thirty-seven items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
| 1.3 | BL-003 | P1 | search form at `src/index.html:450-456`, search-as-you-type at `src/js/main.js:3938-3953` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3916-3936` | Done |
...
| 2.4 | BL-008 | P1 | orders and catalog generated by `scripts/vendor-orders.mjs:197-322`, consumed as data with no view change | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:4120-4167`, with a scope control at `src/index.html:417-421` and a subtitle that stops claiming every list at `src/js/main.js:4134-4136` | Done, shipped as BL-014 |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:4394-4400` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4744-4764`. The work shipped since has
  taken it further: the file is 5,030 lines today. 1,566 is the figure as audited.
...
| BL-128 | Correct the provenance record's description count and record its placeholder shape | Debt | EP-12 | Extends BL-099 | 5 | 5 | 3 | 1 | 13.0 | none | Measured | Shipped | docs/DATA_PROVENANCE.md:91-110 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4443-4480 |
...
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:4355-4383 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:4040-4042 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1529-1530 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4717-4720 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4664-4727 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4077 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:4120-4167 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4744-4758 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4744-4758 |
...
Nothing in the app changed. `groupCatalog` at `src/js/lib/catalog.js:412` was already a general
...
`src/js/main.js:4134-4136` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:4080`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:4080`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:4049-4060`, leaving nothing behind and the offer still
...
`src/js/main.js:4454`, `src/js/main.js:4378` and `src/js/main.js:4394-4400`. Each replaces the whole
...
in `src/js/lib/library.js:28-77` and rendered by a single function at `src/js/main.js:4174-4211`.
...
The settings form keeps its own check and its own message at `src/js/main.js:4405-4406`, unchanged.
...
`src/js/lib/catalog.js:311-321` takes the opposite decision for the catalog, and the difference is
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:4022`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:4097` puts
...
`src/js/main.js:4744` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4777`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4826-4830` records
...
file of 5,030 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:4417-4419`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:12375-12377` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:12396-12398`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:12147-12149` describes
...
`CHANGELOG.md:3166-3175` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:4355-4383`.
...
attack. Evidence: `src/js/main.js:4355-4383`, `src/js/lib/model.js:968-1077`.
...
`src/index.html:125-126`, `src/js/main.js:4664-4727`.
...
size, at `PRODUCT_BACKLOG.md:2625`, and one of the four tests added to the counts suite pins the
...
`src/data/curated-lists.json:82-123`, `src/data/new_ultimate_universe_trades.json:1-10`.
...
button at `src/js/main.js:4443-4480` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4450-4479` repaints the banner
...
`docs/DATA_PROVENANCE.md:119-124` files the order under "Compiled for this project". Review found the
...
button. Evidence: `src/js/main.js:4039-4045`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:4071-4077`,
`scripts/vendor-orders.mjs:222-268`, `scripts/vendor-orders.mjs:321`.
...
here. Evidence: `src/js/main.js:4086`.
...
in place of that button while a copy is live at `src/js/main.js:4567-4573`, which is the state a
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4863-4871`.
...
**Delivered.** The first citation now names `src/js/main.js:4454`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4443-4480`. The other two were already right and
are unchanged: `src/js/main.js:4378` is the call in the successful branch of the restore handler,
and `src/js/main.js:4394-4400` is the whole undo-restore handler, whose call is at
`src/js/main.js:4399`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:4080` is its one call, `src/js/main.js:4049-4060` is the
...
at `PRODUCT_BACKLOG.md:4868-4871`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4863-4866` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4717-4720`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4699-4713` keeps
...
`scripts/vendor-orders.mjs:253-259`.
...
two readings had to win: `scripts/vendor-orders.mjs:306-308` warns that a repeated id will collapse on
...
`src/js/lib/catalog.js:349-355`. Uncanny #129-138 is a story arc, not a book anyone can buy, so
...
check without awaiting it, and `src/js/main.js:4777-4798` shows that the boot path continues into the
...
at `scripts/browser-check.mjs:1642-1649`, and the update scenario at
`scripts/browser-check.mjs:1275-1341` verifies the default current-release response, the explicit
...
`scripts/browser-check.mjs:530-586`: one makes the default answer newer, one suppresses a newer
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:4120-4167` (renderProgress, which now takes a scope), `src/index.html:415`
  (the view's subtitle).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4744-4764` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:2304-2333` (undo), `src/js/main.js:4394-4400` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4744-4764`, `src/js/main.js:1201-1218` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the   file is 5,030 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4402-4435`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1046-1077`, `src/js/main.js:4394-4400`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3916-3936` (attribution rendered in the UI before import). |
...
### Case 1: BL-026 is labelled P0 but ranks eighty-sixth

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 86 of 158.
...
- Calculated: WSJF 1.4, rank 152 of 158, below a hundred and forty-eight unlabelled items and five places above the
  single P2 story.
...
- BL-014, P1, rank 108 of 158. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 88 of 158. Mid-table.
- BL-017, P2, rank 157 of 158. The lowest-ranked scored story other than the one that cannot be
