...
A hundred and thirty-nine items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-183, BL-184, BL-185 and BL-186.
...
remote rather than on the tree, for the reasons recorded in their blocks. Eight remain open on
purpose: confirming BL-027 with a screen reader is a human check no automated run substitutes for,
BL-090's fourth task waits on that same run rather than on a second one, BL-031's axe re-run cannot
be satisfied as written, because axe declines to judge text over a gradient and the finding was
answered by computing the contrast bound instead,
BL-065's third task was answered by measurement rather than ticked, because no colour raises that
pair without reversing which end of a progress bar looks fuller, BL-033's rail task was answered by
measuring the rail at 9 nodes and 2 of them churning on a read toggle, which is not what that item
was raised about, BL-051's README walkthrough has to be done by someone who does not write software,
which is the one thing its author cannot self-certify, BL-130's provenance record waits on an
upstream answer that has been asked for and not yet given, and BL-099's legal review is a judgement
no document can supply, least of all the document written to ask for it. `CHANGELOG.md` carries the
...
open-source readiness study. One of them is still `Ready`: the study records gaps for later
implementation and does not mix those fixes into the roadmap change that identified them.
BL-083, BL-084, BL-085, BL-086, BL-087, BL-088, BL-089, BL-090, BL-093, BL-094, BL-095, BL-096,
BL-097, BL-098, BL-099 and BL-100 have since been delivered, and their detail blocks record what
changed.
BL-091 is the one the study got wrong: re-measuring it refuted its premise, so it is `Dropped` and
its block records the measurement rather than the fix.
...
rather than the question it was asked. All three have since shipped, and none of them needed a new
data source.

BL-113 came from the review of BL-101 and was the narrower half of the same question, left open on
purpose: erasing everything clears both restore-family keys, and the salvage copies it does not
reach are governed by a deliberate rule that only the reader removes one. Which promise wins was a
product decision rather than a defect to fix inside the change that raised it, and it has since
been answered in favour of the rule. The route was not widened; the wording was narrowed to stop
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3816-3877` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:457-459`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:450-456`, search-as-you-type at `src/js/main.js:3942-3957` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3840` and `src/js/main.js:3858-3875` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3916-3940` | Done |
| 2.3 | BL-007 | P1 | three of the five events now ship a main-series variant beside the complete order, grouped in `src/data/catalog.json`; the two whose main series does not open their order are refused by `scripts/build-event-order.mjs:443-453` and say so | Done, shipped as BL-007 |
| 2.4 | BL-008 | P1 | orders and catalog generated by `scripts/vendor-orders.mjs:198-324`, consumed as data with no view change | Done |
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:3317-3373`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3381-3450` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:3279-3297`, manual issue add at `src/js/main.js:3566-3686` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:2193-2213`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:433` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1250-1290` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:4124-4171`, with a scope control at `src/index.html:417-421` and a subtitle that stops claiming every list at `src/js/main.js:4138-4140` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2689` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:340-375` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:675-704`, quoted into the Markdown export at `src/js/lib/markdown.js:214-216` | Done, shipped as BL-017 |
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2785-2786`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2841-2846` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3643` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:1046-1077` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:4398-4404` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2872-2895` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:346-359` and `src/js/main.js:497-518` | Done, shipped as BL-027 |
| 7.3 | BL-028 | P1 | the mobile rail rule at `src/styles.css:229-232` is overridden by `src/styles.css:236-240`, and the reading view overflows by 93 px at 320 px per `docs/ux-artifacts/viewport-sweep-reading.json` | Dropped, product decision |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4748-4768`. The work shipped since has
  taken it further: the file is 5,034 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,263; 224 is the figure as audited.
...
| BL-128 | Correct the provenance record's description count and record its placeholder shape | Debt | EP-12 | Extends BL-099 | 5 | 5 | 3 | 1 | 13.0 | none | Measured | Shipped | docs/DATA_PROVENANCE.md:97-116 |
...
| BL-165 | Let the landing page use the desktop it is given, without stretching prose | Story | EP-01 | Follows BL-154 | 8 | 5 | 5 | 2 | 9.0 | none | Measured | Shipped | src/styles.css:412-421 |
| BL-169 | Make the reading screen say where you are and use the desktop it is given | Story | EP-01 | Follows BL-165 | 8 | 5 | 5 | 2 | 9.0 | none | Observed | Shipped | absent: any on-screen figure for how much of an order is read, the count sitting in a tooltip on the ring instead, and the eighth upcoming issue clipped off the shelf |
| BL-170 | Give the library and progress views a summary before their rows | Story | EP-01 | Follows BL-169 | 8 | 5 | 5 | 2 | 9.0 | none | Measured | Shipped | absent: any figure at all above the rows, so the only way to learn how much had been read was to count the list by eye, no heading anywhere in the three views, no cover on any library row, and the same 876px column at every screen width |
...
| BL-153 | Stop collapsing the sidebar from blanking every pill in the catalog | Defect | EP-08 | Follows BL-152 | 3 | 2 | 2 | 1 | 7.0 | none | Measured | Shipped | `src/styles.css:363-364` |
...
| BL-099 | Clarify the license and provenance boundary for committed data | Debt | EP-12 | Leaves alone | 8 | 8 | 13 | 5 | 5.8 | none | Measured | Shipped | src/data/curated-lists.json:512-553 |
| BL-181 | Publish the first guarded modern Marvel event batch | Feature | EP-03 | Follows BL-165 | 8 | 8 | 13 | 5 | 5.8 | none | Measured | Shipped | absent: any bundled order produced from the maintained modern continuity intake before this batch |
| BL-182 | Publish the second guarded modern Marvel event batch | Feature | EP-03 | Follows BL-181 | 8 | 8 | 13 | 5 | 5.8 | none | Measured | Shipped | absent: any bundled order closing the early continuity gaps before this batch |
| BL-185 | Publish the third guarded modern Marvel event batch | Feature | EP-03 | Follows BL-182 | 8 | 8 | 13 | 5 | 5.8 | none | Measured | Shipped | absent: any bundled order for the approved batch-three event and aftermath sections before this batch |
| BL-186 | Publish the fourth guarded modern Marvel event batch | Feature | EP-03 | Follows BL-185 | 8 | 8 | 13 | 5 | 5.8 | none | Measured | Shipped | absent: any bundled order for the approved batch-four event sections before this batch |
| BL-087 | State the network privacy boundary where the promise appears | Debt | EP-05 | Leaves alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/index.html:569 |
| BL-091 | Let catalog descriptions survive the WCAG text-spacing override | Defect | EP-07 | Leaves BL-028 alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Dropped | src/styles.css:999-1002 |
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4447-4484 |
...
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:4359-4387 |
...
| BL-183 | Keep dated evidence anchored to the repository state it recorded | Enabler | EP-12 | Extends BL-070 | 3 | 5 | 8 | 3 | 5.33 | none | Measured | Shipped | `scripts/check-anchors.mjs:296-403` |
...
| BL-138 | Bring the rest of the synopsis browser evidence into the committed check | Enabler | EP-12 | Follows BL-134 | 3 | 2 | 5 | 2 | 5.0 | none | Measured | Ready | scripts/browser-check.mjs:3-7 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:4042-4044 |
...
| BL-154 | Shelve the events apart from the character runs, and say why the runs sit last | Story | EP-01 | Follows BL-152 | 5 | 3 | 2 | 2 | 5.0 | none | Measured | Shipped | `src/js/lib/catalog.js:594-597` |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1531-1532 |
...
| BL-055 | Record the drift in the audited figures instead of letting them go stale | Debt | EP-12 | Leaves alone | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:203-215 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4721-4724 |
| BL-157 | Show the catalog in the README instead of only describing it | Story | EP-01 | Follows BL-154 | 3 | 1 | 1 | 1 | 5.0 | none | Observed | Shipped | absent: any picture of the app in the README, read of the whole file before this change |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:2304-2333 |
| BL-047 | Split the two meanings of the row class | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Observed | Shipped | src/styles.css:771-787 |
| BL-049 | Decide whether the faint badge borders need to meet the 3:1 non-text minimum | Defect | EP-08 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Measured | Shipped | src/styles.css:720 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:996 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4668-4731 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4081 |
...
| BL-097 | Publish contribution, conduct, support and maintainer governance | Chore | EP-12 | Extends BL-052 | 3 | 3 | 5 | 3 | 3.67 | none | Observed | Shipped | CONTRIBUTING.md:1-213 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:4124-4171 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:1174 |
| BL-073 | Say the recovery instructions once instead of twice | Debt | EP-11 | Leaves alone | 2 | 2 | 2 | 2 | 3.0 | none | Observed | Shipped | src/index.html:151-152 |
...
| BL-071 | Bring the citations in code comments under the anchors gate | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 3 | 2.0 | none | Measured | Shipped | scripts/check-anchors.mjs:261 |
| BL-077 | Bring relative citations under the anchors gate, or stop writing them | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:488-503 |
| BL-078 | Print a first-time citation at bless time, since it has nothing to be compared against | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:1442 |
| BL-079 | Teach the gate the comment syntax of every file it already reads | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-anchors.mjs:477 |
| BL-080 | Pair a citation whose scope alone was renamed, rather than report a loss and an addition | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Observed | Shipped | scripts/check-anchors.mjs:1547 |
...
| BL-126 | Record what a vendoring lookup was told, rather than one empty item for every failure | Debt | EP-05 | Follows BL-109 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | scripts/lib/lookup-issues.mjs:49 |
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:655 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:115 |
...
| BL-144 | Shelve one card per story, and let the reader pick a reading path inside it | Story | EP-01 | Follows BL-143 | 8 | 2 | 3 | 5 | 2.6 | none | Measured | Shipped | absent: any read of the group fields by the home grid, read of all 26 entries in src/data/curated-lists.json and of renderHomeCatalog in src/js/main.js |
| BL-142 | Add twelve reading orders compiled from Comic Book Herald guides | Feature | EP-01 | Follows BL-127 | 8 | 2 | 2 | 5 | 2.4 | none | Measured | Shipped | absent: any Captain America, Doctor Doom, Spider-Man, Thor, Scarlet Witch or Avengers order from src/data/curated-lists.json, read of all 14 entries in it |
...
| BL-117 | Give the ceiling timings a budget that survives a loaded machine | Debt | EP-12 | Leaves BL-085 alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | test/backup-bounds.test.js:294 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3830-3835 |
...
| BL-102 | Send the security headers on the dev server's error responses too | Debt | EP-12 | Follows BL-096 | 1 | 1 | 2 | 2 | 2.0 | none | Measured | Ready | server.mjs:103-113 |
| BL-046 | Share the retry and backoff between the two vendor scripts | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 2 | 2.0 | none | Observed | Shipped | scripts/lib/fetch-json.mjs:72-81 |
| BL-053 | Make the reading filters one list rather than two that must agree | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 2 | 2.0 | none | Observed | Shipped | src/js/lib/readingFilters.js:25-50 |
| BL-067 | Gate the switch and the primary button, which no pair measures | Debt | EP-08 | Leaves alone | 2 | 2 | 2 | 3 | 2.0 | none | Measured | Shipped | src/styles.css:452 |
| BL-173 | Show a cover thumbnail on each Add result row | Story | EP-07 | Follows BL-171 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | absent: any cover on an Add result row, the search, series and creator endpoints returning neither a cover nor a digitalId, which only the per-issue endpoint carries |
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3895-3910` |
| BL-069 | Close the three accent boundaries the BL-067 review found and could not gate | Debt | EP-08 | Leaves alone | 2 | 1 | 2 | 3 | 1.67 | none | Measured | Shipped | src/styles.css:306 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4748-4762 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:684-692 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4748-4762 |
...
| BL-028 | Make the reading view usable on a phone | Story | EP-07 | Leaves alone | 8 | 5 | 5 | 5 | 3.6 | P1 | Measured | Dropped | src/styles.css:229-232 |
...
that function is at `src/js/main.js:3720` today, with the rename-list handler occupying the lines the
...
**BL-148: Hydrate post-2025 issues from the Comic Vine API**

Parked reason: Comic Vine's own terms forbid the redistribution this project does, and it carries
neither of the two fields the gap is actually made of.

Raised because the claim that reached this repository was a good one. Comic Vine's terms do say the
API is "strictly for non-commercial use only", and this tracker is as non-commercial as software
gets. The claim arrived with a list of permitted uses that opened with "personal reading trackers",
which is this product described exactly.

That list is not on the page. All six terms were read in full on 2026-08-18 and none of them
enumerates permitted project types. The list was somebody's reasonable inference presented as a
quotation, and inferring the permission would have skipped the term that decides the question.
Term 5 reads "Don't redistribute in another form. Do not edit, manipulate or reproduce on any other
medium." This project vendors metadata into JSON files, commits them to a public repository and
ships them inside a downloadable archive. That is redistribution in another form three times over,
by an API whose terms name it. Term 4 would also require a visible link back on every page showing
their data, and term 6 makes access revocable at any time, which is a poor foundation for files a
reader has already downloaded.

Two measurements close it independently of the terms, and they are the reason this was not worth
negotiating. Comic Vine has no CORS headers: a request from a page returns no
`Access-Control-Allow-Origin`, and a preflight is answered with a redirect, so the browser could
not call it at all without the proxy this app does not have. And Comic Vine records neither
`digitalId` nor `unlimitedDate`. Those two fields are the entire gap. A source that cannot supply
either would have bought nothing even if every other objection were waived.

No repository constraint was breached, so this is a product decision in the shape of BL-028 rather
than a gate result. It was scored before it was parked and its numbers are left in the row above as
...
rather than curated, at `scripts/build-event-order.mjs:443-453`: the variant is offered only when
...
Nothing in the app changed. `groupCatalog` at `src/js/lib/catalog.js:413` was already a general
...
`h3, .row-title`, which matched nothing in the catalog pane as it then stood, so it compared two
empty lists and reported that they differed. Making both variants share a label did not move it. The
real title is `.result-title`, written at `src/js/main.js:3864`, and with the selector corrected the
same mutation fails it. It is counted above only because it was then seen to fail.
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:723-734`; omitting it
...
`src/index.html:417-421`, matching the reading filter's fieldset pattern rather than inventing a
second idiom, and the subtitle at `src/index.html:415` is now written by the render at
`src/js/main.js:4138-4140` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:489` and load normalises
a stale id at `src/js/lib/model.js:1041`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:963-964`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:684-692`, for the same reason
...
Notes export to Markdown quoted with `> `, at `src/js/lib/markdown.js:214-216`, and the prefix is
...
announced. It now lands on the all-read heading at `src/index.html:379`, which is both what the
reader needs to hear and where the remaining actions are.

The About view carries the shortcut reference at `src/index.html:721-728`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:962` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:352-359` rather
...
`src/js/main.js:2781-2786`, along with the pending badge's. A `title` is unreachable by touch and
...
text, at `src/styles.css:76-82`. BL-166 has since renamed that pair to `--accent` and `--accent-text`
and repainted it purple, so those two hex values are the record of this item rather than the shipped
colours. The `kbd` tint was removed at `src/styles.css:537` and the
...
strikethrough, at `src/styles.css:653-654`; the strikethrough is the non-colour indicator. The
only opacity left on a read row is the cover image at `src/styles.css:656`, which carries no
...
`.badge-expected` at 1.58:1, but that is unconditional design at `src/styles.css:720` rather than
...
top stop was raised from 60 to 88 percent alpha at `src/styles.css:479-486`. The spread collapsed
...
`src/index.html:855`, so resolving the theme in JavaScript would paint dark and then flip, and
...
graphic is `aria-hidden="true"` at `src/index.html:300`, the state is carried in words that
`src/js/main.js:743` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:759` announces,
...
and not `--track` (`src/styles.css:447`), where `--track` is the unfilled part of a progress bar and
...
Reuse is keyed on the whole item, at `src/js/main.js:2564-2579`, rather than on a list of the
...
`src/js/main.js:2554-2562`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:383` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:392-432` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:364`, and both success paths clear that
...
`src/js/main.js:4084`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:508-520` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:451-454`: "in library" and "Continue reading" would both resolve to
...
making sense: `forgetDeleted()` at `src/js/main.js:2271-2274` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:2294-2302` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:4084`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:4053-4064`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:497`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:520-528` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:2317-2327`. The usual reason
...
`src/js/main.js:4458`, `src/js/main.js:4382` and `src/js/main.js:4398-4404`. Each replaces the whole
...
`src/js/main.js:400-413`. Moving a message into the modal makes it readable where the reader is
...
files and returns 404 for anything it cannot find at `server.mjs:158`, with no single-page
...
- **A hash that is not ours is left completely alone.** `src/index.html:20` ships a skip link
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:84-89`. Syncing anywhere else would have left the address naming
...
the id is unknown, at `src/js/lib/model.js:522-524`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:2030-2084`, before the
...
`src/js/lib/readingFilters.js:25-50`, so the restore path above now checks a stored value against
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:754-805`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:28-77` and rendered by a single function at `src/js/main.js:4178-4215`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:490-491`: issue metadata and read state survive the deletion of
...
reason from the other end: an entry with a marvel.com URL keeps that issue's real id, an entry whose
details came from the wiki keeps the real id the wiki carries, and one with neither gets a negative
synthetic id from the clock, per `src/js/main.js:3577`, so the two kinds cannot be ordered against
each other by id at all.
...
rather than naming them, and the sections in `src/index.html:430-442` are empty shells whose heading,
...
`test/library.test.js:216-246` reads both files off disk and fails on any disagreement. That is the
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:644-651`, written the same way
`coerce` writes it at `src/js/lib/model.js:977`, so the two paths cannot disagree and every future
caller inherits it.
...
- [x] Make the run required before merge
...
- **The two cancellation guards in `src/js/hydrate.js:77` and `src/js/hydrate.js:82` are textually
...
Shipped, and the tag is now cut. It was left open because a tag needs a commit to point at, and
`v1.1.0` is the first, published with the Windows archive attached to it. The procedure is written
up under Releasing in `README.md`.

The version was set to 1.0.0 by this item, and what the number means is written down at
`src/js/lib/version.js`: MAJOR means an older build cannot read data saved by this one. That axis
was chosen because reading progress lives only in the reader's own browser and there is no server
that could migrate it for them, so a data format change is the one upgrade that can actually cost
someone their data.
...
BL-086 superseded the second half of that. The mirror argument holds for `connect-src` and was
carried across to `img-src` without being re-tested, but the app never requests an image from the
API base, so a mirror is unaffected by pinning images. `img-src` now names one host.
...
Shipped. The check now runs in the `MarvelApi` constructor at `src/js/api.js:52-67`, which throws a
...
`src/js/main.js:535-567` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:607-636` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:4409-4410`, unchanged.
...
`scripts/lib/fetch-json.mjs`, a factory at `scripts/lib/fetch-json.mjs:58-63` whose limiter, fetch,
...
`scripts/vendor-orders.mjs:47` and `scripts/build-event-order.mjs:209`, and none of them used the
...
`scripts/lib/fetch-json.mjs:72-81`, which also lets the limiter pace the backoff instead of holding a
slot through it.
...
Shipped. The form row is now `.field-row` at `src/styles.css:771-787` and the reading row keeps
`.row`, so the thirteen rules that describe a reading row, from `src/styles.css:639-642` down to the
...
reached every one of them; the reading list at `src/index.html:415` sits inside neither, so it never
...
two at `src/index.html:577` and `src/index.html:584` are direct children of a `.card card-static`,
...
`.card-static > *` at `src/styles.css:753`, which the old rule's 0,2,0 had been masking. Measured
...
unconditional design at `src/styles.css:720`, where the border is 25 percent alpha teal.
...
is written out in words inside the pill at `src/js/main.js:2781-2786`, drawing on the labels at
`src/js/main.js:2841-2846`, and the full sentence from `describe` sits in a visually-hidden span
inside that same pill. Delete the outline and the reader still sees "MU Unlimited", "soon scheduled",
"? unknown", "MUÃ¢Å“â€œ yours: available" or "no yours: not in MU". The outline bounds the label rather
...
The reasoning is recorded at `src/styles.css:694-718`, directly above the rules it governs, along
...
progress-ring transition at `src/styles.css:439` and the preference queries at
`src/styles.css:1119-1126`.
...
So the work was done against the measurement rather than the wording. That 138-word paragraph was
split into three at the time, and its source-evidence procedure now lives at
`docs/MAINTAINING.md:198-210`. The four sentences over 40 words became none: the longest was 36 and
the mean was 17.3. The audit paragraph was the one worth splitting on its own merits, not just its
length, because it was carrying three separate arguments at once: where the catalogue comes from,
why it is read out of `HEAD`, and what a shortcut nobody can check would cost.

The vocabulary was handled by defining all four terms, and by removing an earlier undefined use of
one of them. `vendor` remains defined in the curated-order procedure, at
`docs/MAINTAINING.md:177-179`, as fetching a list once and committing what came back. `depth` gained a gloss
...
`src/js/lib/readingFilters.js:25-50` carries the value, the label the radio shows and the predicate
that decides a row in one entry each, `wireReading` renders the radios from it at
`src/js/main.js:2047-2050`, and `renderRows` asks it at `src/js/main.js:2689`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:2042-2046` exists because
...
`src/js/lib/catalog.js:312-321` takes the opposite decision for the catalog, and the difference is
...
filter and does not decide one, and `filterListProblems` at `src/js/lib/readingFilters.js:75-91`
...
control the two lists build, and `preservingFocus` at `src/js/main.js:264` reads the pair back out of
...
deliberately not to the same control on it. `Ã¢Å¡â€˜` and `Ã¢Å“â€¢` sit in that strip, and Enter auto-repeats on
a held key while Space does not, so restoring `Ã¢Å“â€¢` under a finger already on Enter would delete the
next issue as well. Each list names its least destructive control instead, the shelf at
`src/js/main.js:2529` and the reading rows at `src/js/main.js:2832`, and the reasoning is recorded at
`src/js/main.js:219-223` rather than only
...
while proving nothing. The full order lives inside a `<details>` at `src/index.html:393` that is
...
`src/js/main.js:1867` carries a comment at `src/js/main.js:1877` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:4026`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1587` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:4101` puts
...
`renderRail` at `src/js/main.js:1250` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:1216` and reaches `renderYours` through `renderHome` at
`src/js/main.js:1218`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4748` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1258`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1412` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2874`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1433`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1274` and `src/js/main.js:1822`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4781`, and `src/js/app.js` is the entry
the page loads now, which calls it and nothing else that the view layer has to know about. The one
document read that ran during
...
`src/styles.css:743-744` sets `content` to a right-pointing angle glyph on the card summary marker. It
...
`justify-content`. Only 4 were `content`, and only one of those four carried a glyph. That measurement
...
one, which would blank the copy between them. It cannot fire today, because no `content` value in
the stylesheets holds a comment marker, the glyph-bearing ones included. That is the condition worth
stating: an earlier wording gave the reason as there being only one glyph in the stylesheets, which
stopped being true the moment a second was added. Closing it would need a tokenizer that skips
string literals, which is scope this item did not earn.

Scope is walked rather than listed. `server.mjs:13` resolves the served root to `src/`, so `src/` is
...
argument `scripts/check-anchors.mjs:227-232` makes about the anchors gate. The `design/mockups/`
...
`scripts/check-counts.mjs:501-535`, and `npm run counts` fails on a repeat. That remains true of this
...
already refuses a detail block that appears twice, at `scripts/check-counts.mjs:470-480`, which is
...
enumeration of where to look is the thing `scripts/check-anchors.mjs:227-232` argues against, having
...
built at `src/js/main.js:1274` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1822`, joined an issue count to an invitation to open the
...
at `src/js/main.js:3328`. A reader who imports one of this repository's own checklists therefore saw
...
copy it received. Five of the six are generated, at `scripts/build-event-order.mjs:483`, which wrote
...
takes each list's name from `curated-lists.json`, at `scripts/vendor-orders.mjs:144`, and
...
`src/js/main.js:1768`. That also retired a latent bug rather than fixing it. The old label said
...
`lists[k] = {...}` at `src/js/lib/model.js:996`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:1024`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:1041`.
...
`scripts/check-anchors.mjs:871` already slices the prose immediately preceding each citation into a
`claim` field, and the bless path at `scripts/check-anchors.mjs:1442` then writes only the anchor,
...
change. `reportNearMisses` at `scripts/check-anchors.mjs:1207` is the precedent for the script
...
`src/js/lib/readingFilters.js:25-50` is cited twice in one scope under wholly unlike sentences. The
...
So outside Markdown only the backticked form is collected, at `scripts/check-anchors.mjs:521-533`.
...
argues in its own comment at `scripts/check-anchors.mjs:227-232` that an enumeration is exactly the
defect it exists to catch, and "skip the lock file" is an enumeration of one. It is excluded
structurally at `scripts/check-anchors.mjs:261`, by the same `LOCK` constant that says where the
output is written, so the rule is that the gate does not read its own output and nothing has to be
kept in step. Binary files are dropped the same way, by a NUL byte at `scripts/check-anchors.mjs:142`
...
comment, when it comes from the series name at `src/js/main.js:697-698`. Both are re-aimed, and the
...
`claimBefore` at `scripts/check-anchors.mjs:733-759` now strips the marker outside prose, stops at
...
line, at `scripts/check-anchors.mjs:1188-1203`, and it prints a notice rather than failing, because
...
`scripts/check-palette.mjs:102`, was still correct, which is what made this an item rather than a
...
The check runs before the bless path rather than beside it, at `scripts/check-anchors.mjs:1299-1333`,
...
`scripts/check-anchors.mjs:488-503`. That is not a nicety. This gate's own test builds its fixtures
...
definition, at `scripts/check-anchors.mjs:512-515`, and two tests hold the two halves apart, because
...
The evidence is the bless path itself at `scripts/check-anchors.mjs:1442`, which writes the anchor,
...
no pathspec at `scripts/check-anchors.mjs:251-253`, and binary content is dropped by the reader
...
syntax instead, at `scripts/check-anchors.mjs:497`, `scripts/check-anchors.mjs:764` and
`scripts/check-anchors.mjs:1198`. A YAML comment opens with a hash, an HTML comment with an angle
...
exactly one of each, at `scripts/check-anchors.mjs:1579-1585`. The bucket carries the scope, at
`scripts/check-anchors.mjs:1547`, so a citation whose scope slug is renamed while its anchor and its
...
Delivered as `scopeRenames` at `scripts/check-anchors.mjs:1038-1084`, exported so a test can hold its
...
re-aim buckets are built, at `scripts/check-anchors.mjs:1558`, or a renamed pair could be the second
...
`scripts/check-anchors.mjs:1547` is now cited under two headings with the same ordinal and the same
...
`scripts/check-counts.mjs:521-527`, so it catches a block directly followed by its copy and nothing
...
`src/styles.css:524-528` is already the primary treatment, a red fill with `--on-accent` text, and
`.btn-g` at `src/styles.css:533` is the secondary ghost. `.btn-p` is a modifier that says "primary"
...
destructive one: `src/js/main.js:166-178` confirms Start fresh replaces the unreadable data with an
...
confirm dialog at `src/index.html:849`. The convention there is the same shape as this: the action
...
choose to start fresh." The static paragraph at `src/index.html:151-152` then says "It has not been
...
that boot report, and the comment now standing in its place at `src/js/main.js:4830-4834` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:135-154`, and until BL-075 took its
...
file of 5,034 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
together at `src/js/main.js:77-90`, and behind it sit sixteen library modules, none of which holds
...
limiter and cache when handed neither, at `src/js/api.js:68-69`, and the app always hands it both,
which is what keeps one request budget across the page. Saving a new API base replaces the cache and
the client and leaves the limiter alone, at `src/js/main.js:4421-4423`, because the budget belongs to
...
script, at `scripts/vendor-orders.mjs:28`, so fifteen are reachable from the page and a graph drawn
...
`src/js/storage.js:9-12`, two more belong to the view file at `src/js/main.js:44-45`, and the
...
the one change callback at `src/js/main.js:84-89`, so nothing left on screen said why saving was
paused.
...
`src/index.html:136` declares as an assertive alert. The duplicate predates this change; what this
...
`src/index.html:38`. `.ri[aria-current]::before` at `src/styles.css:306` remains the 3px bar marking
the current destination, which the comment at `src/styles.css:299-300` names as part of the selected
...
not sit on the rail. It sits inside `.ri[aria-current]`, whose background at `src/styles.css:302` is
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:1150`. The choice
...
`scripts/check-palette.mjs:151-179` names a derived background as a fraction of one token over
another, and `resolveSurface` at `scripts/check-palette.mjs:207-227` computes it. One mechanism
...
glyph that no one reads, since the button's label at `src/js/main.js:2765` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:12434-12436` uses that size as the argument for
...
figure as audited" at `PRODUCT_BACKLOG.md:213-215`. The clause is quoted only as far as its fixed
half. The live number beside it moves whenever a test is added, and pinning a copy of it into this
record would be the same defect in a second place, which is the rule BL-059 later had to state
outright. Appendix A does the same thing in its own idiom, correcting a miscount inside the
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:12455-12457`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:12206-12208` describes
...
`docs/UX_STUDY.md:842-845` while this paragraph carries no such marker and reads in the present
...
sentence, which is the enumeration anti-pattern `scripts/check-anchors.mjs:227-232` argues against in
...
`CHANGELOG.md:3195-3204` that says so and points at the backlog clause instead. That clause was
checked before the entry was allowed to defer to it: `PRODUCT_BACKLOG.md:203-207` and
`PRODUCT_BACKLOG.md:213-215` do each carry a live value and are marked as needing re-derivation, so
...
`src/js/storage.js:495-571`, `src/js/main.js:4359-4387`.
...
proxy for it.
Evidence: `src/js/storage.js:426-450`, `src/js/main.js:97-108`.
...
`test/markdown.test.js:203`, and was measured and left alone: it guards a nested quantifier against
...
a 1.6 second cost into a saving. Evidence: `test/backup-bounds.test.js:268-294`.

**BL-118: Notice a distant repeat in the prose documents the gate never reads**

- [x] Decide which documents the scan reads, by deriving the set rather than listing it
- [x] Report a finding against the file it is in, not against the one document the gate names
- [x] Keep the three-line floor honest by re-reading it against whatever population is chosen
...
same shape the size gate already uses, and the third is the one that needed deciding: the order
files are Markdown, but they are the app's data, they are generated, and a checklist that lists a
title once per issue repeats by design. A rule about the directory was chosen over a rule about a
file declaring itself generated, because only 8 of the 12 order files carry the "Edit the script,
not this file." line and one of them says in the page that it is not generated by a script, so a
self-declaration rule would have read four of them as prose. The directory rule covers the
thirteenth order on the day it lands.

Each document is its own corpus, and that is the decision the floor of three rests on. Read on
2026-08-15 against the 18 documents the rules derive from the 36 tracked Markdown files: 0 repeats
at three lines and above, 4 at two lines, 37 at one. The four at two lines are the same four the
2026-08-10 reading found, in the same documents, and every one is deliberate. Pooling those same 18
into a single corpus instead reports 2 at three lines and above, both of them fenced command blocks
that `CONTRIBUTING.md` and `README.md` each state in full, one of them nine lines long. Neither is a
defect, so a pooled reading would have needed the exception list this design exists to avoid. The
floor has now held across populations of seven, nine and eighteen.
...
data-directory rule fails 2, one of which is the assertion that no generated file reaches the pass.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3830-3835`,
`src/data/catalog.json`, `src/js/lib/model.js:134-145`.
...
and choosing between those is the work. Evidence: `.github/workflows/ci.yml:214-216`,
...
attack. Evidence: `src/js/main.js:4359-4387`, `src/js/lib/model.js:968-1077`.
...
`src/js/main.js:1422` and again on the rail, because neither render is bounded by anything. That is a
...
matched the functionÃ¢â‚¬â„¢s own definition eight hundred lines above it. Both passed with the guard
...
- [x] Inventory legitimate cover hosts used by bundled data and the supported metadata service
- [x] Reject or require consent for an unrecognised cover host
- [x] Keep the image CSP and URL policy in step
- [x] Prove a hostile HTTPS cover address makes no request

Constraint gate: checked 1 to 11, none breached.

Cover normalization accepted any HTTPS host and the CSP permitted images from every HTTPS origin. A
compromised or user-selected metadata service could therefore trigger a tracking request to a third
party while the interface said covers come from Marvel's servers. The fix continues to store URLs
only, never image bytes, so Constraint 1 remains intact.

The inventory settled the question the item was uncertain about. All 700 bundled covers across the
shipped catalog files are on `i.annihil.us`, and so are 36 of 36 sampled from the live metadata
service on 2026-08-15, drawn from issues published in 1963, 1985, 2005, 2015, 2021 and 2025. There
is no second legitimate host to accommodate, so consent had nothing to ask about and the host is a
pin rather than a list. The service returns those addresses over `http`, which is why the upgrade
to `https` in `normalizeCover` still runs before the check rather than after it.

The contrarian pass found the reason the hole was left open. BL-044 recorded `connect-src` and
`img-src` as deliberately wide under one rationale, that pinning would break self-hosted mirrors.
That is true of `connect-src`, because the app really does connect to whatever base the reader
configures. It does not reach `img-src`, because the app never requests an image from the API base
at all: a cover address is a field inside the response body, and every service serving the Marvel
API shape reports Marvel's own image paths. A mirror keeps working under the pin, so the one
stated objection to pinning images did not apply to images.

The rule is one module, `src/js/lib/coverHost.js`, imported by `normalizeCover` and by the CSP the
development server sends, so the URL policy and the directive cannot drift apart. It compares the
whole `host` rather than `hostname`, so a port cannot be smuggled past it, and requires `https`, so
a downgrade cannot either.

Review found the pin could be walked through rather than around. The check reads a parsed copy of
the address, and the first implementation then stored the string the service had sent, so a
character the URL parser escapes travelled on unescaped. The double quote is one. A cover path
carrying one really is on the pinned host, so it was accepted, and the hero background is built as
`url("<address>")`, so that quote closed the layer and a second one naming any host could follow it
inside the same declaration. The browser policy blocked the request, which is why nothing left the
machine under `npm start`, but the policy was then carrying the pin by itself and five shipped
sentences credit the refusal rather than the policy. `allowedCoverUrl` now returns the parsed
address serialized again, so what is stored is the string the check was made against, and the
escaping happens at the one place an address is admitted rather than at each place one is
interpolated. All 700 bundled covers serialize to themselves, so nothing that ships changes.

A mutation run is what makes the four boxes evidence rather than assertion. Nine single-edit
mutants were applied one at a time, each restored and the file checked byte-identical afterwards:
dropping the host check, comparing by suffix so `evil-i.annihil.us` passes, using `hostname` so a
port passes, dropping the scheme clause, widening `img-src` back to all of `https:`, narrowing
`connect-src` to the pinned host, adding a second paint site, feeding the painter a raw path, and
giving a bundled catalog entry a foreign host. Eight were caught on the first pass. The survivor
was real: nothing asserted that a plain `http` address on the pinned host is refused, which is the
one case where the host is right and the scheme is not. With that assertion added the run caught
all nine. The fix above added a tenth decision to defend, keeping the sender's string rather than
the parsed address, and the run was extended to match: ten of ten caught, the tenth by the single
test written for it.

Evidence: `src/js/lib/coverHost.js:25`, `src/js/lib/coverHost.js:46`, `src/js/lib/coverHost.js:55`,
`src/js/lib/model.js:226-241`, `server.mjs:53-59`, `src/index.html:609-614`.
...
remain local or absent. Evidence: `README.md:36-52`, `src/index.html:566-644`,
...
`src/js/main.js:2427` builds the reading hero's backdrop URL only when covers are on, and a computed
...
at once. The About view's "Metadata and links only" card, at `src/index.html:695-699`, is the sixth,
...
creator's issues pages the metadata API to completion at `src/js/api.js:194-208`, up to sixty
requests, from `src/js/main.js:3282` and `src/js/main.js:3298`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3391` is named alongside the search it resembles, and the
...
`test/ci-supply-chain.test.js:124-142`, `docs/MAINTAINING.md:149-166`.
...
- [x] Enable secret scanning and push protection where the repository plan supports them
...
- [x] Inventory content that changes without moving focus
- [x] Give each qualifying state one polite or assertive channel
- [x] Avoid announcing every queue tick when start, failure and completion are enough
...
`src/index.html:125-126`, `src/js/main.js:4668-4731`.

**Delivered** for two of the four surfaces, with the other two left silent deliberately and the
reasons written into the code beside them. The inventory is the part of this item that changed the
answer, so it is recorded here rather than summarised: the second task's "each qualifying state" was
read as all four, and measuring the four found that two do not qualify.

Queue depth cannot be announced on its edges. The hydrator awaits one issue at a time at
`src/js/hydrate.js:52-80`, and the limiter's depth counts the queue plus whatever is in flight at
`src/js/lib/limiter.js:22-24`, so depth returns to zero between every pair of requests. A 219-issue
order would cross the empty boundary 219 times and an edge-triggered announcement would speak on
each crossing, which is the fault the third task names rather than a fix for it. The one queue
condition worth hearing is a backoff, and that is already announced directly.

Cache usage cannot be made a live region without saying everything twice. The only change worth
hearing is the one after Clear cached metadata, and that handler already speaks through `notify` at
`src/js/main.js:497-507`, which routes to the announcer precisely when the container is not itself
live. Marking the element live would reintroduce the double-speak BL-027 removed.

So the guard covers API health and hydration, and it is keyed on the state rather than on the
rendered text at `src/js/main.js:327-334`, because both surfaces are rewritten far more often than
they change: `checkHealth` runs at boot and again on every API URL save, and `renderHydration` is
called once per issue fetched. Seeding the health key as reachable at `src/js/main.js:340-342` is
what keeps an ordinary boot silent while a boot that cannot reach the service speaks, and it is also
why the healthy message can only ever be heard as a recovery.

The message policy is a pure function at `src/js/main.js:2928-2937` so a whole run can be replayed
against it. Measured in Edge at 1280x900 on 2026-08-14, against a stubbed metadata service, a
20-issue Markdown import calls the hydration renderer 22 times and a reader hears three messages:
the import's own confirmation, one start and one completion. Hydration had no start announcement at
all before this, so the run a reader was least able to see was the one nothing told them had begun.

Seven single-line mutations were used to check the first twelve of the sixteen new tests can fail,
and six were caught by tests written before the probe ran. The seventh relabelled the stopped phase
as the completed one and nothing noticed, because the two never arrive together today. It is caught
now, by a test that pins the invariant the guard rests on: two phases sharing a state would silence
whichever of their two messages arrived second.

Two defects were found in review, both introduced by this item, and the more serious was in the part
of it that added a message. Giving hydration a start announcement put two messages in one tick on
every add and every import: `addToActive` confirms the add at `src/js/main.js:3267` and then starts
the hydrator at `src/js/main.js:3274`, whose `start()` reports its running phase at
`src/js/hydrate.js:50` before its first await. The announcer was a single slot, clearing the region
and scheduling a write at the same delay for each message, so both writes landed in one timer phase
with no rendering opportunity between them and only the second was ever observed. The confirmation
is the one that lost, and it has no visible live surface to fall back on, so an add would have gone
unconfirmed. Messages raised in one tick are joined into one utterance now, at
`src/js/main.js:282-297`, because two messages produced by one action are one thing to say. Four of
the sixteen tests drive a whole tick against the channel, and both the single-slot original and the
one-line form of the same defect turn three of the four red.

The second defect was the counting vocabulary, widened at the writers and not at the readers.
Extending the number words past ninety-nine left all three of their readers still stopping there,
and two of the three failed silently rather than loudly: the rank-heading pattern captured "a" out
of "ranks a hundred and third" and reported drift no wording could satisfy, while the delivered and
remaining counts skipped the claim entirely, which also stops the id list beside them being checked.
All three are built from one shared fragment now, at `scripts/check-counts.mjs:221`, and the
word-to-number reader follows the writer's own ceiling at `scripts/check-counts.mjs:312` rather than
repeating a number. The guard test meant to catch exactly this looked only at the writers, so it
stayed green throughout; it holds every word the writer produces to being readable back now, which
turns both of the original readers red.

Filing BL-124 alongside this made the ranked table a hundred rows long, and the counts gate refused
the figure rather than printing digits: its number words stopped at ninety-nine, and a guard test
...
size, at `PRODUCT_BACKLOG.md:2627`, and one of the four tests added to the counts suite pins the
...
hundred for the reason the old one stopped below one hundred: nothing here writes a figure that
large, so its wording would be guessed, and refusing to guess is what the null is for. The
governance record's derived count of detail blocks carrying a constraint check moved with the new
item, from 102 and 98 to 103 and 99.

Not closed by this item: the fourth task. The screen-reader run BL-027 still owes is what would
confirm the timing rather than the count, and this item does not discharge it. The browser check
counts what reaches the live region, which is a different claim from what a reader is actually read.
...
- [x] Commit the critical Edge scenarios already used manually
- [x] Keep the browser driver outside runtime and development dependencies
- [x] Give each scenario deterministic fixtures and a proved failing mutation
- [x] Document one command, prerequisites, assertion counts, artifacts and cleanup
...
untested. Evidence: `scripts/browser-check.mjs:1-7`, `docs/UX_STUDY.md:902-945`.

`npm run browser` drove installed Edge through those five journeys and made 28 assertions in 2.5
seconds on the day this shipped. The check has grown since, and BL-134's block records the sixth
journey and what it is for. It is not in CI, which has neither Edge nor a driver, and
`npm run browser:prove` is the separate pass that shows the scenarios can fail.

Three things the first working version got wrong, all of them found by running it rather than by
reading it, and each one a way a check can look green while asserting nothing:

The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:2055-2056`
builds no rows until it opens, so a scenario that waited on a row waited forever against a perfectly
healthy app. That one announced itself. The next two would not have.

Two page-context globals were set on `window` and then read from Node, where they are simply
undefined. Every comparison against them would have been against `undefined` on both sides in the
cases that matter least and thrown in the rest. They are Node constants now, derived from the same
fixture the page is given.

The mutations were injected by stringifying each one into `new Function`, which `server.mjs:53-64`
refuses: the app sends `script-src 'self'` with no `'unsafe-eval'`. The proof pass would have
reported that no mutation broke anything, which reads exactly like a well-covered app. Handing the
function to puppeteer directly is both simpler and the only form that runs, because a
debugger-injected script is not subject to the page's own policy.

The fourth was found by the proof pass itself, and is the one worth keeping. The handoff scenario
recorded `window.open` calls by replacing `window.open` after load. The mutation aimed at it also
...
banner at `src/index.html:154`, so a document-scoped `querySelector` finds them on a healthy app
...
`src/index.html:151`, so the corrupt bytes are now compared byte for byte after the reload. And the
...
stylesheet's own. `src/styles.css:725` puts the per-row actions out of reach with exactly
...
wants: this one writes corrupt state and starts fresh, and structurally cannot reach the reading
progress saved at `127.0.0.1:8787`. It follows that there are no artifacts and no cleanup, which is
what the documentation says rather than a promise to tidy up.

`scripts/check-counts.mjs:352-358` changed with it, for a reason this item caused rather than
found. Shipping BL-093 takes the cohort's `Ready` count to one, and the sentence stating it is then
correctly written "One of them is still `Ready`". The gate's pattern only accepted "are", and that
claim is not covered by the readable backstop, so the grammatical sentence would have gone unread
and unreported rather than red. Both verbs are accepted now, and a test asserts both are read.

Shipping it also moved a figure no gate checks. Appendix B counts the rows still to be picked up
before measuring rank inversions over them alone, and that count is prose rather than a shape the
counts gate reads. It said twelve, which was already two out before this item and three out after
it. Re-derived from the table it is nine, and the population counting the one `Proposed` row
alongside them is ten rather than thirteen. Every other figure in that appendix was re-derived at
the same time and holds: six adjacent inversions across the 101 ranked rows, twelve rows involved,
nine of them shipped, and the same two inversions among the rows still open.

**BL-094: Test the local host and launcher contract**

- [x] Start the server on an ephemeral loopback port in tests
- [x] Verify methods, malformed and escaping paths, ETags, headers and shutdown
- [x] Extract launcher command selection as pure data for Windows, macOS and Linux
- [x] Decide from measured failures whether an operating-system CI smoke is warranted
...
`server.mjs:89-101`, `server.mjs:167-175`, `server.mjs:185-189`.
...
typo in the very advice the busy-port message prints was reported; it now prints a correction and
exits 1. `Number()` accepted `0x2263` and bound 8803, so a value nobody would call a port could move
the origin, which constraint 5 is precisely about; the port is now decimal digits or nothing. And the
comment on the malformed-path branch said such a path was treated as not found while the handler had
always answered 403, so the comment was corrected to the shipped behaviour rather than the behaviour
changed to the comment.

The one mutant that survives is recorded rather than papered over: deleting the `HEAD` arm of the
final `res.end` changes nothing, because node:http suppresses a HEAD body by itself and sends
identical headers either way, measured. The line is kept as intent and the test says in the file that
...
- [x] Enable private vulnerability reporting and verify the Security tab presents the route
...
security policy on every response; the header block at `server.mjs:129-139` is reached only on the
...
The root now routes contribution, conduct, support and maintainer governance to concise public
documents instead of duplicating their policy. Evidence: `README.md:97-107`,
`CONTRIBUTING.md:1-213`, `SUPPORT.md:1-87` and `GOVERNANCE.md:1-95`.

Shipped as four documents, a test that keeps them honest, and one correction to the README that the
work turned up. `CONTRIBUTING.md` is 213 lines, `GOVERNANCE.md` 95, `SUPPORT.md` 87 and
...
- [x] Verify branch rules require the intended reviews and checks
...
Evidence: `.github/copilot-instructions.md:424-456`,
...
`src/data/curated-lists.json:512-553`, `src/data/new_ultimate_universe_trades.json:1-10`.
...
The boundary is a rule rather than a list. `.gitignore:43-44` now holds out the whole of the
...
bitten by, and the reason is written into `scripts/check-anchors.mjs:227-232` for the same class of
defect.
...
button at `src/js/main.js:4447-4484` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4458-4483` repaints the banner
...
policy and this one changes the server. The header set at `server.mjs:129-139` carries the content
security policy, `nosniff`, `no-referrer` and `X-Frame-Options: DENY`, and it is assembled inside the
success path, so only the 200 and 304 replies ever send it. The 405 and 403 exits at `server.mjs:103-113`
...
`scripts/check-counts.mjs:360-435`. It does not derive the second paragraph, which makes the same two
...
`docs/DATA_PROVENANCE.md:125-130` files the order under "Compiled for this project". Review found the
...
repeat is ever written the honest response is to raise it and record why. Only this document was
scanned when this item shipped, so in every other document that response waited on somebody
noticing, and the promise was wider than the check behind it. Widening the scan was a different
change from dating a measurement, and `BL-118` has since carried it.

Evidence: `scripts/check-counts.mjs:503-511`, `scripts/check-counts.mjs:549-571`.
...
it on. Evidence: `src/js/main.js:736-737`, `src/js/main.js:733`, `src/styles.css:495`.
...
over, for the hero backdrop alone: `src/js/main.js:2427` reads the setting before building the URL,
and `src/styles.css:495` sets `background-image: none` under the same class, which does suppress a
...
`src/js/main.js:2578`, and `test/render-rows.test.js` proves the two keys differ.
...
- [x] Stop asserting that every curated item arrived with its metadata
- [x] Keep the reason a lookup failed instead of discarding it
- [x] Leave a refusal out of the retry queue and out of the count on its button
- [x] Show which of the two states an issue is in, on the issue itself
...
`details pending` badge and a **Fetch details for N issues** button. Import mapped every item to
...
button. Evidence: `src/js/main.js:4043-4049`, `src/js/lib/model.js:212`,
`src/js/lib/model.js:807`.
...
is worth showing and not worth retrying. The hydrator could not tell them apart because it discarded
the error deliberately, so that a failure stayed pending and was tried again. That is the right
behaviour for a timeout and the wrong one for a refusal. Evidence: `src/js/hydrate.js:61-73`.

Shipped as one persisted field, `detailsRefused`, inferred in `normalizeIssue` from the only thing
that distinguishes the two absences at import: an item arriving from a curated order with no
metadata has already been looked up, because a curated order is the output of a completed vendoring
run, so its emptiness is an answer rather than a silence. That inference is one step weaker than it
reads, and the review of this change said so: the vendoring run's lookup loop caught every lookup
failure alike, so a 404, an exhausted retry budget and a lost connection all wrote the same empty
item. It holds for the data shipped today, where all 63 empty items are in the two Ultimate orders
and four of them answered 404 against the live API on 2026-08-15, and making the vendoring run
record what it was told is filed as BL-126 rather than folded in here. That item has since shipped,
so the inference described above is now the legacy path rather than the only signal, and the field
it sets is written by the run itself. A refusal met at runtime is recorded
through `markDetailsRefused` instead, and both land in the one field so the rest of the app asks the
question once. No migration was written and none is needed: `coerce` runs every stored issue back
through `normalizeIssue` on load, so a tracker imported before this existed corrects itself the next
time it opens.

`markDetailsRefused` is a dedicated updater rather than a call to `upsertIssue`, and the reason is a
trap rather than a preference. `upsertIssue` normalizes what it is handed, a bare `{ issueId }`
normalizes to a record whose title is the string `Issue 12345`, and that is not null, so
`stripNulls` keeps it and the merge overwrites the real title with a placeholder. The bug would have
shown as issue titles turning into their own numbers as lookups failed. A test holds the shape.

Four surfaces read the field. The retry queue drops a refusal, which takes the button from **Fetch
details for 34 issues** to hidden on a tracker holding only the affected orders. The row badge
became a three-way, `no details held` beside the existing `details pending`, with the reason in the
hidden half the way the availability badge beside it already does it. The issue page gained the
third branch of the same sentence. And the **Details pending** reading filter stopped matching a
refusal, which it would otherwise have listed forever. The row and page decisions were split out of
their renderers as `detailsState` and `synopsisFallback` so they could be measured directly, which
is the same move `backupFileRefusal` records.

Two things were found on the way that the gates do not check. The comment deriving the storage floor
said a coerced issue is a fixed thirteen fields; the tree held fifteen before this change and holds
sixteen after, because two fields had been added since it was written and nothing re-reads a number
in prose. Re-measured with the differencing method the floor test itself uses: the marginal cost of
an issue went from 267 characters to 290, the most generous origin now holds about 36,200 rather
than 39,300, and the ceiling is nearly seven times that rather than six. The test stayed green
throughout and correctly so, because it asserts the clause rather than the numbers. The historical
figures in BL-084's record are left as they were, being an accurate account of what was measured
then. Second, `test/hydrate.test.js` built its fixtures with `source: 'curated'` to mean nothing
more than "not manual", which stopped being true the moment a curated item with no metadata became a
refusal: twelve of its tests went red on a queue that was correctly now empty. Corrected to
`'import'`, which is what a genuinely pending issue is.

The review of this change found a third thing the gates do not check, and it was in the code this
change adds. `detailsState` and `synopsisFallback` both asked the refusal before they asked whether
the record was held. `normalizeIssue` keeps those two apart, but the merge does not: `hydrated` is
OR-preserved across an upsert while `detailsRefused` is last write wins, so an issue added from a
search and then imported inside a curated order that holds nothing for it carries both at once, and
the badge then said no record over a record the tracker was holding. Reproduced against the shipped
modules with issue 7 before it was fixed by asking `hydrated` first on both surfaces. No bundled
order can reach the shape today, because no issue id is empty in one order and populated in another,
but hand-adding one of the 34 ids and then importing the Ultimate order reaches it, and a
re-vendoring that fills the gap in one of the two overlapping orders and not the other would too.

Verified with 21 new tests, each watched failing first. Ten targeted reverts, each the smallest edit
that could reintroduce one defect, and each reddened a named test: dropping the inference, letting a
refusal read as hydrated, taking the field with `clampScalar` so a hand-edited backup could invent a
refusal, leaving refusals in the queue, routing the write through `upsertIssue`, dropping every
failure rather than only a 404, restoring `hydrated: true` at the import call site, asking the badge
questions in the other order, removing the third synopsis branch, and matching a refusal in the
filter. Against the shipped catalog: 34 issues refused, 0 pending, where the app previously offered
to fetch details for 34 that upstream answers 404 for.
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:4075-4081`,
`scripts/vendor-orders.mjs:223-269`, `scripts/vendor-orders.mjs:323`.
...
are green. Evidence: `src/js/lib/model.js:134-145`.
...
`src/js/lib/model.js:141-142`, `src/js/lib/model.js:151-161`.
...
returns one without a `digitalId`. Evidence: `src/js/lib/model.js:88-90`.
...
imports, adds no edge to the module graph at all. Evidence: `docs/ARCHITECTURE.md:108`.
...
here. Evidence: `src/js/main.js:4090`.
...
- [x] Decide whether the erase promise covers data the app could not read
- [x] Hold whichever answer is chosen to a test, so it cannot be reversed in silence
- [x] Say on the erase dialog what it does and does not reach, if anything survives it
...
**Decided and delivered: the rule wins and the wording was narrowed.** The erase reaches the two
restore-family keys and stops, exactly as before, and the sentences either side of it stopped
claiming otherwise. Three arguments carried it. The rule is reasoned rather than incidental, and
overturning it would put a wholesale delete on the one class of data this app admits it cannot
read. The copies are disclosed in plain sight on the same screen, directly above the button, each
with its own Remove, which is what separated this from the undisclosed undo snapshot BL-101 did
withdraw. And narrowing is the half of the choice that can be taken back: a dialog that overstates
can be corrected later against copies that still exist, while an erase that has already destroyed
the last record of unreadable data cannot be.

The wording is a pair of policies rather than a pair of strings, because the two things that can
survive an erase are independent. The snapshot survives when its removal is refused; the salvage
copies always survive. Either, both or neither can be left, and the message the reader hears is
composed from the clauses that are true rather than chosen between two. The plain sentence is kept
for the case where nothing survived, so an ordinary erase still reports plainly. Both surfaces give
three answers rather than two, the same three `renderSalvage()` gives: a browser that will not
enumerate its own storage has not said there is nothing, it has declined to say, and promising that
everything is gone on the strength of a refusal is the one answer that can be wrong in the direction
that costs the reader data.

The erase route now repaints the salvage list, and the reason first written for it was the wrong way
round. That comment said an erase cannot land while a copy is live, because live means the main key
holds the bytes the copy was taken of, so the write is refused either at the latch or at the
compare-before-write. The compare half is false. `persist()` compares write tokens rather than bytes
at `src/js/storage.js:439`, and `tokenOf()` reads only the head of the value at
`src/js/storage.js:41-45`, so a tab that wrote a value still matches after something shortens its
tail past that head. A schema downgrade is the everyday shape of it, and the tab that wrote the
value is not the tab that fails to read it afterwards. The first attempt at the test failed for a
narrower reason than the one recorded: its fixture used a single blocked store, which is the latch
case rather than the compare case, so the comment generalised from the half that held.

So there are two triggers rather than one, and they move a row in opposite directions. An erase that
lands replaces the bytes a live copy was taken of, so that copy stops being live and trades the note
`renderSalvage()` shows for a Remove button that was not there when the screen was painted. An erase
that is refused rolls back by re-reading, the read fails on the bytes that caused the conflict, and
the failure salvages, so pressing Erase can create the first copy this browser has ever held on a
screen that is at that moment saying nothing is being kept aside, with nothing announced because
nothing was saved. Both are the fourth trigger the list's own comment did not have, and neither is
one arrival can cover, because the button is on the screen the list is already showing.

The review of the change found two more sentences making the kind of promise the item was raised to
stop. The dialog named the copies' Remove button unconditionally, and `renderSalvage()` puts a note
in place of that button while a copy is live at `src/js/main.js:4571-4577`, which is the state a
copy is in for as long as the data it copies is still saved: on this screen, right up until the
button is pressed. The policy now reads `live`, claims the location either way, and claims the
button only where there is one. And the branch with no copies at all still said the route clears
everything this browser has stored, which two names outlive: `mrt.settings` and `sidebar.collapsed`
are written by the app and removed by nothing, as `docs/ARCHITECTURE.md:239-240` records. That
branch now says the lists and the progress go and the settings are kept. The message said afterwards
is left alone, because it reports all local data erased and the same rows call those two preferences
rather than data.

Nineteen tests, sixteen of them new in `test/erase-scope.test.js` and three added beside the other
erase tests in the store suite. Eleven mutations across two passes were applied one at a time, each
restored byte for byte, and the first pass of that probe is the reason three of the nineteen exist:
the wording tests drove the exported policies, so the mutation that put the old sentence back at the
call site left every one of them green. The three that close it read the shipped module, as the copy
suite already does for the surfaces ESLint cannot see. Two of the three count a sentence rather than
looking for one, because reverting the call site does not remove the policy but adds a second copy
beside it; the third pins the repaint ahead of the announcement, which is an order rather than a
presence and so is the one of the three a count cannot make.
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1422`
...
derivation in `src/js/lib/model.js:323-324` calls the most generous any browser grants. That figure
...
stops checking. Evidence: `scripts/check-counts.mjs:52-55` and `scripts/check-counts.mjs:61-67`.
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4865-4873`.
...
`forgetDeleted()`, at `src/js/main.js:4458`, `src/js/main.js:4382` and `src/js/main.js:4403`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1531-1532`.
...
**Delivered.** The first citation now names `src/js/main.js:4458`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4447-4484`. The other two were already right and
are unchanged: `src/js/main.js:4382` is the call in the successful branch of the restore handler,
and `src/js/main.js:4398-4404` is the whole undo-restore handler, whose call is at
`src/js/main.js:4403`. The range was left as the handler rather than narrowed to the call, because
...
the code they name, and the twelve besides this one are sound: `src/js/lib/model.js:508-520` is
`restoreList`, `src/js/lib/model.js:451-454` is the comment on a duplicate not inheriting the
catalog claim, `src/js/main.js:2271-2274` is `forgetDeleted` itself, `src/js/main.js:2294-2302` is
`forgetDeletedFor`, `src/js/main.js:4084` is its one call, `src/js/main.js:4053-4064` is the
half-import rollback, `src/js/main.js:497` is `notify` with its `action` parameter,
`src/js/main.js:520-528` is `spoken`, `src/js/main.js:2317-2327` is the failed undo keeping its
buffer, `src/js/main.js:400-413` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:2297`, `src/js/main.js:2313` and `src/js/main.js:2330`. The sentence is right to
...
so at `scripts/check-anchors.mjs:971-973`. Read on 2026-08-14, 159 of the lock's 466 distinct
...
`.github/copilot-instructions.md:180-182` already draws for bare citations outside Markdown. Table
...
at `PRODUCT_BACKLOG.md:4870-4873`.
...
`proseRuns` at `scripts/check-anchors.mjs:572-614` groups consecutive prose lines into the unit a
sentence is read inside, so a sentence may wrap and still be read whole. `repeatedCitations` at
`scripts/check-anchors.mjs:671-704` splits each run on sentence punctuation and reports any anchor
named twice within one sentence. `repeatVerdict` at `scripts/check-anchors.mjs:710-713` refuses
...
after it. The wiring sits at `scripts/check-anchors.mjs:1339-1347`, before the bless path, because
...
the one the filing predicted, at `scripts/check-anchors.mjs:588`, and it removes the single false
...
above it into the one below. A thematic break at `scripts/check-anchors.mjs:593` is a fourth, and it
...
`scripts/check-anchors.mjs:602`, and it is a real shape here rather than a hypothetical one, because
...
The sentence split itself is at `scripts/check-anchors.mjs:643-646`, and it errs towards splitting: a
...
`scripts/check-anchors.mjs:640`, and that list does not have to be complete: one missing from it
...
The fixture form is held in a test at `test/check-anchors.test.js:860-864`.
...
where the status key at `PRODUCT_BACKLOG.md:239` defines an item still to be picked up as `Ready`.
...
all four combinations of the two choices, so no conclusion here turns on which is taken. It is
recorded because the claim this item corrects went stale by exactly this mechanism, one row at a
time, and it has moved three times since, twice by an item shipping and once by one being filed,
which is why `BL-125` is filed against the gap rather than the figure.
...
Re-sorting is therefore safe, and it is declined on cost rather than on risk: a stable sort moved 37
of the 99 rows the table held then, one pass of local swaps leaves six inversions rather than none, nothing gates the
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4865-4868` are the counts of items above
...
**BL-124: Stop the rate-limit backoff saying the same wait twice, and say it grammatically**

- [x] Decide what ends a stall, so a later backoff of the same length is still heard
- [x] Announce each distinct wait once within one stall
- [x] Pluralise the wait, which currently reads "Waiting 1 seconds"
- [x] Prove it against a stubbed service that answers 503

Constraint gate: checked 1 to 11, none breached.

Found while verifying BL-090 in Edge on 2026-08-15. Pointed at a service stubbed to answer 503, the
announcer received four backoff messages before the request gave up, and the first two were
identical: "Waiting 1 seconds", "Waiting 1 seconds", "Waiting 3 seconds", "Waiting 6 seconds". Both
faults were on one line, inside `onApiStatus` at `src/js/main.js:4721-4724`.

This is deliberately not folded into BL-090, whose third task is about queue ticks rather than about
how a failure is worded, and which leaves the backoff announced because a stall is worth hearing.
The repeat is a different question and it needs a decision BL-090 does not: the guard BL-090 added
would silence the second "1 seconds" if keyed on the wait, but it would then also silence a genuine
second stall later in the same session, and the obvious reset, queue depth returning to zero, is
useless here because the hydrator drives depth to zero between every pair of requests anyway. That
reasoning is why this is a separate item rather than three more lines in that one.

The pluralisation could be fixed alone and is the smaller half, but it sits inside the same template
string, so splitting them would mean editing that line twice.

**Decided and delivered: a stall ends when the service answers.** Not when the retry chain ends, and
not when the queue drains. The chain was ruled out on arithmetic: `attempt` resets to 0 on every
`get()` at `src/js/api.js:81`, so the four backoffs a request can produce are one request's worth,
and the hydrator continues past a failed lookup rather than stopping at `src/js/hydrate.js:61-73`.
A 219-issue order against a service that never answers therefore reaches 876 announcements drawn
from eight distinct sentences, because `backoff()` at `src/js/lib/limiter.js:66-69` can only reach
attempts 0 to 3 and those bands round to the integers 1 through 8. Queue depth was ruled out for the
reason filed above and confirmed at `src/js/hydrate.js:52-80`: the hydrator awaits one issue at a
time, so depth returns to zero between every pair of requests and an edge on it would reset the
guard 219 times over.

So the client now reports that the service answered. `src/js/api.js:111` sits past the branch that
refuses to call a 429 or a 5xx an answer and ahead of the one that refuses a 404, which is
deliberate: a 404 means the service is responding, and responding is the whole of what this reports.
Nothing is reported when the retries are exhausted, because that is the service still not answering.
A cache hit reports nothing either, since it returns at `src/js/api.js:82-85` without asking anyone.

The guard is a set rather than the keyed last-value guard `BL-090` added, and the difference is not
stylistic. The waits inside one stall are not monotonic: attempt 1 draws from [1000, 2000) and can
round to the same 1 second attempt 0 drew, which is the reported fault, and the next request opens
at attempt 0 again after the last one drew eight seconds. A guard remembering only the previous wait
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4703-4717` keeps
the set, clears it on an answer, and composes the sentence, so `onApiStatus` is two calls long.

Fourteen tests in `test/backoff-announce.test.js`: seven on the policy, five driving the client
against a service stubbed to answer 503, and two reading the two modules. The client half needed
`fetch` and `sleep` injected into `MarvelApi` at `src/js/api.js:52-54`, because a four-attempt
chain waits about fifteen seconds of real time; that follows `RateLimiter`, which already takes
`now` and `sleep` at `src/js/lib/limiter.js:12`, and `createJsonFetcher`, which already takes both
at `scripts/lib/fetch-json.mjs:58-61`. Three of the fourteen are there for what the other eleven
cannot see, and none of the three is a count. One asserts a stall is audible at all and that its
whole sentence is well formed, against a wait the limiter chose rather than one the test chose.
One reads `src/js/main.js`, because a call site that composes the sentence inline again leaves
every other test here green: the policy tests import the policy, and the client tests hand the
client an announcer of their own. One reads `src/js/api.js` to pin the answer between the retry
branch and the 404, which no test reaches by behaviour, because none of them asks for an issue
that is missing. Eight mutants, one per decision above, were all caught.

A fifteenth was written and then split in two, which is worth recording because the split is the
point. Asserting the announcement count against the real `backoff()` is a coin toss: the draw is
random inside each attempt's band, so four attempts round to four distinct integers about as often
as they collide, and the first version of that test failed on the second full run. The count is now
asserted against the draw that was measured, scripted onto the limiter, and a separate test runs
fifty stalls against the real draw asserting only what is true of every draw, that no sentence is
said twice. A scripted sequence proves the reported case; the sweep proves there is no sequence the
guard lets through.

The reported sequence is now three sentences rather than four, and reads "Waiting 1 second."
rather than "Waiting 1 seconds". The floor is pinned across the two modules rather than clamped in
one: `backoff(0)` draws from [500, 1000), so the smallest wait the limiter can produce rounds to 1
and "Waiting 0 seconds" is unreachable rather than merely unobserved. A test holds that claim
instead of a defensive branch nothing can reach.

Two findings came out of review, and both were about the record rather than the code. Three live
statements of the size of `src/js/main.js` were left at the figure they had before this change,
while the drift clause that exists to carry that figure was updated: two of the three are in this
document and one is in `docs/ARCHITECTURE.md`, and all three were true when they were written.
All three are re-derived and agree now, and `BL-125` is filed for the check that would have
noticed, because nothing here does. The second finding was this block overstating why three of the
fourteen tests exist. It said the other eleven could all pass while the app said nothing, and five
of them assert an exact sentence or an exact count of one, so silence fails each. The paragraph
above is the corrected one, and the comment in the test file that it was compressing was wrong in
the same way and was corrected with it.

**BL-125: Fail the build when a stated line count disagrees with the file it counts**

- [x] Derive the files a stated size could be about rather than naming the sites that state one
- [x] Read the figure out of the prose, in each spelling the documents already use
- [x] Report a stale statement against the file it is written in, and say what the size is now

Constraint gate: checked 1 to 11, none breached.

Raised by the review of `BL-124` and routed here rather than fixed there, because that item
corrected the stale figures and this one is about why nothing noticed them. `BL-055` shipped the
drift clauses, which are the one place a figure is expected to age and is deliberately dated.
Every statement outside them is a plain assertion, and three of those aged while the dated one was
kept current, which is the failure that item was raised to end reappearing somewhere it did not
look.

Neither gate can see it. The counts gate reads the ranked table and recomputes what the prose
derives from it, so it counts rows and never lines. The anchors gate fingerprints the lines a
citation names, so a sentence that states a size without citing anything is not a claim it holds.
A stale size is a well-formed number in a well-formed sentence, which is the one shape both are
blind to, and it has now gone wrong twice.

The work is deciding what counts as a claim rather than writing the comparison, which is three
lines. Both spellings in the tree name their file within a sentence of the number, so the pattern
is small enough to derive and the population too small to be worth a general parser. Deriving it
rather than listing the sites is what keeps this from becoming the enumeration the anchors script
warns about, and it is why this is sized 2 rather than 1. Evidence: absent, read of
`scripts/check-counts.mjs` and `scripts/check-anchors.mjs`.

Shipped as `scripts/check-sizes.mjs:31`, wired as `npm run sizes` and a step of its own in the lint
job. It reads the tracked tree, takes the file a size is about from the nearest backticked path in
the same paragraph, and recomputes the figure. Naming `is` and `of` as the only two spellings is
what does the discriminating: every other use of the word in the tree states a delta, a past
measurement or a quantity of work, so "grew by", "grown by", "from N lines to", "measured at",
...
longer, which made the size stated of it in this document wrong, and the run said so before the
commit rather than three items later. That is the whole of the case for the item, arriving
unprompted on its first day.

**BL-126: Record what a vendoring lookup was told, rather than one empty item for every failure**

- [x] Carry the lookup outcome onto the vendored item rather than inferring it from emptiness
- [x] Key the refusal on that outcome, so a lost connection is not read as a refusal
- [x] Fail a vendoring run loudly when an item is empty for a reason that is not a refusal

Constraint gate: checked 1 to 11, none breached.

Raised by the review of `BL-109` and routed here rather than fixed there, because that item is
about what the app tells a reader and this one is about whether the vendoring run gave it grounds
to say it. `BL-109` infers "upstream has no record of this" from an item arriving in a curated
order with no metadata, on the premise that a curated order is the output of a completed run so
every item in it has already been looked up. The first half is true and the second did not follow,
because the run discarded which answer it got.

The vendoring run's lookup loop caught every error from the lookup identically and only warned, and
the item was then built from whatever the metadata map held, so it landed with every field null.
Four unlike outcomes reached that catch: a 404 from the non-ok throw at
`scripts/lib/fetch-json.mjs:68`, a 429 or 5xx that outlived the retry budget from
`scripts/lib/fetch-json.mjs:76`, a lost connection, and a body that would not parse. Those are
exactly the two kinds `BL-109` exists to separate, and the import path applied none of the test the
runtime path applies at `src/js/hydrate.js:72`.

The consequence is worse at import than at runtime, because it is unrecoverable. A refusal is
dropped from `pendingIssueIds`, therefore from `hydrationOrder`, and the hydrator is the only
caller of the issue lookup in the app, so nothing ever asks again and the one writer that clears
the flag never runs. The reader is then told the snapshot has no record of an issue the snapshot
would hold. Nothing stopped it, either: the run warned once per failed lookup and then wrote the
file anyway, and no check ever counted how many items came back empty, so an outage that lost thirty
issues left thirty warnings scrolled off a build log and an artifact that looked correct.

Not live today, and measured rather than assumed. 69 items across the fourteen shipped orders fail
`hasMetadata`, and they are two unlike things the app already counts apart. Six are placeholders in
the two X-Men orders, rows the vendoring run built deliberately for checklist lines Marvel has no
record of at all: a synthetic negative id, `placeholder: true`, and no lookup ever attempted. The
other 63 came back from a lookup, they are all in the two Ultimate files, and four of them were
checked against the live API on 2026-08-15 and answered 404, so the premise holds for the data that
ships. It is also not a regression: import forced `hydrated: true` before `BL-109`, so a transiently
lost item was already excluded from the queue. What changed is that the exclusion is now permanent
and carries a positive claim, which is what makes the soundness of the inference worth buying rather
than reasoning about. Evidence: `scripts/lib/lookup-issues.mjs:49`, `scripts/lib/fetch-json.mjs:30-36`,
`scripts/vendor-orders.mjs:254-260`.

**The item's own premise was checked rather than inherited, and the check was worth running.** The
block above rests on four issues having been asked about. All 63 lookup-derived empties were put to
the live API instead, and all of them answered 404, so the premise holds completely for the data that
ships, which is a stronger statement than the item makes. Those 63 items are 34 distinct issues, a
thing `BL-109` had already established and this run confirmed rather than discovered: the Ultimate
trades order re-lists what the singles order already carries. 34 is the number of lookups a tracker
holding both orders would otherwise spend, and it is the number that came back refused.

Shipped as a three-way reading of each lookup where there had been one. `scripts/lib/fetch-json.mjs`
now throws a `FetchError` carrying the HTTP status, and `scripts/lib/lookup-issues.mjs` sorts each
outcome into answered, refused or indeterminate. A rejection is a refusal only when
`err?.status === 404`, which is the same test `src/js/hydrate.js:72` already applies at runtime. The
status is a field on the error and not a parse of its message, and a lost connection or an
unparseable body produces an error carrying no status at all, which is exactly right: neither can
pass the test, so neither can pass for an answer.

**The review found the half of that agreement the first pass missed**, and it is the shape this
repository keeps relearning: the most dangerous code in a change is the code added to stop data
loss, because it runs only once something has already gone wrong. Classifying rejections alone left
a lookup that *resolved* with an unusable body counting as answered, so it built an item with every
field null and no `detailsRefused`, which is precisely the unrecoverable outcome this item was
bought to prevent, reached by the one route the new tests did not cover. The runtime path had always
guarded it, declining to upsert a falsy body so the issue stays pending for the next run; the import
path has no next run, so its only guard is declining to write the file. The fix applies the app's own
`hasMetadata` to the resolved body as well, at `scripts/lib/lookup-issues.mjs:43-44`, imported from
`src/js/lib/model.js:88-90` rather than restated, because a producer holding a second copy of the
rule can drift from the consumer without anything going red.

Measured before adopting that test rather than assumed, because a predicate that rejects a real
answer would make an order unvendorable. Across the fourteen shipped orders 1,404 items pass
`hasMetadata` and 69 fail, the 69 are exactly the empties and placeholders already known, and not one
of them carries a cover, a series name, an on-sale date, a description, a page count or a creator, so
the test separates exactly the items a lookup produced nothing for. Reverting the guard reddens 3 of
the 4 tests added for it. The survivor is the one holding a body with a series id and no digital id to
still being an answer, and it passes either way by design: all 1,404 answered items carry both ids,
so no shipped datum is in that shape and nothing derived from the data would notice the rule being
narrowed to `digitalId` alone. Narrowing it reddens that test and nothing else in 957, which is the
whole of its job.

A second review round found three prose claims the code does not support, and no gate caught any of
them. The comment beside that survivor test said plenty of real issues carry no digital id, which the
1,404 refute. This block said the old run warned only when the item count differed from the plan, when
in fact it warned once per failed lookup and then wrote the file anyway, which is both the true
statement and the more damning one. And writing `detailsRefused` only when true was described as
leaving an unchanged order byte for byte, which no re-vendor ever is, because `generatedAt` is a
fresh wall clock on every run. That claim is now scoped to the per-item payload, which is what the
conditional write actually buys.

Every one of those figures was then measured a third time, because this branch was rebased onto two
X-Men orders that landed on `main` while it was open and a measurement is about a tree rather than
about a claim. The rebase is where the numbers moved: 12 orders became 14, 751 items became 1,473,
688 answered became 1,404, and 63 failing the predicate became 69. One sentence in this block
survived that rebase as a silent falsehood, saying every empty item was in the two Ultimate files
when six of them had just arrived in the X-Men orders, and `git` reported no conflict on it because
the two branches had edited different lines of the same paragraph. The shape of every claim held: 0
of the 1,404 carry one id without the other, and 0 of the 69 carry any upstream evidence.

The re-aiming that rebase forced also produced a citation whose end read `null` where a number
belongs, because the end of the range fell inside a changed hunk and no arithmetic could place it.
That is not a citation the anchors gate recognises, so it was not reported as drifted or lost; it was
silently dropped from the corpus, and the range it should have named shrank from three lines to none.
What caught it was a test in `test/check-sizes.test.js:165-180` asserting that the lock still holds a
sentence stating a file's size, which stopped being true the moment the entry vanished. A malformed
anchor is therefore worse than a wrong one, because a wrong one drifts and a malformed one
disappears, and the only reason this one was visible at all is that an unrelated test happened to
depend on the entry it removed.

The abort is inside `lookupIssues` rather than in the caller, and that is a shape choice rather than
a placement one. A run that met an outcome it could not classify never gets its results back, so
"nothing is written" is a property of the function rather than of the order two statements happen to
be in, and no future caller can reintroduce the bug by reading the return value first. The message
names every unclassifiable id rather than the first, because a run that lost thirty issues to an
outage and reports one has told you the least useful true thing it could.

`detailsRefused` is written onto the item only when it is true. An order with nothing refused
therefore re-vendors with no per-item change at all, so the field costs nothing in ten of the twelve
shipped files and the change is legible as a diff in the two where it matters. Not byte for byte,
which is the tempting way to say it and is wrong: `generatedAt` is a fresh wall clock on every run,
so no re-vendor produces an identical file and an empty diff is not the thing to check a run against.

A new build-time module rather than a guard on the existing one. `scripts/vendor-orders.mjs` calls
`main()` unconditionally at module load, so a test cannot import it, and the obvious fix of guarding
that call behind an entry-point check would put `npm run vendor` one subtle path difference away
from doing nothing at all while reporting success. That is the failure this whole item is about, so
buying a testable seam with a copy of it would have been a poor trade. Extracting the logic gives
the seam with no entry-point risk, and the tests drive the real fetcher through it rather than a
stand-in, which is what caught the gap below.

Each of the six decisions was reverted in turn and the tests watched to redden. Making every
failure a refusal reddens 6, handing failures back to the caller instead of aborting reddens 9,
truncating the report to the first id reddens 2, dropping the status from the non-ok throw reddens
2, dropping it from the exhausted-retry throw reddens 1, and storing whatever the lookup resolves
with reddens 3. The abort figure is 9 rather than the 6 measured before the review, because the
three tests added for the resolved-body guard assert a rejection and so notice that revert too; it
is re-derived here rather than carried forward. The first of the two status reverts
reddened only 1 before the seam test existed, because every other test in the file builds its own
error object and would have gone on passing against a producer that had stopped carrying the status.
That gap is the reason one test drives the real fetcher.

The two Ultimate orders were re-vendored against the live API rather than edited, and the diff was
read rather than trusted: `detailsRefused: true` on 34 items in the singles order and 29 in the
trades, the snapshot timestamps, and one payload key the producer has always written since that
file was last built. No other per-item field moved, so the upstream metadata is byte-identical to
the snapshots these replace, taken on 2026-08-05 for the singles order and 2026-08-08 for the
trades, and this change is visibly additive.

What is left, deliberately. `refusedOnArrival` still infers a refusal from emptiness, and removing
it would have been the tidy-looking mistake: `coerce` re-normalizes every stored issue on load, so a
tracker imported before the field existed would have had those items pushed back into the hydration
queue to relearn 34 404s, which is the cost `BL-109` was bought to avoid. The inference is now the
legacy path and the recorded outcome is the primary signal, and the comment above it says so. The
order-gap counts still key on absent metadata rather than on the new field, which is correct
unchanged: they count what a reader is missing, and an issue is missing its cover whether upstream
refused it or nobody has asked yet.
...
two readings had to win: `scripts/vendor-orders.mjs:307-309` warns that a repeated id will collapse on
...
`src/js/lib/catalog.js:350-355`. Uncanny #129-138 is a story arc, not a book anyone can buy, so
...
**BL-128: Correct the provenance record's description count and record its placeholder shape**

- [x] Re-derive every figure the provenance record states and correct the one that disagrees
- [x] Describe the record shape the inventory never mentions
- [x] Keep the six placeholders distinct from the 63 bare records

Constraint gate: checked 1 to 11, none breached.

The provenance record is the document the publication decision rests on, so a stale figure in it
costs more than its size suggests. It stated the description count three times. Two said 798 and one
said 508, which is what the figure was when the tree held twelve orders rather than fourteen.
BL-127 added the other two and re-derived nine figures across the file, and this was the tenth. The
one that survived is inside the list of questions a legal review would have to answer, which is the
worst of the three places to be wrong: there the number is part of the question rather than part of
a description, and nobody reading the answer would know to check it.

The inventory also said every record copies thirteen named fields from upstream, and six records
copy none of them. Two in the Claremont order and four in its complete variant stand for checklist
lines the mirror holds no issue for at all, so nothing was ever fetched for them: they carry
`placeholder: true`, no url, and an id generated locally by hashing the order and the title and
negating the result so it can never be read as Marvel's. BL-126 already recorded that split as
delivery evidence. What it had never done is reach the document written for a reader outside this
project, and that document is the one that has to stand on its own.

Recording it needed care in one direction. The same file says 63 items carry nothing beyond an id, a
title, a number and a marvel.com link, and that number is exactly right as written: all 63 sit in
the two Ultimate universe orders and every one of them has a real Marvel id behind a real link. The
six placeholders fail the opposite way, with no link because no lookup was ever attempted. Added
carelessly they would have turned a correct 63 into an apparent 69, so the two now sit next to each
other with the difference stated rather than left for a reader to reconcile.

Filing this item moved five stated ranks, which the counts gate named and which were written back
from the values it derived. It also surfaced a sixth figure the gate does not read. The WSJF case
for BL-026 said it was outranked by forty-five items, twenty-eight of them cheaper, where the table
gave forty-six and twenty-nine before this change and forty-eight and twenty-nine after. Both had
been one behind since some earlier item was filed above it, and the neighbouring claim about
unlabelled items was correct, which is what made the pair worth checking rather than assuming. They
are re-derived here rather than shifted by two, because carrying a wrong number forward is exactly
how it survived this long.

Three further claims of that kind sit in the appendix that argues against re-sorting the table, and
no gate reads any of them. It counted the table at 102 rows where it held 103, said five rows carry
`Ready` where four do, and said a stable sort by score moves 39 rows where it moves 40. All three
were already wrong before this change, and the row total is the only one this change moved again, to
105. Six sentences carry them, so a reader meeting the figure a second time meets it wrong twice.
The appendix instructs its own reader to re-derive such a figure whenever a row's status changes,
and the item filed above it did not. The governance record states the same kind of derived figure,
of 108 detail blocks and 104 constraint checks, and a test does read that one: it failed on the
first run after the two rows here were added, which is the whole difference between a figure that is
checked and one that is only written down.

**BL-129: Collect the publication-conditional statements into a runbook**

- [x] List the statements that are true only while this repository is private, and record the search that finds them
- [x] Separate the live statements from the historical ones that must not be rewritten
- [x] Record the settings that become available on the day, and the one review that gates them
- [x] Link it from the entry points a maintainer would actually reach it by

Constraint gate: checked 1 to 11, none breached.

Twenty-one passages across ten files are true only while this repository is private, and they were
...
Publication falsifies all twenty-one in the same moment, none of them is a `path:line` claim, and so
no gate will notice. The publication gate and the surface gate both pass on a tree full of them,
because what they check is history and what the remote advertises rather than prose.

The list was wrong twice before it was right, and the second miss is the more instructive. The first
enumeration matched single lines against a pattern requiring the word "repository", so a passage
carried over two lines was invisible to it, and it missed both of `CHANGELOG.md`'s released notes
about protections that cannot be switched on yet. The second read passages rather than lines, and
still keyed every sweep on the private side of the phrasing, so it missed every passage written from
the public side. That is how a file quoting GitHub's own "make this repository public to enable this
feature" stayed off a list that already cited a block pointing straight at it.

So this no longer offers a count as its evidence. The runbook names every passage it holds, states
the rule for membership, and carries the search that finds them, whose recall was measured at
twenty-one of twenty-one with two false positives, both of which are sentences about the list rather
than members of it. A count is a claim a reader cannot check. A list with the search beside it is one
they can, and this item is the second time that difference was paid for rather than argued.

Ten of the twenty-one are in this file and two are in `CHANGELOG.md`. Eleven of those twelve sit
inside blocks recording what was true when a piece of work was delivered. Rewriting those would
corrupt a record to tidy a document, so the runbook marks them as history and leaves them alone. The
twelfth, in this file's introduction, is a live claim about which acceptance items cannot currently
...
refused on this one today, each was checked against the API rather than assumed, and each already
has an item recording the refusal. The runbook adds the ordering, since push protection is accepted
without secret scanning and then does nothing, which is the failure most likely to be mistaken for
success on the day.

None of it is enforced, and the runbook says so in its own last line rather than implying otherwise.
A gate for prose that is deliberately conditional would have to know when the condition changed, and
the only thing that knows that is the person changing it.
...
BL-099 asks for a legal review and cannot be closed without one. This item is about the other
variable, which is the size of the question that review has to answer. Nothing here is a legal
opinion and nothing here decides anything. It makes the ask smaller and the answer cheaper, and
those are the only two things this side of the boundary controls.

The sharpest item is severable, and it was measured on 2026-08-15 rather than estimated. 798 of the
1,473 curated records carried a Marvel description, 151,840 characters in all, and the provenance
record already named that field as the one to look at hardest because it is Marvel's own prose
reproduced verbatim rather than a fact about a publication. Everything else in a record is an id, a
title, a number, a date, a series, a link or a creator credit.

Removing it costs one sentence on one screen. The field reaches the interface through exactly one
consumer at `src/js/main.js:2446`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2630-2639` returns a sentence saying no synopsis is
recorded, for a record that was hydrated and came back without one, and a test already asserts it.
Catalog search does not read the field either, because the description it searches at
`src/js/lib/catalog.js:143` is the order's own one-line summary, written in this repository. So this
is a data change with a shipped fallback already waiting for it rather than a feature to build.

Drift: the field no longer reaches the interface at all. BL-134 closed the last path that could put
one into saved state, so `normalizeIssue` stopped carrying it and there is nothing left on a record
to read. The consumer named above is still the only one and still decides what a reader sees, but
what it now receives is a synopsis fetched for the session and held in memory. The sentence stands
as what this item shipped believing, and the count of screens it costs is now zero rather than one.

It is also reversible, which is worth establishing before anyone deletes anything. The upstream API
is still answering: the contract check ran on 2026-08-15 and 33 of 33 assumptions held across 17
requests. The descriptions can be fetched again by the vendoring script if the review comes back
permissive, so this is a decision that can be taken back rather than a one-way loss.

The second task is smaller and settles something this repository cannot answer from inside itself.
The third of the four open questions asks whether a licence stated in a README, with no licence text
and a package declaration scoped to a source directory, conveys anything for the two reading orders
vendored from that project's `data/` directory, which is outside it. Only the upstream maintainer
knows what was meant. The answer does not unblock publication either way, because that project does
not hold Marvel's rights and so cannot pass them on, but it replaces an inference with a statement
in a document whose whole purpose is to avoid inferring.

The first task shipped on 2026-08-15. All 798 descriptions are `null`, the key stays present on
every record so nothing downstream changes shape, `scripts/vendor-orders.mjs` writes `null` instead
of the fetched value with the reason in a comment beside it, and `test/licence-boundary.test.js`
fails if any returns. What the app loses, measured rather than predicted, is one sentence on one
screen: the 675 records that already carried no description render an absence today, 606 of them
through the sentence saying no synopsis is recorded and 69 through the sentence saying the snapshot
holds no record at all, and the 798 now join the first group. The contract check is deliberately
left asserting the field upstream, because that assertion is the evidence the change is reversible.

That first pass was incomplete, and how it was incomplete matters more than the shortfall. It read
`src/data/catalog.json`, walked the fourteen files listed there, and stopped. `design/mockups/mock-data.js`
is a generated projection of one of those orders, no catalog names it, and it held 41 more
descriptions totalling 7,193 characters, all 40 distinct values byte-identical to ones shipped under
`src/data/`. The guard test inherited the same blind spot, because it read the same catalog, so it
passed on a tree that still shipped the prose. Both now walk the tree rather than a list, and the
widened test was proven to fail on the un-stripped mockup with exactly 41 offenders. The provenance
document had never named `design/` either and now carries a subsection for it. This is the defect
shape the anchors gate's own header warns about, an enumeration somebody has to keep complete, met
in a second place.

One thing the strip does not reach, found while doing it and worth stating plainly rather than
leaving for someone to discover after publication: git history. 243 of the 246 commits then on
`main` carry the prose, and a clone of a public repository carries the whole history, so 455
distinct descriptions and 89,460 characters remain recoverable from it. The distinct figures run
below the 798 and 151,840 because the same issue sits in more than one reading order. Both figures
are raw character counts, measured the same way, which is worth saying because the first version of
this paragraph reported 89,558, the length after JSON escaping, and compared it to a raw count.
...
Drift: the first clause of that sentence is false, and the measurement that falsifies it is recorded
under BL-131. Rewriting history does not reach this prose, because the forge does not collect what a
force-push orphans and serves the orphans through the pull request refs among other doors. The
sentence stands as what this item shipped believing, which is the treatment this document gives a
superseded measured statement; what it asserts does not stand.
...
- [x] Verify the rewrite by re-running the evidence anchors gate and the full suite on the result
- [x] Choose a route that reaches what a force-push cannot, because pushing here does not
- [x] Record the chosen route and its reason in the publication runbook either way

Constraint gate: checked 1 to 11, none breached.

This exists because BL-130 removed 798 Marvel descriptions from the working tree and that is not the
same as removing them from the repository. Measured on 2026-08-15: 243 of the 246 commits then on
`main` carry item description prose in the vendored orders, and 455 distinct descriptions totalling
89,460 characters are recoverable from them. The distinct figures sit below BL-130's 798 and 151,840
because the same issue appears in more than one reading order. A clone takes the whole history, so
what BL-130 achieved is that the app serves none of it and a checkout contains none of it, which is
most of the value and not all of it.

The reason this is not a note inside BL-130 is that it expires, and nothing else in this backlog
does. The repository has never been public and has no forks, so a rewrite today invalidates nobody's
clone and breaks nobody's fork. The moment it is published, both of those stop being true and the
prose is out irretrievably. That is the whole of the time criticality score, and it is why an item
worth 6.8 sits above things that will still be worth doing next year.

Neither answer is recommended here. A rewrite is destructive, it changes every commit id in the
repository, and it would invalidate `docs/anchors.lock.json` wholesale, so it is not a thing to do
casually or on an agent's judgement. Accepting the history is a legitimate answer too, and it is the
cheaper one. What is not legitimate is arriving at it by not noticing, which is what this item
prevents.

One thing to not misread on the way. `npm run publication` also scans history, and what it reports
is true of what it looks for, which is personal and credential-shaped content and the ignore rules
that keep session evidence local. It says nothing about third-party prose and was never meant to.
Its own header sets out both halves of its population at `scripts/check-publication.mjs:7-13`. A
green run there is not an answer to this item.

That paragraph was written while the gate still signed off with the words "the history is clean",
which is why it was needed. The wording was corrected in the same change, because the sentence was
accurate about the population and wrong about the reader: the one person most likely to run this
...
**A force-push does not reach it.** The forge does not collect what a force-push orphans, and its own
guidance names three places the commits survive one: clones and forks, cached views addressed
directly by their SHA-1, and any pull request that references them, retrieved 2026-08-16 from
`docs.github.com/en/authentication/keeping-your-account-and-data-secure/removing-sensitive-data-from-a-repository`.
So the pull request refs below are the easiest door onto the residue rather than the cause of it, and
a repository with no pull requests at all would not be cleaned by a force-push either. This one has
116, and a forge serves every one of them a permanent `refs/pull/<n>/head` that the owner cannot
rewrite, delete or force past.
...
What the refs hold is not what the discussions hold, and the difference bears on the route choice.
Scanning every pull request title and body here, 116 of them, together with the 44 issue and review
comments the forge returns for this repository, finds none of the 455 descriptions reproduced in
full. The one hit is a deliberately truncated 60-character excerpt in this branch's own pull request
body, quoted there to show what was being removed. So the exposure sits entirely in the code the
refs point at rather than in anything a person wrote, which makes losing the discussions a cost of
the new-repository route rather than a second leak that route would have to clean up. That scan also
re-derived the set it was searching for, taking it from the pre-strip `main` rather than from the
refs, and got 455 distinct descriptions totalling 89,460 characters: the same two numbers again, from
...
does not have. Across 251 commits it rewrote 248 and left 3 untouched. That 248 is not the 243 of
246 above with this branch's commits added, and an earlier draft of this paragraph reconciled them
that way: a rewrite changes the object id of every commit from the first one it touches onward,
whether or not that commit carries prose, so 248 counts descendants where 243 counted carriers. The
comparable figure at the branch tip is 244 of 251 carrying prose, and the 3 untouched are the only
common ground between the two counts. Commit subjects, authors and dates are identical either side.
Two properties make it checkable rather than merely finished: applied to the
...
One risk in the new-repository route was hypothesised and then tested rather than left standing. The
lock holds 9 anchors targeting `src/data/` or `design/mockups/`, so a rewritten `main` could
plausibly have landed with a red gate on its first day. Two qualifications, both measured, and the
first corrects an earlier draft of this paragraph which called those nine exactly the files the
rewrite edits. They are not. The rewrite edits 14 files and only 2 of the 9 sit on one of them: the
mockups' generated data file at its first line, and the New Ultimate Universe trades order at its
first ten. The other seven cite a catalog file, two order notes and two mockup pages, none of which
carries an `items[]` description or is ever opened by the transform. Second, nothing moves in any
case, and that is a property of the transform rather than luck: it replaces each description value
with `null` in place instead of deleting the entry, so every file keeps its line count. Measured on
the file likeliest to break that, the mockups' generated data file is 1,215 lines before the rewrite
and 1,215 after. Do not read that across to the shipped strip on this branch, which is 1,219,
because BL-130 also replaced a one-line provenance header with five; the transform accounts for none
of those four lines. Run
against the rewritten `main` all seven gates are green: 0 drifted anchors, 963 tests passing, and
lint, counts, sizes, contrast and the publication gate clean. Note the 963 rather than 966, which is
main's own suite before this branch adds three, and note that it passes on a stripped tree, so
nothing on `main` asserted the prose was there.
...
working artifacts. Asking the forge's support to purge what is left was described here as the only
route that keeps both, and that was wrong on both halves. Its published policy on the page cited
above says it will not remove non-sensitive data, and will assist on sensitive data only where the
risk cannot be mitigated by rotating a credential; third-party marketing prose is neither, so on the
published terms the route probably does not exist. The same page says the removal takes the internal
references that render pull request diffs with it, so even granted it would cost the diff view on
every closed pull request, which is most of what the route was supposed to preserve. What is genuinely
unknowable in advance is not the shape of the residue, which `git ls-remote origin 'refs/pull/*/head'`
enumerates today, but whether support would exercise discretion against its own published policy.
...
**Resolved on 2026-08-16: accept.** The owner chose to leave the 455 descriptions in history and
behind the pull request refs, and to publish this repository as it stands rather than rebuild it
somewhere clean. Three things made that the cheaper answer. What is exposed is a third party's
marketing copy rather than anything private, so the harm it can do is a licensing question rather
than a security one. The working tree, the app and every checkout are already clean of it under
BL-130, so what remains is reachable only by someone who goes looking through the object store. And
the only route that would have removed it costs all 116 pull request discussions, which are most of
the record of how this repository was built. The rewrite is kept rather than discarded, because it
is what makes the decision reversible in the one direction that matters: if the rights holder ever
asks, the repository can be made private again and the new-repository route run then, on a transform
already proven to reproduce the tip byte for byte. What is not reversible is the exposure between
publication and that moment, and that is the risk being accepted rather than a gap in the plan.

**BL-132: Measure what the day-of rewrites cost the anchors gate, before they are made**

- [x] Count the citations pointing into the files that carry the publication-conditional passages
- [x] Separate the ones that break whatever is written from the ones that break only on a line shift
- [x] Prove the separation by experiment rather than by reading the gate
- [x] Record the cheaper method in the runbook, above the edits rather than below them
- [x] Check whether any test defends the prose, so a red suite is not mistaken for one that does

Constraint gate: checked 1 to 11, none breached.

BL-129 collected the twenty-one passages publication makes false and said what each one asserts. It
did not say what rewriting them costs, and the answer is not small: nine files carry the ten live
passages, and 54 citations point into those nine. Left alone that is a 53 anchor round, run on the
night of the flip by whoever is also turning on three settings and reading a table of twenty-one
passages.

The useful part is that 39 of the 53 are avoidable. A citation sitting after an edit in the same
file breaks only when the replacement has a different line count from the passage it replaced, so
matching the count confines the round to the 14 that overlap a passage and cannot be saved. That is
a constraint on prose being rewritten anyway, which is what makes it worth stating rather than
merely true.

It was proved rather than reasoned, because reasoning about this gate has gone wrong here before.
The security policy's publication-conditional paragraph is six lines, and that file carries a
citation ninety lines below it. Replacing the six with six drifted two anchors, both overlapping the
paragraph. Replacing the six with eight drifted three, the third being the distant citation that no
edit had gone near. The experiment also corrected this item's own first count: classifying citations
by whether they begin before the first edit called a range running from line 29 to line 42 safe,
when it contains the paragraph and in fact breaks in both arms. Overlap is the test, not the start
...
**BL-133: Turn on the three settings publication unlocks, and make the conditional prose true**
...
The repository was published on 2026-08-16, which is the event BL-129 wrote the runbook for and
BL-132 costed. Three settings that had been refused outright became available. All three were turned
on and then read back rather than assumed to have taken: secret scanning and push protection both
report `enabled`, private vulnerability reporting answers `{"enabled":true}`, and the branch rules
read back with three required checks. Secret scanning found nothing, which is the answer a
repository whose history was examined at length in BL-131 wanted to hear.

The order the runbook insisted on was real rather than cautious. Push protection was left alone
until the alerts endpoint stopped answering 404, and it answered with an empty list once scanning
was on. Asking for push protection first is accepted and changes nothing, and that trap is still
written down in the security policy, because it stays true for anyone forking this repository even
though it is no longer true here.

Branch protection needed a decision the task had left open, because "the intended reviews and
checks" cannot both be had. The checks are documented: `GOVERNANCE.md:44-46` says the maintainer
does not merge past a red gate, so the three continuous integration contexts are required, taken
from a real run rather than derived from the workflow file, which is where a matrix job's name is
easy to guess wrong. A required review is not documented anywhere and cannot be satisfied here:
there is one collaborator, GitHub never asks a pull request's author to approve it, and the rule
would either block every merge or be bypassed by the same person it was meant to constrain. It is
refused for that reason rather than omitted. Force pushes and branch deletion are refused outright,
which is the part that matters most to a repository that decided in BL-131 to keep its history. The
...
wrong about its citations: the nine rows name passages by `path:line`, the gate holds those claims
live whether the document is spent or not, and rewriting the passages changed what those lines say.
Leaving it alone would have meant blessing "private reporting cannot be turned on here" onto a line
that says it was turned on, which is the corruption the gate exists to prevent rather than a pass.
The document is stamped as executed and its table header put into the past tense instead, which is
cheaper than rewriting it and keeps every row true as a record of what the passage used to say.

**BL-134: Fetch a synopsis at read time instead of shipping one**

- [x] Close every path that writes an issue synopsis to saved state
- [x] Keep the prose out of both caches, and empty the one that already holds it
- [x] Fetch a whole reading order on request, sequentially, behind a provenance disclaimer
- [x] Prove each promise with a check observed to fail without its fix

Constraint gate: checked 1 to 11, none breached.

BL-130 removed 798 Marvel descriptions from the vendored data and measured the cost as one sentence
on one screen. That measurement was honest and it was also the whole of what the app then offered:
every curated issue now answers "No synopsis is recorded for this issue", including the 798 that used
to say something. This item is the other half of that trade. The prose is fetched from the same
service the app already asks for covers and reader links, it is shown, and it is never written down.

The four promises are structural rather than procedural, which is the only reason they are worth
believing. `normalizeIssue` no longer carries the field, and both `upsertIssue` and `coerce` reach
saved state through it, so there is one gate rather than a rule to remember. `SynopsisRunner` at
`src/js/synopsis.js:89` writes only to a `Map` and never calls `store.update`, so the promise holds by
construction. `withoutSynopsis` at `src/js/api.js:34` strips the field before the response cache is
written, and every request now carries `cache: 'no-store'` at `src/js/api.js:95` and again on the
reader launch path, which fetches the same endpoint from a different module.

The browser's own cache is the store the plan missed and the critique found. It is on disk, it is
written by the same fetch, and the settings screen's button to clear cached metadata cannot reach it.
Without the directive the memory-only promise would have held in every store except the one a reader
has no way to inspect, which is the worst place for it to fail.

Two more critique findings were quiet failures rather than visible ones. `ResponseCache.clear` used to
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:649` would have recorded
a purge that never ran and left thirty days of unstripped prose sitting there with the app believing it
gone; the clear now reports, and the marker only advances on success. The marker is merged from storage
at write time rather than from the settings snapshot, because settings are read once at boot and
written whole, so a tab left open across the upgrade would have erased the marker on the reader's next
theme toggle.

The run is sequential with a lookahead of eight, which was the owner's decision and turned out to
match a shape the hydrator already had. Extracting it as `lookaheadPriority` at
`src/js/lib/model.js:831` exposed a defect in the original: it sliced the first nine ids and then
filtered, so a reader resuming a list whose first issues were already answered got a group of nothing
and a run that stopped. Applying the predicate first is a behaviour change to hydration as well, made
deliberately, because eight useful answers is what the number was always for.

The disclaimer at `src/js/main.js:2977` is shown every time a run starts rather than once and
remembered, by owner decision, and the reading order is captured before the dialog opens so a reader
who switches list while it is up does not start a run against the list they left.

Ten mechanisms were each reverted on their own and every one named the tests that defend it, which is
a sharper measurement than reverting the modules and watching the suite go red. The proof script had
to be repaired before it proved anything: its first run reported five of the ten as skipped, because
its anchors were written with Unix line endings against a tree checked out with Windows ones, so it
matched nothing and said so in the same shape as a pass. That is this repository's rule about checks
never seen to fail, met one level above the checks.

What this does not do is make the app work offline or faster. A run competes with hydration for the
same twenty requests per ten seconds, so "a synopsis in seconds" is the best case rather than the
promise, and the first session after upgrading is slower because the cache is emptied once. Both are
recorded in the changelog rather than left for a reader to report as a fault.

Two independent reviews of the change found nine defects between them and all nine were fixed in it.
Three were the same class of fault, a claim the interface makes that the code cannot support: a run
in which every lookup failed still announced that all synopses were fetched, an issue the service had
just answered "no synopsis" for still read as not yet fetched, and the disclaimer named one service
by hand while the reader is free to point the app at another. The remaining six were quieter. A
response landing after a cancel was recorded before the guard that was supposed to stop it, so prose
appeared that the "stopped after n of m" count had already excluded. `synopsisOrder` queued
already-read issues ahead of unread ones, against its own comment. Nothing capped the length of a
fetched synopsis once `normalizeIssue` stopped carrying the field that used to. `ResponseCache.clear`
reported success from a transaction that could still roll back, which is the same false success the
purge marker exists to prevent. Changing the service address left the run still asking the old one.
And saved state that already held prose from before the upgrade was cleaned in memory on every load
and never written back, so it sat on disk indefinitely; it is now rewritten once, on the load that
finds it. Seven of the nine are proven by reverting the fix alone and naming the test that goes red,
and the other two by driving a real browser.

A third review, this one aimed only at the nine fixes, found one defect they had left behind and
confirmed the rest. The partial ending stopped a finished run claiming answers it never got, but the
same count runs through the stopped ending too, and `done` is incremented for a refused request
exactly as for an answered one. So a reader who stopped a failing run was told it had fetched what it
had in fact failed to reach, next to a hero still saying no synopsis is recorded. The count now
subtracts what could not be reached and names it, as the finished ending already did. Extracting the
line as `synopsisStatusLine` is what makes that testable: it was inline in a renderer that needs a
document, so the visible half of the earlier fix had been proven only in a browser while its spoken
half had a unit test. Both halves of this one are proven by reverting each in turn.

That review also found the transaction fault's own test weaker than it looked. The double that stands
in for IndexedDB did not roll back an aborted transaction, so the test could assert only the returned
value and not the thing the value describes, which is that the prose is still in the store. The
double rolls back now and the test says so, and removing the rollback turns exactly that assertion
red. A fake that cannot express the difference between two contracts cannot tell you which one you
have.

A fourth review, aimed at that fix, found the same counter read a third time and left unsubtracted:
the line shown while a run is going. The arithmetic there was no more wrong than it had been, but
fixing two of the three reads is worse than fixing none, because the three had at least agreed with
each other before. Measured against the real runner on a run losing two of its first three, the
reader watched the number climb to 3 and pressing stop rewrote it to 1, so the count appeared to go
backwards at the one moment it was being looked at. The running line now subtracts and names the
failures exactly as the two endings do, so the number a stop leaves behind is the number that was
already showing. It also holds still while a wholly failing run works through its queue, which reads
as stalled and is the honest report, so the failure count beside it is what shows the run is moving.

That review also noted `synopsisStatusLine` was exported without the guard its only caller applies,
so calling it before a run had started returned the sentence for a run that had finished. It returns
nothing for that case now, which is what the renderer has always done with it. Both are proven by
reverting each alone, and the harness that proves them was itself wrong first: it filtered for a TAP
failure line against a runner that prints the spec reporter, so it reported every mechanism green
including ones that were genuinely broken. It reads the fail total back as a cross-check now.
...
refuse, which starved the wait and reddened only the harness's catch-all row, so
...
pair as its own evidence. The pair was reproducible before by reverting the tree by hand, which is a
thing a reader has to be told to do and has to do correctly; it is printed by a committed check now.

**BL-135: Refuse an issue synopsis at the boundary that writes, not only at the one that reads**

- [ ] Reject issue prose where saved state is written, rather than where each writer builds its value
- [ ] Reject it again where a backup is exported, so a copy cannot carry what state may not
- [ ] Prove the guard fires by writing prose past `normalizeIssue` and watching it refuse

Constraint gate: checked 1 to 11, none breached.

BL-134's promise that no synopsis reaches saved state holds today, and it holds by enumeration. Five
places write `state.issues`; four reach it through `normalizeIssue`, which no longer carries the
field, and the fifth is a cache read whose single caller strips it first. That was checked rather
than assumed, and finding no live defect is why this is an enabler rather than a fault. But an
enumeration is a list somebody has to keep complete, which is the shape of defect this repository's
anchors gate exists because of, and a sixth writer added later would breach the promise silently.

A guard at `Store.update` and at `exportBackup` turns the promise into one that cannot be broken by
adding code elsewhere. `exportBackup` copies the map verbatim at `src/js/lib/model.js:1120`, so it is
the second boundary rather than a duplicate of the first: a backup written before the guard existed,
restored after it, would otherwise reintroduce exactly what the guard refuses.

The open question is what refusing should mean. Throwing turns a licensing promise into a crash on a
path readers depend on; stripping silently keeps the app working and hides the bug that a test would
otherwise catch. Stripping plus a console warning is the likely answer, because the promise is about
what reaches disk and nothing about the reader's session needs to end for it to be kept.

**BL-136: Decide what happens to the prose in copies kept for recovery**

- [ ] Say in one place whether a recovery copy is rewritten, left alone, or discarded
- [ ] Apply that decision to the pre-restore copy and to every salvage copy
- [ ] Prove the decision by observing a copy taken before the upgrade and read after it

Constraint gate: checked 1 to 11, none breached.

The load path now rewrites saved state once when it finds descriptions left over from before
BL-134, which closes the ordinary case. It does not touch the copies kept so that a failed restore
or an unreadable state can still be recovered. The pre-restore copy is written from the raw prior
value at `src/js/storage.js:547`, and the salvage copies are the same shape, so any of them taken
before the upgrade still holds the prose that the main key no longer does.

Rewriting them is not obviously right and that is the whole of this item. These are the copies that
exist because something already went wrong, they are the least exercised paths in the app, and a
rewrite that fails leaves a reader with neither their state nor the copy of it. Discarding them
outright is worse. Leaving them is defensible: they are transient, the pre-restore copy is cleared
on the next successful restore, and nothing displays their contents.

So the deliverable is a decision written down and applied consistently, not a change of behaviour
chosen in advance. Whichever way it goes, the promise BL-134 makes should say what it means for a
copy taken to survive a failure, because at present it is silent and a reader would reasonably read
it as covering everything on disk.

**BL-137: Close the window a tab left open across the one-time purge is writing into**

- [ ] Stop a tab loaded before the upgrade from writing unstripped responses into the shared cache
- [ ] Stop it from rewinding the purge marker the new tab has already advanced
- [ ] Prove both by driving two contexts against one cache

Constraint gate: checked 1 to 11, none breached.

The one-time purge at `src/js/main.js:655` empties the response cache once, so the prose cached
before BL-134 goes. A tab that was already open when the upgrade landed is still running the old
code, still caching unstripped responses into the same shared store, and still holding the old
settings snapshot, so its next write can move the purge marker back and buy the next session a
second purge.

Neither consequence is severe, which is why this ranks where it does. The stale tab's own writes are
bounded by how long it stays open, the marker rewind self-heals as one repeat slow session, and both
require the reader to have left a tab open across a version change and then used it. It is recorded
because the cost of finding it later, from a report of the app being slow twice, is far higher than
the cost of writing it down now.

The likely shape is a version stamp the cache carries, so a write from code that predates the purge
...
browser holds the real IndexedDB, the real `localStorage` and the real page, so it is the one place
...
- [x] Check each manifest claim against the file or origin it claims about
- [x] Say in the README that installing does not remove the need to start the server
...
that decide where an installed window lands, at `src/manifest.webmanifest:6-7`, are origin-relative
for exactly that reason. An absolute address
would have pinned an installed window to 8787 and quietly emptied the app for anyone running on
another port, which is the same defect the README's warning about the address is written against.
...
would go red for a reason that has nothing to do with the picture.

A maskable icon was considered and refused. It exists so Android can crop an icon to whatever shape
the launcher uses, and this app cannot run on a phone at all, because the page is served by a
program running on the same machine. Shipping one would have been a third file, drawn to a padding
rule nobody here can check against a device.

Both new checks were proved able to fail before they were trusted. Removing the manifest link from
the page turns one red, and widening the glyph stroke in the generator without regenerating turns
the pixel comparison red, which is the case that matters: it is the one that catches a committed
icon quietly ceasing to be the icon the source describes.
...
- [x] Say what to do when Node.js is missing, in the window, rather than closing it
- [x] Leave the address alone, so a launched app is the same app with the same progress
- [x] Test the contract, because the reader of these files cannot diagnose them

Constraint gate: checked 1 to 11, none breached. Constraint 5 is the one this item is most able to
breach and the one its tests are built around: a launcher that set `MRT_PORT`, or that opened
...
double-clicking it is the whole feature, and there is almost nothing to it.

Almost nothing, but not nothing, and the parts that are not nothing are all failure modes that
happen on a machine nobody here is sitting at. A double-clicked script does not start in the
project folder, so both files move there first and both tests check the move happens before the
server is started. A `.command` with Windows line endings is refused by the kernel before its first
line runs, with a message naming an interpreter the reader never typed, so `.gitattributes` pins
the endings of both extensions and a test reads the blob as git holds it rather than as this
machine checked it out. The executable bit is recorded in the git index for the same reason: a
clone that loses it produces a permission error rather than an app.

The Windows file is written as one-line statements and gotos rather than parenthesised blocks. That
is not a style preference. `cmd.exe` has never parsed a multi-line block reliably in a file with
Unix line endings, and the reader this is for arrives by "Download ZIP", where the endings are the
archive's decision rather than this repository's. The pinning above makes CRLF likely; writing the
file so it does not care makes it certain, and a test holds that shape by refusing any line that
ends in an opening parenthesis.

Every one of the 8 checks was proved able to fail, by eleven single-change mutations applied one at a
time and each reverted before the next. Nine were edits to the two launchers and to `.gitattributes`:
removing either directory change, setting a port, naming localhost, opening a parenthesised block,
removing the address for Node.js, removing the pauses, misspelling the server file, and unpinning the
endings. The remaining two checks are about how git holds the file rather than what is in it, so no
edit reaches them and the mutations are index operations: untracking the macOS launcher, which turns
both of those checks red, and clearing its executable bit, which turns one. All eleven named the test
that defends them. Two earlier drafts of the suite did not: one assertion read the file with its
comments left in and so was satisfied by a paragraph describing the command rather than by the
command, and it was rewritten before it was trusted.

The Windows launcher was then run for real. The server printed its startup banner and `curl`
returned 200 for the page, the manifest and the 192 pixel icon. The macOS file is the same contract
...
- [x] Prove the whole thing in a real browser, with the server genuinely stopped

Constraint gate: checked 1 to 11, none breached. Constraint 1 is the one a service worker is most
able to breach, because caching whatever the page asks for is the obvious way to write one and
cover images are exactly what the page asks for most. It is answered structurally rather than by
care: the worker declines to answer any request for another origin at all, covers come from Marvel's
...
response comes from it, so an edited file is seen immediately and the no-cache and ETag behaviour in
`server.mjs` keeps working exactly as written. The store is consulted only when the fetch fails,
which is the case this exists for. A cache-first worker is the shape people reach for and it is
worse in the one way that is hardest to notice: it works perfectly until somebody edits a file, and
then it serves yesterday's copy with nothing on screen to say so. A deliberately cache-first version
of this worker passed every other test in the suite, which is why the test that catches it exists.
...
write can be refused, for quota or for a partial response, and an uncaught refusal would turn a page
that had just been fetched successfully into a failed load. So the store is best effort and its
...
seen and what you have already saved, and says nothing new. The metadata service being unreachable
...
**BL-142: Add twelve reading orders compiled from Comic Book Herald guides**

- [x] Compile a Captain America order, a Doctor Doom primer, a Spider-Man order, a Thor order and a
  Scarlet Witch order
- [x] Compile the seven Avengers orders that together run from 1963 to the Fresh Start era
- [x] Resolve every issue against Marvel's own metadata rather than against the guide's prose
- [x] Name every row that could not be resolved, in the file, rather than dropping it quietly
- [x] Credit the guides for what was actually taken from them, and no more

Constraint gate: checked 1 to 11, none breached. Constraint 4 is the one a batch this size invites a
breach of, because twelve orders is where somebody reaches for a scraper library. Nothing was added
to `package.json`: the generators are throwaway scripts that ran outside the tree and are not
committed, and the only thing committed is their output. Constraint 2 is answered the same way. No
page on `marvel.com` or `read.marvel.com` was fetched by anything here; the guides were read by a
person, and every issue behind them was resolved from the metadata mirror the repository already
uses.

Twelve orders, 1,178 items, 1,151 distinct issues drawn from 87 series. Every one resolved: no
placeholders, no missing digital ids, no missing covers, and no record that came back empty. That
last figure is not luck. The latest on-sale date anywhere in the twelve is 2020-02-12, comfortably
inside the window the retired developer portal covered, so none of them land in the gap that leaves
63 bare records in the two Ultimate orders.

The debt to Comic Book Herald is real and it is narrow, and BL-106 is the precedent for saying so
carefully. What was taken is the selection and the grouping: which runs are worth reading and which
arcs belong together. What was not taken is the issue-level sequence, which the guides mostly do not
give, or the identifiers, or the metadata. Those were resolved here, issue by issue, and in one case
the resolution contradicted the guide outright. So the credit line reads "Compiled for this project
from Comic Book Herald's guide" and links to the page, which says both halves: the guide chose the
shape, this repository built the contents.

The resolution rule was settled once, in advance, rather than row by row, because the alternative was
forty separate judgement calls made in the dark. Marvel's metadata wins wherever a guide's volume
label disagrees with it. A nested range ships once, as the containing range. A row with no issue
numbers resolves to the whole run. Sections keep the guide's order. And anything genuinely
unresolvable is named in the file's own header, never guessed at and never silently dropped.

That last clause did most of the work. Nine rows are named as unresolvable, and they fall into three
honest kinds. Two are not issues at all: `Triumph & Torment` is a trade paperback in the metadata and
`Marvel Graphic Novel #27` is a graphic novel, so neither has an issue record to link. One is absent
from Marvel's series index entirely, the 1994 `Scarlet Witch` mini. And one is a hole in the mirror
rather than in the guide: `Astonishing Tales (1970) #8` is missing while #7 and #9 to #24 are
present. That last pair took the whole Cynthia von Doom section of the Doom primer with them, which
is recorded in that file rather than papered over, because a section that quietly shrank to nothing
is worse than a section that says why it did.

The single most dangerous row was the one that looked easiest. The Spider-Man guide lists "Coming
Home" as `Amazing Spider-Man #30-35`, and there is an `Amazing Spider-Man #30-35` in the 1963 volume:
six real issues, published 1965, about the Cat Burglar. The arc meant is the 1999 relaunch. Nothing
in the guide's text distinguishes them and both resolve cleanly, so this is the kind of error that
ships looking correct and is found by a reader years later. It was caught by checking each resolved
issue's on-sale year against the era the section claims, which is now the reason that check exists.

Two smaller traps were found the same way. Marvel numbers point issues inside several of the named
ranges, and they are real issues rather than artefacts: `#699.1`, `#6.5`, `#1.5`, `#12.1`, `#21.1`,
and `#1.1` through `#4.1` of Avengers (2016). All are included and all are recorded, and the last
four are the entire reason the All-New All-Different order holds 92 items where the arithmetic on the
guide says 88. Separately, the mirror carries four genuine duplicates, two ids for one issue, in
Avengers/Invaders #2, Savage Avengers #1, and Avengers (2016) #5 and #1.1. Those are deduplicated to
the lower id, which keeps the rule BL-135 set down: a list holds an issue once.

Every count was reconciled against hand arithmetic before it was accepted. Ten of the twelve matched
first time; the two that did not were both diagnosed to a cause rather than adjusted to fit, which is
how the point issues and the duplicates were found at all. A count that disagrees is evidence, and
the temptation is to trust the machine and move on.

**BL-143: Shelve the catalog in reading order rather than in the order the manifest happens to list**

- [x] Record when each order's reading starts, as a fact about the order rather than a guess
- [x] Leave an order that ranges across the timeline out of the sequence instead of forcing it in
- [x] Sort once, where the catalog is parsed, so every view of it agrees
- [x] Keep grouped variants of one order together

Constraint gate: checked 1 to 11, none breached.

Twelve new orders turned a shelf you could read into a list you had to search. The order they
appeared in was the order the manifest happened to list them, which is the order they were added,
which is no order at all: Essential Avengers, which opens in 1963, sat below an order that opens in
2023.

The year is recorded rather than derived, and that is the decision worth defending. Deriving it from
the earliest on-sale date in the file gives the wrong answer for exactly the orders that need it
most: a best-of that opens in 2004 and then works back to 1966 would sort to 1966, where nobody
looking for it would think to look. Deriving it from `type` is worse, because the type does not carry
the distinction. A creator run can sit squarely on the timeline and a character run can range across
the whole of it, so the field would be answering a different question from the one asked.

So the absent value means something specific: not "unknown", but "this order ranges across the
timeline instead of sitting on it". Seven orders are undated on that basis and all seven are
retrospectives. The other nineteen carry a year, each checked against the on-sale year of the first
issue in its own pinned file.

The sort is stable, which is load-bearing rather than incidental. Six orders share a year with
another, and three of those pairs are the same event at two reading depths. An unstable sort would
separate the pair that the catalog groups and renders as one card with a depth toggle. Ties keeping
manifest order is what holds them together.

It is applied inside `parseCatalog`, at the single point where the catalog becomes data the app can
use, rather than at each place the app renders it. The home page shows only the first few cards, so a
sort applied at render time in one place and not the other would have produced a home page whose
"first" orders were a different set from the catalog's first orders. One call site cannot disagree
with itself.

Two judgement calls are worth flagging, because each is a one-value change if the reading is wrong.
The Essential Avengers order was retyped from a character run to an era and dated 1963, so that it
heads the Avengers spine the seven new orders form. The Claremont X-Men order was left undated,
because it is a single title's retrospective rather than a place in the story. Neither is enforced by
anything; both are editorial and recorded here so they can be argued with.

One thing was deliberately not done. The new Civil War Avengers order could be folded into the
existing Civil War group as a third reading path alongside the full event and the essentials, and
there is a case for it. It was left out because it is a different question from the one asked, and
grouping is a decision about how a reader chooses between orders rather than about where they sit.

**BL-144: Shelve one card per story, and let the reader pick a reading path inside it**

- [x] Read the grouping the catalog already carries, instead of shelving each reading path
- [x] Fold the Civil War Avengers order in as a third path, which BL-143 left open
- [x] Group before the twelve-card cap, so folding frees a slot rather than wasting one
- [x] Count stories wherever the app says how many orders there are
- [x] Keep the choice off the shelf card, and put it where the reader has already decided to look

Constraint gate: checked 1 to 11, none breached.

Twelve new orders made a problem visible that had been latent since the second reading depth was
added. The catalog carried `group`, `groupName` and `variant` on every entry that is one way of
reading a story someone can read more than one way, and the home shelf ignored all three. So the
shelf showed twenty-six cards for nineteen stories, and seven of the twelve cards it had room for
were three stories: Civil War three times, House of M twice, Secret Invasion twice. A reader
scanning for something to read met the same title three times before meeting the second story.

Reading them is the whole change. Folding the six groups leaves nineteen stories, so the twelve
cards on the landing page are now twelve different stories rather than eight. The grouping happens
before the cap rather than after it, which is what makes the freed slots useful: fold after the cap
and the shelf shows nine cards, fold before it and the shelf shows twelve.

Where the app said how many orders there are, it now counts stories. That is not cosmetic. The
filter chips, the "Showing 12 of 19" line, the "See all 19 orders" link and the threshold that
decides when the search box is worth offering all counted rows, and a reader told there are
twenty-six things and shown nineteen has been given a number they cannot check.

**Where the choice goes was the expensive part, and the first answer was wrong.** The chooser
shipped on the shelf card, as a row of pills under the title. Measured in Edge at 1280x900 with
storage cleared, against the identical page built from the tree before the change: the grid went
from 1149px to 1458px, the tallest card from 293px to 445px, the spread between the tallest and
shortest card from 29px to 181px, the focusable controls in the grid from 24 to 33, and the
speaking surfaces from 65 to 77. A card offering a choice averaged 399px against 331px for one that
did not, and the chooser alone was 123px of a 277px card. The labels are authored prose of 17 to 46
characters and the card column is about 28, so every one of them wrapped to two lines. They were
also pill-shaped and grey, which is exactly what the filter chips 100px above them look like.

The fix is to ask the question once, where it is being asked of somebody who has already decided
they are interested. The card says only that there is a choice and how many ways there are, in one
quiet underlined line reading "3 ways to read this", and pressing it opens the same preview dialog
the card's own button opens. The dialog is full width, so both pills fit on one line, and it lists
the issues right underneath, so choosing a path shows you what you just chose. Re-measured on the
shipped tree: the grid is 1188px against 1149px before any of this, the tallest card 309px against
293px, the spread 45px against 29px, the focusable controls 28 against 24 and the speaking surfaces
68 against 65. The four extra controls and the three extra surfaces are the four cards that now say
there is a choice, which is the whole of the cost.

The catalog keeps its visible pills, and the difference is the column width rather than a second
opinion about the design. A catalog row is the full width of the pane, both labels fit on one line,
and the reader arriving there is comparing orders rather than scanning a shelf.

Three things in the implementation are worth naming because each is a defect that was designed out
rather than found. The chooser is built once when the dialog opens and never inside the repaint,
because rebuilding a radio group destroys the radio holding focus and a group that loses its
focused radio silently hands focus, and the appearance of the selection, to its first one.
Ownership is written into the labels a chooser already has, for the same reason, rather than the
group being rebuilt when an order is added. And the radio `name` carries the surface it is on, so
the catalog pane and the dialog, which are both in the document at once, cannot join into one group
in which choosing a path in one unchecks the other.

Library identity stays per reading path rather than per story, which means the mark reading "in
your library" is information and not prevention. Keeping both the main series and the complete order
is a reasonable thing to want, and nothing here deletes or replaces a list a reader already has.

Verified with 1,093 unit tests passing, 16 of them new, and forty browser checks in Edge at
1280x900, all passing, two of which were proved to fail on the tree without their fix: removing the
re-render that carries a dialog choice back to the card failed exactly one check, and removing the
in-place ownership update failed exactly one other. The first of those checks had to be rewritten
before it was worth anything, because the first version imported an order in the same journey, and
an import re-renders the grid on its own and carried the choice back whether or not closing the
dialog did.

**Review of the pull request found four defects, and the worst was again in the code written to be
careful.** Focus restoration after a repaint matches on `data-key`, and the preview button carried
the key of the list rather than of the story. Choosing a different path rebuilds the card around a
different list, so the key the repaint captured no longer exists, and the miss falls through to the
card's primary action, which is Add. A reader who opened the preview, changed path and pressed
Escape landed on a control that imports an order, one Enter away from importing it. The ways button
was already keyed by the story, so the two routes into the same dialog behaved differently, which is
why nothing caught it: the route that was tested was the safe one. Keying both by the story fixes
it.

The other three are smaller and all of a kind. The lift that holds the ways button above the
full-card preview overlay sat on the paragraph wrapping the button rather than on the button, so an
85px by 29px strip of empty text column was raised with it and took clicks meant for the card;
`.ocard-foot .btn` in the same stylesheet was already doing it the right way. Every chooser in the
catalog pane carried one identical literal legend, and three stories offer radio labels that match
word for word, so six choosers announced as one name repeated; the dialog's chooser now takes the
story's name. Promoting each row title from a `div` to an `h3` answers the six group headings this
change had removed, which had left the pane with none at all. And `.ocard-desc` sets
`display: -webkit-box`, an author declaration that outranks the user agent's rule for `[hidden]`, so
a card whose description was absent would have shown an empty clamped box; unreachable from the
bundled data, reachable through the manual entry form.

Twelve further browser checks cover the four fixes and all twelve pass. Nine of them fail on the
tree without the fixes, each reproducing what the review observed rather than merely going red:
...
- [x] Put the app and a Windows Node runtime in one archive, so the reader installs nothing
- [x] Make the launcher prefer the bundled runtime and still work for anyone who has their own
- [x] Carry the runtime distribution's whole licence file, rather than a summary of it
- [x] Publish the archive as a release asset, so one link replaces the download instructions
- [x] Leave the address alone, so an unzipped app is the same app with the same progress
- [x] Test the packaging contract, because the person this is for cannot diagnose it
...
explaining. The shipped build came out at 35,308,148 bytes across 100 files on 2026-08-18, the
difference being the launcher's new branch and the short text file the reader is meant to open
first. The upstream figure was confirmed byte for byte by the build, which is the half of the
...
the distribution names dozens of bundled components, OpenSSL and V8 and ICU among them, each
carrying its own attribution terms. Writing MIT beside the binary would under-attribute what is
inside it, so the archive carries that whole file. This repository already treats a licence question
as first-class, and BL-130 exists to close one before publication, so the record should not be the
thing that sets the bar too low.

Two figures were taken here and they disagree, which is worth recording rather than tidying away.
The estimate above was 157,606 bytes naming 44 components, read from the v24.x branch on 2026-08-17.
The file the built archive actually contains is 160,552 bytes and names 47, counted from the
`LICENSE` inside `node-v24.19.0-win-x64.zip` on 2026-08-18. Neither reading was careless: they are
different files, a moving branch against a pinned release, and the branch had moved on. The shipped
number is the one the code and its tests state, because the only licence that can be wrong is the
...
the running guide says so at `docs/RUNNING.md:41-55`, where the reader is already told their
...
than merely questioned. `docs/RUNNING.md:46-55` already carries a workaround for the milder version of
...
**BL-146: Put a way out of the capped landing grid where the reader runs out of it**

- [x] Give the reader something to press at the end of the grid, not only above it
- [x] Say the same thing in both places, so neither can go stale against the other
- [x] Derive "is there more" once, rather than from two expressions that happen to agree
- [x] Keep the cap, because a wall of cards is not a landing page

Constraint gate: checked 1 to 11, none breached.

Reported as "the dashboard displays only 12 reading orders and there is no way to scroll or load
more". Nothing was broken, which is what made it worth measuring rather than arguing about. The
landing grid caps at twelve by design, states the shortfall underneath, and offers `See all` in the
section header. All three worked. The reader was still right.

Measured in Edge at 1280x900 against the 19 orders bundled today, with storage cleared. The grid
ends at y=1526. The only control sat at y=134, so a reader who had scrolled to the last card was
1,392px past it, more than one and a half viewports. Scrolling to the end of the grid put the header
button off screen and left exactly one thing in view: a sentence reading "Showing 12 of 19 reading
orders." with nothing to press. An affordance that is never on screen when it is wanted is
indistinguishable from one that does not exist, and the report was the honest reading of what was
there.

The cap is kept. Its stated reason is that the page stays one you can take in at a glance however
far the catalog grows, and that reason gets stronger as orders are added, not weaker: the ask that
prompted this was for many more lists, and the answer to a hundred of them is not a hundred cards on
the landing page. The catalog view already takes the whole catalog with no cap, and it has the
facets and the search box for going deeper. What was missing was the door, not the room.

So the same action now sits under the grid as well, 44px below its last row, and the two labels are
generated from one call rather than written twice. After the change, a reader scrolled to the end of
the grid has the control on screen and the header one off it, which is the exact inversion of the
measurement above; pressing it arrives at the catalog with all 19 rows.

The label names the whole number rather than the remainder. `See all 19 reading orders` describes
where the button goes; `Show the other 7` would describe an append, which is a different interaction
from the one it performs.

One thing the first version of the test got wrong, recorded because the mistake is the reusable
part. The check for "the count and the action never disagree" swept matched counts 0 to 40 with
`shown` set to `min(matched, 12)`, and under that sweep a rule written against the cap and a rule
written against the shown length give the same answer for every input. Mutating the code to the
cap-based form left it green. It needed the case the sweep cannot construct, nineteen shown out of
nineteen, before it could fail. A test that cannot distinguish the fix from the defect is not
evidence, however many inputs it runs.

**BL-147: Stop the startup failure messages naming a tool the download does not carry**

- [x] Say nothing about npm, which the packaged download does not contain
- [x] Stop offering a different port as the way past a busy one
- [x] Say what a different port would cost, rather than only refusing it
- [x] Cover the port being held by something that is not the tracker
- [x] Keep every line inside the width the console window opens at

Constraint gate: checked 1 to 11, none breached. Constraint 5 is the reason this is a defect at all:
the origin is load-bearing, and the message was recommending that the reader move it.

Found while verifying the archive BL-145 ships, by starting it against an occupied port rather than
by reading the code. The launcher handled it correctly and the server printed this underneath:
`Otherwise start it on another port:  set MRT_PORT=8788 && npm start`.

Two things are wrong with that line, and the second is the serious one.

There is no npm in the download. The archive carries a runtime, the app and the launcher, and no
`package.json`, which is the whole point of it: the reader it was built for has installed nothing.
Extraction of the built archive confirms it, 100 files and no `package.json`. The advice was sound
when a clone was the only way to run this, and it survived into a distribution where it cannot be
followed.

Worse, following it loses the reader their reading. Progress is saved by the browser against the
exact origin it was saved at, so a reader who moves to 8788 opens an app with nothing in it while
everything they have read sits at 8787 where they cannot see it. That reader has just been told
their port is busy, which is not the moment they are best placed to work that out. The repository
already knew this: `test/launcher.test.js:77` forbids the launcher from setting `MRT_PORT` on
exactly these grounds. The rule was enforced on the launcher and not on the advice the server
prints beside it.

The message now refuses the port change and says what it would cost, and it covers the case that
prompted the original report: the port held by something that is not the tracker at all, where
"open the address instead" answers `Not found`.

Both messages moved out of the error branch and into `server.mjs:214-235`, returned as lines rather
than printed, which is the same reason `browserCommand` is a table: a branch that only runs when a
socket is taken is a branch nobody reads. `test/startup-messages.test.js` reads the words without
binding a port.

Five of its seven assertions were run against the shipped strings and fail on them, which is the
evidence that they test the fix rather than describe it. The remaining two are honest regression
guards: the address line was always correct, and the width check earns its place by having caught
the replacement copy at 88 characters before it was committed.

**BL-149: Read the Marvel Unlimited book id off an address the reader pastes**

- [x] Take the book id from a read.marvel.com address given to the hand-entry form
- [x] Store it, so the Read button opens the reader instead of falling back to a catalogue page
- [x] Say which of the two outcomes happened, rather than reporting both the same way
- [x] Tell the reader in the form which address does the work, and why
- [x] Refuse a lookalike host, a zero and anything that would build a link to nothing
- [x] Answer schemes the way the two sibling checks already do, and stop at the length the launcher will accept
- [x] Keep a pasted reader address out of the detail link, so Info does not become a second name for Read
- [x] Point the address field at the hint that explains which of the two addresses does the work

Constraint gate: checked 1 to 11, none breached. Constraint 7 is the one this had to be written
around: it forbids awaiting a lookup before opening the window, and this change removes the need for
the lookup rather than moving it.

The metadata service this app hydrates from stopped on 2025-10-29. Asking it for 2026 returns a
total of zero. That is not a lag that will catch up: Marvel's own API was withdrawn without notice
or replacement, and the service is a mirror of a mirror of it.

The hand-entry form already existed for exactly this, and the form's own hint has said since 2025
that anything newer has to be added by hand. What it could not do was make such an issue openable.
The Read button sends the issue to `/open.html`, which resolves the digital book id from the
metadata service, and for an issue the service has never heard of that lookup can only fail. The
launch then degrades to the marvel.com catalogue page, which is honest and is not reading the comic.

The fix is that the id was already on the reader's screen. A subscriber reading a book in Marvel
Unlimited has `https://read.marvel.com/#/book/129648` in the address bar, and that number is the
digital book id the launcher wanted. Pasting it needs no key, no account, no request to anyone and
no agreement with any third party. The form already accepted the host, because `isSafeMarvelUrl`
has always allowed `read.marvel.com`. Nothing read the number out of it, so the entry fell back to
a synthetic negative id and lost the one field that would have made it work.

This is worth stating plainly because it bounds what any amount of further work can achieve. Five
candidate catalogues were checked on 2026-08-18, and not one records Marvel's digital book ids:
Comic Vine, Metron, the Grand Comics Database, Wikidata and the Marvel Fandom wiki. The id is
generated inside Marvel Unlimited and the only machine-readable sources that ever carried it were
Marvel's own API and the two mirrors that cached it, all three of which are frozen or gone. There is
a second floor beneath that one: Marvel adds an issue to Unlimited about three months after print,
so an issue published last month has no digital id anywhere, including at Marvel. A subscriber
pasting an address is therefore not a workaround for a missing integration. It is the supply.

Availability is deliberately not touched. An issue added this way still reports `unknown`, because
the Unlimited date is a separate field and nothing in the pasted address carries it. Saying so in
the confirmation is the point: the two outcomes used to read identically, and whether Read will
reach the reader is the one thing the row does not show afterwards.

The parser is stricter than the host check that precedes it. It parses the address rather than
matching a pattern over it, so `read.marvel.com.evil.com` is rejected on the hostname and cannot
smuggle a chosen number into the launcher. It rejects a marvel.com issue address outright, because
that carries an issue id, and reading one as the other would open a different comic with no sign
that anything was wrong. Zero and anything beyond the safe integer range are refused for the same
reason `readerUrl` refuses them: a Read button that goes nowhere is worse than one that is absent,
because it looks like it worked.

Three further refusals were added after a review rather than before it, and none of them was
reachable, which is the reason worth recording. This module's own host check and the private copy of
it in the reader module both refuse anything outside http and https, and the parser did not: probed
with crafted addresses, ftp, file, javascript and data URLs naming the reader host each yielded a
book id here while being rejected there. The caller gates on the sibling first, so nothing could
reach it, but two functions answering the same question differently is a contract waiting to be
relied on. The length bound has the same shape. The launcher will not build a reader address from
more than twelve digits, so a longer id was storable here and refused there, which is exactly the
dead Read button this function's comment claims to rule out.

The pasted reader address is deliberately not kept as the issue url. It would satisfy the detail
check in the reader module, which accepts that host, so the row's Info link and the hero's Info
button would both appear, name themselves after marvel.com, and open the very reader the Read button
already opens: one destination offered twice under two names, one of them wrong. With no url and a
synthetic negative id there is no detail link at all, which is the honest answer, because nothing
here knows a marvel.com page for an issue this new. A www.marvel.com address pasted instead is kept,
because that one is a detail page.

The address field points at its hint through `aria-describedby`, the first use of that attribute
anywhere in the page. It is recorded rather than slipped in, because it changes the page's own
convention: every other field here explains itself through its label. This one cannot, because which
of the two accepted addresses yields a working Read button is a paragraph rather than a label.

The parser sits in this module beside `issueIdFromUrl` and `isSafeMarvelUrl` rather than in the
reader module beside `READER_PREFIX`, and the reason is direction of travel. This module reads ids
out of addresses; the reader module builds addresses out of ids. Filing a reader with the builders
would have split three parsers across two files to keep one pair of related constants together.

Checked by removing the pieces rather than by running the whole thing, one line at a time, because a
revert that takes out a whole module only tells you the suite noticed something. Withdrawing the
parser's answer turns four tests red. Removing the scheme check on its own turns exactly one red,
and it is the one written for it. Removing the twelve digit bound on its own turns exactly one red,
and it is the one written for that. Returning the pasted address to the issue url, which is the
whole of the Info decision, turns exactly one red. In the browser the same discipline holds: the two
mutations aimed at this scenario are each caught by it and redden nothing else.
...
- [x] Decide whether anything derived from it is used at runtime only or written into the tree
- [x] Weigh the share-alike obligation that vendoring its text would attach to this repository
- [x] Fill the hand-entry form from a lookup the reader asks for, and store nothing until they add
- [x] Take Marvel's own issue id from the page, and refuse it when the tracker already holds it
- [ ] Decide whether covers are worth widening the pinned image host for, or are better left out

Constraint gate: checked 1 to 11, none breached. Constraints 1, 3 and 4 are the ones that will
decide the shape: no image bytes, no accounts, and no runtime dependency.
...
from a title and not backwards from an id this app already holds. The content is wikitext and would
...
Worth recording so the question is not reopened: those same five catalogues were checked on that day
for the digital book id, and not one of them holds it. Not Comic Vine, not Metron, not the Grand
Comics Database, which is CC0 and would otherwise be the ideal source, not Wikidata, which has
Marvel properties for characters and creators and none for issues, and not the Fandom wiki itself,
whose `MarvelUnlimitedID` is Marvel's issue id under a misleading name rather than the book id.
Price and review aggregators carry no such field either, and no public dump of the retired Marvel
API was found to have preserved it. The field appears to exist nowhere outside Marvel's own systems,
which is why BL-149 takes it from the reader's address bar instead.
...
server's content policy. Widening that is a real change to a deliberate boundary and should be
decided on its own merits, not carried along by a metadata change. That task stays open: nothing
shipped here fetches an image from the wiki, and the pinned host is untouched.

Shipped on 2026-08-19. The hand-entry form has a lookup button, and pressing it is the only thing
that makes a request. The search is fuzzy enough that picking the top hit would routinely file the
wrong comic's date against the entry, measured: asking for `X-Men Vol 7 26` returns the series page
first and issue 25 second. So the matches are offered and the reader picks one, and even then
nothing is stored until they press Add issue. Filled from the page: the release date, the page
count, the credits by role, and the series and number when the page title parses as one. The title
box is overwritten with the page title so the box shows exactly what will be stored.

The second and third tasks resolved together, and in the only direction that keeps this repository's
licence position where it is. Nothing derived from the wiki is written into the tree. The parser
admits a field only if the field is named in an allowlist, so a page's quotation, its appearance
list and its story titles are dropped because they were never asked for rather than because
something excludes them, and a test asserts none of them can reach a caller. The fixtures are
synthetic wikitext written here with real field names and invented values, so no wiki text enters
the tree even as test data. Bare facts carry no share-alike obligation; prose would, and none is
taken.

The decision on Marvel's issue id was reversed during implementation, and the reversal is the more
interesting half of this item. The first decision was to leave the id alone, reasoning that an
entry carrying a real id would show a Read button that could not open anything. That reasoning was
wrong, and reading `src/open.js` is what showed it: a missing digital id is not an unhandled state
but a designed one, which says that the issue has no Marvel Unlimited link recorded and opens its
page on marvel.com instead. Every catalogue issue lacking a digital id already behaves that way.

Measured on 2026-08-19 with a browser user agent, because Marvel's CDN refuses a bare one: the
official page for the April 2026 issue whose id came from the wiki answers 200, and an invented id
answers 404, so the 200 is a real page rather than a soft failure. The metadata service still
answers 404 for that same issue, so the id reaches the comic's official page and never the reader.
That distinction is worth stating plainly wherever this is described, because overstating it would
promise a reader something this does not do.

Two things were found while checking that, and both make the id worth more than it first appeared.
A hand entry has no link to its own official page today, because the launcher refuses to build one
for a negative synthetic id, so adopting the id fixes an absence nobody had written down as a
defect. And the address needs no new construction: the launcher already builds exactly that page
for any positive id, so the whole benefit arrives through code that was already there.

The collision hazard is real and is guarded. `addIssuesToList` merges the incoming issue into
`state.issues` before it decides whether the list already held that id, so a colliding id overwrites
the held issue's title, series, release date, page count and credits even on the path that reports
nothing was added. The reader would be told the issue was already in that list and would not be
told that a curated issue's metadata had just been replaced by this form's. The snapshot's 2025
cutoff does not protect against this, which was the tempting argument and is false: the bound
applies to the catalogue, not to what a reader types, and the wiki page for Amazing Spider-Man Vol 1
1 reports 6482, which is in the vendored data.

The guard refuses the entry and names the lists that hold the issue, rather than adding it under a
synthetic id. That follows what this form already does when the list holds the issue, and it is the
recoverable choice: nothing is lost and the reader is told something true, where a second row for a
comic they already track would sit in their reading list permanently.

What the guard tests is the id that will actually be written, and it fires whenever a wiki match has
been accepted. The first shape of it tested the wiki's id instead, on the reasoning that an id read
from a pasted address names one issue the reader deliberately pointed at. That reasoning is still
right about the id, and it is still honoured: a pasted address with no lookup behind it merges
exactly as it did before, because no match is held and the guard does not run. It was wrong about
which id gets written. `issueIdFromUrl(url)` outranks the wiki id, while the accepted match's facts
are spread into the payload either way, so a lookup for one comic and a pasted address for another
put the first comic's facts onto the second comic's id, and that id had been examined by nothing.
Nothing withdraws an accepted match in that sequence either, because the withdrawal listener watches
the title box and `acceptManualMatch` sets that box programmatically, which fires no input event.
Measured on the unguarded build in real Edge: a held issue's release date became 2026-07-01, its page
count became 99 and its writer was replaced, while the reader was told the issue was already in that
list and that nothing was added. Found by review on a sibling branch, and the correction is the one
that review proposed: gate on the presence of the fuzzy-search facts rather than on the id's
provenance, so what becomes impossible is facts this code searched for landing on something already
held, which is the principle the first shape had stated and failed to implement.

Constraint 3 is what shaped the request itself. It is never automatic, it sends the words in the
title box and nothing else, it carries no cookie and no referrer, and it is disclosed beside the
button that fires it and in the three surfaces the privacy copy test holds to that standard, which
now enforce two further rules: that the lookup sends the typed title to this wiki, and that it
happens only when the reader presses the button. Constraint 4 is untouched: the two new modules use
`fetch` and nothing else, and the dependency count is unchanged. Constraint 6 is untouched because
nothing here writes the availability field, so all five states survive.

Verified: 1,152 tests pass and 0 fail, up from 1,122, with the 30 new ones proved to fail against
the unfixed tree. All seven CI checks pass. The browser check grew to 53 assertions across 8
scenarios from 43 across 7, and the new scenario drives real Edge through a lookup, a pick, an add,
a refused duplicate and a refused cross-id collision with the wiki stubbed. A further injected
fault was added and aimed at it, taking `npm run browser:prove` to 13 of 13 caught. The first version
of that fault hooked the write and turned nothing red, which is the harness reporting honestly: a
refused add performs no write, so there is nothing there to intercept. What it injects instead is the
hazard's own signature, the stored entry carrying the second answer's date, page count and writer at
the moment the reader is told nothing was added, and that reddens exactly one assertion and no other
scenario. The two assertions covering the cross-id route were proved the same way and more cheaply:
reverting the guard's condition alone, one line, reddens exactly those two and nothing else.

**BL-151: Carry a pasted reader address through export and back in again**

- [ ] Read the book id from a reader address met while parsing a checklist
- [ ] Give such a line an identity that survives being imported twice
- [ ] Keep it out of the hydration queue, which can only spend requests learning it is not there

Constraint gate: checked 1 to 11, none breached.

Exporting a list writes each issue as a Markdown link, and for a hand-added issue that link is
whatever address was pasted. So a list containing Marvel Unlimited addresses exports correctly and
re-imports as unresolved lines, because the checklist parser only recognises the marvel.com issue
shape. The reader is then offered a title search to resolve them, and for an issue newer than the
snapshot that search cannot succeed.

This predates BL-149. The hand-entry form has always accepted `read.marvel.com`, so the round trip
has always lost these. BL-149 makes it matter more, because the address now carries something worth
losing.

Deliberately not folded into BL-149. Importing raises two questions the hand-entry path does not.
An imported line needs an identity, and the synthetic id the form uses is minted from the clock,
which is not stable across two imports of the same file and is not even distinct between two lines
of one paste. And an imported issue enters the hydration queue, which excludes hand-added issues by
source and would otherwise spend the reader's request budget on ids the service cannot resolve.
Both are answerable, and neither is answerable by the change that ships BL-149.
**BL-152: Say which order to read after this one, for a reader who has read none of them**

- [x] Declare the sequence once, as data, beside the lists it names
- [x] Refuse a path naming an order that does not exist, doubling back on a story, or running to fewer than two stops
- [x] Say on each row which path it belongs to, where it sits on it, and what comes next
- [x] Say "Start here" on the first stop, because that is the question actually being asked
- [x] Prove no two stops overlap, at either reading depth a story offers
- [x] Print the start year the shelf was already sorted by but never showed
- [x] Measure the badge's contrast on the surface it actually renders on, not the one it borrows its colour from

Constraint gate: checked 1 to 11, none breached. Constraint 8 shaped the path's end: it stops at the
last story the snapshot covers rather than gesturing past the boundary.

The catalog was shown to someone who wanted to start reading Marvel comics, and they could not. That
is not a discovery problem. All 19 stories were on screen, each with a name, a length and a
description, and the shelf had already been sorted into reading order by BL-143. What was missing is
that a shelf is a set and reading is a sequence, and nothing on it said any order followed any other.
Sorting them put them in the right order without ever claiming they were in one.

The evidence that they do connect was already in the data and had never been used. Ten of the
nineteen stories are the Avengers spine, from Essential Avengers to Fresh Start, 1963 to 2018. Every
pair of them was measured: 45 pairs across 671 issues, zero shared. So the shelf held a sequence that
no reader could see, and every stop on it is disjoint, which is the property that makes calling it a
sequence honest rather than a suggestion.

The first design was a `follows` field on each entry, and the plan critique killed it. Pairwise links
admit a cycle, admit two orders both claiming to follow a third, and have no answer to "where does it
begin" that is not a scan. A named ordered array has none of those failure modes by construction:
there is no cycle to check for, the next stop is the next element, and it starts where it starts. The
critique's two blocking findings were dissolved rather than answered, which is the better outcome and
took a smaller change to reach.

A step names an order id rather than a story key. Ids are already checked for uniqueness across the
manifest, so a typo fails the vendor run instead of resolving to the wrong story; naming the order
also records which reading of a story the source chose; and a story key is an internal shape that has
no business being typed into hand-authored data. The app resolves each step to its story, so naming
either reading of House of M places the House of M row, not one version of it. Measured against the
shipped data, all 10 stops resolve and the two grouped stops name the story rather than a reading.

At story level the safety measurement is wider than the pairwise one: 10 stories, 15 orders behind
them, 99 cross-story pairs, 821 unique issues, zero overlap. That measurement is a test rather than a
note, because it is the one property a path asserts that nothing else in the catalog does. Two orders
sharing an issue is legitimate everywhere else, so no validator can refuse it in general; what the
test refuses is two *stops* sharing one, which is the claim being made. Add an overlapping stop and it
fails with the pair named.

The line reads `Start here Â· The Modern Avengers Â· Step 1 of 10 Â· Next: Avengers Disassembled` on the
first stop and `Step 3 of 10 Â· The Modern Avengers Â· Next: Civil War` in the middle. Two drafts were
thrown away against a screenshot of the real shelf. The first repeated the path's name in a badge on
all ten rows, which made the most prominent element on the shelf the one word that never changed. The
second printed "After Essential Avengers", and since the shelf is sorted by year, that restated the
row directly above it. What a reader cannot get by looking up is what comes next, so that is what it
says.

`sourceOrigin` is required on a path for the same reason it is required on a list. A path is a claim
this repository makes about how stories connect, and no upstream publishes one in this form, so the
shelf has to be able to say who compiled it and from what. Seven of the ten stops follow Comic Book
Herald's Avengers guide; the other three are placed by their start year, and the wording says so.

19 unit tests in `test/reading-path.test.js`, and every guard was proven to fail without its fix by a
mutation harness, 13 of them plus a control. Two browser journeys and four mutations were added to
`scripts/browser-check.mjs`, which now runs 53 assertions across 8 scenarios with all 14 mutations
caught by the scenario aimed at them.

Two of those proofs were false negatives before they were true ones, and both are worth recording
because both reported success. The first mutation reported that removing the duplicate-id guard broke
nothing: `else if (seen.has(id))` appears twice in `src/js/lib/curated.js`, once for entries and once
for paths, and a plain string replacement took the first. It really did break the entry check, which
these tests do not cover, so the run was honest about a guard nobody had asked it about. Occurrence
indexing fixed it. The second pair reported that removing a stylesheet rule broke nothing, because the
app sends `style-src 'self'` and the injected `<style>` element was silently dropped. Rewritten
through `insertRule`, both caught their target. A mutation that cannot reach the code reads exactly
like a scenario that does not test it.

A review of the pull request found a third check of that shape, and this one was green rather than
falsely red. The "Start here" badge reuses `.pill-ok`'s two values exactly, so reusing a measured
colour looked like reusing its measurement. It is not: the contrast gate carried `--green` against
`--bg` and `--card`, and a catalog row is neither, it is `--card-2`. The badge shipped on a surface
nothing had measured, and the CSS comment beside it said the opposite. The pair is now gated at
`scripts/check-palette.mjs:61` and measures 8.38:1 in the dark theme and 6.07:1 in the light one
against a 4.5:1 floor, so the badge was in fact legible and the claim was still unearned. The
assertion that should have forced the question lists which surfaces each foreground is measured on,
and it did not name `--green`; it does now, and removing the pair fails it. The gate's other
coverage assertion, that every declared surface has something rendered on it, passes on the broken
tree and always would have: it walks the three named surfaces the file composites by hand, and a
token background is not one of them, so a missing token pair was never inside its reach.

**BL-153: Stop collapsing the sidebar from blanking every pill in the catalog**

- [x] Scope the collapsed-rail pill rule to the rail
- [x] Keep the two status pills collapsing to dots as they were meant to
- [x] Hold the scope with a browser mutation, since no unit test draws a stylesheet

Constraint gate: checked 1 to 11, none breached.

Found while checking that the new "Start here" badge survived a collapsed sidebar. It did not, and
neither did anything else. `.railed .pill` was written for the two status pills in the rail's foot and
set `width: 10px` with `color: transparent`. But `.railed` is set on `#shell`, and `#shell` contains
`main`, so the rule reached every pill in the catalog. Measured in Edge at 1280x900 with the sidebar
collapsed: the depth label became 89px of transparent text, and the badge this change adds would have
become a 10px dot.

This shipped before the reading path existed and has been wrong the whole time, which is the argument
for fixing it here rather than filing it: the change that found it is the change whose new element it
breaks, and the fix is one selector. Scoping to `.railed .rail-foot .pill` fixes it; the rail's own
pill still collapses, measured 132px to 10px.

Two mutations hold it. One unscopes the selector back to what it was, and one deletes the width, so
neither the scope nor the collapse can be lost silently. Both were verified to redden the scenario
they were aimed at.

**BL-154: Shelve the events apart from the character runs, and say why the runs sit last**

- [x] Divide the shelf into events and character-focused reading, keyed on the order's type
- [x] Say what each section is for, in a sentence a new reader can act on
- [x] Keep the reading path intact across the division, since its stops span both
- [x] Leave the facet chips working, so a filtered shelf still says which section it is showing

Constraint gate: checked 1 to 11, none breached.

Asked for directly by the owner: the event orders should be separated from the ones that follow a
single character. The data already supports it. `type` has four values and all 26 orders carry one,
and the facet chips above the shelf already filter on it.

Two sections rather than four. `type` splits four ways, but the reading path runs through orders
typed `era`, `event` and `creator-run`, so dividing by all four would cut the sequence this change
just shipped into three pieces on three parts of the page. Events and eras read as one thing to a
new reader, and the character runs read as another.

That sentence named `character-run` when this item was written, and it was wrong. Measured against
the shipped catalog: the path's stops are typed `era`, `event`, `event`, `event`, `event`, `era`,
`era`, `creator-run`, `era`, `era`. No stop is a character run, and the one creator run is stop 8,
Hickman's Avengers. Filing creator runs with the character runs, as the original sentence implied,
would have taken a stop out of the middle of the sequence and put it in the half the sequence does
not run through, breaking this item's own third acceptance criterion. So `creator-run` sits with the
shared story, on the evidence rather than on the label. A test holds the division against the real
catalog, and reversing the one line that decides it fails both that test and the unit test beside it.

The third acceptance criterion is written on that same mistaken premise, and is left standing rather
than rewritten, because a criterion edited after the fact records nothing. Its "since its stops span
both" is false on the measurement above: no stop is a character run, so the stops span one section,
not two. It is ticked because the path is intact and wholly inside the shared story, which is what
the criterion was for. Had the premise been true, the same tick would have required the opposite
arrangement, and that is the reading of it this paragraph exists to prevent.

There is a second finding worth carrying into that change. All 7 character-run orders have no
`timeline` year, so the year sort already puts every one of them at the bottom of the shelf. A reader
sees the grouping and is told nothing about it. The sections would make an accident into a statement,
which is the actual value here rather than the reordering.

Key it on `type`, not on whether a year exists. Today those two produce the same split, but that is a
property of the data as it stands rather than a rule anything enforces, and a dated character run
would silently land in the events half.

Shipped as two headed sections, "The shared story" and "Character spotlights", each with a sentence
under it. Nothing is reordered: the year sort already put the boundary where it is, and the headings
only name it. A story belongs to the spotlights only when every one of its readings is a character
run, which keeps a story with any shared-universe reading on the side a path can run through. An
empty section is dropped rather than headed, so a facet or a search that narrows the shelf to one
kind still names the kind it is showing.

**BL-155: Draw the reading path as a path, instead of a badge on each row of a list**

- [ ] Show the whole sequence in one place, so its shape is visible without scrolling a shelf
- [ ] Mark where the reader has got to, using progress the app already holds
- [ ] Keep the shelf's badges, which answer the same question from the other direction
- [ ] Decide what a second path does to the view before adding one

Constraint gate: checked 1 to 11, none breached.

BL-152 answers "what do I read after this" on the row a reader is already looking at. It does not
answer "what is the shape of this", because ten badges spread down a shelf are a sequence only to
someone who scrolls the whole thing and holds it in their head.

The original request was a timeline page, and this is the honest version of it: not a proportional
year axis, which would put 1963 and 2015 on one line and waste most of it, but a linear spine with
named stops, which is how continuityguide.net presents the same material and why it works. The stops
already have names, positions and start years.

It is ranked below the shelf sections because it is a new view rather than a change to one, and
because the badge already delivers most of the value for the reader who prompted all this. It is worth
...
Measured, which is what makes the request actionable rather than a matter of taste. As originally
raised, the rule now at `src/styles.css:392-394` set two properties, a text colour and a border
colour, and both were the same two values `.pill-ok` already used. Everything that made a pill a
pill, the `.72rem` size, the inherited weight and the padding, was shared with every other pill on
the row. So the one element on the shelf whose job is to answer "where do I start" was the same size
and weight as a status chip, and differed from it only in hue. A reader scanning the page had nothing
to catch their eye, which is exactly what the owner reported.
...
Partly overtaken on 2026-08-20 by BL-166, which had to touch this exact rule for another reason. The
badge now carries a tinted fill and a heavier weight rather than a text and border colour alone, so
it is distinguished by more than hue and it clears the floor on the surface it is drawn on. What
remains open is the second criterion, the line above the shelf that names the first stop for a reader
who has not scrolled to it. That reader is the one who prompted the original request, so the item
stays `Ready` rather than being closed on the part that was incidental to someone else's change.

**BL-157: Show the catalog in the README instead of only describing it**

- [x] Put a picture of the shelf in the README, above the instructions for running it
- [x] Show the reading orders that can be imported, not the empty first-run home view
- [x] Guarantee no comic artwork is committed, rather than checking the frame by eye
- [x] Describe the picture in text, for a reader who cannot see it

Constraint gate: checked 1 to 11, none breached. Constraint 1 is the one that bites and is treated
below. Constraint 10 caps the value: this is not an acquisition asset, and the score reflects that.

Direction from the owner: the README "needs something for users to visualize what they're getting
into", refined to a picture that "shows the reading orders one can add to their library". The second
sentence rules out the obvious shot. The landing view is the home panel, which on a fresh install is
mostly empty, and `docs/ux-artifacts/01-landing-firstrun-1280.png` already records what that looks
like. What answers the request is the catalog, because that is the screen the imports come from.

Constraint 1 forbids storing comic image bytes, and a screenshot is bytes, so a capture that catches
cover art breaches it no matter what the surrounding prose says. The page requests twelve covers from
`i.annihil.us` on load. Framing around them would be a judgement made by eye and would silently rot
the first time the layout moved, so the guarantee is made mechanical instead: the capture writes
`{"covers":false}` into the settings key before first paint, and the app then requests no cover at
all rather than fetching and hiding them. The run asserted both halves, zero image requests observed
and `document.images` empty at the shutter.

Switching covers off does not make the picture unrepresentative, which is the objection this answers.
The catalog draws no covers in either setting: captures taken with them on and off are the same
picture. So the file is the shelf as it ships, and needs no caveat in the README saying otherwise.

No capture script is committed. Producing the file needs `puppeteer-core`, which the browser checks
deliberately keep outside this repository, and committing a script that cannot run without it would
imply a dependency the project does not have. The cost is that the picture can go stale without
anything failing. That is accepted rather than overlooked: the alternative is either a dependency the
repository has refused, or a gate that cannot run in CI for the same reason.

**BL-158: Let the reader close the undo notice a deleted list leaves on every screen**

- [x] Give every message raised under the undo key a control that closes it
- [x] Spend the undo when it is closed, rather than hiding the words and leaving the buffer behind
- [x] Speak the way out, as the offer beside it is already spoken
- [x] Land focus somewhere honest when the button that held it is removed

Constraint gate: checked 1 to 11, none breached. Nothing here touches the network, storage or the
origin; the buffer this ends is in memory and always was.

Reported by the owner, with a screenshot: a list deleted hours earlier still had its banner and its
button above every view. The expectation was a message that lasted five or ten seconds.

BL-035 held the offer for the whole session on purpose, and that decision is not reversed here. Its
reasoning survives re-reading: deleting the list you were reading moves you somewhere else, and a
clock that expires while the reader is still deciding takes the only way back at the worst possible
moment. What BL-035 missed is that a clock was doing two jobs. It ended the offer, which it was bad
at, and it ended the message, which nothing else did. Only the second job was ever wanted, so it is
given to the reader instead of to a timer: the words go when they say so, and the undo lasts exactly
as long as it did.

Closing spends the undo rather than only hiding it. A buffer still live behind a message the reader
has closed is an offer they can no longer see and cannot take, and it would not stay quiet: putting
the same order back from the catalog consults that buffer and raises a fresh message about a deletion
the reader had already finished with. That is the shape of defect this repository keeps finding in
its own recovery paths, so the dismiss runs `forgetDeleted()`, which is the same withdrawal a backup
restore and an erase already use.

All four messages under that key got the control, not just the offer. Two of them report something
that has already happened and offer nothing to do about it, so they are the ones a reader is most
likely to want gone, and they were the ones with no button at all. They are raised from a shared
record rather than written out at each call, and a test reads the source for every `notify` call
carrying that key and fails on any raised without one, or with one written into the argument before
it, because the defect was never in one message: it was in the key, and a fifth message added later
with no way out puts the reader back where they started.

The fourth is not settled, and it does not say "Dismiss". A restore that failed for want of space
can succeed once the reader frees some, so that message keeps its buffer and offers "Try again", and
the buffer is by then the only place the deleted list still exists. Closing that message throws it
away. "Dismiss" names clearing the screen, so on this one message it would be a destructive act
wearing the label of a tidy-up, pressed by a reader who has just asked for the list back. It reads
"Give up", which is the same withdrawal under the name of what it costs.

The way out is spoken as well as drawn. BL-035's own argument for announcing the undo was that a
button nobody is told about is a button a screen reader user cannot know to look for, and it reaches
this control for the opposite reason. This is the one notice that stays until it is closed, so a
reader who is not told it can be closed is the reader who keeps hearing it.

Dismissing destroys the button that was pressed, and a focused node leaving the document drops focus
to `<body>` at the top of the page, which is the silent landing BL-054 was filed for. The heading of
the current view is where `showView` already puts a reader, so that is where a dismissed notice puts
them too, and the code that does it is now one function serving both rather than two that can drift.
Inside an open dialog the page behind is inert and focusing it does nothing, so that case is left to
the browser, which keeps focus in the dialog.

Which of those two applies is asked before the click runs, not after. Closing the notice detaches the
button, and `closest` on a detached node walks up to null, so the same question asked afterwards
answers "not in a dialog" every time: the one case the guard exists to skip would be the one case it
could never see. The first draft of this asked afterwards. Nothing observable depended on it, because
an inert page ignores `focus()` anyway, but a guard that cannot return one of its two answers is a
guard in name only, and it was found by review rather than by any check here.

Dismiss is drawn as a second text button rather than a bare glyph, because the notice already holds
one text button and a word beside a word reads as a choice between two things. It loses its outline
until it is pointed at or focused, which is the whole of its being secondary: its colour is left
alone, since dimming text is the one way to say "secondary" that costs contrast, and this is the
control a reader is looking for when they want the message gone.

Thirteen tests were added, and every one of them has been watched to fail. Thirteen single-line
reverts were made in turn and each turned a named test red, with all thirteen tests appearing in
that list, so no test in the file is merely passing alongside the code it is meant to defend.
Raising the offer without the control, and raising the two withdrawals without it, each fail the
source scan, and so does handing the control to the action slot rather than the dismiss slot, which
the first version of that scan waved through. Drawing no control at all fails six tests, three of
them the ones about focus. Dropping `dismiss` from the spoken message fails two. Restoring the old
layout rule leaves a dismiss-only notice without the flex row that makes space for a button, and
applying the new one to every notice makes a message with nothing to press claim a button it has
not got. Closing the message without spending the undo, deleting the focus move, and asking the
dialog question after the click rather than before each fail the single test written for it.

None of those runs a browser, so the claim was also settled in one. A scenario in the committed
browser check drives installed Edge through the reported journey: import an order, delete it, read
the notice, carry it to another screen, close it, and then delete and undo a second time. Ten
assertions, all passing, within a check of ninety-one across twelve scenarios, and they cover the
two things the unit tests cannot see, which are that the message really does survive a change of
screen and that focus lands on the heading of the screen being read rather than on `<body>`. That
second one was worth nothing until this review. A synthetic click does not move focus, so the
button being removed could never have dropped focus anywhere, and the assertion passed on every
tree it could ever be run against. Focusing the button before pressing it, which is what a reader
does, makes it discriminating: it now reports `BODY` on a tree without the fix. Two mutations prove
the scenario can fail, and both are among the twenty the mutation run carries, all twenty caught.
Stripping the second control reproduces the notice exactly as it shipped and reddens three named
assertions before starving a wait; making the button hide the paragraph without spending the undo,
which is the tempting shape of this fix, now reddens two, the focus landing and the assertion that
the closed message does not come back on the next screen.

One path is left to the unit tests alone. The message shown when putting a list back fails needs a
storage failure to reach, which the browser check has no way to stage, so its wiring is pinned by
...
- [x] Offer the stable release zip as the primary action and the release notes as the secondary link
- [x] Say that reading progress stays in the browser and that the old folder can be deleted
- [x] Drive that upgrade against two real installs, so the sentence is verified rather than asserted
- [x] Keep the browser harness deterministic by stubbing the release request and proving the scenario

Constraint gate: checked 1 to 11, none breached. Constraint 3 is satisfied because the request asks
GitHub only for the latest release JSON and carries no reading data, no note and no identifier.
Constraint 4 is satisfied because no runtime dependency was added. Constraint 5 is untouched because
the app still serves the same 127.0.0.1:8787 origin. Constraint 11 is checked by the dash scan.

Readers previously had no in-app way to learn that a release existed. The distribution work done in
BL-145 made a zip easy to download, but the app still depended on the owner telling people to go get
it. This closes that gap without making the app a self-updater: the browser asks GitHub for the
latest release and, if the tag is newer than the running version, shows a notice. It never downloads,
extracts, installs or starts anything on the reader's machine.

The release logic is deliberately small and stateless. `src/js/lib/updateCheck.js:3-4` holds the
latest-release endpoint and the stable zip URL, and `src/js/lib/updateCheck.js:55-95` performs the
injected fetch, strips the leading `v`, compares dotted numeric versions and turns every failure into
a silent result. The unit test covers current, older, newer, malformed and missing tags, a rejected
request, non-200 and rate-limited responses, and the not-due path where the injected fetch count must
stay at zero.

The setting lives with the other reader preferences rather than with reading progress. The visible
switch is in the settings screen at `src/index.html:651-658`, and the manual button is in the About
screen at `src/index.html:770-773`. `src/js/main.js:762-778` applies and saves the switch, while
`src/js/main.js:864-901` lets the About button force a check even when the automatic one is off.
The stored record only gained fields, so the launch page that reads the same key for the API base
keeps seeing the field it already understood.

The notice copy is the important part of the feature, not decoration. `src/js/main.js:784-811` names
the new and current versions, keeps the download action pointed at the stable zip URL, adds the
release-notes link, says reading progress is saved by the browser, and tells the reader they can
delete the old folder once the new version opens. `src/js/main.js:841-852` starts the automatic
check without awaiting it, and `src/js/main.js:4781-4802` shows that the boot path continues into the
...
at `scripts/browser-check.mjs:1646-1653`, and the update scenario at
`scripts/browser-check.mjs:1279-1345` verifies the default current-release response, the explicit
...
`scripts/browser-check.mjs:532-588`: one makes the default answer newer, one suppresses a newer
answer, one corrupts the download link, one removes the release-notes link, and one rewrites the
delivered module so boot waits for the update check.
...
while their shelf comes up empty, which is the distinction between storage agreeing and a reader
seeing their progress. Discarding read state leaves the order in place but returns visible progress
to zero. Starting the upgraded copy on a different port breaks the survival assertions, and is the
one way a reader following this advice really can lose everything. Pointing the control at the origin
it is supposed to differ from breaks the control. Neither command runs in CI, for the same reason
...
never once loaded the thing it was testing.
...
the stylesheet for failing both, at `src/styles.css:79-82`. The icon kept it. So the mark is drawn
...
nominative fair use test set out in New Kids on the Block v. News America Publishing, 971 F.2d 302
(9th Cir. 1992), asks first whether the product can be identified at all without the mark, and a
Marvel reading order cannot be. The second and third parts are where a borrowed logo fails: only so
much of the mark as is reasonably necessary may be used, and nothing may suggest sponsorship. The
court's own application of it turned on the defendant having used no "distinctive logo". A red box
with a white letter is not necessary to identify anything, so it takes the weaker half of the test
while adding nothing the word had not already established.

It also reaches past this repository. Microsoft Store policy 11.2 requires all content "and
associated metadata" to be originally created, appropriately licensed, used as permitted by the
rights holder, or used as otherwise permitted by law, and the section preamble defines that
metadata to include the product icon. Read from the [Store
policies](https://learn.microsoft.com/en-us/windows/apps/publish/store-policies), document version
7.19, on 2026-08-20. So the icon is assessed at certification whether or not anyone complains.

Sized 1 because it is one drawing and two exports. It is ranked first of the three because it is
the cheapest of them and the only one that needs no decision taken before the work can start.
...
The first is Marvel's own stated position. Section 5 of the Marvel API terms says a developer may
"not create derivatives of Marvel Property, or use the 'Marvel' name or any other Marvel Marks in
the title of your App without Marvel's prior written approval", read from the [2025-10-08 archive
snapshot](https://web.archive.org/web/20251008063626/http://developer.marvel.com/terms) on
2026-08-20 because the live page answers 403. That clause binds users of the Marvel API, and
`docs/DATA_PROVENANCE.md` records that this project never called it, so the clause is evidence of
Marvel's posture rather than a term this repository agreed to. It remains the clearest statement
Marvel has published about third-party naming, and it names the title slot specifically.

The second is Microsoft's, and it does not depend on Marvel at all. Store policy 10.1.1 requires
that a product "must not in any way attempt to mislead customers as to its actual features,
functionality, or relationship to other products", that a product title "must not contain marketing
or descriptive text, including extraneous use of keywords", and that a product "must not claim to
be from a company, government body, or other entity if you do not have permission to make that
representation". Same source and date as BL-161. A reviewer can decline the listing under any of
those three without Marvel being involved at all, which makes it the likelier of the two to be met
first.

What comparable apps are called is the most useful evidence available, because it records what
survived rather than what anyone argued. The established comic trackers take arbitrary names and
mention Marvel in the description instead: Comic Geeks, CLZ Comics, iCollect Comic Books and
ComicsTracker, listings read on 2026-08-20. Riot, TMDB and Letterboxd each publish a third-party
policy saying the same thing in their own words, which is to keep the mark out of the title, say
the product is unofficial, and keep the developer's own branding dominant.

The app already carries the disclaimer half of that pattern, at `src/index.html:686-690`, and it is
...
Displaying it anyway was the cautious choice, and as a claim about origin it is defensible, since
the facts did begin with Marvel. The problem is narrower than that and runs the other way. The
string is the official one, so it reads as attribution given under an agreement, which is the one
impression the rest of this repository works to avoid. Under the endorsement question that BL-161
and BL-162 both turn on, borrowed official wording is a liability rather than a mitigation.

Two smaller things go with it. The required text is literally "Data provided by Marvel. Â© 2014
Marvel", with the year hard-coded by Marvel and never templated, read from the [attribution
page](https://web.archive.org/web/20251008073256/https://developer.marvel.com/documentation/attribution)
...
**BL-165: Let the landing page use the desktop it is given, without stretching prose**

- [x] Widen the shell past the fixed measure it has carried since the first release
- [x] Hold every prose view at the width it already had, so reading is not made worse to make browsing better
- [x] Get more cards per row at a desktop width and again on an ultrawide one
- [x] Measure the result in a browser rather than deriving it from the stylesheet

Constraint gate: checked 1 to 11, none breached. Constraint 5 is the live one and it is untouched:
this changes a max-width, not an origin or a port. Constraint 11 is checked by the dash scan.

Implemented on 2026-08-20. Measured in Edge against the running app: at 1280x900 the shell is 1028px
and the landing view 964px, giving 3 columns; at 2560x1080 the shell stops at the 1360px cap and the
landing view is 1296px, giving 4. The reading measure is 876px at both, unchanged. Neither width
overflows horizontally.

The shell was `max-width: 940px` with `1.6rem 2rem 3rem` of padding. That is the whole finding,
because `box-sizing: border-box` means the content box was 876px and not 940px. Anything that set
the new prose measure to 940 would have widened every prose view by 64px while claiming to have
left them alone, and nothing in the suite would have said so. So the reading measure is pinned at
876 and only the landing page is allowed past it, through one modifier class.

The catalog grid's own minimum deliberately did not move. `auto-fill` fits
`floor((available + gap) / (min + gap))` with a 14.4px gap, and at 260px that is already 3 columns
in 964px of space and 4 in 1296px. Raising it to 300px was considered and rejected on arithmetic:
it changes neither of the two widths this item is about, drops the 881px to 960px band from two
columns to one, and overflows a 320px viewport. A number that buys nothing at the sizes named and
costs something at the sizes not named is the wrong number.

Sized 2 because the width change is small and the checking is not: every view had to be looked at
at both widths, in both themes, and the failure mode is a prose column that silently got wider.

**BL-166: Move the app's identity off Marvel's red and onto its own purple**

- [x] Take the brand colour from the app's own icon rather than choosing one that looks close
- [x] Keep red for danger only, so a warning still reads as a warning
- [x] Give links, success and warnings each their own hue instead of overloading one
- [x] Measure every pair in both themes before shipping, and put none below the contrast floor

Constraint gate: checked 1 to 11, none breached. Constraint 6 is the live one and it is honoured:
all five availability states stay distinct, and the availability badge is untouched apart from the
token rename behind it. Constraint 11 is checked by the dash scan.

Implemented on 2026-08-20. The palette gate measures 88 pairs across both themes and reports 0 new
below the floor. The five that sit below it are the five already recorded before this change, all of
them progress-bar tracks and one checkbox tick, none of them introduced here. One of the five did get
worse: the white tick on the dark teal fill fell from 2.30:1 to 1.93:1, and rather than inherit
BL-069's acceptance the justification in `scripts/check-palette.mjs` was re-derived on the new fill,
because a pair already recorded is the pair a later change can most easily worsen unwatched.

The brand colour was read out of the shipped icon rather than picked: decoding the 512-pixel PNG,
`#6d28d9` covers 69.41% of its opaque pixels, and the committed vector is byte-identical to it. That
is the light-theme accent. It cannot be the dark-theme accent, which is the part worth writing down:
measured against the progress track it is 2.14:1, and it fails five of its eight pairs on a dark
page. Mixing it 20% toward white gives `#8a53e1`, which is the least white that clears all eight, and
white on it measures 4.77:1 against a 4.5 floor.

Red is now danger and nothing else. The row you are reading used to be marked with the danger line,
which is why it carried a faint red wash; it now has its own accent line, because selection is not
danger. Blue carries links and focus, teal carries success and positive availability, amber carries
warnings and the one featured panel. Neutrals were rotated to hue 240 with saturation scaled to keep
them quiet, holding lightness rather than luminance, so ratios did move: across the 88 pairs 52 went
up, 30 came down and none crossed a floor in either direction. The largest movement of a pair with no
recoloured token in it is the dark secondary text on the page, 11.83:1 down to 11.28:1.

One bug was found by the owner during review and is worth recording because the shape of it recurs.
A filled badge cannot take a fixed foreground when its fill flips polarity between themes: the teal
token is light on a dark page and dark on a light one, so a dark glyph on it measured 9.55:1 in one
theme and 2.83:1 in the other. The fix is the tint pattern the stylesheet already used elsewhere, a
12% wash behind the theme's own text colour, which measures 7.16:1 and 6.02:1 on a card.

Review found two shipped surfaces the token rename could not reach, and both are worth recording
because the reason they were missed is structural rather than careless. The Marvel Unlimited launch
page is opened in its own tab and never loads the app's stylesheet, so `src/open.css` writes its
colours out as literals and was still carrying the retired red mark and the pre-rotation neutrals.
The web app manifest names the dark page colour twice, for the standalone splash and the browser's
own chrome, and it too was still `#0f1115`. Both are now the shipped dark values. The palette gate
reads `src/styles.css` alone, so neither file is measured by anything, which is why they drifted
without a gate saying so and why the launch page now states its four measured ratios in a comment.
Widening the gate to every file that paints a colour is filed as BL-168 rather than done here.

Sized 2 because the colours are one file and the checking is three: every pair measured in both
themes, every renamed usage read to confirm it was brand rather than danger, and the whole thing
looked at in a browser.

**BL-167: Answer "where do I start" on the first screen, with one order named**

- [x] Name one reading order on the first-run landing page, rather than offering nineteen equally
- [x] Say what the commitment is, in issues and in hours, where the decision is made
- [x] Choose it by a rule anyone can check, not by an editor's pick that nobody can audit
- [x] Withdraw the offer the moment the reader has said something more specific
- [x] Leave the returning reader's own progress exactly where it was

Constraint gate: checked 1 to 11, none breached. Constraint 1 is the live one and it is honoured:
the panel shows the same cover URL the shelf already shows and stores no bytes. Constraint 11 is
checked by the dash scan.

Implemented on 2026-08-20. Measured in Edge: the panel is shown on a genuine first run and names
Avengers Disassembled; it is hidden while a search is typed, hidden once a facet is chosen, and
hidden once a single order has been imported, at which point the continue-reading panel is showing
and the library holds one list.

The rule is the beginner-friendly order with the fewest issues, ties broken by catalog order. That
is deliberately the smallest real commitment the data can offer, because "how much am I signing up
for" is the question a first-time reader is actually asking and an editor's pick answers a different
one. Against the bundled catalog it selects a five-issue event. The tie-break matters more than it
looks: without it two orders of equal length can swap places between renders, and the landing page
then offers a different starting point each time it is opened.

Withdrawal is the half that is easy to skip. A reader with a library has chosen before, and a reader
who is filtering or searching is choosing right now; in all three cases the panel would be answering
a question that has already been answered. Withdrawing the offer when it stops making sense is
cheaper than explaining it afterwards, and it is why the returning reader sees no change at all.

Sized 3 because the panel is new markup, new selection code and new tests, and because the failure
mode is quiet: a panel that keeps offering a starting point to someone who has already started reads
as the app not noticing them.

**BL-168: Measure every file that paints a colour, not only the stylesheet**

- [ ] Enumerate the files that set a colour, rather than naming the stylesheet and trusting that
- [ ] Measure the literals in a standalone document against each other, not against tokens it
      cannot see
- [ ] Fail when a shipped colour has no pair watching it, so a blind spot is a finding rather than
      a silence
- [ ] Keep the manifest's page colour and the stylesheet's page colour from disagreeing

Constraint gate: checked 1 to 11, none breached. Constraint 4 is the live one: this is gate work and
stays inside the existing zero-dependency script, so nothing reaches the browser. Constraint 6 is
untouched, because the availability badge's five states are a modelling question and not a contrast
one. Constraint 11 is checked by the dash scan.

Raised on 2026-08-20 by the review of BL-166. The palette gate reads one file, at
`scripts/check-palette.mjs:426`, so its 88 pairs describe `src/styles.css` and nothing else. Two
shipped surfaces sit outside that file and both had drifted from the app before the review found
them: the Marvel Unlimited launch page writes its colours as literals because it opens in its own tab
and never loads the stylesheet, and the web app manifest names the dark page colour twice for the
standalone splash and the browser's own chrome. BL-166 corrected both by hand, which is exactly the
mechanism this item exists to replace.

The shape of the defect is the one this repository keeps meeting: a check whose scope is an
enumeration somebody has to remember to extend. `scripts/check-anchors.mjs:227-232` states the same
argument for the anchors gate and resolves it by walking every tracked file instead of a list. The
palette gate should reach the same way, discovering the files that paint rather than being told.

Sized 2 rather than 1 because the standalone page has no token layer, so measuring it means pairing
...
**BL-181: Publish the first guarded modern Marvel event batch**
...
Constraint gate: constraints 1 through 11 checked; none breached. Constraint 1 is protected by vendoring cover URLs
...
**BL-182: Publish the second guarded modern Marvel event batch**

- [x] Audit skipped early sources before advancing to the next pending inventory position
- [x] Resolve the approved ten and replace blocked candidates only with the next chronological events
- [x] Publish ten complete issue sequences with exact source credit and no copied commentary or images
- [x] Hold the catalog chronology and every generated issue sequence to the reviewed mappings
- [x] Keep missing identities and every exact, subset, or partial overlap out of the shipped packet

Constraint gate: checked 1 to 11, none breached. The batch keeps only cover URLs under Constraint 1
and adds no runtime dependency under Constraint 4. Constraints 6, 7, 8 and 9 are untouched.
Constraint 11 is checked by the added-line dash scan.

Implemented on 2026-08-21. The early-source audit selected Maximum Security, Decimation, Planet Hulk,
Annihilation: Conquest, War of Kings, Realm of Kings, The Thanos Imperative, Silent War, Messiah
Complex, and World War Hulk. Decimation stayed blocked because five Generation M issues have no
repository metadata and its Day After issue is already in House of M. Realm of Kings stayed blocked
because Nova #29-31 are also in War of Kings. World War Hulk stayed blocked because three issues are
already in two shipped orders. Messiah War, Necrosha, and Second Coming were the next chronological
missing events that passed both gates.

The final ten add 178 distinct issues. Every card credits Comic Book Herald and links to the exact
page or section followed. All 450 comparisons against the 36 existing orders and nine packet peers
report no shared issue, and vendoring reports zero unresolved rows, placeholders, count warnings,
duplicate warnings, missing digital ids, and missing covers. Focused assertions hold both the ten
generated sequences and their independently verified catalog chronology.

Sized 5 because each proposed substitution had to be proved rather than chosen by title: thirteen
candidates, 178 live identities, twelve complete overlap reports, ten generated payloads, and one
chronological catalog change had to agree before the batch was safe to ship.

**BL-185: Publish the third guarded modern Marvel event batch**

- [x] Represent distinct guides on one page without inventing URL fragments or DOM ids
- [x] Resolve Manifest Destiny's anthology material and Regenesis's repeated issue through evidence
- [x] Publish ten complete story sequences with exact source credit and no copied commentary or images
- [x] Hold every generated issue sequence and source section to the reviewed mappings
- [x] Keep unresolved identities, duplicate sequences, and every semantic overlap out of the packet

Constraint gate: all 11 checked and none breached. The packet stores only cover URLs, uses build-time
code, and adds no dependency. It leaves availability, reader launching, the metadata boundary, and
the browser-app decision unchanged. The added-line dash scan checks the writing convention.

Implemented on 2026-08-21. The approved packet adds X-Men: Divided We Stand, X-Men: Manifest
Destiny, X-Men: Nation X, X-Men: Curse of the Mutants, Wolverine Goes to Hell, X-Men: Age of X,
X-Men: Schism, X-Men: Regenesis, Doomwar, and Spider-Island. Eight orders share one exact source
page and carry distinct visible `sourceSection` values. The URL remains unchanged and has no
invented fragment.

The final ten add 200 distinct story issues. Manifest Destiny's two unnumbered anthology groups
resolve to the same five numbered anthology issues and appear once. Regenesis's consecutive repeated
Uncanny X-Men #3 is recorded and represented once, consistent with the source's #1-4 collection
summary. Spider-Island records its non-story Spotlight magazine separately and ships the thirteen
story issues the source names. All 550 comparisons against 46 existing orders and nine packet peers
report no shared issue.

Sized 5 because the batch combines a source-identity contract with ten mappings, 200 live issue
identities, ten complete overlap reports, ten generated payloads, and one chronological catalog
change that must agree before any guide is safe to ship.

**BL-186: Publish the fourth guarded modern Marvel event batch**

- [x] Refresh the ten exact source scopes and settle every open count boundary from the source
- [x] Preserve six shared-page identities as visible sections without inventing URL fragments
- [x] Resolve 128 distinct issue identities, including all eight `1.MU` Monsters Unleashed tie-ins
- [x] Publish ten complete sequences with exact source credit and no copied commentary or images
- [x] Keep unresolved metadata, duplicate identities, semantic duplicates, and every issue overlap out

Constraint gate: checked 1 to 11, none breached. Constraint 1 is protected by vendoring cover URLs
only. Constraint 4 is protected by build-time scripts and zero new dependencies. Constraints 6, 7,
8 and 9 are untouched. Constraint 11 is checked by the added-line dash scan.

Implemented on 2026-08-21. The batch adds Minimum Carnage, X-Men: X-Termination, Avengers: The Enemy
Within, X-Men: Battle of the Atom, Revolutionary War, X-Men: The Trial of Jean Grey, Monsters
Unleashed, Venomverse, Infinity Countdown and Infinity Wars, and Marvel Damnation. Six orders use
two shared pages with distinct visible `sourceSection` values, while four use dedicated source pages.

The final ten add 128 distinct issues. Minimum Carnage stops at the source's explicit six-issue
reading-order line instead of taking Scarlet Spider #12 from the following block. Venomverse stops
at its 11-issue core. Infinity keeps the 46 explicit issue-by-issue rows instead of four issues found
only in a collected lead-in, and Damnation keeps its 15-row event checklist. The eight Monsters
Unleashed tie-ins preserve `1.MU` even though Marvel metadata writes `1.1`. All 650 comparisons
against 56 existing orders and nine packet peers report no shared issue.

Sized 5 because two source pages hold six distinct identities, two issue numbers require explicit
metadata normalization, two duplicate metadata records require evidence-based selections, and ten
mappings, overlap reports, generated payloads, and catalog cards all have to agree.

**BL-169: Make the reading screen say where you are and use the desktop it is given**

- [x] Let the reading order screen past the prose measure, the way the landing page already goes
- [x] Give the next issue a cover big enough to be the thing the eye lands on, and one obvious way in
- [x] Put the progress figure on the screen instead of inside a tooltip
- [x] Stop the upcoming shelf clipping its last issue, and fill an ultrawide with it rather than white space
- [x] Quieten rename, note, duplicate, export and delete without hiding them or taking them out of the tab order
- [x] Make the full order scan as a list, with the current issue marked by something other than a tint
- [x] Measure the result in a browser at both widths rather than deriving it from the stylesheet

Constraint gate: checked 1 to 11, none breached. Constraint 7 is the live one and it is untouched:
the Marvel Unlimited button keeps its handler, its synchronous `window.open`, and its place in the
markup, so the launch is still made under user activation. Only its size and colour changed.
Constraint 6 is honoured, because the availability badge and its five states are not touched at all.
Constraint 11 is checked by the dash scan.

Implemented on 2026-08-20. Measured in Edge against the running app, importing House of M for the
seven-tile case and Essential Avengers for the eight-tile one. At 1280x900 the reading view goes from
876px to 964px and the hero cover from 176x264 to 248x372. At 2560x1080 the reading view goes from
876px to 1296px, which is the whole of the point: before this change the second measurement was
identical to the first, because the reading screen sat on the prose measure at every width.

The shelf was the clearest defect and the easiest to prove. On an order with eight issues still to
come, the old flex row overflowed its own box by 62px at 1280 and by 62px again at 2560, so the
eighth issue was cut in half and no width fixed it. The shelf now wraps: 0px of overflow at both
widths, eight tiles over two rows of 125px at 1280 and eight across one row of 149px at 2560.
Nothing scrolls sideways at either width.

It wraps as a flex row rather than as a grid, and the grid it started as is worth recording because
it passed every gate while being wrong. An `auto-fit` track list collapses its empty tracks and hands
their width to the survivors, and a tile with a capped width inside a stretched track is placed at
the track's start, so the shelf held its tiles at the declared gap only when it was full. A review
measured 487px between two tiles on a shelf with two left on it. That is the ordinary end of every
order, since the shelf shows what is still to come and empties as the order is read, and neither the
unit test nor the browser check had ever looked at a shelf with fewer tiles than it had room for. A
flex row with `flex: 1 1 112px` and the same cap reproduces the full-shelf measurements exactly and
packs a partial shelf from the left, and both gates now measure the widest gap between two tiles
sharing a row, at a full shelf and again after reading down to three or fewer.

Progress was carried by a 36px ring whose only figure, `0 / 8`, was a tooltip on the ring's wrapper.
A tooltip is not available to touch, is not read out on focus, and takes a hover and a pause on a
mouse. The ring is now 44px and the count is text beside it: `0 of 8 read` with `8 to go · 0%` under
it. The `title` attribute is gone rather than kept as a duplicate, because two sources for one number
is how they come to disagree. The circumference constant moved with the radius, from 94.2 to 119.4,
and a test now asserts the two agree so a future radius change cannot silently leave the arc drawing
the wrong fraction.

The list-management row was five outlined pills of the same weight as the hero's own buttons. They
are now borderless until hovered or focused, inside a single bounded strip, so the row reads as one
group of secondary actions rather than five competing offers. Every button keeps its label, its
place in the DOM and its tab stop, and the border returns on `:focus-visible`, so keyboard use is if
anything clearer than before. Under forced colors the strip and each button take a system border,
which is correct: the demotion is a visual hierarchy the user's own colours are entitled to discard.

The current issue is marked in the full order by an accent outline rather than by a tint alone, and
that outline was for a while drawn on three sides. Rows are separated by a top border on every row
after the first, and both rules weigh the same, so the later one took the top edge of whichever row
it landed on. The current issue is the first *unread* row, so in any order with progress in it that
row is preceded by a read one and lost its top edge every time; only an untouched order, which is
what both gates imported, showed it whole. The separator now stands down for the rows that have
something of their own to say, and the browser check reads the four border colours of the current
row after marking an issue read.

This partly overtakes BL-165's second task, "hold every prose view at the width it already had". That
task was right about the landing page and about prose, and the reading order screen is neither: it is
a working surface with a cover, a shelf and a table on it, and holding it at a reading measure was
costing it the shelf. The prose measure itself has not moved, and the views that carry prose still
sit on it.

Sized 2 because the visual change is broad but the mechanism is narrow, and because the checking is
the expensive half: both widths, both themes, covers off, forced colors, and a hero button whose
click path must not gain a single `await`.




**BL-170: Give the library and progress views a summary before their rows**

- [x] Put a summary band above the rows on the three library and progress views
- [x] Group Everything read by when issues were read, with real headings and counts
- [x] Add cover thumbnails to library rows without introducing a new image size
- [x] Group Progress by series by state, and name the state in words rather than in colour
- [x] Let the three views use the wider layout the landing page and the reading screen already use
- [x] Mark completed, active, empty and hand-added states by something other than colour alone
- [x] Measure the result in a browser at both widths rather than deriving it from the stylesheet

Constraint gate: checked 1 to 11, none breached. Constraint 1 is untouched because no image bytes
are hosted or stored, only cover URLs are used, and the thumbnails request the same variant the
reading rows already request, so no new image enters the cache. Constraint 3 is untouched because
nothing is uploaded and every figure is derived at render time from state already on the device,
with none of it stored. Constraint 4 is untouched because runtime dependencies stay at zero and the
new module adds none. Constraint 6 is untouched because the availability badge and its five states
are not touched at all. Constraint 11 is checked by the dash scan.

Implemented on 2026-08-21. Measured in Edge at 1280x900 and again at 2560x1080, against a seeded
fixture of 37 issues with 21 marked read across 7 series, 4 of them added by hand, and 3 read issues
belonging to no list. Every figure below is a before and after reading of that same fixture in the
same browser, taken from the running tree rather than derived from the stylesheet.

At 1280x900 the content column went from 876px to 964px on Everything read, Added by hand and
Progress by series. At 2560x1080 it went from 876px to 1296px on Everything read and on Progress by
series. Before this change the second measurement was identical to the first, because all three
views sat on the prose measure at every width. Horizontal overflow is 0px on every one of them at
both widths, before and after.

Everything read now groups its rows by when they were read, as Today, Yesterday, In the past week,
Earlier this month and then by month, each group a real heading with its count beside it. That took
the view from 0 headings to 9, which is nine places a reader can jump between on a screen that
offered none. Progress by series groups by state and names that state in words, taking it from 0 to
1. Added by hand stays ungrouped at 4 rows, for the reason two paragraphs below.

Cover thumbnails were added to the library rows at the same image variant the reading rows already
request: 0 to 21 covers on Everything read, and 0 to 4 on Added by hand. The summary bands did not
exist before at any width. They now read "21 issues read / 7 series / 3 in no list", "4 added by
hand / 2 read / 4 in no list", and "1 series / 7 of 8 tracked issues read / 0 series fully read".
The progress band says fully read rather than complete, because the app only knows about the issues
the reader has chosen to track, and a series can be finished in print while the tracker has one
issue of it.

Two fixes came after the rendered result was reviewed, and both are worth recording rather than
smoothing over. A chip repeating "In no list" was removed: the row's own meta line already says
those words, so a screen reader heard them twice, and on the hand-added view, where every row is in
no list by definition, the chip drew a column that distinguished nothing. And a derived grouping is
now drawn only above twelve rows, because below that it degrades into one heading per row. Measured
on the hand-added view with four entries, it produced four headings each printing a count of 1. The
progress groupings are deliberately exempt from that threshold, because there the group is the state
itself, and "Not started" is worth saying about a single series.

Sized 2 because the visual change is broad but the mechanism is narrow, and because the checking is
the expensive half: both widths, both themes, covers off, forced colors and reduced motion. Nothing
a reader has saved is touched. Every figure is computed from the existing state as the view renders,
and none of it is written back.

**BL-171: Make search the default way to add issues and group the other four paths**

- [x] Make the search panel always open rather than one collapsed disclosure among five
- [x] Move the other four paths under one "Other ways to add" section with a one line lead
- [x] Give each of those four a purpose line so a reader can choose a path without opening it
- [x] Head search first and drop the grouped headings a level so no heading level is skipped
- [x] Summarise each result set and say how many of it are already in your library
- [x] Mark a result already held, and keep that mark legible without colour
- [x] Name the destination reading order in the series and creator panes, not only in search
- [x] Give the loading messages their own notice kind so they carry no success tick

Implemented on 2026-08-21. The Add view offered five ways to add an issue as five equal collapsed
disclosures, search folded among them, so the fastest path for a reader who already knows a title
had no more prominence than importing a Markdown file. Search is now an always-open panel at the top
of the view, and the other four paths sit under one "Other ways to add" section with a one line lead
and a short purpose line on each summary, so a reader can tell what a path does before opening it.
The four leads read "Every issue of one series, oldest first.", "Every issue credited to one writer
or artist.", "A Markdown checklist or one title per line." and "Anything the 2025 metadata snapshot
does not have."

Verified in Microsoft Edge across the whole view: 38 checks, all passing. Heading levels went from a
run of equal disclosures to 1, 2, 2, 3, 3, 3, 3, with no level skipped, so the search panel and the
grouping read as peers and the four grouped paths one below. All 13 visible controls carry an
accessible name. Each of the five rail entries lands focus in its own control, and walking all five
in turn never left a second path open at once.

A result set now opens with a one line summary that a reader and a screen reader take the same words
from, for instance "12 results, 1 already in your library. Adding to My reading order.". That
sentence names the destination reading order in the search, series and creator panes alike; before
this change only the search pane named it, so two of the three left a reader guessing where an add
would land. A result already in the issue store carries an Already in your library marker. The held
count in that summary is derived at render time by one new pure function that reads the store and
counts distinct ids, with nothing uploaded or stored to produce it, so the promise that nothing
leaves the device is untouched. Under forced colours the marker stays a legible label with a tick
glyph rather than a colour, and the panels named in it keep a CanvasText border, checked in the Edge
high contrast mode.

The three loading messages moved to their own busy notice kind, so a message that means work is in
flight no longer wears the tick a success notice carries, and it shows no animation under
prefers-reduced-motion reduce. The results pane is still not a live region, so a screen reader is
not interrupted while rows move under it. Horizontal overflow is 0px at 640, 1280, 1920 and 2560 CSS
pixels, where 640 is what 200 per cent zoom reports. Lint reports 0 problems and the palette gate
exits 0. The one new function is covered by 8 tests, and a second test file guards the view's markup
and the repeated button classes.

Sized 2 because the change is broad in the view but narrow in mechanism, one pure reader of existing
state and a regrouping of markup that already existed, and because the checking is the expensive
half: both themes, forced colours, reduced motion and four widths. The availability model and its
five states are neither read nor changed here, and runtime dependencies stay at zero.

Constraint gate: checked 1 to 11, none breached.

**BL-172: Let the reader stop a long series or creator load once it has started**

- [ ] Add a cancel control to the series and creator adds so a running load can be stopped
- [ ] Leave a stopped add in a clean state, keeping the issues already fetched
- [ ] Follow the cancellation shape the reading view loaders already use, without moving them

Routed out of the research for the progressive disclosure work rather than built in it. Adding a
whole series fetches every page of that series in turn, and there is at present no way to stop it
once it is running: a reader who started the wrong series waits for it to finish or reloads the
page and loses the run. The app already has two loaders that can be cancelled, the cover hydrator
and the synopsis loader, so the pattern is known here. It is not reuse, though, because both of
those are wired into the reading view, not the Add view, and the series and creator runners have no
cancellation seam at all, so this is new capability rather than a wire from one view to another.

Scored V5, TC2, RE3 over Size3 for a WSJF of 3.33, and left Ready rather than started, because it is
a distinct piece of work from the disclosure change and the standing rule here is one feature to a
change. The measure of done is that a stopped add leaves the library exactly as a completed add of
the same issues would, minus the pages not yet fetched, with nothing half written.

Constraint gate: checked 1 to 11, none breached.

**BL-173: Show a cover thumbnail on each Add result row**

- [ ] Weigh a per-row cover request against the traffic it adds before building it
- [ ] If pursued, fetch each cover without blocking the row's own Add button
- [ ] Store only the cover URL, never image bytes, as every other view here does

Deliberately not done in the progressive disclosure work, and recorded so the omission reads as a
decision rather than an oversight. The search, series and creator endpoints are list endpoints, and
they return neither a cover nor a digitalId; only the per-issue endpoint carries those, which is the
whole reason hydration runs after every add rather than at list time. A thumbnail on an Add result
would therefore need a new per-row request pattern against the per-issue endpoint, one call a row,
which is a performance question about traffic and pacing rather than a styling one, and it belongs
in its own change with its own measurement.

Scored V3, TC1, RE2 over Size3 for a WSJF of 2.0, and left Ready. If it is taken up, constraint 1
governs it: the row would store the cover URL only and never the image bytes, exactly as the reading
and library rows already do, so no comic image enters a cache.

Constraint gate: checked 1 to 11, none breached.

**BL-174: Group the settings screen and give each control its own confirmation**

- [x] Group the settings view into Data safety, Personalization, Connectivity and Advanced
- [x] Put backup and restore first, inside Data safety, with the primary purple treatment
- [x] Move cached metadata out of the Metadata source card into its own card under Advanced
- [x] Give the metadata source control and the cache control each their own report pane
- [x] Carry a shape glyph on each of the four notice kinds so they read without colour
- [x] Keep every card title outranking its sub-heading, with no heading level skipped
- [x] Leave the danger zone card last, and reading progress and its storage key untouched

Implemented on 2026-08-21. The Backup and settings screen was a flat run of seven cards with no
grouping, so backup sat visually level with the cache size readout and nothing told a reader which
controls were safe to touch. The view is now four labelled groups, Data safety, Personalization,
Connectivity and Advanced, with backup and restore first inside Data safety and carrying the primary
purple treatment already used by the Add view's search panel. Cached metadata moved out of the
Metadata source card into its own card under Advanced, and the danger zone card stays last.

All three of the API-URL-refused, API-URL-saved and cache-cleared messages had been written into one
shared report pane that lived in the Backup card, so a confirmation for the metadata source control
appeared 905px above the button that caused it, in a different card and off screen, and a cache clear
silently overwrote a restore refusal the reader had not yet read. Two new report panes were added so
the metadata source control and the cache control each report into their own card, and the restore
pane now keeps only restore and export messages. Measured in Edge, the metadata source confirmation
moved from 905px above its button in another card to 38px below it in its own card, fully on screen,
and it stayed on screen at 200 per cent zoom. A refused restore now survives an unrelated cache
clear.

Under forced colours the notice kinds had been told apart by colour alone, which that mode discards,
so three of the four were indistinguishable from each other. Each kind now carries its own ::before
glyph, a tick for ok, a triangle for warn, a cross for error and a dot for busy, and all four keep a
1px border under forced colours. The finished-order empty state gained a simple glyph as well.
Measured in Edge, ordinary card titles render at 15.2px, the one primary card title at 18.4px, and
the view's only sub-heading, inside that primary card, at 13.6px, so where a title and a sub-heading
actually meet the title still outranks it. The heading levels run 1, 2, 3, 4, 3, 2, 3, 3, 2, 3, 3, 2,
3, 2 with no level skipped. The
settings view grew from 1,672px to 1,950px tall, 278px more, which is the honest cost of the four
group headings.

The four groups were expected to break a stated size claim about src/js/main.js and did not. The
only change to that file was repointing three message destinations from the shared pane to the two
new ones, which added and removed no lines, so it stayed at exactly 5,030 lines and every stated size
claim about it stayed true. Reading progress, its storage key and the recovery layer in
src/js/storage.js were not modified, nothing was uploaded, and no new storage key was introduced.

Verified with 7 new tests, taking the suite from 1,263 to 1,270, all passing. The repository browser
check passes 119 assertions across 14 scenarios, including the one that drives an unreadable saved
store against the salvage and start-fresh offers, and a 39-check acceptance run in Microsoft Edge
passes with 0 failures across both themes, forced colours, reduced motion and 200 per cent zoom.
Lint reports 0 problems, and the evidence anchors gate holds 993 citations with 0 drifted after 72
were re-aimed by this change, 42 of them by the source edits and 30 more by these records.
...
instead, after both Escape and a Cancel click. Measured on this change's own base commit as well as
on its changed tree, the behaviour is identical on both, so it is pre-existing rather than introduced
here, and the settings change left dialog behaviour still by holding to the binding rules it already
had.

This is a WCAG 2.4.3 focus order concern: a keyboard or screen reader user who dismisses a dialog is
dropped at the top of the view rather than back where they were working. The fix belongs with dialog
...
**BL-183: Keep dated evidence anchored to the repository state it recorded**

- [x] Recognise every dated tracking artifact structurally rather than by a list of current files
- [x] Resolve each committed citation line from the Git commit that last authored that line
- [x] Keep current documents, edited lines and new ignored artifacts on the active tree
- [x] Preserve historical extensionless targets through source-specific repository membership
- [x] Refuse shallow history, missing provenance, missing source targets and malformed range ends
- [x] Prove the behavior in disposable repositories without changing application dependencies

Constraint gate: checked 1 to 11, none breached. Constraint 3 is untouched because the mechanism
reads local Git objects and uploads nothing. Constraint 4 is untouched because runtime dependencies
stay at zero and no development dependency was added. Constraint 11 is checked by the added-line
dash scan.

Implemented on 2026-08-21. Dated tracking records now select a target tree for each citation
occurrence from that document line's Git provenance. A record can therefore contain lines authored
in several commits, and each keeps the repository state it actually recorded. Current product
documents still use the working tree or named revision, while an edited or new historical line uses
the active tree until it has a real source commit.

The corpus remains structural and complete. The checker still walks every tracked file, also enrolls
new dated artifacts hidden by the tracking-root ignore rule, and derives extensionless membership
from the same source tree used for fingerprinting. It does not exclude historical filenames, search
for repeated head text, fall back to current content, or keep a list of affected citations.

Five semantic process tests cover mixed historical and current sources, multiple source commits in
one artifact, direct and nested date paths, working lines, deleted extensionless targets, malformed
ranges, full and shallow clones, missing historical targets, and named revisions. All five failed
against the live-only checker and all pass with provenance enabled. A sixth test pins the existing
full-history checkout and was observed failing when that checkout was made shallow.

Sized 3 because the behavior crosses document discovery, Git history, citation membership and target
reads, while preserving the existing lock identity and comparison model. The implementation caches
one blame result per dated artifact and each source plus path read, and the complete repository
corpus resolves in under ten seconds.

**BL-184: Turn the root project page into a focused reader landing page**

- [x] Keep the product description, screenshot, reader benefits, and qualified privacy disclosure
- [x] Keep the stable Windows download, source startup, exact origin, and safe upgrade path
- [x] Move detailed reader operations into a focused running guide
- [x] Move checks, data authoring, and release procedures into a focused maintainer guide
- [x] Route support, contribution, and governance documents to the new canonical sections
- [x] Keep the root project page at or below 250 lines

Constraint gate: checked 1 to 11, none breached. Constraint 3 remains explicit in the root privacy
section, including every direct request the app makes. Constraint 4 remains intact because the two
guides add no runtime or development dependency. Constraint 5 is more prominent in the short run and
upgrade paths. Constraint 11 is checked by the added-line dash scan.

Implemented on 2026-08-21. The root project page fell from 849 lines to 118 while keeping the
reader-facing product story, privacy contract, stable Windows download, source startup, exact
`127.0.0.1:8787` origin, safe upgrade sequence, disclaimer, and license. Detailed startup,
installation, restart, port, and troubleshooting procedures now live in one running guide.
Repository checks, live API verification, browser and upgrade proof modes, workflow pin review, data
authoring, generated indexes, and merge-before-tag release mechanics now live in one maintainer
guide.

Sized 3 because the change moved four document roles without changing runtime behavior, then
reconnected support, contribution, governance, semantic tests, and evidence anchors to the new
ownership boundary.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:4124-4171` (renderProgress, which now takes a scope), `src/index.html:415`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3865` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4748-4768` (renderAll rebuilds
every region), `src/js/main.js:84-89` (store.onChange is wired straight to renderAll),
`src/js/main.js:2661-2839` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2841-2846`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:2304-2333` (undo), `src/js/main.js:4398-4404` (undoRestore exists).
- User engagement and inclusivity: gap. The interface is hard-locked to a dark scheme, and a light
  ...
  Evidence: `docs/ux-artifacts/live-inspection.json`, `src/styles.css:55`, `src/index.html:6`.
...
Evidence: `src/index.html:146-161` (blocked banner, saving paused, salvage offered),
`src/js/lib/model.js:888-964` (migrate refuses an unsupported schema version),
...
Evidence: `server.mjs:129-139`.

Resolved: `BL-030` shipped both. The policy is assembled at `server.mjs:53-64` and sent at
`server.mjs:134`, with `frame-ancestors 'none'` at `server.mjs:63` and the companion
`x-frame-options: DENY` at `server.mjs:137`.
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4748-4768`, `src/js/main.js:1201-1218` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the   file is 5,034 lines now, so nearly every item shipped
...
- Modifiability gap: the retry and backoff logic is duplicated between the two vendoring scripts.
  Evidence: `scripts/lib/fetch-json.mjs:72-81`, `scripts/lib/fetch-json.mjs:42-57`.
...
- Minor analysability gap: the `.row` class carries two unrelated meanings, a reading row and a form
  row, and a leftover empty rule sits between them. Evidence: `src/styles.css:639-642`,
  `src/styles.css:771-787`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4406-4439`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:888-964`.
...
| Category | Verdict |
|----------|---------|
| First-run experience | Gap. The first-run DOM ships an empty `<h2 id="hero-title">`, so the first heading a screen reader meets on an unseeded install is blank. Evidence: `src/index.html:340-375`, `docs/ux-artifacts/pa11y-landing.json`. |
| Empty states | No gap. The unseeded landing state explains what the app is for and routes to the catalog rather than showing a bare shell. Evidence: `docs/ux-artifacts/01-landing-firstrun-1280.png`, `src/index.html:42-45`. |
| Error handling and recovery | Gap, closed by BL-034. Curated import used to report failure through native `alert()` while every other path used the in-page notice system. It now writes to a pane chosen when the message is written, so on the landing page the reason appears beside the catalog it is about rather than stopping the page, and it is not left in a view the reader has already scrolled or navigated away from. Evidence: `src/js/main.js:4037`, `src/js/main.js:4061-4063`, `src/js/main.js:4093` against `src/js/main.js:364-514`. |
| Offline behavior | No gap, and no proposal. Probed as required rather than treated as a caching problem. With the local server running and no internet, the app starts, reads saved state, imports any bundled curated list and marks issues read, because those paths touch only same-origin files. Only cover images, metadata hydration and search degrade, and hydration failure is already surfaced as a pending state rather than as silence. Evidence: `src/data/house_of_m.json`, `src/js/main.js:2785-2786` (pending and by-hand badges), `absent: serviceWorker|navigator.onLine|manifest.json, case-insensitive grep across src/`. Repository Constraint 1 forbids caching cover bytes, so no cover-caching improvement is proposed. |
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1046-1077`, `src/js/main.js:4398-4404`. |
| Schema migration | No gap. Stored state carries `SCHEMA_VERSION`, migrations run forward, and a future schema is refused rather than silently coerced, with a test pinning that behaviour. Evidence: `src/js/lib/model.js:12`, `src/js/lib/model.js:888-964`, `test/model.test.js:564-566`. |
| Observability | Partial gap, bounded by Repository Constraint 3. Product analytics are forbidden and are not proposed. What is missing is local and private: there is no way for the reader to see why hydration stalled beyond a queue-depth pill. Evidence: `src/js/main.js:4721-4724`. |
| Performance | Gap, measured. See characteristic 2. Evidence: `docs/ux-artifacts/render-cost.json`. |
| Security, OWASP Top 10 | Gap under A05 Security Misconfiguration: no CSP and no `x-frame-options` on the dev server. Evidence: `server.mjs:129-139`. Resolved: `BL-030` shipped both, assembled at `server.mjs:53-64` and sent at `server.mjs:134` and `server.mjs:137`. Partial gap under A10 Server-Side Request Forgery by analogy: `MarvelApi` accepted any base URL and only stripped trailing slashes, with the https-or-local check living in the settings form rather than in the client. Resolved: `BL-045` moved the rule into the constructor at `src/js/api.js:52-67` and onto the read out of storage at `src/js/main.js:535-567`, so a base the rule refuses cannot reach a fetch from any of the three call sites, and the form keeps its own message at `src/js/main.js:4409-4410`. A01, A02, A03, A07 and A09 are not applicable, because there is no server-side authorisation boundary, no credential store, no server-side query language, no account system and no central log to protect. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3916-3940` (attribution rendered in the UI before import). |
...
eighty-one further items filed, none of them labelled, one of which, BL-060, was parked in its
turn, which is how it reaches 107 rows now. The ranks below are positions in it as it stands.

Positions, not scores, and the two have come apart in six places. Read on 2026-08-15 across the 107
...
the status key at `PRODUCT_BACKLOG.md:239` means by an item still to be picked up. Measured over
...
its turn, and BL-126 has since shipped as well, so both members of the last pair it named are now
outside the population it measures. Nothing checks that: the counts gate reads the ledger figures and
the id list, not a pair of ids named in prose, so an item shipping leaves this paragraph naming a row
that is no longer in the population it claims to measure. Re-derive it from the table whenever a
row's status changes, rather than reading it as settled.
...
stale unnoticed, and that one is already covered: `checkRanks` at `scripts/check-counts.mjs:180-212`
rebuilds every row's rank from the table and fails any `rank N of M` stated against it, and
`checkOrdinalHeadings` at `scripts/check-counts.mjs:225-249` does the same for a rank spelled as a
...
The cost is what decides it. A stable sort by score moves 40 of the 105 rows, and the six pairs do not
...
### Case 1: BL-026 is labelled P0 but ranks eighty-eighth

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 88 of 160.
...
- Calculated: WSJF 1.4, rank 154 of 160, below a hundred and forty-nine unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3865` and `src/js/main.js:3858-3875`. What is missing is data, in
...
- Consequence for the ranking: with BL-028 removed the table was 34 rows, and was 36 once BL-058
  ...
  34, carried by BL-131 alone. That figure was 16, shared by BL-029, BL-039 and BL-050, until
  the 2026-08-10 study filed four items scoring above the 18 BL-028 carried, and it was 29 until
  BL-131 was filed. It had also been
...
- BL-014, P1, rank 110 of 160. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 90 of 160. Mid-table.
- BL-017, P2, rank 159 of 160. The lowest-ranked scored story other than the one that cannot be
