...
A hundred and twelve items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
remote rather than on the tree, for the reasons recorded in their blocks. Nine remain open on
purpose: tagging a release needs a commit to point at, confirming BL-027 with a screen reader is a
human check no automated run substitutes for, BL-090's fourth task waits on that same run rather
than on a second one, BL-031's axe re-run cannot be satisfied as written, because axe declines to
judge text over a gradient and the finding was answered by computing the contrast bound instead,
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3242-3298` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3326-3341` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3265` and `src/js/main.js:3283-3296` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3304-3324` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2956-3012`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3020-3089` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2918-2936`, manual issue add at `src/js/main.js:3091-3135` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1876-1896`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:432` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1008-1048` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3508-3542`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3522-3524` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2347` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2443-2444`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2499-2504` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3114` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3735-3741` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2530-2553` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4085-4105`. The work shipped since has
  taken it further: the file is 4,193 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,093; 224 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2288-2297 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3784-3821 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3696-3724 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3900-3903 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1395-1396 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4058-4061 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1964-1991 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4005-4068 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3465 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3508-3542 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3256-3260 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4085-4099 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4085-4099 |
...
that function is at `src/js/main.js:3169` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3289`, and with the selector corrected the
...
`src/js/main.js:3522-3524` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:2439-2444`, along with the pending badge's. A `title` is unreachable by touch and
...
Reuse is keyed on the whole item, at `src/js/main.js:2222-2237`, rather than on a list of the
...
`src/js/main.js:2212-2220`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3468`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1931-1934` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1954-1962` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3468`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3437-3448`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1977-1985`. The usual reason
...
`src/js/main.js:3795`, `src/js/main.js:3719` and `src/js/main.js:3735-3741`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1713-1767`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3549-3567`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:3101`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3746-3747`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2439-2444`, drawing on the labels at
`src/js/main.js:2499-2504`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1730-1733`, and `renderRows` asks it at `src/js/main.js:2347`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1725-1729` exists because
...
`src/js/main.js:2187` and the reading rows at `src/js/main.js:2490`, and the reasoning is recorded at
...
`src/js/main.js:1550` carries a comment at `src/js/main.js:1560` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3410`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1271` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3485` puts
...
`src/js/main.js:4085` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2532`,
...
`src/js/main.js:1032` and `src/js/main.js:1505`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4118`, and `src/js/app.js` is the entry
...
on every catalog card, at `src/js/main.js:1505`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2967`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1452`. That also retired a latent bug rather than fixing it. The old label said
...
that boot report, and the comment now standing in its place at `src/js/main.js:4165-4169` records
...
file of 4,193 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3758-3760`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2423` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10596-10598` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10615-10619`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10430-10432` describes
...
`CHANGELOG.md:2601-2610` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3696-3724`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3256-3260`,
...
attack. Evidence: `src/js/main.js:3696-3724`, `src/js/lib/model.js:965-1074`.
...
`src/js/main.js:2085` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2921` and `src/js/main.js:2937`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3030` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:4005-4068`.
...
The message policy is a pure function at `src/js/main.js:2586-2595` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2906` and then starts
the hydrator at `src/js/main.js:2913`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2488`, and one of the four tests added to the counts suite pins the
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1738-1739`
...
button at `src/js/main.js:3784-3821` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3791-3820` repaints the banner
...
over, for the hero backdrop alone: `src/js/main.js:2085` reads the setting before building the URL,
...
`src/js/main.js:2236`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3427-3433`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3459-3465`,
...
here. Evidence: `src/js/main.js:3474`.
...
in place of that button while a copy is live at `src/js/main.js:3908-3914`, which is the state a
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4723-4731`.
...
`forgetDeleted()`, at `src/js/main.js:3795`, `src/js/main.js:3719` and `src/js/main.js:3740`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1395-1396`.
...
**Delivered.** The first citation now names `src/js/main.js:3795`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3784-3821`. The other two were already right and
are unchanged: `src/js/main.js:3719` is the call in the successful branch of the restore handler,
and `src/js/main.js:3735-3741` is the whole undo-restore handler, whose call is at
`src/js/main.js:3740`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1931-1934` is `forgetDeleted` itself, `src/js/main.js:1954-1962` is
`forgetDeletedFor`, `src/js/main.js:3468` is its one call, `src/js/main.js:3437-3448` is the
half-import rollback, `src/js/main.js:437` is `notify` with its `action` parameter,
`src/js/main.js:460-464` is `spoken`, `src/js/main.js:1977-1985` is the failed undo keeping its
...
`src/js/main.js:1957`, `src/js/main.js:1973` and `src/js/main.js:1988`. The sentence is right to
...
at `PRODUCT_BACKLOG.md:4728-4729`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4723-4726` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4058-4061`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4040-4054` keeps
...
consumer at `src/js/main.js:2104`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2288-2297` returns a sentence saying no synopsis is
...
The disclaimer at `src/js/main.js:2635` is shown every time a run starts rather than once and
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3508-3542` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3290` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4085-4105` (renderAll rebuilds
every region), `src/js/main.js:75-80` (store.onChange is wired straight to renderAll),
`src/js/main.js:2319-2497` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2499-2504`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1964-1991` (undo), `src/js/main.js:3735-3741` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4085-4105`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,193 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3743-3776`, `src/js/lib/cachePolicy.js:16-20`,
...
| Release and versioning | Gap. Version is pinned at `0.1.0` with no tags and no changelog, so there is no way to say which build a backup or a bug report came from. Evidence: `package.json:3`, `absent: CHANGELOG.md and git tags, glob of repository root and git tag --list`. Resolved: `BL-043` set the version to `1.0.0` at `package.json:3`, added `CHANGELOG.md`, and wired a `version` script at `package.json:27` that syncs the version the app reports. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3304-3324` (attribution rendered in the UI before import). |
...
- Calculated: WSJF 3.67, rank 59 of 122.
...
- Calculated: WSJF 1.4, rank 116 of 122, below a hundred and twelve unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3290` and `src/js/main.js:3283-3296`. What is missing is data, in
...
- BL-014, P1, rank 75 of 122. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 61 of 122. Mid-table.
- BL-017, P2, rank 121 of 122. The lowest-ranked scored story other than the one that cannot be
