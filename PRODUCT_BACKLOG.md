...
A hundred and fourteen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147 and BL-149.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3273-3329` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3357-3372` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3296` and `src/js/main.js:3314-3327` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3335-3355` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2960-3016`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3024-3093` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2922-2940`, manual issue add at `src/js/main.js:3105-3166` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1880-1900`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:432` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1008-1048` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3539-3573`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3553-3555` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2351` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2447-2448`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2503-2508` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3136` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3766-3772` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2534-2557` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:335-348` and `src/js/main.js:437-458` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4116-4136`. The work shipped since has
  taken it further: the file is 4,224 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,122; 224 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2292-2301 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3815-3852 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3727-3755 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3940-3943 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4089-4092 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1968-1995 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4036-4099 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3496 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:437-458 |
...
| BL-146 | Put a way out of the capped landing grid where the reader runs out of it | Defect | EP-10 | Follows BL-145 | 3 | 1 | 2 | 2 | 3.0 | none | Measured | Shipped | `src/index.html:247`, `src/js/main.js:1205-1212` |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3539-3573 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1176 |
...
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:560 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:106 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3287-3291 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4116-4130 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4116-4130 |
...
that function is at `src/js/main.js:3200` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3320`, and with the selector corrected the
...
`src/js/main.js:3553-3555` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:726` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:341-348` rather
...
`src/js/main.js:2443-2448`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:648` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:664` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:2226-2241`, rather than on a list of the
...
`src/js/main.js:2216-2224`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:372` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:381-419` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:353`, and both success paths clear that
...
`src/js/main.js:3499`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1935-1938` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1958-1966` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3499`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3468-3479`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:437`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:460-464` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1981-1989`. The usual reason
...
`src/js/main.js:3826`, `src/js/main.js:3750` and `src/js/main.js:3766-3772`. Each replaces the whole
...
`src/js/main.js:389-400`. Moving a message into the modal makes it readable where the reader is
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:75-80`. Syncing anywhere else would have left the address naming
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1717-1771`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3580-3598`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:3115`, so the two kinds cannot be
...
`src/js/main.js:471-500` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:525-541` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:3777-3778`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2443-2448`, drawing on the labels at
`src/js/main.js:2503-2508`, and the full sentence from `describe` sits in a visually-hidden span
...
now three, at `README.md:630-641`, and the four sentences over 40 words are now none: the longest
...
`src/js/main.js:1734-1737`, and `renderRows` asks it at `src/js/main.js:2351`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1729-1733` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:253` reads the pair back out of
...
`src/js/main.js:2191` and the reading rows at `src/js/main.js:2494`, and the reasoning is recorded at
`src/js/main.js:208-212` rather than only
...
`src/js/main.js:1554` carries a comment at `src/js/main.js:1564` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3441`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1275` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3516` puts
...
`renderRail` at `src/js/main.js:1008` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:980` and reaches `renderYours` through `renderHome` at
`src/js/main.js:982`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4116` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1016`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1170` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2536`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1186`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1032` and `src/js/main.js:1509`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4149`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:1032` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1509`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2971`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1456`. That also retired a latent bug rather than fixing it. The old label said
...
comment, when it comes from the series name at `src/js/main.js:602-603`. Both are re-aimed, and the
...
destructive one: `src/js/main.js:155-167` confirms Start fresh replaces the unreadable data with an
...
that boot report, and the comment now standing in its place at `src/js/main.js:4196-4200` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:124-143`, and until BL-075 took its
...
file of 4,224 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
together at `src/js/main.js:68-81`, and behind it sit sixteen library modules, none of which holds
...
the client and leaves the limiter alone, at `src/js/main.js:3789-3791`, because the budget belongs to
...
`src/js/storage.js:9-12`, two more belong to the view file at `src/js/main.js:39-40`, and the
...
the one change callback at `src/js/main.js:75-80`, so nothing left on screen said why saving was
paused.
...
glyph that no one reads, since the button's label at `src/js/main.js:2427` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10861-10863` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10880-10884`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10695-10697` describes
...
`src/js/storage.js:495-571`, `src/js/main.js:3727-3755`.
...
proxy for it.
Evidence: `src/js/storage.js:426-450`, `src/js/main.js:88-99`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3287-3291`,
...
attack. Evidence: `src/js/main.js:3727-3755`, `src/js/lib/model.js:965-1074`.
...
`src/js/main.js:1176` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/main.js:2089` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2925` and `src/js/main.js:2941`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3034` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:4036-4099`.
...
`src/js/main.js:437-447`, which routes to the announcer precisely when the container is not itself
...
rendered text at `src/js/main.js:316-323`, because both surfaces are rewritten far more often than
they change: `checkHealth` runs at boot and again on every API URL save, and `renderHydration` is
called once per issue fetched. Seeding the health key as reachable at `src/js/main.js:329-331` is
...
The message policy is a pure function at `src/js/main.js:2590-2599` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2910` and then starts
the hydrator at `src/js/main.js:2917`, whose `start()` reports its running phase at
...
`src/js/main.js:271-286`, because two messages produced by one action are one thing to say. Four of
...
size, at `PRODUCT_BACKLOG.md:2528`, and one of the four tests added to the counts suite pins the
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1742-1743`
...
are valuable history and are not a concise public policy. Evidence: `README.md:381-696`,
...
button at `src/js/main.js:3815-3852` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3822-3851` repaints the banner
...
it on. Evidence: `src/js/main.js:641-642`, `src/js/main.js:638`, `src/styles.css:436`.
...
over, for the hero backdrop alone: `src/js/main.js:2089` reads the setting before building the URL,
...
`src/js/main.js:2240`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3458-3464`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3490-3496`,
...
here. Evidence: `src/js/main.js:3505`.
...
in place of that button while a copy is live at `src/js/main.js:3939-3945`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1176`
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4763-4771`.
...
`forgetDeleted()`, at `src/js/main.js:3826`, `src/js/main.js:3750` and `src/js/main.js:3771`
...
**Delivered.** The first citation now names `src/js/main.js:3826`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3815-3852`. The other two were already right and
are unchanged: `src/js/main.js:3750` is the call in the successful branch of the restore handler,
and `src/js/main.js:3766-3772` is the whole undo-restore handler, whose call is at
`src/js/main.js:3771`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1935-1938` is `forgetDeleted` itself, `src/js/main.js:1958-1966` is
`forgetDeletedFor`, `src/js/main.js:3499` is its one call, `src/js/main.js:3468-3479` is the
half-import rollback, `src/js/main.js:437` is `notify` with its `action` parameter,
`src/js/main.js:460-464` is `spoken`, `src/js/main.js:1981-1989` is the failed undo keeping its
buffer, `src/js/main.js:389-400` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:1961`, `src/js/main.js:1977` and `src/js/main.js:1992`. The sentence is right to
...
at `PRODUCT_BACKLOG.md:4768-4769`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4763-4766` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4089-4092`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4071-4085` keeps
...
consumer at `src/js/main.js:2108`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2292-2301` returns a sentence saying no synopsis is
...
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:554` would have recorded
...
The disclaimer at `src/js/main.js:2639` is shown every time a run starts rather than once and
...
The one-time purge at `src/js/main.js:560` empties the response cache once, so the prose cached
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3539-3573` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3321` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4116-4136` (renderAll rebuilds
every region), `src/js/main.js:75-80` (store.onChange is wired straight to renderAll),
`src/js/main.js:2323-2501` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2503-2508`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1968-1995` (undo), `src/js/main.js:3766-3772` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4116-4136`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,224 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3774-3807`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3335-3355` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3321` and `src/js/main.js:3314-3327`. What is missing is data, in
