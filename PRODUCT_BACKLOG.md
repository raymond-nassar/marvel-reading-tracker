...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2442-2476` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2504-2519` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2450` and `src/js/main.js:2459-2464` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2482-2502` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2143-2199`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2207-2276` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2105-2123`, manual issue add at `src/js/main.js:2278-2322` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1369-1389`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:179` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:765-805` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2681-2715`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2695-2697` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1676` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1763-1769`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1821-1826` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2301` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2812-2818` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1852-1875` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2928-2948`. The work shipped since has
  taken it to 2,980; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1455-1482 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2681-2715 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2928-2942 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2928-2942 |
...
that function is at `src/js/main.js:2356` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2454`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2695-2697` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1759-1769`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2641`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1422-1425` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1445-1453` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2641`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2609-2620`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1468-1476`. The usual reason
...
`src/js/main.js:2805`, `src/js/main.js:2805` and `src/js/main.js:2812-2818`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1268-1312`, before the
...
Verified: `npm run lint` 0, `npm test` 480 pass and 0 fail, up from 468 by the 12 added to
`test/route.test.js`. Seven mutations were applied one at a time and 7 of 7 turned the suite red,
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2722-2740`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2288`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2822-2823`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1759-1769`, drawing on the labels at
`src/js/main.js:1821-1826`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1285-1288`, and `renderRows` asks it at `src/js/main.js:1676`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1280-1284` exists because
...
`src/js/main.js:1854`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
`src/js/main.js:1140` carries a comment at `src/js/main.js:1150` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2588`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:993` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2658` puts
...
`renderRail` at `src/js/main.js:765` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:743` and reaches `renderYours` through `renderHome` at
`src/js/main.js:745`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:2928` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:773`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:916` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1854`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:929`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:789` and `src/js/main.js:1108`. Both predate this change and neither is part of its
...
built at `src/js/main.js:789` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1108`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2154`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1092`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3394-3395` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3414-3416`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:3236-3238` describes
...
`CHANGELOG.md:400-403` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2681-2715` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2425` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2928-2948` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1665-1819` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1821-1826`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1455-1482` (undo), `src/js/main.js:2812-2818` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2928-2948`, `src/js/main.js:730-761` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 2,980 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2820-2840`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2482-2502` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2425` and `src/js/main.js:2459-2464`. What is missing is data, in
