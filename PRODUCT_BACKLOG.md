...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2347-2377` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2405-2420` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2351` and `src/js/main.js:2360-2365` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2383-2403` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2051-2104`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2112-2181` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2013-2031`, manual issue add at `src/js/main.js:2183-2227` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2582-2616`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2596-2598` | Done, shipped as BL-014 |
...
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1729-1734` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2206` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2713-2719` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1760-1783` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2829-2849`. The work shipped since has
  taken it to 2,923; 1,566 is the figure as audited.
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2582-2616 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2829-2843 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2829-2843 |
...
that function is at `src/js/main.js:2261` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2355`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2596-2598` instead of asserting "every list" whatever is being counted.
...
Verified: 24 new tests, every one of 20 mutations caught with none skipped, and 32 browser checks in
...
`src/js/main.js:2485`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:2542`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2510-2521`, leaving nothing behind and the offer still
...
`src/js/main.js:2706`, `src/js/main.js:2706` and `src/js/main.js:2713-2719`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2623-2641`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2193`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2723-2724`, unchanged.
...
`src/js/main.js:1729-1734`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1720`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2489`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2559` puts
...
`src/js/main.js:2829` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1762`,
...
at `src/js/main.js:2062`. A reader who imports one of this repository's own checklists therefore saw
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3156-3157` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3176-3178`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2998-3000` describes
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2582-2616` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2330` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2829-2849` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1608-1727` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1729-1734`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1398-1425` (undo), `src/js/main.js:2713-2719` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2829-2849`, `src/js/main.js:711-742` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 2,923 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2721-2741`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:592-620`, `src/js/main.js:2713-2719`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2383-2403` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2330` and `src/js/main.js:2360-2365`. What is missing is data, in
