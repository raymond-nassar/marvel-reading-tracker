...
Thirty-six items have since been delivered and are marked `Shipped` in the table below: BL-007,
BL-014, BL-026, BL-027, BL-029, BL-030, BL-031, BL-032, BL-034, BL-035, BL-036, BL-037, BL-038,
BL-039, BL-040, BL-041, BL-043, BL-044, BL-045, BL-046, BL-047, BL-048, BL-049, BL-050, BL-051,
BL-052, BL-053, BL-054, BL-055, BL-056, BL-057, BL-058, BL-059, BL-061, BL-062 and BL-063.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2287-2317` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:400-402`, filterByFacet imported at `src/js/main.js:17` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:393-399`, search-as-you-type at `src/js/main.js:2345-2360` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2291` and `src/js/main.js:2300-2305` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2323-2343` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1991-2044`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2052-2121` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1953-1971`, manual issue add at `src/js/main.js:2123-2167` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1294-1314`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:745-785` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2522-2556`, with a scope control at `src/index.html:360-364` and a subtitle that stops claiming every list at `src/js/main.js:2536-2538` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1597` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:284-319` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1649-1655`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1685-1690` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2146` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:543-567` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2652-2658` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1700-1723` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:226-239` and `src/js/main.js:328-349` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2768-2788`. The work shipped since has
  taken it to 2,862; 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 403; 224 is the figure as audited.
...
|----|-------|------|------|--------------|---|----|----|------|------|---|-------|--------|----------|
| BL-030 | Stop dimming read rows with a blanket opacity | Defect | EP-08 | Leaves alone | 5 | 3 | 2 | 1 | 10.0 | none | Measured | Shipped | src/styles.css:541-550 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1380-1407 |
| BL-047 | Split the two meanings of the row class | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Observed | Shipped | src/styles.css:628-644 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:328-349 |
| BL-031 | Put a scrim behind hero text so its contrast stops depending on the cover | Defect | EP-08 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | none | Measured | Shipped | src/index.html:284-319 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2522-2556 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:207 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:79 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2768-2782 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2768-2782 |
...
that function is at `src/js/main.js:2201` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2295`, and with the selector corrected the same mutation fails it. It is
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:355-366`; omitting it
...
`src/index.html:360-364`, matching the reading filter's fieldset pattern rather than inventing a
second idiom, and the subtitle at `src/index.html:358` is now written by the render at
`src/js/main.js:2536-2538` instead of asserting "every list" whatever is being counted.
...
a stale id at `src/js/lib/model.js:533`, so the two cannot come apart. That invariant also rules out
...
announced. It now lands on the all-read heading at `src/index.html:322`, which is both what the
reader needs to hear and where the remaining actions are.

The About view carries the shortcut reference at `src/index.html:595-602`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:544` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:232-239` rather
...
`src/js/main.js:1645-1655`, along with the pending badge's. A `title` is unreachable by touch and
...
strikethrough, at `src/styles.css:547-548`; the strikethrough is the non-colour indicator. The
only opacity left on a read row is the cover image at `src/styles.css:550`, which carries no
...
`src/index.html:692`, so resolving the theme in JavaScript would paint dark and then flip, and
...
outstanding notice is kept as a record at `src/js/main.js:263` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:272-310` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:244`, and both success paths clear that
...
`src/js/main.js:2425`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1347-1350` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1370-1378` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2482`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2450-2461`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:328`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:351-355` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1393-1401`. The usual reason
...
`src/js/main.js:2645`, `src/js/main.js:2645` and `src/js/main.js:2652-2658`. Each replaces the whole
...
`src/js/main.js:280-291`. Moving a message into the modal makes it readable where the reader is
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:66-71`. Syncing anywhere else would have left the address naming
...
the id is unknown, at `src/js/lib/model.js:225-227`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1211-1264`, before the
...
The fourth task stays open, but not for the reason it was written. BL-036 has landed and decided
the scheme, so nothing has to be invented to carry the filter in it. It was kept out of that change
to hold to one major feature per pull request, and is the one task left in this block.
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:386-436`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2563-2581`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:193-194`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:2133`, so the two kinds cannot be
...
reader presses the button. `VIEWS` at `src/js/lib/route.js:13` now spreads the Library entries in
rather than naming them, and the sections in `src/index.html:373-385` are empty shells whose heading,
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:307-314`, written the same way
`coerce` writes it at `src/js/lib/model.js:505`, so the two paths cannot disagree and every future
caller inherits it.
...
`src/js/main.js:362-386` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:387-400` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:2662-2663`, unchanged.
...
Shipped. The form row is now `.field-row` at `src/styles.css:628-644` and the reading row keeps
`.row`, so the thirteen rules that describe a reading row, from `src/styles.css:535-538` down to the
...
reached every one of them; the reading list at `src/index.html:358` sits inside neither, so it never
...
two at `src/index.html:464` and `src/index.html:464` are direct children of a `.card card-static`,
...
`.card-static > *` at `src/styles.css:621`, which the old rule's 0,2,0 had been masking. Measured
...
is written out in words inside the pill at `src/js/main.js:1645-1655`, drawing on the labels at
`src/js/main.js:1685-1690`, and the full sentence from `describe` sits in a visually-hidden span
...
The reasoning is recorded at `src/styles.css:570-594`, directly above the rules it governs, along
...
`src/js/main.js:1228-1231`, and `renderRows` asks it at `src/js/main.js:1597`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1223-1227` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:207` reads the pair back out of
...
`src/js/main.js:1676`, and the reasoning is recorded at `src/js/main.js:162-166` rather than only
...
while proving nothing. The full order lives inside a `<details>` at `src/index.html:336` that is
...
`src/js/main.js:1119` carries a comment at `src/js/main.js:1129` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2429`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:973` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2499` puts
...
`renderRail` at `src/js/main.js:745` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:723` and reaches `renderYours` through `renderHome` at
`src/js/main.js:725`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:2768` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:753`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:896` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1702`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:909`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:769` and `src/js/main.js:1087`. Both predate this change and neither is part of its
...
`src/styles.css:615` sets `content` to a right-pointing angle glyph on the card summary marker. It
...
`content` in the stylesheets is the one at `src/styles.css:615`. Closing it would need a tokenizer
...
built at `src/js/main.js:769` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1087`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2002`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1071`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3101-3102` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3121-3123`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2943-2945` describes
...
`docs/UX_STUDY.md:776-779` while this paragraph carries no such marker and reads in the present
...
`CHANGELOG.md:340-343` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2522-2556` (renderProgress, which now takes a scope), `src/index.html:358`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2270` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2768-2788` (renderAll rebuilds
every region), `src/js/main.js:66-71` (store.onChange is wired straight to renderAll),
`src/js/main.js:1586-1683` (renderRows builds every row with no virtualisation and no early exit when
...
`src/js/lib/model.js:543-567` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1685-1690`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1380-1407` (undo), `src/js/main.js:2652-2658` (undoRestore exists).
...
`src/js/lib/model.js:466-492` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2768-2788`, `src/js/main.js:710-741` (showView switches views by
...
- Minor analysability gap: the `.row` class carries two unrelated meanings, a reading row and a form
  row, and a leftover empty rule sits between them. Evidence: `src/styles.css:535-538`,
  `src/styles.css:628-644`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2660-2680`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:466-492`.
...
| Category | Verdict |
|----------|---------|
| First-run experience | Gap. The first-run DOM ships an empty `<h2 id="hero-title">`, so the first heading a screen reader meets on an unseeded install is blank. Evidence: `src/index.html:284-319`, `docs/ux-artifacts/pa11y-landing.json`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2323-2343` (attribution rendered in the UI before import). |
...
- Calculated: WSJF 1.4, rank 35 of 41, below thirty-one unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2270` and `src/js/main.js:2300-2305`. What is missing is data, in
