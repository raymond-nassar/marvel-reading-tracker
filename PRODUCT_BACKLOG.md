...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3004-3038` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:417-419`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:410-416`, search-as-you-type at `src/js/main.js:3066-3081` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3012` and `src/js/main.js:3021-3026` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3044-3064` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2705-2761`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2769-2838` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2667-2685`, manual issue add at `src/js/main.js:2840-2884` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1681-1701`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:432` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1000-1040` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3248-3282`, with a scope control at `src/index.html:377-381` and a subtitle that stops claiming every list at `src/js/main.js:3262-3264` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2148` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2244-2245`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2300-2305` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2863` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:1037-1068` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3475-3481` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2331-2354` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:3812-3832`. The work shipped since has
  taken it further: the file is 3,920 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,003; 224 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2093-2098 |
...
| BL-134 | Fetch a synopsis at read time instead of shipping one | Feature | EP-05 | Follows BL-130 | 8 | 5 | 8 | 3 | 7.0 | none | Measured | Shipped | src/js/synopsis.js:85 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3511-3548 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3436-3464 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3886-3889 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1382-1383 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3785-3788 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1769-1796 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:987 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3732-3795 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3205 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/main.js:621-625 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3248-3282 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1159 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3007-3011 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3812-3826 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3812-3826 |
...
that function is at `src/js/main.js:2918` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:3016`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:3262-3264` instead of asserting "every list" whatever is being counted.
...
a stale id at `src/js/lib/model.js:1032`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:954-955`, which latches the "Could not read your saved data" banner. Measured
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:718` and
...
`src/js/main.js:2240-2245`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:640` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:656` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:2027-2042`, rather than on a list of the
...
`src/js/main.js:2017-2025`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3208`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1736-1739` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1759-1767` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3208`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3177-3188`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1782-1790`. The usual reason
...
`src/js/main.js:3522`, `src/js/main.js:3459` and `src/js/main.js:3475-3481`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1518-1572`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3289-3307`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2850`, so the two kinds cannot be
...
`coerce` writes it at `src/js/lib/model.js:968`, so the two paths cannot disagree and every future
caller inherits it.
...
The settings form keeps its own check and its own message at `src/js/main.js:3486-3487`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2240-2245`, drawing on the labels at
`src/js/main.js:2300-2305`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1535-1538`, and `renderRows` asks it at `src/js/main.js:2148`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1530-1534` exists because
...
`src/js/main.js:1992` and the reading rows at `src/js/main.js:2291`, and the reasoning is recorded at
...
`src/js/main.js:1390` carries a comment at `src/js/main.js:1400` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3150`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1233` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3225` puts
...
`renderRail` at `src/js/main.js:1000` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:972` and reaches `renderYours` through `renderHome` at
`src/js/main.js:974`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:3812` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1008`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1153` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2333`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1169`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1024` and `src/js/main.js:1350`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:3845`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:1024` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1350`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2716`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1332`. That also retired a latent bug rather than fixing it. The old label said
...
`lists[k] = {...}` at `src/js/lib/model.js:987`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:1015`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:1032`.
...
comment, when it comes from the series name at `src/js/main.js:594-595`. Both are re-aimed, and the
...
that boot report, and the comment now standing in its place at `src/js/main.js:3892-3896` records
...
file of 3,920 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3498-3500`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2224` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:9820-9822` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:9839-9843`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:9654-9656` describes
...
`CHANGELOG.md:2312-2321` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3436-3464`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3007-3011`,
...
attack. Evidence: `src/js/main.js:3436-3464`, `src/js/lib/model.js:959-1068`.
...
`src/js/main.js:1159` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/main.js:1890` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2670` and `src/js/main.js:2686`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2779` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3732-3795`.
...
The message policy is a pure function at `src/js/main.js:2387-2396` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2655` and then starts
the hydrator at `src/js/main.js:2662`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2475`, and one of the four tests added to the counts suite pins the
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1543-1544`
...
button at `src/js/main.js:3511-3548` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3518-3547` repaints the banner
...
it on. Evidence: `src/js/main.js:633-634`, `src/js/main.js:630`, `src/styles.css:436`.
...
over, for the hero backdrop alone: `src/js/main.js:1890` reads the setting before building the URL,
...
`src/js/main.js:2041`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3167-3173`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3199-3205`,
...
here. Evidence: `src/js/main.js:3214`.
...
in place of that button while a copy is live at `src/js/main.js:3635-3641`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1159`
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4709-4717`.
...
`forgetDeleted()`, at `src/js/main.js:3522`, `src/js/main.js:3459` and `src/js/main.js:3480`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1382-1383`.
...
**Delivered.** The first citation now names `src/js/main.js:3522`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3511-3548`. The other two were already right and
are unchanged: `src/js/main.js:3459` is the call in the successful branch of the restore handler,
and `src/js/main.js:3475-3481` is the whole undo-restore handler, whose call is at
`src/js/main.js:3480`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1736-1739` is `forgetDeleted` itself, `src/js/main.js:1759-1767` is
`forgetDeletedFor`, `src/js/main.js:3208` is its one call, `src/js/main.js:3177-3188` is the
half-import rollback, `src/js/main.js:437` is `notify` with its `action` parameter,
`src/js/main.js:460-464` is `spoken`, `src/js/main.js:1782-1790` is the failed undo keeping its
...
`src/js/main.js:1762`, `src/js/main.js:1778` and `src/js/main.js:1793`. The sentence is right to
...
at `PRODUCT_BACKLOG.md:4714-4715`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4709-4712` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3785-3788`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3767-3781` keeps
...
consumer at `src/js/main.js:1909`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2093-2098` returns a sentence saying no synopsis is
...
`src/js/synopsis.js:85` writes only to a `Map` and never calls `store.update`, so the promise holds by
...
The disclaimer at `src/js/main.js:2422` is shown every time a run starts rather than once and
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3248-3282` (renderProgress, which now takes a scope), `src/index.html:375`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2987` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3812-3832` (renderAll rebuilds
every region), `src/js/main.js:75-80` (store.onChange is wired straight to renderAll),
`src/js/main.js:2120-2298` (renderRows, which then built every row with no virtualisation and no
...
`src/js/lib/model.js:1037-1068` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2300-2305`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1769-1796` (undo), `src/js/main.js:3475-3481` (undoRestore exists).
...
`src/js/lib/model.js:879-955` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:3812-3832`, `src/js/main.js:957-974` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,920 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3483-3503`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:879-955`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3044-3064` (attribution rendered in the UI before import). |
...
### Case 1: BL-026 is labelled P0 but ranks fifty-fourth

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 54 of 110.
- Driver: job size, not value. Its Cost of Delay of 11 is the tenth highest figure in the backlog.
  It is outranked by fifty-three items, twenty-nine of them sized 1, 2 or 3 whose Cost of Delay
...
- Calculated: WSJF 1.4, rank 104 of 110, below a hundred unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2987` and `src/js/main.js:3021-3026`. What is missing is data, in
...
- BL-014, P1, rank 67 of 110. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 56 of 110. Mid-table.
- BL-017, P2, rank 109 of 110. The lowest-ranked scored story other than the one that cannot be
