...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2046-2076` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:366-368`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:359-365`, search-as-you-type at `src/js/main.js:2104-2119` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2050` and `src/js/main.js:2059-2064` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2082-2102` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2029` and `src/js/main.js:2059-2064`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
| 2.4 | BL-008 | P1 | orders and catalog generated by `scripts/vendor-orders.mjs:201-301`, consumed as data with no view change | Done |
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1750-1803`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1811-1880` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1712-1730`, manual issue add at `src/js/main.js:1882-1926` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1081-1101`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:590-615` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:2266-2284`, and the view states it counts across every list at `src/index.html:346-348` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1447-1457` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1412-1418`, pending filter at `src/js/main.js:1451` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1440-1445` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1905` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2329-2335` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1467-1485` | Partial |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:155-168` and `src/js/main.js:257-278` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2445-2458`.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1167-1194 |
...
| BL-026 | Make every action reachable and repeatable from the keyboard | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P0 | Measured | Ready | src/js/main.js:1467-1485 |
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:257-278 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:2266-2284 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2445-2457 |
...
Which channel to use is now decided by reading the container at `src/js/main.js:161-168` rather
...
`src/js/main.js:1408-1418`, along with the pending badge's. A `title` is unreachable by touch and
...
outstanding notice is kept as a record at `src/js/main.js:192` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:201-239` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:173`, and both success paths clear that
...
`src/js/main.js:2184`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1134-1137` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1157-1165` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2241`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2209-2220`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:257`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:280-284` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1180-1188`. The usual reason
...
`src/js/main.js:2376`, `src/js/main.js:2322` and `src/js/main.js:2329-2335`. Each replaces the whole
...
`src/js/main.js:209-220`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1017-1051`, before the
...
the code says so: `matchesFilter` at `src/js/main.js:1447-1457` still enumerates the five and
...
`src/js/main.js:291-315` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:317-330` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:2339-2340`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1408-1418`, drawing on the labels at
`src/js/main.js:1440-1445`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1431-1436` shows it plainly: at
...
honoured only when a radio offers it. `matchesFilter` at `src/js/main.js:1447-1457` still
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2266-2284` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2029` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2445-2458` (renderAll rebuilds
every region), `src/js/main.js:61-66` (store.onChange is wired straight to renderAll),
`src/js/main.js:1352-1438` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1440-1445`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1167-1194` (undo), `src/js/main.js:2329-2335` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2445-2458`, `src/js/main.js:558-586` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2337-2357`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2082-2102` (attribution rendered in the UI before import). |
...
intent that no one stated. Six original stories were still open when the pass ran, so the table was
28 rows then. BL-028 has since been parked and three further items filed, none of them labelled,
which is how it reaches 30 rows now. The ranks below are positions in it as it stands.
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2029` and `src/js/main.js:2059-2064`. What is missing is data, in
