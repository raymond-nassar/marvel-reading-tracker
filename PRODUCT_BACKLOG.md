...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1974-2004` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:366-368`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:359-365`, search-as-you-type at `src/js/main.js:2032-2047` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1978` and `src/js/main.js:1987-1992` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2010-2030` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1957` and `src/js/main.js:1987-1992`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1678-1731`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1739-1808` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1640-1658`, manual issue add at `src/js/main.js:1810-1854` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1015-1035`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:555-580` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:2194-2212`, and the view states it counts across every list at `src/index.html:346-348` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1375-1385` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1340-1346`, pending filter at `src/js/main.js:1379` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1368-1373` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1833` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2257-2263` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1395-1413` | Partial |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2366-2379`.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1095-1122 |
...
| BL-026 | Make every action reachable and repeatable from the keyboard | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P0 | Measured | Ready | src/js/main.js:1395-1413 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:2194-2212 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2366-2378 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2366-2378 |
...
`src/js/main.js:1336-1346`, along with the pending badge's. A `title` is unreachable by touch and
...
the page, and `placeNotices()` at `src/js/main.js:191-229` renders every outstanding record into
...
`src/js/main.js:2112`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1068-1071` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1086-1093` runs when the deleted list's own order is imported again. An offer the reader
can see and press should do what it says, so the case where it cannot is prevented rather than
handled.

Withdrawing it says so. Deleting the list you were reading hands you to the home view, where the
card for that order has already reverted to "+ Add to library", so the wrong way back and the right
one sit on the same screen. A reader who had renamed or reordered their copy would otherwise press
it, be told the order is now in their sidebar, and lose the route back to that copy in the same tick
with nothing said about it. The sentence is returned as well as shown, and the caller folds it into
the announcement it is about to make at `src/js/main.js:2169`, because two announcements in one tick
leave only the last.
...
rolls the half-import back at `src/js/main.js:2137-2148`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:247`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:270-274` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1108-1116`. The usual reason
...
`src/js/main.js:2297`, `src/js/main.js:2250` and `src/js/main.js:2257-2263`. Each replaces the whole
...
`src/js/main.js:199-210`. Moving a message into the modal makes it readable where the reader is
...
is written out in words inside the pill at `src/js/main.js:1336-1346`, drawing on the labels at
`src/js/main.js:1368-1373`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1359-1364` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2194-2212` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1957` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2366-2379` (renderAll rebuilds
every region), `src/js/main.js:61-66` (store.onChange is wired straight to renderAll),
`src/js/main.js:1280-1366` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1368-1373`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1095-1122` (undo), `src/js/main.js:2257-2263` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2366-2379`, `src/js/main.js:523-551` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2265-2278`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2010-2030` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1957` and `src/js/main.js:1987-1992`. What is missing is data, in
