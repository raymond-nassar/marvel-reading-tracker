...
BL-014, BL-026, BL-027, BL-029, BL-030, BL-031, BL-032, BL-034, BL-035, BL-037, BL-038, BL-039,
BL-040, BL-041, BL-043, BL-044, BL-045, BL-046, BL-047, BL-048, BL-049, BL-050, BL-051, BL-052,
BL-053, BL-054, BL-055, BL-056, BL-057, BL-058, BL-059, BL-061, BL-062, BL-063 and BL-066.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2297-2331` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:400-402`, filterByFacet imported at `src/js/main.js:17` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:393-399`, search-as-you-type at `src/js/main.js:2359-2374` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2305` and `src/js/main.js:2314-2319` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2337-2357` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1998-2054`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2062-2131` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1960-1978`, manual issue add at `src/js/main.js:2133-2177` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1266-1286`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:169` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:702-742` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2536-2570`, with a scope control at `src/index.html:360-364` and a subtitle that stops claiming every list at `src/js/main.js:2550-2552` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1569` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1656-1662`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1692-1697` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2156` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2666-2672` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1707-1730` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:225-238` and `src/js/main.js:327-348` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2782-2797`. The work shipped since has
  taken it to 2,852; 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 423; 224 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1352-1379 |
| BL-047 | Split the two meanings of the row class | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Observed | Shipped | src/styles.css:641-657 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:327-348 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2536-2570 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:206 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:78 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2782-2796 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2782-2796 |
...
that function is at `src/js/main.js:2211` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2309`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2550-2552` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:543` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:231-238` rather
...
`src/js/main.js:1652-1662`, along with the pending badge's. A `title` is unreachable by touch and
...
outstanding notice is kept as a record at `src/js/main.js:262` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:271-309` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:243`, and both success paths clear that
...
`src/js/main.js:2496`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1319-1322` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1342-1350` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2496`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2464-2475`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:327`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:350-354` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1365-1373`. The usual reason
...
`src/js/main.js:2659`, `src/js/main.js:2659` and `src/js/main.js:2666-2672`. Each replaces the whole
...
`src/js/main.js:279-290`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1183-1236`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2577-2595`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2143`, so the two kinds cannot be
...
reader presses the button. `VIEWS` at `src/js/main.js:667-670` now spreads the Library entries in
...
same way, it is 93 lines against main.js's 2,852, and it holds the module-scope `pending` that every
...
`src/js/main.js:361-385` now applies the same rule on the way out of storage and falls back to the
...
The settings form keeps its own check and its own message at `src/js/main.js:2676-2677`, unchanged.
...
Shipped. The form row is now `.field-row` at `src/styles.css:641-657` and the reading row keeps
...
`.card-static > *` at `src/styles.css:634`, which the old rule's 0,2,0 had been masking. Measured
...
is written out in words inside the pill at `src/js/main.js:1652-1662`, drawing on the labels at
`src/js/main.js:1692-1697`, and the full sentence from `describe` sits in a visually-hidden span
...
The reasoning is recorded at `src/styles.css:583-607`, directly above the rules it governs, along
...
`src/js/main.js:1200-1203`, and `renderRows` asks it at `src/js/main.js:1569`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1195-1199` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:206` reads the pair back out of
...
`src/js/main.js:1683`, and the reasoning is recorded at `src/js/main.js:161-165` rather than only
...
`src/js/main.js:1077` carries a comment at `src/js/main.js:1087` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2443`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:930` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2513` puts
...
`renderRail` at `src/js/main.js:702` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:683` and reaches `renderYours` through `renderHome` at
`src/js/main.js:685`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:2782` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:710`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:853` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1709`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:866`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:726` and `src/js/main.js:1045`. Both predate this change and neither is part of its
...
`src/styles.css:628` sets `content` to a right-pointing angle glyph on the card summary marker. It
...
`content` in the stylesheets is the one at `src/styles.css:628`. Closing it would need a tokenizer
...
built at `src/js/main.js:726` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1045`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2009`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1029`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3095-3096` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3115-3117`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2937-2939` describes
...
`CHANGELOG.md:350-353` that says so and points at the backlog clause instead. That clause was
...
Verified with 20 new tests, 423 passing in total, and 18 browser checks in Edge at 1280x900. Each new
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2536-2570` (renderProgress, which now takes a scope), `src/index.html:358`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2280` (groupCatalog renders variant rows only where a group exists).
...
- Correctness is well defended: 224 unit tests pass, 235 when this pass shipped and 423 now, and
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2782-2797` (renderAll rebuilds
every region), `src/js/main.js:65-70` (store.onChange is wired straight to renderAll),
`src/js/main.js:1558-1690` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1692-1697`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1352-1379` (undo), `src/js/main.js:2666-2672` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2782-2797`, `src/js/main.js:670-698` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 2,852 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2674-2694`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2337-2357` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2280` and `src/js/main.js:2314-2319`. What is missing is data, in
