...
A hundred and eighteen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147, BL-149, BL-150, BL-152, BL-153 and BL-154.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3465-3526` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3587-3602` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3489` and `src/js/main.js:3507-3524` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3565-3585` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3769-3803`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3783-3785` | Done, shipped as BL-014 |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3996-4002` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4346-4366`. The work shipped since has
  taken it further: the file is 4,454 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,180; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4045-4082 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3957-3985 |
...
| BL-154 | Shelve the events apart from the character runs, and say why the runs sit last | Story | EP-01 | Follows BL-152 | 5 | 3 | 2 | 2 | 5.0 | none | Measured | Shipped | `src/js/lib/catalog.js:587-590` |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4319-4322 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4266-4329 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3726 |
...
| BL-097 | Publish contribution, conduct, support and maintainer governance | Chore | EP-12 | Extends BL-052 | 3 | 3 | 5 | 3 | 3.67 | none | Observed | Shipped | README.md:388-455 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3769-3803 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3479-3484 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3544-3559` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4346-4360 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4346-4360 |
...
real title is `.result-title`, written at `src/js/main.js:3513`, and with the selector corrected the
...
`src/js/main.js:3783-3785` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3729`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3729`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3698-3709`, leaving nothing behind and the offer still
...
`src/js/main.js:4056`, `src/js/main.js:3980` and `src/js/main.js:3996-4002`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3810-3828`.
...
The settings form keeps its own check and its own message at `src/js/main.js:4007-4008`, unchanged.
...
now three, at `README.md:677-688`, and the four sentences over 40 words are now none: the longest
...
`README.md:547-551`, as fetching a list once and committing what came back. `depth` gained a gloss
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3671`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3746` puts
...
`src/js/main.js:4346` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4379`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4426-4430` records
...
file of 4,454 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:4019-4021`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11133-11135` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11152-11156`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10967-10969` describes
...
`CHANGELOG.md:2770-2779` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3957-3985`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3479-3484`,
...
attack. Evidence: `src/js/main.js:3957-3985`, `src/js/lib/model.js:968-1077`.
...
remain local or absent. Evidence: `README.md:84-103`, `src/index.html:529-587`,
...
`test/ci-supply-chain.test.js:124-142`, `README.md:505-540`.
...
`src/index.html:125-126`, `src/js/main.js:4266-4329`.
...
are valuable history and are not a concise public policy. Evidence: `README.md:388-743`,
...
button at `src/js/main.js:4045-4082` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4052-4081` repaints the banner
...
button. Evidence: `src/js/main.js:3688-3694`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3720-3726`,
...
here. Evidence: `src/js/main.js:3735`.
...
in place of that button while a copy is live at `src/js/main.js:4169-4175`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:4056`, `src/js/main.js:3980` and `src/js/main.js:4001`
...
**Delivered.** The first citation now names `src/js/main.js:4056`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4045-4082`. The other two were already right and
are unchanged: `src/js/main.js:3980` is the call in the successful branch of the restore handler,
and `src/js/main.js:3996-4002` is the whole undo-restore handler, whose call is at
`src/js/main.js:4001`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3729` is its one call, `src/js/main.js:3698-3709` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4319-4322`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4301-4315` keeps
...
this repository's own README says so at `README.md:162-164`, where the reader is already told their
...
than merely questioned. `README.md:163-164` already carries a workaround for the milder version of
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3769-3803` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3514` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4346-4366` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1970-1997` (undo), `src/js/main.js:3996-4002` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4346-4366`, `src/js/main.js:967-984` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,454 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4004-4037`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1046-1077`, `src/js/main.js:3996-4002`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3565-3585` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3514` and `src/js/main.js:3507-3524`. What is missing is data, in
