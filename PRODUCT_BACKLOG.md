...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1846-1876` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:366-368`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:359-365`, search-as-you-type at `src/js/main.js:1904-1919` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1850` and `src/js/main.js:1859-1864` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:1882-1902` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1829` and `src/js/main.js:1859-1864`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1552-1605`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1613-1682` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1514-1532`, manual issue add at `src/js/main.js:1684-1728` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:964-984`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:507-532` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:2049-2067`, and the view states it counts across every list at `src/index.html:346-348` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1249-1259` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1214-1220`, pending filter at `src/js/main.js:1253` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1242-1247` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1707` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:453-477` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2101-2104` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1269-1287` | Partial |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:150-163` and `src/js/main.js:199-208` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2212-2225`.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Ready | src/js/main.js:950-962 |
...
| BL-026 | Make every action reachable and repeatable from the keyboard | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P0 | Measured | Ready | src/js/main.js:1269-1287 |
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:199-208 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:2049-2067 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Ready | src/js/main.js:69 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2212-2224 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2212-2224 |
...
Which channel to use is now decided by reading the container at `src/js/main.js:156-163` rather
...
`src/js/main.js:1210-1220`, along with the pending badge's. A `title` is unreachable by touch and
...
viewport. `paneFor()` at `src/js/main.js:165-186` therefore picks the pane when the message is
written rather than when the import started, preferring a `.report` inside an open modal, then the
named pane, then any `.report` in the current view, and `notify()` scrolls it into view with
`block: 'nearest'`, which is a no-op when it is already visible.

Choosing at write time is not enough on its own. The failure is usually written before the reader
navigates, not after, so `rehomeNotices()` at `src/js/main.js:459-471` moves a notice that has been
left in a hidden view or a closed dialog into one the reader can see, on every view change and when
the preview closes. The message is moved rather than dropped because what it describes, an import
that did not happen, is still true afterwards.

That needs somewhere to move it to. Five of the seven views carry no notice pane of their own, so
`#app-report` sits above every view and is the destination when nothing else is showing. It is
deliberately not a live region: the message was already announced once when it was written, and
moving it into an assertive region would say it a second time, which is the double-speak BL-027
removed. `rehomeNotices()` replaces rather than appends, because `notify()` clears only the pane it
chose and the copy left behind is the same message, which stacked an identical paragraph per round
trip, measured at one, then two, then three in about six clicks.

Only panes carrying `report` are relocatable. `#save-report` is above every view and assertive
...
Seventeen scenarios are measured in Edge, including every view that has no pane of its own, and the
case where a notice already in view must not scroll the page out from under the reader.
...
Not done here: the undo after a delete. The confirmation says only that read progress is kept,
which is what is true today. BL-035 adds the undo and the wording that goes with it.
...
is written out in words inside the pill at `src/js/main.js:1210-1220`, drawing on the labels at
`src/js/main.js:1242-1247`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1233-1238` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2049-2067` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1829` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2212-2225` (renderAll rebuilds
every region), `src/js/main.js:61-66` (store.onChange is wired straight to renderAll),
`src/js/main.js:1154-1240` (renderRows builds every row with no virtualisation and no early exit when
...
`src/js/lib/model.js:453-477` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1242-1247`.
...
- User error protection: gap, narrowed by BL-034. Deleting a list is confirmed in the page rather
  than by a native `confirm()`, but it still cannot be undone afterwards, while restoring a backup
  does have an undo. Evidence:
  `src/js/main.js:950-962` (delete, confirm only), `src/js/main.js:2101-2104` (undoRestore exists).
...
`src/js/lib/model.js:376-402` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2212-2225`, `src/js/main.js:476-503` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2114-2127`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:376-402`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:1882-1902` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1829` and `src/js/main.js:1859-1864`. What is missing is data, in
