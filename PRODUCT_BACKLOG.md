...
Twenty items have since been delivered and are marked `Shipped` in the table below: BL-026, BL-027,
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2056-2086` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:366-368`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:359-365`, search-as-you-type at `src/js/main.js:2114-2129` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2060` and `src/js/main.js:2069-2074` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2092-2112` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2039` and `src/js/main.js:2069-2074`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1760-1813`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1821-1890` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1722-1740`, manual issue add at `src/js/main.js:1892-1936` | Done |
...
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:2276-2294`, and the view states it counts across every list at `src/index.html:346-348` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1455-1465` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1420-1426`, pending filter at `src/js/main.js:1459` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1448-1453` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1915` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:480-504` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2339-2345` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now refuses only text entry at `src/js/main.js:1475-1495` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2455-2468`.
...
| BL-026 | Make every action reachable and repeatable from the keyboard | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P0 | Measured | Shipped | src/js/lib/shortcuts.js:26-58 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:2276-2294 |
...
| BL-053 | Make the reading filters one list rather than two that must agree | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 2 | 2.0 | none | Observed | Ready | src/js/main.js:1455-1465 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2455-2467 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2455-2467 |
...
`src/js/lib/shortcuts.js:26-58`: does the control consume typed characters, and would the browser
...
The About view carries the shortcut reference at `src/index.html:550-557`, naming all three
bindings including `Ctrl` + `\` for the sidebar, which was previously advertised nowhere at all.
The hero's two `kbd` hints stay, because they are the discoverable spot for a first-time reader,
but About is now the maintained list.

Sixty-five assertions across two harnesses. Fourteen unit tests make 45 of them about the
predicate alone, and eighteen mutations of it are all caught, including one that reinstates the
original blanket guard. Twenty browser checks cover the behaviour the unit tests cannot reach;
eight of them fail against the pre-change tree, which is what makes them evidence rather than
decoration. The other twelve are regression guards that pass either way, the tab-ring walk among
them: 142 stops, every one with a visible focus ring, focus order strictly following document
order, no stop visited twice and the reverse walk escaping cleanly. That walk is deliberately not
offered as proof of the fix, since the ring was never the defect.
...
`src/js/main.js:1416-1426`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2194`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:2251`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2219-2230`, leaving nothing behind and the offer still
...
`src/js/main.js:2386`, `src/js/main.js:2332` and `src/js/main.js:2339-2345`. Each replaces the whole
...
the code says so: `matchesFilter` at `src/js/main.js:1455-1465` still enumerates the five and
...
The settings form keeps its own check and its own message at `src/js/main.js:2349-2350`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1416-1426`, drawing on the labels at
`src/js/main.js:1448-1453`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1439-1444` shows it plainly: at
...
honoured only when a radio offers it. `matchesFilter` at `src/js/main.js:1455-1465` still
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2276-2294` (renderProgress calls seriesProgress on the whole state, with
  no list filter), `src/index.html:346-348` (the view's own subtitle states it counts across every
  list).
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2039` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2455-2468` (renderAll rebuilds
every region), `src/js/main.js:62-67` (store.onChange is wired straight to renderAll),
`src/js/main.js:1360-1446` (renderRows builds every row with no virtualisation and no early exit when
...
`src/js/lib/model.js:480-504` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1448-1453`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1168-1195` (undo), `src/js/main.js:2339-2345` (undoRestore exists).
...
`src/js/lib/model.js:403-429` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2455-2468`, `src/js/main.js:559-587` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2347-2367`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:403-429`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2092-2112` (attribution rendered in the UI before import). |
...
Only three of the 28 original stories are still open work and therefore carry both a label and a
score that can disagree: BL-007, BL-014 and BL-017. Four more keep a label and a score but can no
longer disagree, because BL-026 and BL-027 have shipped and BL-025 and BL-028 were dropped. The
...
open when the pass ran, so the table was 28 rows then. BL-028 has since been parked and three
further items filed, none of them labelled, which is how it reaches 30 rows now. The ranks below are
...
- Calculated: WSJF 3.67, rank 12 of 30.
...
### Case 2: BL-007 is labelled P1 but ranks twenty-sixth

- Stated: P1 Core product value, event order variants.
- Calculated: WSJF 1.4, rank 26 of 30, below twenty-two unlabelled items and three places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2039` and `src/js/main.js:2069-2074`. What is missing is data, in
...
- Consequence for the ranking: with BL-028 removed the table is 30 rows, and the highest Cost of
...
- BL-014, P1, rank 17 of 30. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 13 of 30. Mid-table.
- BL-017, P2, rank 29 of 30. The lowest-ranked scored story other than the one that cannot be
