...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1200-1230` | Done |
| 1.2 | BL-002 | P0 | category fieldset at `src/index.html:214-216`, filterByCategory imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:207-213`, search-as-you-type at `src/js/main.js:1258-1266` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1204` and `src/js/main.js:1213-1218` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:1236-1256` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1183` and `src/js/main.js:1200-1205`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:913-966`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:974-1020` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:883-901`, manual issue add at `src/js/main.js:1055-1075` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:343-348`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:139` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:290-315` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:1388-1406`, and the view states it counts across every list at `src/index.html:194-196` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:180-184`, applied without touching stored order at `src/js/main.js:617` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:581-584`, pending filter at `src/js/main.js:617` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:12-18`, hedged short labels at `src/js/main.js:606-611` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1061` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:1448-1451` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:634-650` | Partial |
| 7.2 | BL-027 | P1 | announcements exist at `src/js/main.js:138-143`, but they fire into two live regions at once and the first-run heading is empty per `docs/ux-artifacts/pa11y-landing.json` | Partial |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:1551-1566`.
...
| BL-044 | Send a content security policy and frame options from the dev server | Enabler | EP-12 | Leaves alone | 2 | 1 | 3 | 1 | 6.0 | none | Observed | Shipped | server.mjs:84-90 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:1388-1406 |
| BL-034 | Replace the native dialogs with the app's own notice system | Debt | EP-11 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Observed | Ready | src/js/main.js:328 |
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Ready | src/js/main.js:49 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:1551-1566 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:1551-1566 |
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:1388-1406` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1183` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:1551-1566` (renderAll rebuilds
every region), `src/js/main.js:41-46` (store.onChange is wired straight to renderAll),
`src/js/main.js:525-604` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:606-611`.
...
- User error protection: gap. Deleting a list is guarded only by a native `confirm()` and cannot be
  ...
  `src/js/main.js:334-341` (delete, confirm only), `src/js/main.js:1448-1451` (undoRestore exists).
...
- Self-descriptiveness: gap. The full availability description is carried only in a `title`
  attribute, which never reaches a keyboard or touch user. Evidence: `src/js/main.js:579`.
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:1551-1566`, `src/js/main.js:266-286` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:1453-1470`, `src/js/lib/cachePolicy.js:16-20`,
...
| Error handling and recovery | Gap. Error surfaces are inconsistent: curated import reports failure through native `alert()`, while every other path uses the in-page notice system. Evidence: `src/js/main.js:1345`, `src/js/main.js:1356`, `src/js/main.js:1373` against `src/js/main.js:138-143`. |
| Offline behavior | No gap, and no proposal. Probed as required rather than treated as a caching problem. With the local server running and no internet, the app starts, reads saved state, imports any bundled curated list and marks issues read, because those paths touch only same-origin files. Only cover images, metadata hydration and search degrade, and hydration failure is already surfaced as a pending state rather than as silence. Evidence: `src/data/house_of_m.json`, `src/js/main.js:581-584` (pending and by-hand badges), `absent: serviceWorker|navigator.onLine|manifest.json, case-insensitive grep across src/`. Repository Constraint 1 forbids caching cover bytes, so no cover-caching improvement is proposed. |
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:434-458`, `src/js/main.js:1448-1451`. |
...
| Observability | Partial gap, bounded by Repository Constraint 3. Product analytics are forbidden and are not proposed. What is missing is local and private: there is no way for the reader to see why hydration stalled beyond a queue-depth pill. Evidence: `src/js/main.js:1536-1538`. |
| Performance | Gap, measured. See characteristic 2. Evidence: `docs/ux-artifacts/render-cost.json`. |
| Security, OWASP Top 10 | Gap under A05 Security Misconfiguration: no CSP and no `x-frame-options` on the dev server. Evidence: `server.mjs:84-90`. Partial gap under A10 Server-Side Request Forgery by analogy: `MarvelApi` accepts any base URL and only strips trailing slashes, with the https-or-local check living in the settings form rather than in the client. Evidence: `src/js/api.js:18-26` against `src/js/main.js:1453-1463`. A01, A02, A03, A07 and A09 are not applicable, because there is no server-side authorisation boundary, no credential store, no server-side query language, no account system and no central log to protect. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:1236-1256` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1183` and `src/js/main.js:1200-1205`. What is missing is data, in
