...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3734-3795` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3856-3871` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3758` and `src/js/main.js:3776-3793` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3834-3854` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:3235-3291`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3299-3368` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:3197-3215`, manual issue add at `src/js/main.js:3484-3604` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:2120-2140`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:433` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1248-1288` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:4038-4072`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:4052-4054` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2616` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:309-344` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2712-2713`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2768-2773` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3561` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:4267-4273` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2799-2822` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:344-357` and `src/js/main.js:495-516` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4617-4637`. The work shipped since has
  taken it further: the file is 4,728 lines today. 1,566 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2557-2566 |
...
| BL-153 | Stop collapsing the sidebar from blanking every pill in the catalog | Defect | EP-08 | Follows BL-152 | 3 | 2 | 2 | 1 | 7.0 | none | Measured | Shipped | `src/styles.css:347-348` |
...
| BL-087 | State the network privacy boundary where the promise appears | Debt | EP-05 | Leaves alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/index.html:532 |
...
| BL-055 | Record the drift in the audited figures instead of letting them go stale | Debt | EP-12 | Leaves alone | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:193-205 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4590-4593 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:2231-2260 |
| BL-047 | Split the two meanings of the row class | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Observed | Shipped | src/styles.css:693-709 |
| BL-049 | Decide whether the faint badge borders need to meet the 3:1 non-text minimum | Defect | EP-08 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Measured | Shipped | src/styles.css:661 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4537-4600 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3995 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:495-516 |
| BL-031 | Put a scrim behind hero text so its contrast stops depending on the cover | Defect | EP-08 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | none | Measured | Shipped | src/index.html:309-344 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:1016 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:262 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1416 |
...
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:653 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:113 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3748-3753 |
...
| BL-067 | Gate the switch and the primary button, which no pair measures | Debt | EP-08 | Leaves alone | 2 | 2 | 2 | 3 | 2.0 | none | Measured | Shipped | src/styles.css:410 |
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3813-3828` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4617-4631 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4617-4631 |
...
that function is at `src/js/main.js:3638` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3782`, and with the selector corrected the
...
`src/index.html:385-389`, matching the reading filter's fieldset pattern rather than inventing a
second idiom, and the subtitle at `src/index.html:383` is now written by the render at
`src/js/main.js:4052-4054` instead of asserting "every list" whatever is being counted.
...
announced. It now lands on the all-read heading at `src/index.html:347`, which is both what the
reader needs to hear and where the remaining actions are.

The About view carries the shortcut reference at `src/index.html:668-675`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:960` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:350-357` rather
...
`src/js/main.js:2708-2713`, along with the pending badge's. A `title` is unreachable by touch and
...
text, at `src/styles.css:21-29`. The `kbd` tint was removed at `src/styles.css:495` and the
...
strikethrough, at `src/styles.css:594-595`; the strikethrough is the non-colour indicator. The
only opacity left on a read row is the cover image at `src/styles.css:597`, which carries no
...
`.badge-expected` at 1.58:1, but that is unconditional design at `src/styles.css:661` rather than
...
top stop was raised from 60 to 88 percent alpha at `src/styles.css:437-444`. The spread collapsed
...
`src/index.html:802`, so resolving the theme in JavaScript would paint dark and then flip, and
...
graphic is `aria-hidden="true"` at `src/index.html:269`, the state is carried in words that
`src/js/main.js:741` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:757` announces,
...
and not `--track` (`src/styles.css:405`), where `--track` is the unfilled part of a progress bar and
...
Reuse is keyed on the whole item, at `src/js/main.js:2491-2506`, rather than on a list of the
...
`src/js/main.js:2481-2489`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:381` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:390-430` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:362`, and both success paths clear that
...
`src/js/main.js:3998`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:2198-2201` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:2221-2229` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3998`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3967-3978`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:495`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:518-526` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:2244-2254`. The usual reason
...
`src/js/main.js:4327`, `src/js/main.js:4251` and `src/js/main.js:4267-4273`. Each replaces the whole
...
`src/js/main.js:398-411`. Moving a message into the modal makes it readable where the reader is
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:82-87`. Syncing anywhere else would have left the address naming
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1957-2011`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:4079-4097`.
...
synthetic id from the clock, per `src/js/main.js:3495`, so the two kinds cannot be ordered against
...
rather than naming them, and the sections in `src/index.html:398-410` are empty shells whose heading,
...
`src/js/main.js:533-565` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:605-634` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:4278-4279`, unchanged.
...
Shipped. The form row is now `.field-row` at `src/styles.css:693-709` and the reading row keeps
`.row`, so the thirteen rules that describe a reading row, from `src/styles.css:582-585` down to the
...
reached every one of them; the reading list at `src/index.html:383` sits inside neither, so it never
...
two at `src/index.html:538` and `src/index.html:545` are direct children of a `.card card-static`,
...
`.card-static > *` at `src/styles.css:686`, which the old rule's 0,2,0 had been masking. Measured
...
unconditional design at `src/styles.css:661`, where the border is 25 percent alpha green.
...
is written out in words inside the pill at `src/js/main.js:2708-2713`, drawing on the labels at
`src/js/main.js:2768-2773`, and the full sentence from `describe` sits in a visually-hidden span
...
The reasoning is recorded at `src/styles.css:635-659`, directly above the rules it governs, along
...
progress-ring transition at `src/styles.css:397` and the preference queries at
`src/styles.css:973-980`.
...
`src/js/main.js:1974-1977`, and `renderRows` asks it at `src/js/main.js:2616`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1969-1973` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:262` reads the pair back out of
...
`src/js/main.js:2456` and the reading rows at `src/js/main.js:2759`, and the reasoning is recorded at
`src/js/main.js:217-221` rather than only
...
while proving nothing. The full order lives inside a `<details>` at `src/index.html:361` that is
...
`src/js/main.js:1794` carries a comment at `src/js/main.js:1804` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3819`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1515` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:4015` puts
...
`renderRail` at `src/js/main.js:1248` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:1214` and reaches `renderYours` through `renderHome` at
`src/js/main.js:1216`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4617` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1256`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1410` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2801`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1426`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1272` and `src/js/main.js:1749`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4650`, and `src/js/app.js` is the entry
...
`src/styles.css:680` sets `content` to a right-pointing angle glyph on the card summary marker. It
...
`content` in the stylesheets is the one at `src/styles.css:680`. Closing it would need a tokenizer
...
built at `src/js/main.js:1272` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1749`, joined an issue count to an invitation to open the
...
at `src/js/main.js:3246`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1696`. That also retired a latent bug rather than fixing it. The old label said
...
comment, when it comes from the series name at `src/js/main.js:695-696`. Both are re-aimed, and the
...
`scripts/check-palette.mjs:100`, was still correct, which is what made this an item rather than a
...
`src/styles.css:482-486` is already the primary treatment, a red fill with `--on-accent` text, and
`.btn-g` at `src/styles.css:491` is the secondary ghost. `.btn-p` is a modifier that says "primary"
...
destructive one: `src/js/main.js:164-176` confirms Start fresh replaces the unreadable data with an
...
confirm dialog at `src/index.html:796`. The convention there is the same shape as this: the action
...
that boot report, and the comment now standing in its place at `src/js/main.js:4699-4703` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:133-152`, and until BL-075 took its
...
file of 4,728 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
together at `src/js/main.js:75-88`, and behind it sit sixteen library modules, none of which holds
...
the client and leaves the limiter alone, at `src/js/main.js:4290-4292`, because the budget belongs to
...
the one change callback at `src/js/main.js:82-87`, so nothing left on screen said why saving was
paused.
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:992`. The choice
...
glyph that no one reads, since the button's label at `src/js/main.js:2692` reads "Mark X as unread"
...
`src/js/storage.js:495-571`, `src/js/main.js:4228-4256`.
...
proxy for it.
Evidence: `src/js/storage.js:426-450`, `src/js/main.js:95-106`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3748-3753`,
...
attack. Evidence: `src/js/main.js:4228-4256`, `src/js/lib/model.js:968-1077`.
...
`src/js/main.js:1416` and again on the rail, because neither render is bounded by anything. That is a
...
matched the functionâ€™s own definition eight hundred lines above it. Both passed with the guard
...
`src/js/lib/model.js:226-241`, `server.mjs:53-59`, `src/index.html:555-560`.
...
remain local or absent. Evidence: `README.md:108-127`, `src/index.html:529-597`,
...
`src/js/main.js:2354` builds the reading hero's backdrop URL only when covers are on, and a computed
...
at once. The About view's "Metadata and links only" card, at `src/index.html:642-646`, is the sixth,
...
requests, from `src/js/main.js:3200` and `src/js/main.js:3216`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3309` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:4537-4600`.
...
`src/js/main.js:495-505`, which routes to the announcer precisely when the container is not itself
...
rendered text at `src/js/main.js:325-332`, because both surfaces are rewritten far more often than
they change: `checkHealth` runs at boot and again on every API URL save, and `renderHydration` is
called once per issue fetched. Seeding the health key as reachable at `src/js/main.js:338-340` is
...
The message policy is a pure function at `src/js/main.js:2855-2864` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:3185` and then starts
the hydrator at `src/js/main.js:3192`, whose `start()` reports its running phase at
...
`src/js/main.js:280-295`, because two messages produced by one action are one thing to say. Four of
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1982-1983`
...
stylesheet's own. `src/styles.css:666` puts the per-row actions out of reach with exactly
...
button at `src/js/main.js:4316-4353` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4323-4352` repaints the banner
...
it on. Evidence: `src/js/main.js:734-735`, `src/js/main.js:731`, `src/styles.css:453`.
...
over, for the hero backdrop alone: `src/js/main.js:2354` reads the setting before building the URL,
and `src/styles.css:453` sets `background-image: none` under the same class, which does suppress a
...
`src/js/main.js:2505`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3957-3963`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3989-3995`,
...
here. Evidence: `src/js/main.js:4004`.
...
in place of that button while a copy is live at `src/js/main.js:4440-4446`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1416`
...
**Delivered.** The first citation now names `src/js/main.js:4327`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4316-4353`. The other two were already right and
are unchanged: `src/js/main.js:4251` is the call in the successful branch of the restore handler,
and `src/js/main.js:4267-4273` is the whole undo-restore handler, whose call is at
`src/js/main.js:4272`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:2198-2201` is `forgetDeleted` itself, `src/js/main.js:2221-2229` is
`forgetDeletedFor`, `src/js/main.js:3998` is its one call, `src/js/main.js:3967-3978` is the
half-import rollback, `src/js/main.js:495` is `notify` with its `action` parameter,
`src/js/main.js:518-526` is `spoken`, `src/js/main.js:2244-2254` is the failed undo keeping its
buffer, `src/js/main.js:398-411` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:2224`, `src/js/main.js:2240` and `src/js/main.js:2257`. The sentence is right to
...
where the status key at `PRODUCT_BACKLOG.md:229` defines an item still to be picked up as `Ready`.
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4590-4593`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4572-4586` keeps
...
consumer at `src/js/main.js:2373`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2557-2566` returns a sentence saying no synopsis is
...
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:647` would have recorded
...
The disclaimer at `src/js/main.js:2904` is shown every time a run starts rather than once and
...
The one-time purge at `src/js/main.js:653` empties the response cache once, so the prose cached
...
The line reads `Start here · The Modern Avengers · Step 1 of 10 · Next: Avengers Disassembled` on the
first stop and `Step 3 of 10 · The Modern Avengers · Next: Civil War` in the middle. Two drafts were
...
`scripts/check-palette.mjs:59` and measures 7.10:1 in the dark theme and 6.10:1 in the light one
...
switch is in the settings screen at `src/index.html:565-572`, and the manual button is in the About
screen at `src/index.html:717-720`. `src/js/main.js:760-776` applies and saves the switch, while
`src/js/main.js:862-899` lets the About button force a check even when the automatic one is off.
...
The notice copy is the important part of the feature, not decoration. `src/js/main.js:782-809` names
...
delete the old folder once the new version opens. `src/js/main.js:839-850` starts the automatic
check without awaiting it, and `src/js/main.js:4650-4671` shows that the boot path continues into the
...
Two smaller things go with it. The required text is literally "Data provided by Marvel. © 2014
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:4038-4072` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3783` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4617-4637` (renderAll rebuilds
every region), `src/js/main.js:82-87` (store.onChange is wired straight to renderAll),
`src/js/main.js:2588-2766` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2768-2773`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:2231-2260` (undo), `src/js/main.js:4267-4273` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4617-4637`, `src/js/main.js:1199-1216` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,728 lines now, so nearly every item shipped
...
- Minor analysability gap: the `.row` class carries two unrelated meanings, a reading row and a form
  row, and a leftover empty rule sits between them. Evidence: `src/styles.css:582-585`,
  `src/styles.css:693-709`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4275-4308`, `src/js/lib/cachePolicy.js:16-20`,
...
| Category | Verdict |
|----------|---------|
| First-run experience | Gap. The first-run DOM ships an empty `<h2 id="hero-title">`, so the first heading a screen reader meets on an unseeded install is blank. Evidence: `src/index.html:309-344`, `docs/ux-artifacts/pa11y-landing.json`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3834-3854` (attribution rendered in the UI before import). |
...
the status key at `PRODUCT_BACKLOG.md:229` means by an item still to be picked up. Measured over
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3783` and `src/js/main.js:3776-3793`. What is missing is data, in
