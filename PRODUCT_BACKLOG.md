...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2693-2727` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2755-2770` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2701` and `src/js/main.js:2710-2715` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2733-2753` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2394-2450`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2458-2527` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2356-2374`, manual issue add at `src/js/main.js:2529-2573` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2932-2966`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:2946-2948` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1915` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2011-2017`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2072-2077` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2552` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3076-3082` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2103-2126` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3343-3363`. The work shipped since has
  taken it to 3,449; 1,566 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Ready | src/js/main.js:3112-3125 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Ready | src/js/main.js:3046-3074 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Ready | src/js/main.js:3282-3330 |
...
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Ready | src/js/main.js:2856 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2932-2966 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3343-3357 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3343-3357 |
...
that function is at `src/js/main.js:2607` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2705`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2946-2948` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:2007-2017`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2892`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:2892`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2860-2871`, leaving nothing behind and the offer still
...
`src/js/main.js:3060`, `src/js/main.js:3060` and `src/js/main.js:3076-3082`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2973-2991`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2539`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3086-3087`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2007-2017`, drawing on the labels at
`src/js/main.js:2072-2077`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1388-1391`, and `renderRows` asks it at `src/js/main.js:1915`. "In the markup but
...
`src/js/main.js:1841` and the reading rows at `src/js/main.js:2063`, and the reasoning is recorded at
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2839`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2909` puts
...
`src/js/main.js:3343` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2105`,
...
sequence moved into an exported `boot()` at `src/js/main.js:3376`, and `src/js/app.js` is the entry
...
at `src/js/main.js:2405`. A reader who imports one of this repository's own checklists therefore saw
...
that boot report, and the comment now standing in its place at `src/js/main.js:3421-3425` records
...
file of 3,449 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3099-3101`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:1991` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:6778-6780` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:6797-6801`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:6612-6614` describes
...
`src/js/storage.js:341-408`, `src/js/main.js:3046-3074`.
...
`src/js/main.js:3046-3074`, `src/js/lib/model.js:612-699`.
...
requests, from `src/js/main.js:2359` and `src/js/main.js:2375`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2468` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3282-3330`.
...
button at `src/js/main.js:3112-3125` and the escape hatch out of the blocked state at
...
button. Evidence: `src/js/main.js:2856`, `src/js/lib/model.js:88`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:2882-2889`,
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2932-2966` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2676` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3343-3363` (renderAll rebuilds
every region), `src/js/main.js:73-78` (store.onChange is wired straight to renderAll),
`src/js/main.js:1887-2070` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2072-2077`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1620-1647` (undo), `src/js/main.js:3076-3082` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3343-3363`, `src/js/main.js:812-829` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,449 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3084-3104`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2733-2753` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2676` and `src/js/main.js:2710-2715`. What is missing is data, in
