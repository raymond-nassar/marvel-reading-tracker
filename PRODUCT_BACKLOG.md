...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3422-3478` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3506-3521` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3445` and `src/js/main.js:3463-3476` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3484-3504` | Done |
...
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2933-2951`, manual issue add at `src/js/main.js:3210-3315` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3688-3722`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3702-3704` | Done, shipped as BL-014 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3272` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3915-3921` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4265-4285`. The work shipped since has
  taken it further: the file is 4,373 lines today. 1,566 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3964-4001 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3876-3904 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4238-4241 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4185-4248 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3645 |
...
| BL-150 | Fill in what the metadata snapshot cannot, from the one catalogue a browser can reach | Feature | EP-03 | Follows BL-149 | 5 | 5 | 5 | 5 | 3.0 | none | Measured | Shipped | `src/js/lib/wikitext.js:38-42`, `src/js/main.js:3234-3247`, `absent: any record newer than 2025-10-29 upstream, measured by asking the metadata service for 2026 and being told the total is zero` |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3688-3722 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3436-3440 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4265-4279 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4265-4279 |
...
that function is at `src/js/main.js:3349` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3469`, and with the selector corrected the
...
`src/js/main.js:3702-3704` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3648`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3648`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3617-3628`, leaving nothing behind and the offer still
...
`src/js/main.js:3975`, `src/js/main.js:3899` and `src/js/main.js:3915-3921`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3729-3747`.
...
synthetic id from the clock, per `src/js/main.js:3251`, so the two kinds cannot be ordered against
...
The settings form keeps its own check and its own message at `src/js/main.js:3926-3927`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3590`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3665` puts
...
`src/js/main.js:4265` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4298`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4345-4349` records
...
file of 4,373 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3938-3940`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10903-10905` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10922-10926`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10737-10739` describes
...
`CHANGELOG.md:2715-2724` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3876-3904`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3436-3440`,
...
attack. Evidence: `src/js/main.js:3876-3904`, `src/js/lib/model.js:968-1077`.
...
`src/index.html:125-126`, `src/js/main.js:4185-4248`.
...
button at `src/js/main.js:3964-4001` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3971-4000` repaints the banner
...
button. Evidence: `src/js/main.js:3607-3613`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3639-3645`,
...
here. Evidence: `src/js/main.js:3654`.
...
in place of that button while a copy is live at `src/js/main.js:4088-4094`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:3975`, `src/js/main.js:3899` and `src/js/main.js:3920`
...
**Delivered.** The first citation now names `src/js/main.js:3975`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3964-4001`. The other two were already right and
are unchanged: `src/js/main.js:3899` is the call in the successful branch of the restore handler,
and `src/js/main.js:3915-3921` is the whole undo-restore handler, whose call is at
`src/js/main.js:3920`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3648` is its one call, `src/js/main.js:3617-3628` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4238-4241`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4220-4234` keeps
...
comic they already track would sit in their reading list permanently. Only the wiki's id is
guarded. An id read from a pasted address names one issue the reader deliberately pointed at, so
merging into it is what they asked for; a wiki id comes from a fuzzy search this code ran, and a
search that returns the wrong page must never be able to rewrite something the tracker already
holds.
...
the unfixed tree. All seven CI checks pass. The browser check grew to 43 assertions across 7
scenarios from 35 across 6, and the new scenario drives real Edge through a lookup, a pick, an add
and a refused duplicate with the wiki stubbed. An eleventh injected fault was added and aimed at
it, taking `npm run browser:prove` to 11 of 11 caught. The first version of that fault hooked the
write and turned nothing red, which is the harness reporting honestly: a refused add performs no
write, so there is nothing there to intercept. What it injects instead is the hazard's own
signature, the stored entry carrying the second answer's date, page count and writer at the moment
the reader is told nothing was added, and that reddens exactly one assertion and no other scenario.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3688-3722` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3470` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4265-4285` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1969-1996` (undo), `src/js/main.js:3915-3921` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4265-4285`, `src/js/main.js:966-983` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,373 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3923-3956`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1046-1077`, `src/js/main.js:3915-3921`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3484-3504` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3470` and `src/js/main.js:3463-3476`. What is missing is data, in
