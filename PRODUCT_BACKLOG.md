...
A hundred and six items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-129, BL-130, BL-131, BL-132, BL-133, BL-134, BL-135 and BL-136.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2871-2905` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:415-417`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:408-414`, search-as-you-type at `src/js/main.js:2933-2948` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2879` and `src/js/main.js:2888-2893` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2911-2931` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2572-2628`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2636-2705` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2534-2552`, manual issue add at `src/js/main.js:2707-2751` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1623-1643`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:424` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:942-982` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3115-3149`, with a scope control at `src/index.html:375-379` and a subtitle that stops claiming every list at `src/js/main.js:3129-3131` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2082` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:299-334` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:666-695`, quoted into the Markdown export at `src/js/lib/markdown.js:180-182` | Done, shipped as BL-017 |
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2178-2179`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2234-2239` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2730` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:988-1019` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3342-3348` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2265-2288` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:329-342` and `src/js/main.js:431-452` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:3679-3699`. The work shipped since has
  taken it further: the file is 3,785 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 957; 224 is the figure as audited.
...
| BL-135 | Give the tracker a file a non-engineer can double-click to start it | Story | EP-10 | Extends BL-134 | 5 | 2 | 3 | 1 | 10.0 | none | Observed | Shipped | absent: any way to start the app that is not a terminal command, read of the README's step 2 and a listing of the repository root |
...
| BL-087 | State the network privacy boundary where the promise appears | Debt | EP-05 | Leaves alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/index.html:507 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3378-3415 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3303-3331 |
...
| BL-136 | Let the installed window open when the server is not running | Story | EP-10 | Extends BL-134 | 5 | 2 | 3 | 2 | 5.0 | none | Measured | Shipped | absent: any service worker, listing of src/ and a launch of the installed address in Edge with the server stopped |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1384-1385 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3652-3655 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1709-1736 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:938 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3599-3662 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3072 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:431-452 |
| BL-031 | Put a scrim behind hero text so its contrast stops depending on the cover | Defect | EP-08 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | none | Measured | Shipped | src/index.html:299-334 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/lib/model.js:217-232 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/main.js:563-567 |
...
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Shipped | src/js/lib/model.js:206 |
| BL-045 | Move the API base URL check into the client that uses it | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Observed | Shipped | src/js/api.js:25-40 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3115-3149 |
...
| BL-134 | Let the browser install the tracker, so it has a window and an icon of its own | Story | EP-10 | Leaves alone | 3 | 1 | 2 | 2 | 3.0 | none | Observed | Shipped | absent: any web app manifest or app icon, read of the head of src/index.html and a listing of src/ |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3679-3693 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:675-683 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3679-3693 |
...
that function is at `src/js/main.js:2785` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2883`, and with the selector corrected the same mutation fails it. It is
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:714-725`; omitting it
...
`src/index.html:375-379`, matching the reading filter's fieldset pattern rather than inventing a
second idiom, and the subtitle at `src/index.html:373` is now written by the render at
`src/js/main.js:3129-3131` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:480` and load normalises
a stale id at `src/js/lib/model.js:983`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:905-906`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:675-683`, for the same reason
...
announced. It now lands on the all-read heading at `src/index.html:337`, which is both what the
reader needs to hear and where the remaining actions are.

The About view carries the shortcut reference at `src/index.html:633-640`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:660` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:335-342` rather
...
`src/js/main.js:2174-2179`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/index.html:752`, so resolving the theme in JavaScript would paint dark and then flip, and
...
`src/js/main.js:582` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:598` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:1966-1981`, rather than on a list of the
...
`src/js/main.js:1956-1964`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:366` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:375-413` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:347`, and both success paths clear that
...
`src/js/main.js:3075`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:499-511` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:442-445`: "in library" and "Continue reading" would both resolve to
...
making sense: `forgetDeleted()` at `src/js/main.js:1676-1679` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1699-1707` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3075`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3044-3055`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:431`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:454-458` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1722-1730`. The usual reason
...
`src/js/main.js:3389`, `src/js/main.js:3326` and `src/js/main.js:3342-3348`. Each replaces the whole
...
`src/js/main.js:383-394`. Moving a message into the modal makes it readable where the reader is
...
the id is unknown, at `src/js/lib/model.js:513-515`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1460-1514`, before the
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:745-795`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3156-3174`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:481-482`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:2717`, so the two kinds cannot be
...
rather than naming them, and the sections in `src/index.html:388-400` are empty shells whose heading,
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:635-642`, written the same way
`coerce` writes it at `src/js/lib/model.js:919`, so the two paths cannot disagree and every future
caller inherits it.
...
Shipped. The check now runs in the `MarvelApi` constructor at `src/js/api.js:25-40`, which throws a
...
`src/js/main.js:465-489` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:490-503` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:3353-3354`, unchanged.
...
reached every one of them; the reading list at `src/index.html:373` sits inside neither, so it never
...
two at `src/index.html:513` and `src/index.html:520` are direct children of a `.card card-static`,
...
is written out in words inside the pill at `src/js/main.js:2174-2179`, drawing on the labels at
`src/js/main.js:2234-2239`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1477-1480`, and `renderRows` asks it at `src/js/main.js:2082`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1472-1476` exists because
...
`src/js/main.js:1931` and the reading rows at `src/js/main.js:2225`, and the reasoning is recorded at
...
while proving nothing. The full order lives inside a `<details>` at `src/index.html:351` that is
...
`src/js/main.js:1332` carries a comment at `src/js/main.js:1342` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3017`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1175` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3092` puts
...
`renderRail` at `src/js/main.js:942` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:914` and reaches `renderYours` through `renderHome` at
`src/js/main.js:916`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:3679` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:950`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1095` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2267`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1111`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:966` and `src/js/main.js:1292`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:3712`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:966` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1292`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2583`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1274`. That also retired a latent bug rather than fixing it. The old label said
...
`lists[k] = {...}` at `src/js/lib/model.js:938`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:966`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:983`.
...
comment, when it comes from the series name at `src/js/main.js:536-537`. Both are re-aimed, and the
...
confirm dialog at `src/index.html:746`. The convention there is the same shape as this: the action
...
that boot report, and the comment now standing in its place at `src/js/main.js:3757-3761` records
...
file of 3,785 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
limiter and cache when handed neither, at `src/js/api.js:41-42`, and the app always hands it both,
which is what keeps one request budget across the page. Saving a new API base replaces the cache and
the client and leaves the limiter alone, at `src/js/main.js:3365-3367`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2158` reads "Mark X as unread"
...
`src/js/storage.js:495-571`, `src/js/main.js:3303-3331`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:2874-2878`,
...
attack. Evidence: `src/js/main.js:3303-3331`, `src/js/lib/model.js:910-1019`.
...
`src/js/main.js:1101` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/lib/model.js:217-232`, `server.mjs:53-59`, `src/index.html:530-535`.
...
remain local or absent. Evidence: `README.md:51-70`, `src/index.html:504-562`,
`src/js/api.js:104-116`.
...
`src/js/main.js:1829` builds the reading hero's backdrop URL only when covers are on, and a computed
...
at once. The About view's "Metadata and links only" card, at `src/index.html:607-611`, is the sixth,
...
creator's issues pages the metadata API to completion at `src/js/api.js:161-175`, up to sixty
requests, from `src/js/main.js:2537` and `src/js/main.js:2553`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2646` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:3599-3662`.
...
`src/js/main.js:431-441`, which routes to the announcer precisely when the container is not itself
...
rendered text at `src/js/main.js:310-317`, because both surfaces are rewritten far more often than
they change: `checkHealth` runs at boot and again on every API URL save, and `renderHydration` is
called once per issue fetched. Seeding the health key as reachable at `src/js/main.js:323-325` is
...
The message policy is a pure function at `src/js/main.js:2321-2330` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2522` and then starts
the hydrator at `src/js/main.js:2529`, whose `start()` reports its running phase at
...
`src/js/main.js:265-280`, because two messages produced by one action are one thing to say. Four of
...
size, at `PRODUCT_BACKLOG.md:2477`, and one of the four tests added to the counts suite pins the
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1485-1486`
...
button at `src/js/main.js:3378-3415` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3385-3414` repaints the banner
...
it on. Evidence: `src/js/main.js:575-576`, `src/js/main.js:572`, `src/styles.css:436`.
...
over, for the hero backdrop alone: `src/js/main.js:1829` reads the setting before building the URL,
...
`src/js/main.js:1980`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3034-3040`, `src/js/lib/model.js:203`,
`src/js/lib/model.js:797`.
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3066-3072`,
...
here. Evidence: `src/js/main.js:3081`.
...
in place of that button while a copy is live at `src/js/main.js:3502-3508`, which is the state a
...
derivation in `src/js/lib/model.js:314-315` calls the most generous any browser grants. That figure
...
`forgetDeleted()`, at `src/js/main.js:3389`, `src/js/main.js:3326` and `src/js/main.js:3347`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1384-1385`.
...
**Delivered.** The first citation now names `src/js/main.js:3389`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3378-3415`. The other two were already right and
are unchanged: `src/js/main.js:3326` is the call in the successful branch of the restore handler,
and `src/js/main.js:3342-3348` is the whole undo-restore handler, whose call is at
`src/js/main.js:3347`. The range was left as the handler rather than narrowed to the call, because
...
the code they name, and the twelve besides this one are sound: `src/js/lib/model.js:499-511` is
`restoreList`, `src/js/lib/model.js:442-445` is the comment on a duplicate not inheriting the
catalog claim, `src/js/main.js:1676-1679` is `forgetDeleted` itself, `src/js/main.js:1699-1707` is
`forgetDeletedFor`, `src/js/main.js:3075` is its one call, `src/js/main.js:3044-3055` is the
half-import rollback, `src/js/main.js:431` is `notify` with its `action` parameter,
`src/js/main.js:454-458` is `spoken`, `src/js/main.js:1722-1730` is the failed undo keeping its
buffer, `src/js/main.js:383-394` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:1702`, `src/js/main.js:1718` and `src/js/main.js:1733`. The sentence is right to
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3652-3655`.
...
`get()` at `src/js/api.js:54`, so the four backoffs a request can produce are one request's worth,
...
So the client now reports that the service answered. `src/js/api.js:78` sits past the branch that
...
A cache hit reports nothing either, since it returns at `src/js/api.js:55-58` without asking anyone.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3634-3648` keeps
...
`fetch` and `sleep` injected into `MarvelApi` at `src/js/api.js:25-27`, because a four-attempt
...
consumer at `src/js/main.js:1848`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2027-2032` returns a sentence saying no synopsis is
...
**BL-134: Let the browser install the tracker, so it has a window and an icon of its own**
...
BL-134 gave the tracker an icon, and an icon is a promise that clicking it opens something. What it
...
BL-134's own record said the install "does not start the app for you", and that an installed window
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3115-3149` (renderProgress, which now takes a scope), `src/index.html:373`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2854` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3679-3699` (renderAll rebuilds
every region), `src/js/main.js:74-79` (store.onChange is wired straight to renderAll),
`src/js/main.js:2054-2232` (renderRows, which then built every row with no virtualisation and no
...
`src/js/lib/model.js:988-1019` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2234-2239`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1709-1736` (undo), `src/js/main.js:3342-3348` (undoRestore exists).
...
`src/js/lib/model.js:830-906` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:3679-3699`, `src/js/main.js:899-916` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,785 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3350-3370`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:830-906`.
...
| Category | Verdict |
|----------|---------|
| First-run experience | Gap. The first-run DOM ships an empty `<h2 id="hero-title">`, so the first heading a screen reader meets on an unseeded install is blank. Evidence: `src/index.html:299-334`, `docs/ux-artifacts/pa11y-landing.json`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2911-2931` (attribution rendered in the UI before import). |
...
- Calculated: WSJF 3.67, rank 55 of 112.
...
- Calculated: WSJF 1.4, rank 106 of 112, below a hundred unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2854` and `src/js/main.js:2888-2893`. What is missing is data, in
...
- BL-014, P1, rank 68 of 112. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 57 of 112. Mid-table.
- BL-017, P2, rank 111 of 112. The lowest-ranked scored story other than the one that cannot be
