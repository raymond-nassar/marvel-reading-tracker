...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:1938-1968` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:366-368`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:359-365`, search-as-you-type at `src/js/main.js:1996-2011` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:1942` and `src/js/main.js:1951-1956` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:1974-1994` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:1921` and `src/js/main.js:1951-1956`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1642-1695`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1703-1772` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1604-1622`, manual issue add at `src/js/main.js:1774-1818` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1011-1031`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:551-576` | Done |
| 4.2 | BL-014 | P1 | seriesProgress is called on the whole state with no list filter at `src/js/main.js:2147-2165`, and the view states it counts across every list at `src/index.html:346-348` | Partial |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/index.html:332-336`, applied without touching stored order at `src/js/main.js:1339-1349` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1304-1310`, pending filter at `src/js/main.js:1343` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1332-1337` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:1797` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:477-501` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2210-2213` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, but the shortcut handler bails on any focused control at `src/js/main.js:1359-1377` | Partial |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:150-163` and `src/js/main.js:243-264` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2316-2329`.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1069-1085 |
...
| BL-026 | Make every action reachable and repeatable from the keyboard | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P0 | Measured | Ready | src/js/main.js:1359-1377 |
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:243-264 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Ready | src/js/main.js:2147-2165 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2316-2328 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2316-2328 |
...
`src/js/main.js:1300-1310`, along with the pending badge's. A `title` is unreachable by touch and
...
outstanding notice is kept as a record at `src/js/main.js:182` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:191-225` renders every outstanding record into
...
`src/js/main.js:2076`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:209-219` puts a list back at the index it held,
...
A list whose id is present again is not restored. The offer is held for the rest of the session, so
the buffer can still be live after the reader has imported the same curated order a second time,
and overwriting that fresh list with the stale copy would destroy work rather than recover it.
`restoreList` returns the state unchanged in that case, which is covered by a test.

Read progress is not a consideration in either direction. It is global and keyed by issue id, so
deleting a list never touched it and restoring one never has to put it back. Two of the four new
tests assert exactly that, in both directions, because the confirmation copy now promises it.
...
`notify` gained an `action` at `src/js/main.js:243`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:266-270` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1072-1080`. The usual reason
...
Erasing everything and restoring a backup both call `forgetDeleted()`, at `src/js/main.js:2247` and
`src/js/main.js:2203`. Each replaces the whole state, so the buffered list belongs to data that is
no longer there: putting it back would splice a list out of the old tracker into the restored one,
or resurrect one list out of a tracker the reader asked to be emptied.

Six scenarios are measured in Edge on top of the fourteen for the dialogs, covering the offer
surviving all six other views with its button still working, the announcement carrying the button,
erasing dropping the offer, a second delete replacing rather than stacking the first, and an
unrelated catalog failure and the offer coexisting without destroying each other.
...
is written out in words inside the pill at `src/js/main.js:1300-1310`, drawing on the labels at
`src/js/main.js:1332-1337`, and the full sentence from `describe` sits in a visually-hidden span
...
citations were stale the moment they were committed. `src/js/main.js:1323-1328` shows it plainly: at
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2147-2165` (renderProgress calls seriesProgress on the whole state, with
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:1921` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2316-2329` (renderAll rebuilds
every region), `src/js/main.js:61-66` (store.onChange is wired straight to renderAll),
`src/js/main.js:1244-1330` (renderRows builds every row with no virtualisation and no early exit when
...
`src/js/lib/model.js:477-501` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1332-1337`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1069-1085` (undo), `src/js/main.js:2210-2213` (undoRestore exists).
...
`src/js/lib/model.js:400-426` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2316-2329`, `src/js/main.js:519-547` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2215-2228`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:400-426`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:1974-1994` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:1921` and `src/js/main.js:1951-1956`. What is missing is data, in
