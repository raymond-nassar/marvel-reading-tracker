...
A hundred and sixteen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147, BL-149, BL-152 and BL-153.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3277-3338` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3399-3414` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3301` and `src/js/main.js:3319-3336` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3377-3397` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2961-3017`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3025-3094` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2923-2941`, manual issue add at `src/js/main.js:3106-3167` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1881-1901`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:432` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1009-1049` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3581-3615`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3595-3597` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2352` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:309-344` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:674-703`, quoted into the Markdown export at `src/js/lib/markdown.js:214-216` | Done, shipped as BL-017 |
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2448-2449`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2504-2509` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3137` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:1043-1074` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3808-3814` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2535-2558` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:336-349` and `src/js/main.js:438-459` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4158-4178`. The work shipped since has
  taken it further: the file is 4,266 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,141; 224 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2293-2302 |
...
| BL-087 | State the network privacy boundary where the promise appears | Debt | EP-05 | Leaves alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/index.html:523 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3857-3894 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3769-3797 |
| BL-135 | Refuse an issue synopsis at the boundary that writes, not only at the one that reads | Enabler | EP-05 | Extends BL-134 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Ready | src/js/lib/model.js:1117 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3944-3947 |
...
| BL-154 | Shelve the events apart from the character runs, and say why the runs sit last | Story | EP-01 | Follows BL-152 | 5 | 3 | 2 | 2 | 5.0 | none | Measured | Ready | `src/js/main.js:3263-3264` |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4131-4134 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1969-1996 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:993 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4078-4141 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3538 |
...
| BL-097 | Publish contribution, conduct, support and maintainer governance | Chore | EP-12 | Extends BL-052 | 3 | 3 | 5 | 3 | 3.67 | none | Observed | Shipped | README.md:381-448 |
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:438-459 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/lib/model.js:225-240 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/main.js:630-634 |
...
| BL-146 | Put a way out of the capped landing grid where the reader runs out of it | Defect | EP-10 | Follows BL-145 | 3 | 1 | 2 | 2 | 3.0 | none | Measured | Shipped | `src/index.html:247`, `src/js/main.js:1206-1213` |
...
| BL-150 | Fill in what the metadata snapshot cannot, from the one catalogue a browser can reach | Feature | EP-03 | Follows BL-149 | 5 | 5 | 5 | 5 | 3.0 | none | Measured | Ready | absent: any record newer than 2025-10-29 upstream, measured by asking the metadata service for 2026 and being told the total is zero |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3581-3615 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:254 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1177 |
...
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:561 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:107 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3291-3296 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3356-3371` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4158-4172 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:683-691 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4158-4172 |
...
that function is at `src/js/main.js:3201` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3325`, and with the selector corrected the
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:722-733`; omitting it
...
`src/js/main.js:3595-3597` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:488` and load normalises
a stale id at `src/js/lib/model.js:1038`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:960-961`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:683-691`, for the same reason
...
The About view carries the shortcut reference at `src/index.html:649-656`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:727` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:342-349` rather
...
`src/js/main.js:2444-2449`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/index.html:774`, so resolving the theme in JavaScript would paint dark and then flip, and
...
`src/js/main.js:649` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:665` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:2227-2242`, rather than on a list of the
...
`src/js/main.js:2217-2225`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:373` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:382-420` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:354`, and both success paths clear that
...
`src/js/main.js:3541`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:507-519` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:450-453`: "in library" and "Continue reading" would both resolve to
...
making sense: `forgetDeleted()` at `src/js/main.js:1936-1939` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1959-1967` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3541`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3510-3521`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:438`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:461-465` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1982-1990`. The usual reason
...
`src/js/main.js:3868`, `src/js/main.js:3792` and `src/js/main.js:3808-3814`. Each replaces the whole
...
`src/js/main.js:390-401`. Moving a message into the modal makes it readable where the reader is
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:76-81`. Syncing anywhere else would have left the address naming
...
the id is unknown, at `src/js/lib/model.js:521-523`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1718-1772`, before the
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:753-803`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3622-3640`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:489-490`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:3116`, so the two kinds cannot be
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:643-650`, written the same way
`coerce` writes it at `src/js/lib/model.js:974`, so the two paths cannot disagree and every future
caller inherits it.
...
`src/js/main.js:472-501` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:526-542` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:3819-3820`, unchanged.
...
two at `src/index.html:529` and `src/index.html:536` are direct children of a `.card card-static`,
...
is written out in words inside the pill at `src/js/main.js:2444-2449`, drawing on the labels at
`src/js/main.js:2504-2509`, and the full sentence from `describe` sits in a visually-hidden span
...
now three, at `README.md:668-679`, and the four sentences over 40 words are now none: the longest
...
`README.md:538-542`, as fetching a list once and committing what came back. `depth` gained a gloss
...
`src/js/main.js:1735-1738`, and `renderRows` asks it at `src/js/main.js:2352`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1730-1734` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:254` reads the pair back out of
...
`src/js/main.js:2192` and the reading rows at `src/js/main.js:2495`, and the reasoning is recorded at
`src/js/main.js:209-213` rather than only
...
`src/js/main.js:1555` carries a comment at `src/js/main.js:1565` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3483`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1276` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3558` puts
...
`renderRail` at `src/js/main.js:1009` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:981` and reaches `renderYours` through `renderHome` at
`src/js/main.js:983`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4158` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1017`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1171` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2537`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1187`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1033` and `src/js/main.js:1510`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4191`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:1033` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1510`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2972`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1457`. That also retired a latent bug rather than fixing it. The old label said
...
`lists[k] = {...}` at `src/js/lib/model.js:993`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:1021`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:1038`.
...
comment, when it comes from the series name at `src/js/main.js:603-604`. Both are re-aimed, and the
...
destructive one: `src/js/main.js:156-168` confirms Start fresh replaces the unreadable data with an
...
confirm dialog at `src/index.html:768`. The convention there is the same shape as this: the action
...
that boot report, and the comment now standing in its place at `src/js/main.js:4238-4242` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:125-144`, and until BL-075 took its
...
file of 4,266 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
together at `src/js/main.js:69-82`, and behind it sit sixteen library modules, none of which holds
...
the client and leaves the limiter alone, at `src/js/main.js:3831-3833`, because the budget belongs to
...
`src/js/storage.js:9-12`, two more belong to the view file at `src/js/main.js:40-41`, and the
...
the one change callback at `src/js/main.js:76-81`, so nothing left on screen said why saving was
paused.
...
glyph that no one reads, since the button's label at `src/js/main.js:2428` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11025-11027` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11044-11048`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10859-10861` describes
...
`CHANGELOG.md:2728-2737` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3769-3797`.
...
proxy for it.
Evidence: `src/js/storage.js:426-450`, `src/js/main.js:89-100`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3291-3296`,
...
attack. Evidence: `src/js/main.js:3769-3797`, `src/js/lib/model.js:965-1074`.
...
`src/js/main.js:1177` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/lib/model.js:225-240`, `server.mjs:53-59`, `src/index.html:546-551`.
...
remain local or absent. Evidence: `README.md:84-103`, `src/index.html:520-578`,
...
`src/js/main.js:2090` builds the reading hero's backdrop URL only when covers are on, and a computed
...
at once. The About view's "Metadata and links only" card, at `src/index.html:623-627`, is the sixth,
...
requests, from `src/js/main.js:2926` and `src/js/main.js:2942`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3035` is named alongside the search it resembles, and the
...
`test/ci-supply-chain.test.js:124-142`, `README.md:496-531`.
...
`src/index.html:125-126`, `src/js/main.js:4078-4141`.
...
`src/js/main.js:438-448`, which routes to the announcer precisely when the container is not itself
...
rendered text at `src/js/main.js:317-324`, because both surfaces are rewritten far more often than
they change: `checkHealth` runs at boot and again on every API URL save, and `renderHydration` is
called once per issue fetched. Seeding the health key as reachable at `src/js/main.js:330-332` is
...
The message policy is a pure function at `src/js/main.js:2591-2600` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2911` and then starts
the hydrator at `src/js/main.js:2918`, whose `start()` reports its running phase at
...
`src/js/main.js:272-287`, because two messages produced by one action are one thing to say. Four of
...
size, at `PRODUCT_BACKLOG.md:2532`, and one of the four tests added to the counts suite pins the
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1743-1744`
...
are valuable history and are not a concise public policy. Evidence: `README.md:381-734`,
...
button at `src/js/main.js:3857-3894` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3864-3893` repaints the banner
...
it on. Evidence: `src/js/main.js:642-643`, `src/js/main.js:639`, `src/styles.css:453`.
...
over, for the hero backdrop alone: `src/js/main.js:2090` reads the setting before building the URL,
...
`src/js/main.js:2241`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3500-3506`, `src/js/lib/model.js:211`,
`src/js/lib/model.js:805`.
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3532-3538`,
...
here. Evidence: `src/js/main.js:3547`.
...
in place of that button while a copy is live at `src/js/main.js:3981-3987`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1177`
...
derivation in `src/js/lib/model.js:322-323` calls the most generous any browser grants. That figure
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4767-4775`.
...
`forgetDeleted()`, at `src/js/main.js:3868`, `src/js/main.js:3792` and `src/js/main.js:3813`
...
**Delivered.** The first citation now names `src/js/main.js:3868`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3857-3894`. The other two were already right and
are unchanged: `src/js/main.js:3792` is the call in the successful branch of the restore handler,
and `src/js/main.js:3808-3814` is the whole undo-restore handler, whose call is at
`src/js/main.js:3813`. The range was left as the handler rather than narrowed to the call, because
...
the code they name, and the twelve besides this one are sound: `src/js/lib/model.js:507-519` is
`restoreList`, `src/js/lib/model.js:450-453` is the comment on a duplicate not inheriting the
catalog claim, `src/js/main.js:1936-1939` is `forgetDeleted` itself, `src/js/main.js:1959-1967` is
`forgetDeletedFor`, `src/js/main.js:3541` is its one call, `src/js/main.js:3510-3521` is the
half-import rollback, `src/js/main.js:438` is `notify` with its `action` parameter,
`src/js/main.js:461-465` is `spoken`, `src/js/main.js:1982-1990` is the failed undo keeping its
buffer, `src/js/main.js:390-401` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:1962`, `src/js/main.js:1978` and `src/js/main.js:1993`. The sentence is right to
...
at `PRODUCT_BACKLOG.md:4772-4773`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4767-4770` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4131-4134`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4113-4127` keeps
...
consumer at `src/js/main.js:2109`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2293-2302` returns a sentence saying no synopsis is
...
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:555` would have recorded
...
`src/js/lib/model.js:829` exposed a defect in the original: it sliced the first nine ids and then
...
The disclaimer at `src/js/main.js:2640` is shown every time a run starts rather than once and
...
adding code elsewhere. `exportBackup` copies the map verbatim at `src/js/lib/model.js:1117`, so it is
...
The one-time purge at `src/js/main.js:561` empties the response cache once, so the prose cached
...
this repository's own README says so at `README.md:155-157`, where the reader is already told their
...
than merely questioned. `README.md:156-157` already carries a workaround for the milder version of
...
decided on its own merits, not carried along by a metadata change.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3581-3615` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3326` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4158-4178` (renderAll rebuilds
every region), `src/js/main.js:76-81` (store.onChange is wired straight to renderAll),
`src/js/main.js:2324-2502` (renderRows, which then built every row with no virtualisation and no
...
`src/js/lib/model.js:1043-1074` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2504-2509`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1969-1996` (undo), `src/js/main.js:3808-3814` (undoRestore exists).
...
`src/js/lib/model.js:885-961` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4158-4178`, `src/js/main.js:966-983` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,266 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3816-3849`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:885-961`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3377-3397` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3326` and `src/js/main.js:3319-3336`. What is missing is data, in
