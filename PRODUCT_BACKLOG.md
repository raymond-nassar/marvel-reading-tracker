...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2485-2519` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2547-2562` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2493` and `src/js/main.js:2502-2507` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2525-2545` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2186-2242`, `src/js/lib/markdown.js:36-112` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2250-2319` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2148-2166`, manual issue add at `src/js/main.js:2321-2365` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1412-1432`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:179` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:783-823` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2724-2758`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2738-2740` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1719` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1806-1812`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1864-1869` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2344` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2855-2861` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1895-1918` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2971-2991`. The work shipped since has
  taken it to 3,065; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1498-1525 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2724-2758 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2971-2985 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2971-2985 |
...
that function is at `src/js/main.js:2399` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2497`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2738-2740` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1802-1812`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2684`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1465-1468` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1488-1496` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2684`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2652-2663`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1511-1519`. The usual reason
...
`src/js/main.js:2848`, `src/js/main.js:2848` and `src/js/main.js:2855-2861`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1286-1333`, before the
...
- **Arrowing through the filter group left one history entry per stop passed over.** Arrow keys move
  ...
  first stop of a traversal now pushes and the rest overwrite it, so a traversal leaves one entry
  whose Back returns to the filter in force before it began. A change no arrow key produced still
  pushes on its own, so two pointer choices still get an entry each, and leaving the group closes
  the traversal so a reader who returns to it later is not overwriting a minutes-old entry.
...
either lookup back, pushing on every stop again, letting any key open a traversal, and letting a
move between two radios close one. A browser check of the fixes passes 24 of 24, over the five
prototype members and eight history sequences, and each of the two was measured failing on this tree
first.
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2765-2783`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2331`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:2865-2866`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1802-1812`, drawing on the labels at
`src/js/main.js:1864-1869`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1303-1306`, and `renderRows` asks it at `src/js/main.js:1719`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1298-1302` exists because
...
`src/js/main.js:1897`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
`src/js/main.js:1158` carries a comment at `src/js/main.js:1168` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2631`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1011` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2701` puts
...
`renderRail` at `src/js/main.js:783` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:761` and reaches `renderYours` through `renderHome` at
`src/js/main.js:763`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:2971` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:791`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:934` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1897`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:947`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:807` and `src/js/main.js:1126`. Both predate this change and neither is part of its
...
built at `src/js/main.js:807` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1126`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2197`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1110`. That also retired a latent bug rather than fixing it. The old label said
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3484-3485` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3504-3506`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:3326-3328` describes
...
`CHANGELOG.md:407-410` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2724-2758` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2468` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2971-2991` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1708-1862` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1864-1869`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1498-1525` (undo), `src/js/main.js:2855-2861` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2971-2991`, `src/js/main.js:746-763` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,065 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2863-2883`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2525-2545` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2468` and `src/js/main.js:2502-2507`. What is missing is data, in
