...
A hundred and ten items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-129, BL-130, BL-131, BL-132, BL-133, BL-134, BL-139, BL-140, BL-141, BL-142, BL-143 and BL-144.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3215-3271` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:421-423`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:414-420`, search-as-you-type at `src/js/main.js:3299-3314` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3238` and `src/js/main.js:3256-3269` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3277-3297` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2929-2985`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2993-3062` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2891-2909`, manual issue add at `src/js/main.js:3064-3108` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1849-1869`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:432` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1008-1048` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3481-3515`, with a scope control at `src/index.html:381-385` and a subtitle that stops claiming every list at `src/js/main.js:3495-3497` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2320` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:305-340` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2416-2417`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2472-2477` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3087` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3708-3714` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2503-2526` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4058-4078`. The work shipped since has
  taken it further: the file is 4,166 lines today. 1,566 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2261-2270 |
...
| BL-087 | State the network privacy boundary where the promise appears | Debt | EP-05 | Leaves alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/index.html:513 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3757-3794 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3669-3697 |
...
| BL-055 | Record the drift in the audited figures instead of letting them go stale | Debt | EP-12 | Leaves alone | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:192-204 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4031-4034 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1937-1964 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3978-4041 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3438 |
...
| BL-097 | Publish contribution, conduct, support and maintainer governance | Chore | EP-12 | Extends BL-052 | 3 | 3 | 5 | 3 | 3.67 | none | Observed | Shipped | README.md:348-415 |
...
| BL-031 | Put a scrim behind hero text so its contrast stops depending on the cover | Defect | EP-08 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | none | Measured | Shipped | src/index.html:305-340 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3481-3515 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:974 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1167 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3229-3233 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4058-4072 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4058-4072 |
...
that function is at `src/js/main.js:3142` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3262`, and with the selector corrected the
...
`src/index.html:381-385`, matching the reading filter's fieldset pattern rather than inventing a
second idiom, and the subtitle at `src/index.html:379` is now written by the render at
`src/js/main.js:3495-3497` instead of asserting "every list" whatever is being counted.
...
announced. It now lands on the all-read heading at `src/index.html:343`, which is both what the
reader needs to hear and where the remaining actions are.

The About view carries the shortcut reference at `src/index.html:639-646`, naming all three
...
`src/js/main.js:2412-2417`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/index.html:764`, so resolving the theme in JavaScript would paint dark and then flip, and
...
graphic is `aria-hidden="true"` at `src/index.html:265`, the state is carried in words that
...
Reuse is keyed on the whole item, at `src/js/main.js:2195-2210`, rather than on a list of the
...
`src/js/main.js:2185-2193`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3441`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1904-1907` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1927-1935` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3441`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3410-3421`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1950-1958`. The usual reason
...
`src/js/main.js:3768`, `src/js/main.js:3692` and `src/js/main.js:3708-3714`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1686-1740`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3522-3540`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:3074`, so the two kinds cannot be
...
rather than naming them, and the sections in `src/index.html:394-406` are empty shells whose heading,
...
The settings form keeps its own check and its own message at `src/js/main.js:3719-3720`, unchanged.
...
reached every one of them; the reading list at `src/index.html:379` sits inside neither, so it never
...
two at `src/index.html:519` and `src/index.html:526` are direct children of a `.card card-static`,
...
is written out in words inside the pill at `src/js/main.js:2412-2417`, drawing on the labels at
`src/js/main.js:2472-2477`, and the full sentence from `describe` sits in a visually-hidden span
...
now three, at `README.md:597-608`, and the four sentences over 40 words are now none: the longest
...
`README.md:505-509`, as fetching a list once and committing what came back. `depth` gained a gloss
...
`src/js/main.js:1703-1706`, and `renderRows` asks it at `src/js/main.js:2320`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1698-1702` exists because
...
`src/js/main.js:2160` and the reading rows at `src/js/main.js:2463`, and the reasoning is recorded at
...
while proving nothing. The full order lives inside a `<details>` at `src/index.html:357` that is
...
`src/js/main.js:1523` carries a comment at `src/js/main.js:1533` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3383`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1245` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3458` puts
...
`src/js/main.js:4058` calls both on every `store.update` with no navigation at all. Measured on that
...
`renderYours` at `src/js/main.js:1161` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2505`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1177`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1032` and `src/js/main.js:1478`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4091`, and `src/js/app.js` is the entry
...
on every catalog card, at `src/js/main.js:1478`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2940`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1425`. That also retired a latent bug rather than fixing it. The old label said
...
confirm dialog at `src/index.html:758`. The convention there is the same shape as this: the action
...
that boot report, and the comment now standing in its place at `src/js/main.js:4138-4142` records
...
file of 4,166 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3731-3733`, because the budget belongs to
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:950`. The choice
...
glyph that no one reads, since the button's label at `src/js/main.js:2396` reads "Mark X as unread"
...
figure as audited" at `PRODUCT_BACKLOG.md:202-204`. The clause is quoted only as far as its fixed
...
`CHANGELOG.md:2543-2552` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3669-3697`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3229-3233`,
...
attack. Evidence: `src/js/main.js:3669-3697`, `src/js/lib/model.js:965-1074`.
...
`src/js/main.js:1167` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/lib/model.js:225-240`, `server.mjs:53-59`, `src/index.html:536-541`.
...
remain local or absent. Evidence: `README.md:51-70`, `src/index.html:510-568`,
...
`src/js/main.js:2058` builds the reading hero's backdrop URL only when covers are on, and a computed
...
at once. The About view's "Metadata and links only" card, at `src/index.html:613-617`, is the sixth,
...
requests, from `src/js/main.js:2894` and `src/js/main.js:2910`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3003` is named alongside the search it resembles, and the
...
`test/ci-supply-chain.test.js:124-142`, `README.md:463-498`.
...
`package.json:26-36`, `package-lock.json:1-22`, `absent: .github/dependabot.yml, tracked-file inventory`.
...
`src/index.html:125-126`, `src/js/main.js:3978-4041`.
...
The message policy is a pure function at `src/js/main.js:2559-2568` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2879` and then starts
the hydrator at `src/js/main.js:2886`, whose `start()` reports its running phase at
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1711-1712`
...
are valuable history and are not a concise public policy. Evidence: `README.md:348-655`,
...
button at `src/js/main.js:3757-3794` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3764-3793` repaints the banner
...
over, for the hero backdrop alone: `src/js/main.js:2058` reads the setting before building the URL,
...
`src/js/main.js:2209`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3400-3406`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3432-3438`,
...
here. Evidence: `src/js/main.js:3447`.
...
in place of that button while a copy is live at `src/js/main.js:3881-3887`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1167`
...
config that imports it needs it declared. Evidence: `package.json:32-35`, `eslint.config.mjs:13`.
...
**Delivered.** The first citation now names `src/js/main.js:3768`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3757-3794`. The other two were already right and
are unchanged: `src/js/main.js:3692` is the call in the successful branch of the restore handler,
and `src/js/main.js:3708-3714` is the whole undo-restore handler, whose call is at
`src/js/main.js:3713`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1904-1907` is `forgetDeleted` itself, `src/js/main.js:1927-1935` is
`forgetDeletedFor`, `src/js/main.js:3441` is its one call, `src/js/main.js:3410-3421` is the
half-import rollback, `src/js/main.js:437` is `notify` with its `action` parameter,
`src/js/main.js:460-464` is `spoken`, `src/js/main.js:1950-1958` is the failed undo keeping its
...
`src/js/main.js:1930`, `src/js/main.js:1946` and `src/js/main.js:1961`. The sentence is right to
...
where the status key at `PRODUCT_BACKLOG.md:228` defines an item still to be picked up as `Ready`.
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4031-4034`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4013-4027` keeps
...
consumer at `src/js/main.js:2077`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2261-2270` returns a sentence saying no synopsis is
...
The disclaimer at `src/js/main.js:2608` is shown every time a run starts rather than once and
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3481-3515` (renderProgress, which now takes a scope), `src/index.html:379`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3263` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4058-4078` (renderAll rebuilds
every region), `src/js/main.js:75-80` (store.onChange is wired straight to renderAll),
`src/js/main.js:2292-2470` (renderRows, which then built every row with no virtualisation and no
...
Evidence: `package.json:1-36` (no `dependencies` key at all, `engines.node >= 20`),
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2472-2477`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1937-1964` (undo), `src/js/main.js:3708-3714` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4058-4078`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,166 lines now, so nearly every item shipped
...
- Installability: good, and suited to an app cloned and run by hand. `npm start` runs the server
  with no install step, because there is nothing to install. Evidence: `package.json:8-26`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3716-3749`, `src/js/lib/cachePolicy.js:16-20`,
...
| Category | Verdict |
|----------|---------|
| First-run experience | Gap. The first-run DOM ships an empty `<h2 id="hero-title">`, so the first heading a screen reader meets on an unseeded install is blank. Evidence: `src/index.html:305-340`, `docs/ux-artifacts/pa11y-landing.json`. |
...
| Observability | Partial gap, bounded by Repository Constraint 3. Product analytics are forbidden and are not proposed. What is missing is local and private: there is no way for the reader to see why hydration stalled beyond a queue-depth pill. Evidence: `src/js/main.js:4031-4034`. |
| Performance | Gap, measured. See characteristic 2. Evidence: `docs/ux-artifacts/render-cost.json`. |
| Security, OWASP Top 10 | Gap under A05 Security Misconfiguration: no CSP and no `x-frame-options` on the dev server. Evidence: `server.mjs:129-139`. Resolved: `BL-030` shipped both, assembled at `server.mjs:53-64` and sent at `server.mjs:134` and `server.mjs:137`. Partial gap under A10 Server-Side Request Forgery by analogy: `MarvelApi` accepted any base URL and only stripped trailing slashes, with the https-or-local check living in the settings form rather than in the client. Resolved: `BL-045` moved the rule into the constructor at `src/js/api.js:52-67` and onto the read out of storage at `src/js/main.js:471-500`, so a base the rule refuses cannot reach a fetch from any of the three call sites, and the form keeps its own message at `src/js/main.js:3719-3720`. A01, A02, A03, A07 and A09 are not applicable, because there is no server-side authorisation boundary, no credential store, no server-side query language, no account system and no central log to protect. |
| Privacy | No gap. Nothing is uploaded, there is no account and there is no telemetry, which is the product promise itself. Evidence: `package.json:1-36` (no dependency that could exfiltrate), `absent: analytics|telemetry|gtag|beacon, grep across src/ and scripts/`. |
...
| Release and versioning | Gap. Version is pinned at `0.1.0` with no tags and no changelog, so there is no way to say which build a backup or a bug report came from. Evidence: `package.json:3`, `absent: CHANGELOG.md and git tags, glob of repository root and git tag --list`. Resolved: `BL-043` set the version to `1.0.0` at `package.json:3`, added `CHANGELOG.md`, and wired a `version` script at `package.json:26` that syncs the version the app reports. |
| Dependency management | Not applicable, because runtime dependencies are zero by Repository Constraint 4, there are no `devDependencies`, and there is therefore no lockfile and no dependency graph to manage or audit. The repository invokes no package-fetching tool at all. Evidence: `package.json:1-36` (neither a `dependencies` nor a `devDependencies` key), `absent: npx, grep across the repository returning only this appendix's own text`. The absence of dev tooling is recorded as a maintainability and CI gap above rather than counted twice here. Changed since: the "not applicable" verdict no longer holds. `BL-040` added three `devDependencies` at `package.json:31-36` and a tracked `package-lock.json`, and `BL-115` took that to four, so there is now a dev dependency graph to audit even though runtime dependencies remain zero. |
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3277-3297` (attribution rendered in the UI before import). |
...
the status key at `PRODUCT_BACKLOG.md:228` means by an item still to be picked up. Measured over
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3263` and `src/js/main.js:3256-3269`. What is missing is data, in
