...
A hundred and seventeen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147, BL-149, BL-150, BL-152 and BL-153.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3451-3512` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3573-3588` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3475` and `src/js/main.js:3493-3510` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3551-3571` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3755-3789`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3769-3771` | Done, shipped as BL-014 |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3982-3988` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4332-4352`. The work shipped since has
  taken it further: the file is 4,440 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,171; 224 is the figure as audited.
...
| BL-091 | Let catalog descriptions survive the WCAG text-spacing override | Defect | EP-07 | Leaves BL-028 alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Dropped | src/styles.css:842-845 |
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4031-4068 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3943-3971 |
...
| BL-154 | Shelve the events apart from the character runs, and say why the runs sit last | Story | EP-01 | Follows BL-152 | 5 | 3 | 2 | 2 | 5.0 | none | Measured | Ready | `src/js/main.js:3437-3438` |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4305-4308 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4252-4315 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3712 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3755-3789 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:995 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3465-3470 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3530-3545` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4332-4346 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4332-4346 |
...
real title is `.result-title`, written at `src/js/main.js:3499`, and with the selector corrected the
...
`src/js/main.js:3769-3771` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3715`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3715`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3684-3695`, leaving nothing behind and the offer still
...
`src/js/main.js:4042`, `src/js/main.js:3966` and `src/js/main.js:3982-3988`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3796-3814`.
...
The settings form keeps its own check and its own message at `src/js/main.js:3993-3994`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3657`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3732` puts
...
`src/js/main.js:4332` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4365`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4412-4416` records
...
file of 4,440 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:4005-4007`, because the budget belongs to
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:971`. The choice
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11117-11119` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11136-11140`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10951-10953` describes
...
`CHANGELOG.md:2752-2761` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3943-3971`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3465-3470`,
...
attack. Evidence: `src/js/main.js:3943-3971`, `src/js/lib/model.js:968-1077`.
...
`src/index.html:125-126`, `src/js/main.js:4252-4315`.
...
button at `src/js/main.js:4031-4068` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4038-4067` repaints the banner
...
button. Evidence: `src/js/main.js:3674-3680`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3706-3712`,
...
here. Evidence: `src/js/main.js:3721`.
...
in place of that button while a copy is live at `src/js/main.js:4155-4161`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:4042`, `src/js/main.js:3966` and `src/js/main.js:3987`
...
**Delivered.** The first citation now names `src/js/main.js:4042`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4031-4068`. The other two were already right and
are unchanged: `src/js/main.js:3966` is the call in the successful branch of the restore handler,
and `src/js/main.js:3982-3988` is the whole undo-restore handler, whose call is at
`src/js/main.js:3987`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3715` is its one call, `src/js/main.js:3684-3695` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4305-4308`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4287-4301` keeps
...
- [ ] Divide the shelf into events and character-focused reading, keyed on the order's type
- [ ] Say what each section is for, in a sentence a new reader can act on
- [ ] Keep the reading path intact across the division, since its stops span both
- [ ] Leave the facet chips working, so a filtered shelf still says which section it is showing
...
Two sections rather than four. `type` splits four ways, but the reading path runs through orders typed
`era`, `event` and `character-run`, so dividing by all four would cut the sequence this change just
shipped into three pieces on three parts of the page. Events and eras read as one thing to a new
reader, and creator runs and character runs read as another.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3755-3789` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3500` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4332-4352` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1970-1997` (undo), `src/js/main.js:3982-3988` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4332-4352`, `src/js/main.js:967-984` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,440 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3990-4023`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1046-1077`, `src/js/main.js:3982-3988`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3551-3571` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3500` and `src/js/main.js:3493-3510`. What is missing is data, in
