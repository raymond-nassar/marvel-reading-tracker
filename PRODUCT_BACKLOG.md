...
A hundred and nineteen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147, BL-149, BL-150, BL-152, BL-153, BL-154 and BL-157.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3471-3532` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3593-3608` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3495` and `src/js/main.js:3513-3530` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3571-3591` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2972-3028`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3036-3105` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2934-2952`, manual issue add at `src/js/main.js:3221-3341` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1882-1902`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:433` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1010-1050` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3775-3809`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3789-3791` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2353` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2449-2450`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2505-2510` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3298` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:4002-4008` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2536-2559` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:337-350` and `src/js/main.js:439-460` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4352-4372`. The work shipped since has
  taken it further: the file is 4,460 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,182; 224 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2294-2303 |
...
| BL-091 | Let catalog descriptions survive the WCAG text-spacing override | Defect | EP-07 | Leaves BL-028 alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Dropped | src/styles.css:852-855 |
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4051-4088 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3963-3991 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3947-3950 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1439-1440 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4325-4328 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1970-1997 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4272-4335 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3732 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:439-460 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/main.js:631-635 |
...
| BL-146 | Put a way out of the capped landing grid where the reader runs out of it | Defect | EP-10 | Follows BL-145 | 3 | 1 | 2 | 2 | 3.0 | none | Measured | Shipped | `src/index.html:247`, `src/js/main.js:1207-1214` |
...
| BL-150 | Fill in what the metadata snapshot cannot, from the one catalogue a browser can reach | Feature | EP-03 | Follows BL-149 | 5 | 5 | 5 | 5 | 3.0 | none | Measured | Shipped | `src/js/lib/wikitext.js:38-42`, `src/js/main.js:3263-3275`, `absent: any record newer than 2025-10-29 upstream, measured by asking the metadata service for 2026 and being told the total is zero` |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3775-3809 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:1005 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1178 |
...
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:562 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3485-3490 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3550-3565` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4352-4366 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4352-4366 |
...
that function is at `src/js/main.js:3375` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3519`, and with the selector corrected the
...
`src/js/main.js:3789-3791` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:728` and
...
`src/js/main.js:2445-2450`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:650` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:666` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:2228-2243`, rather than on a list of the
...
`src/js/main.js:2218-2226`. Doing it the other way round leaves the stale node in front of the
...
the page, and `placeNotices()` at `src/js/main.js:383-421` renders every outstanding record into
...
`src/js/main.js:3735`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1937-1940` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1960-1968` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3735`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3704-3715`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:439`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:462-466` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1983-1991`. The usual reason
...
`src/js/main.js:4062`, `src/js/main.js:3986` and `src/js/main.js:4002-4008`. Each replaces the whole
...
`src/js/main.js:391-402`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1719-1773`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3816-3834`.
...
synthetic id from the clock, per `src/js/main.js:3232`, so the two kinds cannot be ordered against
...
`src/js/main.js:473-502` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:527-543` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:4013-4014`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2445-2450`, drawing on the labels at
`src/js/main.js:2505-2510`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1736-1739`, and `renderRows` asks it at `src/js/main.js:2353`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1731-1735` exists because
...
`src/js/main.js:2193` and the reading rows at `src/js/main.js:2496`, and the reasoning is recorded at
...
`src/js/main.js:1556` carries a comment at `src/js/main.js:1566` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3677`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1277` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3752` puts
...
`renderRail` at `src/js/main.js:1010` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:982` and reaches `renderYours` through `renderHome` at
`src/js/main.js:984`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4352` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1018`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1172` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2538`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1188`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1034` and `src/js/main.js:1511`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4385`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:1034` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1511`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2983`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1458`. That also retired a latent bug rather than fixing it. The old label said
...
comment, when it comes from the series name at `src/js/main.js:604-605`. Both are re-aimed, and the
...
that boot report, and the comment now standing in its place at `src/js/main.js:4432-4436` records
...
file of 4,460 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:4025-4027`, because the budget belongs to
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:981`. The choice
...
glyph that no one reads, since the button's label at `src/js/main.js:2429` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11211-11213` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11230-11234`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:11045-11047` describes
...
`CHANGELOG.md:2785-2794` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3963-3991`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3485-3490`,
...
attack. Evidence: `src/js/main.js:3963-3991`, `src/js/lib/model.js:968-1077`.
...
`src/js/main.js:1178` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/main.js:2091` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2937` and `src/js/main.js:2953`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3046` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:4272-4335`.
...
`src/js/main.js:439-449`, which routes to the announcer precisely when the container is not itself
...
The message policy is a pure function at `src/js/main.js:2592-2601` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2922` and then starts
the hydrator at `src/js/main.js:2929`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2535`, and one of the four tests added to the counts suite pins the
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1744-1745`
...
button at `src/js/main.js:4051-4088` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4058-4087` repaints the banner
...
it on. Evidence: `src/js/main.js:643-644`, `src/js/main.js:640`, `src/styles.css:453`.
...
over, for the hero backdrop alone: `src/js/main.js:2091` reads the setting before building the URL,
...
`src/js/main.js:2242`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3694-3700`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3726-3732`,
...
here. Evidence: `src/js/main.js:3741`.
...
in place of that button while a copy is live at `src/js/main.js:4175-4181`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1178`
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4770-4778`.
...
**Delivered.** The first citation now names `src/js/main.js:4062`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4051-4088`. The other two were already right and
are unchanged: `src/js/main.js:3986` is the call in the successful branch of the restore handler,
and `src/js/main.js:4002-4008` is the whole undo-restore handler, whose call is at
`src/js/main.js:4007`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1937-1940` is `forgetDeleted` itself, `src/js/main.js:1960-1968` is
`forgetDeletedFor`, `src/js/main.js:3735` is its one call, `src/js/main.js:3704-3715` is the
half-import rollback, `src/js/main.js:439` is `notify` with its `action` parameter,
`src/js/main.js:462-466` is `spoken`, `src/js/main.js:1983-1991` is the failed undo keeping its
buffer, `src/js/main.js:391-402` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:1963`, `src/js/main.js:1979` and `src/js/main.js:1994`. The sentence is right to
...
at `PRODUCT_BACKLOG.md:4775-4776`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4770-4773` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4325-4328`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4307-4321` keeps
...
consumer at `src/js/main.js:2110`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2294-2303` returns a sentence saying no synopsis is
...
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:556` would have recorded
...
The disclaimer at `src/js/main.js:2641` is shown every time a run starts rather than once and
...
The one-time purge at `src/js/main.js:562` empties the response cache once, so the prose cached
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3775-3809` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3520` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4352-4372` (renderAll rebuilds
every region), `src/js/main.js:77-82` (store.onChange is wired straight to renderAll),
`src/js/main.js:2325-2503` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2505-2510`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1970-1997` (undo), `src/js/main.js:4002-4008` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4352-4372`, `src/js/main.js:967-984` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,460 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4010-4043`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3571-3591` (attribution rendered in the UI before import). |
...
### Case 1: BL-026 is labelled P0 but ranks sixty-sixth

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 66 of 132.
- Driver: job size, not value. Its Cost of Delay of 11 is the tenth highest figure in the backlog.
  It is outranked by sixty-five items, sixty-two of them sized 1, 2 or 3 whose Cost of Delay
...
- Calculated: WSJF 1.4, rank 126 of 132, below a hundred and twenty-two unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3520` and `src/js/main.js:3513-3530`. What is missing is data, in
...
- BL-014, P1, rank 84 of 132. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 68 of 132. Mid-table.
- BL-017, P2, rank 131 of 132. The lowest-ranked scored story other than the one that cannot be
