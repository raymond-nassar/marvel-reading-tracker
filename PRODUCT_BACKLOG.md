...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3055-3089` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:417-419`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:410-416`, search-as-you-type at `src/js/main.js:3117-3132` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3063` and `src/js/main.js:3072-3077` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3095-3115` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2756-2812`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2820-2889` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2718-2736`, manual issue add at `src/js/main.js:2891-2935` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3299-3333`, with a scope control at `src/index.html:377-381` and a subtitle that stops claiming every list at `src/js/main.js:3313-3315` | Done, shipped as BL-014 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2914` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3526-3532` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:3876-3896`. The work shipped since has
  taken it further: the file is 3,984 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,023; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3575-3612 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3487-3515 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3849-3852 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3796-3859 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3256 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3299-3333 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3058-3062 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3876-3890 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3876-3890 |
...
that function is at `src/js/main.js:2969` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:3067`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:3313-3315` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3259`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3259`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3228-3239`, leaving nothing behind and the offer still
...
`src/js/main.js:3586`, `src/js/main.js:3510` and `src/js/main.js:3526-3532`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3340-3358`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2901`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3537-3538`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3201`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3276` puts
...
`src/js/main.js:3876` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:3909`, and `src/js/app.js` is the entry
...
at `src/js/main.js:2767`. A reader who imports one of this repository's own checklists therefore saw
...
that boot report, and the comment now standing in its place at `src/js/main.js:3956-3960` records
...
file of 3,984 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3549-3551`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:9930-9932` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:9949-9953`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:9764-9766` describes
...
`src/js/storage.js:495-571`, `src/js/main.js:3487-3515`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3058-3062`,
...
attack. Evidence: `src/js/main.js:3487-3515`, `src/js/lib/model.js:965-1074`.
...
requests, from `src/js/main.js:2721` and `src/js/main.js:2737`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2830` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3796-3859`.
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2706` and then starts
the hydrator at `src/js/main.js:2713`, whose `start()` reports its running phase at
...
button at `src/js/main.js:3575-3612` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3582-3611` repaints the banner
...
button. Evidence: `src/js/main.js:3218-3224`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3250-3256`,
...
here. Evidence: `src/js/main.js:3265`.
...
in place of that button while a copy is live at `src/js/main.js:3699-3705`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:3586`, `src/js/main.js:3510` and `src/js/main.js:3531`
...
**Delivered.** The first citation now names `src/js/main.js:3586`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3575-3612`. The other two were already right and
are unchanged: `src/js/main.js:3510` is the call in the successful branch of the restore handler,
and `src/js/main.js:3526-3532` is the whole undo-restore handler, whose call is at
`src/js/main.js:3531`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3259` is its one call, `src/js/main.js:3228-3239` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3849-3852`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3831-3845` keeps
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3299-3333` (renderProgress, which now takes a scope), `src/index.html:375`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3038` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3876-3896` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1777-1804` (undo), `src/js/main.js:3526-3532` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:3876-3896`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,984 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3534-3567`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1043-1074`, `src/js/main.js:3526-3532`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3095-3115` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3038` and `src/js/main.js:3072-3077`. What is missing is data, in
