...
BL-051, BL-052, BL-053, BL-054, BL-055, BL-056, BL-057, BL-058, BL-059, BL-061, BL-062 and BL-063.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2335-2365` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:407-409`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:400-406`, search-as-you-type at `src/js/main.js:2393-2408` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2339` and `src/js/main.js:2348-2353` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2371-2391` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2039-2092`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2100-2169` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2001-2019`, manual issue add at `src/js/main.js:2171-2215` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1312-1332`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:152` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:746-786` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2570-2604`, with a scope control at `src/index.html:367-371` and a subtitle that stops claiming every list at `src/js/main.js:2584-2586` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1619` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:291-326` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:359-379`, quoted into the Markdown export at `src/js/lib/markdown.js:149-151` | Done, shipped as BL-017 |
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1671-1677`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1717-1722` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2194` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:592-620` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2701-2707` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1748-1771` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2817-2837`. The work shipped since has
  taken it to 2,911; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1398-1425 |
| BL-047 | Split the two meanings of the row class | Debt | EP-12 | Leaves alone | 1 | 1 | 2 | 1 | 4.0 | none | Observed | Shipped | src/styles.css:646-662 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2570-2604 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2817-2831 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:359-367 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2817-2831 |
...
that function is at `src/js/main.js:2249` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2343`, and with the selector corrected the same mutation fails it. It is
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:398-409`; omitting it
...
`src/js/main.js:2584-2586` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:204` and load normalises
a stale id at `src/js/lib/model.js:587`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:534-535`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:359-367`, for the same reason
...
Notes export to Markdown quoted with `> `, at `src/js/lib/markdown.js:149-151`, and the prefix is
...
Verified: 23 new tests, every one of 18 mutations caught with none skipped, and 26 browser checks in
...
`src/js/main.js:1667-1677`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2473`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:223-235` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:170-173`: "in library" and "Continue reading" would both resolve to
...
making sense: `forgetDeleted()` at `src/js/main.js:1365-1368` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1388-1396` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2530`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2498-2509`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1411-1419`. The usual reason
...
`src/js/main.js:2694`, `src/js/main.js:2694` and `src/js/main.js:2701-2707`. Each replaces the whole
...
the id is unknown, at `src/js/lib/model.js:237-239`, so an address naming a deleted list would
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1212-1265`, before the
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:429-479`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2611-2629`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:205-206`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:2181`, so the two kinds cannot be
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:319-326`, written the same way
`coerce` writes it at `src/js/lib/model.js:548`, so the two paths cannot disagree and every future
caller inherits it.
...
The settings form keeps its own check and its own message at `src/js/main.js:2711-2712`, unchanged.
...
Shipped. The form row is now `.field-row` at `src/styles.css:646-662` and the reading row keeps
...
`.card-static > *` at `src/styles.css:639`, which the old rule's 0,2,0 had been masking. Measured
...
is written out in words inside the pill at `src/js/main.js:1667-1677`, drawing on the labels at
`src/js/main.js:1717-1722`, and the full sentence from `describe` sits in a visually-hidden span
...
The reasoning is recorded at `src/styles.css:588-612`, directly above the rules it governs, along
...
`src/js/main.js:1229-1232`, and `renderRows` asks it at `src/js/main.js:1619`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1224-1228` exists because
...
`src/js/main.js:1708`, and the reasoning is recorded at `src/js/main.js:163-167` rather than only
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2477`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:974` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2547` puts
...
`renderRail` at `src/js/main.js:746` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:724` and reaches `renderYours` through `renderHome` at
`src/js/main.js:726`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:2817` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:754`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:897` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1750`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:910`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:770` and `src/js/main.js:1088`. Both predate this change and neither is part of its
...
`src/styles.css:633` sets `content` to a right-pointing angle glyph on the card summary marker. It
...
`content` in the stylesheets is the one at `src/styles.css:633`. Closing it would need a tokenizer
...
built at `src/js/main.js:770` and joined the list's name to its progress with one; the preview button
...
at `src/js/main.js:2050`. A reader who imports one of this repository's own checklists therefore saw
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:3146-3147` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:3166-3168`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:2988-2990` describes
...
`CHANGELOG.md:349-352` that says so and points at the backlog clause instead. That clause was
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2570-2604` (renderProgress, which now takes a scope), `src/index.html:365`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2318` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2817-2837` (renderAll rebuilds
every region), `src/js/main.js:67-72` (store.onChange is wired straight to renderAll),
`src/js/main.js:1608-1715` (renderRows builds every row with no virtualisation and no early exit when
...
`src/js/lib/model.js:592-620` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1717-1722`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1398-1425` (undo), `src/js/main.js:2701-2707` (undoRestore exists).
...
`src/js/lib/model.js:509-535` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2817-2837`, `src/js/main.js:711-742` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 2,911 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2709-2729`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:11` and `src/js/lib/model.js:509-535`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2371-2391` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2318` and `src/js/main.js:2348-2353`. What is missing is data, in
