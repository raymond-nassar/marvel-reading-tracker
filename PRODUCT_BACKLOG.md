...
Twenty-eight items have since been delivered and are marked `Shipped` in the table below: BL-014,
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2198-2228` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:370-372`, filterByFacet imported at `src/js/main.js:16` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:363-369`, search-as-you-type at `src/js/main.js:2256-2271` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2202` and `src/js/main.js:2211-2216` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2234-2254` | Done |
| 2.3 | BL-007 | P1 | grouping and variant labelling exist at `src/js/main.js:2181` and `src/js/main.js:2211-2216`, but only the Hickman creator run populates `group` and `variant` in `src/data/catalog.json`; all six event lists carry null | Partial |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:1902-1955`, `src/js/lib/markdown.js:36-99` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:1963-2032` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:1864-1882`, manual issue add at `src/js/main.js:2034-2078` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1205-1225`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:143` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:658-698` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2433-2467`, with a scope control at `src/index.html:348-352` and a subtitle that stops claiming every list at `src/js/main.js:2447-2449` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1508` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:1560-1566`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:1596-1601` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2057` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:2512-2518` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:1611-1634` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:2628-2641`. The work shipped since has
  taken it to 2,696; 1,566 is the figure as audited.
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1291-1318 |
...
| BL-061 | Take the two em dashes out of the copy the app puts on screen | Chore | EP-12 | Leaves alone | 2 | 1 | 1 | 1 | 4.0 | none | Measured | Ready | src/js/main.js:682 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Ready | src/js/main.js:2628-2640 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:2628-2640 |
...
that function is at `src/js/main.js:2112` today, with the rename-list handler occupying the lines the
...
`src/js/main.js:2447-2449` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:1556-1566`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:2336`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1258-1261` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1281-1289` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2393`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2361-2372`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1304-1312`. The usual reason
...
`src/js/main.js:2505`, `src/js/main.js:2505` and `src/js/main.js:2512-2518`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1122-1175`, before the
...
The settings form keeps its own check and its own message at `src/js/main.js:2522-2523`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:1556-1566`, drawing on the labels at
`src/js/main.js:1596-1601`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1139-1142`, and `renderRows` asks it at `src/js/main.js:1508`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1134-1138` exists because
...
`src/js/main.js:1587`, and the reasoning is recorded at `src/js/main.js:159-163` rather than only
...
`src/js/main.js:1030` carries a comment at `src/js/main.js:1040` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2340`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2410` puts
...
`src/js/main.js:2628` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:1613`,
...
`src/js/main.js:682` and `src/js/main.js:998`. Both predate this change and neither is part of its
...
- [ ] Check whether anything else the app puts on screen carries one

Constraint gate: checked 1 to 11, none breached.

Constraint 11 says shipped surfaces contain no em dashes, and two of them do. The rail tooltip is
built at `src/js/main.js:682` and reads `<name> — <read> of <total> read`; the preview button on
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2433-2467` (renderProgress, which now takes a scope), `src/index.html:346`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2181` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:2628-2641` (renderAll rebuilds
every region), `src/js/main.js:63-68` (store.onChange is wired straight to renderAll),
`src/js/main.js:1497-1594` (renderRows builds every row with no virtualisation and no early exit when
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:1596-1601`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1291-1318` (undo), `src/js/main.js:2512-2518` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:2628-2641`, `src/js/main.js:626-654` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:2520-2540`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2234-2254` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2181` and `src/js/main.js:2211-2216`. What is missing is data, in
