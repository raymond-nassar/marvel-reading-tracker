...
Ninety items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-103, BL-104, BL-105, BL-106, BL-107, BL-108, BL-110, BL-111, BL-112, BL-113, BL-115, BL-116,
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2824-2858` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2886-2901` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2832` and `src/js/main.js:2841-2846` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2864-2884` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2525-2581`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2589-2658` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2487-2505`, manual issue add at `src/js/main.js:2660-2704` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1623-1643`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:368` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:942-982` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3062-3096`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:3076-3078` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:2030` | Done |
| 4.4 | BL-016 | P1 | hero next-unread and Done, next at `src/index.html:295-330` | Done |
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:610-639`, quoted into the Markdown export at `src/js/lib/markdown.js:180-182` | Done, shipped as BL-017 |
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2126-2132`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2187-2192` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2683` and render, reorder, export and back up like any other issue | Done |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:927-958` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3289-3295` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2218-2241` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3626-3646`. The work shipped since has
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3325-3362 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3250-3278 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3599-3602 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:877 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3546-3609 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3019 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/lib/model.js:179-194 |
...
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Ready | src/js/main.js:2987 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3062-3096 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:2827-2831 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3626-3640 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:619-627 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3626-3640 |
...
that function is at `src/js/main.js:2738` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2836`, and with the selector corrected the same mutation fails it. It is
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:658-669`; omitting it
...
`src/js/main.js:3076-3078` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:424` and load normalises
a stale id at `src/js/lib/model.js:922`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:844-845`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:619-627`, for the same reason
...
`src/js/main.js:2122-2132`, along with the pending badge's. A `title` is unreachable by touch and
...
Reuse is keyed on the whole item, at `src/js/main.js:1965-1980`, rather than on a list of the
...
`src/js/main.js:1955-1963`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3022`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:443-455` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:386-389`: "in library" and "Continue reading" would both resolve to
...
about to make at `src/js/main.js:3022`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2991-3002`, leaving nothing behind and the offer still
...
`src/js/main.js:3336`, `src/js/main.js:3273` and `src/js/main.js:3289-3295`. Each replaces the whole
...
the id is unknown, at `src/js/lib/model.js:457-459`, so an address naming a deleted list would
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:689-739`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3103-3121`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:425-426`: issue metadata and read state survive the deletion of
...
gets a negative synthetic id from the clock, per `src/js/main.js:2670`, so the two kinds cannot be
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:579-586`, written the same way
`coerce` writes it at `src/js/lib/model.js:858`, so the two paths cannot disagree and every future
caller inherits it.
...
The settings form keeps its own check and its own message at `src/js/main.js:3299-3300`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2122-2132`, drawing on the labels at
`src/js/main.js:2187-2192`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1477-1480`, and `renderRows` asks it at `src/js/main.js:2030`. "In the markup but
...
`src/js/main.js:1930` and the reading rows at `src/js/main.js:2178`, and the reasoning is recorded at
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2970`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3039` puts
...
`src/js/main.js:3626` calls both on every `store.update` with no navigation at all. Measured on that
...
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2220`,
...
sequence moved into an exported `boot()` at `src/js/main.js:3659`, and `src/js/app.js` is the entry
...
at `src/js/main.js:2536`. A reader who imports one of this repository's own checklists therefore saw
...
`lists[k] = {...}` at `src/js/lib/model.js:877`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:905`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:922`.
...
that boot report, and the comment now standing in its place at `src/js/main.js:3704-3708` records
...
the client and leaves the limiter alone, at `src/js/main.js:3312-3314`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2106` reads "Mark X as unread"
...
`src/js/storage.js:495-571`, `src/js/main.js:3250-3278`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:2827-2831`,
`src/data/catalog.json`, `src/js/lib/model.js:105-116`.
...
attack. Evidence: `src/js/main.js:3250-3278`, `src/js/lib/model.js:849-958`.
...
`src/js/lib/model.js:179-194`, `server.mjs:53-59`, `src/index.html:526-531`.
...
requests, from `src/js/main.js:2490` and `src/js/main.js:2506`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2599` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3546-3609`.
...
`src/js/hydrate.js:52-72`, and the limiter's depth counts the queue plus whatever is in flight at
...
The message policy is a pure function at `src/js/main.js:2274-2283` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2475` and then starts
the hydrator at `src/js/main.js:2482`, whose `start()` reports its running phase at
...
button at `src/js/main.js:3325-3362` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3332-3361` repaints the banner
...
`src/js/main.js:1979`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:2987`, `src/js/lib/model.js:168`,
`src/js/lib/model.js:741`.
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3013-3019`,
...
are green. Evidence: `src/js/lib/model.js:105-116`.
...
`src/js/lib/model.js:112-113`, `src/js/lib/model.js:122-132`.
...
here. Evidence: `src/js/main.js:3028`.
...
in place of that button while a copy is live at `src/js/main.js:3449-3455`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:3336`, `src/js/main.js:3273` and `src/js/main.js:3294`
...
**Delivered.** The first citation now names `src/js/main.js:3336`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3325-3362`. The other two were already right and
are unchanged: `src/js/main.js:3273` is the call in the successful branch of the restore handler,
and `src/js/main.js:3289-3295` is the whole undo-restore handler, whose call is at
`src/js/main.js:3294`. The range was left as the handler rather than narrowed to the call, because
...
the code they name, and the twelve besides this one are sound: `src/js/lib/model.js:443-455` is
`restoreList`, `src/js/lib/model.js:386-389` is the comment on a duplicate not inheriting the
catalog claim, `src/js/main.js:1676-1679` is `forgetDeleted` itself, `src/js/main.js:1699-1707` is
`forgetDeletedFor`, `src/js/main.js:3022` is its one call, `src/js/main.js:2991-3002` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3599-3602`.
...
and the hydrator continues past a failed lookup rather than stopping at `src/js/hydrate.js:61-65`.
...
reason filed above and confirmed at `src/js/hydrate.js:52-72`: the hydrator awaits one issue at a
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3581-3595` keeps
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3062-3096` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2807` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3626-3646` (renderAll rebuilds
every region), `src/js/main.js:74-79` (store.onChange is wired straight to renderAll),
`src/js/main.js:2002-2185` (renderRows, which then built every row with no virtualisation and no
...
`src/js/lib/model.js:927-958` (validated backup shape).
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2187-2192`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1709-1736` (undo), `src/js/main.js:3289-3295` (undoRestore exists).
...
`src/js/lib/model.js:769-845` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3626-3646`, `src/js/main.js:899-916` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3297-3317`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:769-845`.
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2864-2884` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2807` and `src/js/main.js:2841-2846`. What is missing is data, in
