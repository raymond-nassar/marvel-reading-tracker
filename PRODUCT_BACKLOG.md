...
Ninety-one items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-121, BL-122, BL-123 and BL-124.
...
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1623-1643`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:415` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:942-982` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3114-3148`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:3128-3130` | Done, shipped as BL-014 |
...
| 4.5 | BL-017 | P2 | notes on both shapes at `src/js/lib/model.js:657-686`, quoted into the Markdown export at `src/js/lib/markdown.js:180-182` | Done, shipped as BL-017 |
...
| 6.1 | BL-022 | P0 | validated backup shape at `src/js/lib/model.js:979-1010` | Done |
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3341-3347` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3678-3698`. The work shipped since has
  taken it to 3,784; 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 920; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3377-3414 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3302-3330 |
...
| BL-105 | Derive the roadmap paragraph's status split in the counts gate | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | scripts/check-counts.mjs:357-432 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:3651-3654 |
...
| BL-068 | Stop the model reading a list id that names a prototype member | Defect | EP-12 | Leaves alone | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/lib/model.js:929 |
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3598-3661 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3071 |
...
| BL-097 | Publish contribution, conduct, support and maintainer governance | Chore | EP-12 | Extends BL-052 | 3 | 3 | 5 | 3 | 3.67 | none | Observed | Shipped | README.md:272-333 |
...
| BL-086 | Keep cover requests inside the stated trust boundary | Defect | EP-05 | Extends BL-044 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/lib/model.js:208-223 |
...
| BL-093 | Make real-browser regression evidence reproducible | Enabler | EP-12 | Extends BL-041 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Shipped | `scripts/browser-check.mjs:1-7`, `package.json:22-23` |
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Shipped | src/js/lib/model.js:197 |
...
| BL-062 | Delete the paragraph that BL-054's block states twice over | Debt | EP-12 | Leaves alone | 1 | 1 | 1 | 1 | 3.0 | none | Measured | Shipped | scripts/check-counts.mjs:498-532 |
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3114-3148 |
...
| BL-081 | Let the repetition check see a copy that is not next to its original | Debt | EP-12 | Leaves alone | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Shipped | scripts/check-counts.mjs:569-607 |
...
| BL-125 | Fail the build when a stated line count disagrees with the file it counts | Enabler | EP-12 | Extends BL-056 | 2 | 1 | 3 | 2 | 3.0 | none | Measured | Ready | absent: any comparison of a stated line count against the file it counts, read of the counts gate and the anchors gate |
...
| BL-118 | Notice a distant repeat in the prose documents the gate never reads | Enabler | EP-12 | Extends BL-058 | 2 | 1 | 2 | 2 | 2.5 | none | Measured | Ready | scripts/check-counts.mjs:626 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3678-3692 |
...
| BL-017 | Let a reader keep notes on a list or an issue | Story | EP-04 | Leaves alone | 2 | 1 | 1 | 5 | 0.8 | P2 | Observed | Shipped | src/js/lib/model.js:666-674 |
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3678-3692 |
...
Shipped. `seriesProgress` takes an optional list id at `src/js/lib/model.js:705-716`; omitting it
...
`src/js/main.js:3128-3130` instead of asserting "every list" whatever is being counted.
...
`deleteList` falls back to the first surviving list at `src/js/lib/model.js:471` and load normalises
a stale id at `src/js/lib/model.js:974`, so the two cannot come apart. That invariant also rules out
...
`src/js/lib/model.js:896-897`, which latches the "Could not read your saved data" banner. Measured
...
An issue note is global, keyed by issue id at `src/js/lib/model.js:666-674`, for the same reason
...
`src/js/main.js:3074`. The same order can be added from the landing page and imported from the
...
Shipped. `restoreList` at `src/js/lib/model.js:490-502` puts a list back at the index it held,
...
avoid at `src/js/lib/model.js:433-436`: "in library" and "Continue reading" would both resolve to
...
about to make at `src/js/main.js:3074`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3043-3054`, leaving nothing behind and the offer still
...
`src/js/main.js:3388`, `src/js/main.js:3325` and `src/js/main.js:3341-3347`. Each replaces the whole
...
the id is unknown, at `src/js/lib/model.js:504-506`, so an address naming a deleted list would
...
Shipped. The two selectors are `readIssues` and `manualIssues` in `src/js/lib/model.js:736-786`,
sitting in the derived section beside `seriesProgress`, and the pages themselves are described once
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3155-3173`.

The reason Everything read reads the read map rather than walking the lists is the sentence above
`deleteList` at `src/js/lib/model.js:472-473`: issue metadata and read state survive the deletion of
...
believed. The coercion now lives in `markRead` at `src/js/lib/model.js:626-633`, written the same way
`coerce` writes it at `src/js/lib/model.js:910`, so the two paths cannot disagree and every future
caller inherits it.
...
The settings form keeps its own check and its own message at `src/js/main.js:3351-3352`, unchanged.
...
now three, at `README.md:515-526`, and the four sentences over 40 words are now none: the longest
...
`README.md:423-427`, as fetching a list once and committing what came back. `depth` gained a gloss
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3091` puts
...
`src/js/main.js:3678` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:3711`, and `src/js/app.js` is the entry
...
`scripts/check-counts.mjs:498-532`, and `npm run counts` fails on a repeat. That remains true of this
...
already refuses a detail block that appears twice, at `scripts/check-counts.mjs:467-477`, which is
...
`lists[k] = {...}` at `src/js/lib/model.js:929`, filters the stored order with `lists[id]` at
`src/js/lib/model.js:957`, and picks the active list with `lists[raw.active]` at
`src/js/lib/model.js:974`.
...
`scripts/check-counts.mjs:518-524`, so it catches a block directly followed by its copy and nothing
...
`scripts/check-counts.mjs:569-607`, descending from the longest possible repeat down to a floor and
...
that boot report, and the comment now standing in its place at `src/js/main.js:3756-3760` records
...
file of 3,732 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3364-3366`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:8888-8890` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:8907-8911`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:8722-8724` describes
...
`CHANGELOG.md:1938-1947` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3302-3330`.
...
Evidence: `scripts/check-counts.mjs:626`.
...
`src/data/catalog.json`, `src/js/lib/model.js:125-136`.
...
attack. Evidence: `src/js/main.js:3302-3330`, `src/js/lib/model.js:901-1010`.
...
`src/js/lib/model.js:208-223`, `server.mjs:53-59`, `src/index.html:526-531`.
...
`test/ci-supply-chain.test.js:124-142`, `README.md:381-416`.
...
`package.json:24-34`, `package-lock.json:1-22`, `absent: .github/dependabot.yml, tracked-file inventory`.
...
`src/index.html:121-122`, `src/js/main.js:3598-3661`.
...
All three are built from one shared fragment now, at `scripts/check-counts.mjs:218`, and the
word-to-number reader follows the writer's own ceiling at `scripts/check-counts.mjs:309` rather than
...
`scripts/check-counts.mjs:349-355` changed with it, for a reason this item caused rather than
...
are valuable history and are not a concise public policy. Evidence: `README.md:272-573`,
...
button at `src/js/main.js:3377-3414` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3384-3413` repaints the banner
...
`scripts/check-counts.mjs:357-432`. It does not derive the second paragraph, which makes the same two
...
Evidence: `scripts/check-counts.mjs:500-508`, `scripts/check-counts.mjs:546-568`.
...
button. Evidence: `src/js/main.js:3034-3039`, `src/js/lib/model.js:194`,
`src/js/lib/model.js:788`.
...
reads, and the review of this change said so: scripts/vendor-orders.mjs:200-209 catches every lookup
failure alike, so a 404, an exhausted retry budget and a lost connection all write the same empty
...
record what it was told is filed as BL-126 rather than folded in here. A refusal met at runtime is recorded
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3065-3071`,
...
are green. Evidence: `src/js/lib/model.js:125-136`.
...
`src/js/lib/model.js:132-133`, `src/js/lib/model.js:142-152`.
...
in place of that button while a copy is live at `src/js/main.js:3501-3507`, which is the state a
...
config that imports it needs it declared. Evidence: `package.json:30-33`, `eslint.config.mjs:13`.
...
stops checking. Evidence: `scripts/check-counts.mjs:49-52` and `scripts/check-counts.mjs:58-64`.
...
**Delivered.** The first citation now names `src/js/main.js:3388`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3377-3414`. The other two were already right and
are unchanged: `src/js/main.js:3325` is the call in the successful branch of the restore handler,
and `src/js/main.js:3341-3347` is the whole undo-restore handler, whose call is at
`src/js/main.js:3346`. The range was left as the handler rather than narrowed to the call, because
...
the code they name, and the twelve besides this one are sound: `src/js/lib/model.js:490-502` is
`restoreList`, `src/js/lib/model.js:433-436` is the comment on a duplicate not inheriting the
catalog claim, `src/js/main.js:1676-1679` is `forgetDeleted` itself, `src/js/main.js:1699-1707` is
`forgetDeletedFor`, `src/js/main.js:3074` is its one call, `src/js/main.js:3043-3054` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:3651-3654`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:3633-3647` keeps
...
at `scripts/lib/fetch-json.mjs:38-41`. Three of the fourteen are there for what the other eleven
...
- [ ] Derive the files a stated size could be about rather than naming the sites that state one
- [ ] Read the figure out of the prose, in each spelling the documents already use
- [ ] Report a stale statement against the file it is written in, and say what the size is now
...
- [ ] Carry the lookup outcome onto the vendored item rather than inferring it from emptiness
- [ ] Key the refusal on that outcome, so a lost connection is not read as a refusal
- [ ] Fail a vendoring run loudly when an item is empty for a reason that is not a refusal
...
every item in it has already been looked up. The first half is true and the second does not
follow, because the run discards which answer it got.

`scripts/vendor-orders.mjs:200-209` catches every error from the lookup identically and only warns,
and the item is then built from whatever the metadata map holds, so it lands with every field null.
Four unlike outcomes reach that catch: a 404 from `scripts/lib/fetch-json.mjs:48`, a 429 or 5xx
that outlived the retry budget from `scripts/lib/fetch-json.mjs:56`, a lost connection, and a body
that would not parse. Those are exactly the two kinds `BL-109` exists to separate, and the import
path applies none of the test the runtime path applies at `src/js/hydrate.js:72`.
...
would hold. Nothing notices, either: the run warns only when the item count differs from the plan
...
excluded from the queue. What changed is that the exclusion is now permanent and carries a positive
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3114-3148` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3678-3698` (renderAll rebuilds
...
Evidence: `package.json:1-34` (no `dependencies` key at all, `engines.node >= 20`),
`src/js/lib/model.js:979-1010` (validated backup shape).
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1709-1736` (undo), `src/js/main.js:3341-3347` (undoRestore exists).
...
`src/js/lib/model.js:821-897` (migrate refuses an unsupported schema version),
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3678-3698`, `src/js/main.js:899-916` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,732 lines now, so nearly every item shipped
...
- Installability: good, and suited to an app cloned and run by hand. `npm start` runs the server
  with no install step, because there is nothing to install. Evidence: `package.json:8-24`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3349-3369`, `src/js/lib/cachePolicy.js:16-20`,
  `src/js/lib/model.js:12` and `src/js/lib/model.js:821-897`.
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:979-1010`, `src/js/main.js:3341-3347`. |
| Schema migration | No gap. Stored state carries `SCHEMA_VERSION`, migrations run forward, and a future schema is refused rather than silently coerced, with a test pinning that behaviour. Evidence: `src/js/lib/model.js:12`, `src/js/lib/model.js:821-897`, `test/model.test.js:559-561`. |
| Observability | Partial gap, bounded by Repository Constraint 3. Product analytics are forbidden and are not proposed. What is missing is local and private: there is no way for the reader to see why hydration stalled beyond a queue-depth pill. Evidence: `src/js/main.js:3651-3654`. |
| Performance | Gap, measured. See characteristic 2. Evidence: `docs/ux-artifacts/render-cost.json`. |
| Security, OWASP Top 10 | Gap under A05 Security Misconfiguration: no CSP and no `x-frame-options` on the dev server. Evidence: `server.mjs:129-139`. Resolved: `BL-030` shipped both, assembled at `server.mjs:53-64` and sent at `server.mjs:134` and `server.mjs:137`. Partial gap under A10 Server-Side Request Forgery by analogy: `MarvelApi` accepted any base URL and only stripped trailing slashes, with the https-or-local check living in the settings form rather than in the client. Resolved: `BL-045` moved the rule into the constructor at `src/js/api.js:25-40` and onto the read out of storage at `src/js/main.js:465-489`, so a base the rule refuses cannot reach a fetch from any of the three call sites, and the form keeps its own message at `src/js/main.js:3352-3353`. A01, A02, A03, A07 and A09 are not applicable, because there is no server-side authorisation boundary, no credential store, no server-side query language, no account system and no central log to protect. |
| Privacy | No gap. Nothing is uploaded, there is no account and there is no telemetry, which is the product promise itself. Evidence: `package.json:1-34` (no dependency that could exfiltrate), `absent: analytics|telemetry|gtag|beacon, grep across src/ and scripts/`. |
...
| Release and versioning | Gap. Version is pinned at `0.1.0` with no tags and no changelog, so there is no way to say which build a backup or a bug report came from. Evidence: `package.json:3`, `absent: CHANGELOG.md and git tags, glob of repository root and git tag --list`. Resolved: `BL-043` set the version to `1.0.0` at `package.json:3`, added `CHANGELOG.md`, and wired a `version` script at `package.json:24` that syncs the version the app reports. |
| Dependency management | Not applicable, because runtime dependencies are zero by Repository Constraint 4, there are no `devDependencies`, and there is therefore no lockfile and no dependency graph to manage or audit. The repository invokes no package-fetching tool at all. Evidence: `package.json:1-34` (neither a `dependencies` nor a `devDependencies` key), `absent: npx, grep across the repository returning only this appendix's own text`. The absence of dev tooling is recorded as a maintainability and CI gap above rather than counted twice here. Changed since: the "not applicable" verdict no longer holds. `BL-040` added three `devDependencies` at `package.json:29-34` and a tracked `package-lock.json`, and `BL-115` took that to four, so there is now a dev dependency graph to audit even though runtime dependencies remain zero. |
...
stale unnoticed, and that one is already covered: `checkRanks` at `scripts/check-counts.mjs:177-209`
rebuilds every row's rank from the table and fails any `rank N of M` stated against it, and
`checkOrdinalHeadings` at `scripts/check-counts.mjs:222-246` does the same for a rank spelled as a
