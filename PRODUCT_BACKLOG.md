...
Seventy-one items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-075, BL-076, BL-077, BL-078, BL-079, BL-080, BL-081, BL-082, BL-083, BL-085, BL-087, BL-088,
BL-089, BL-095, BL-096, BL-099, BL-100, BL-101, BL-106, BL-112, BL-115 and BL-116.
...
open-source readiness study. Nine of them are still `Ready`: the study records gaps for later
...
detail blocks record what changed. BL-091 is the one the study got wrong: re-measuring it refuted
its premise, so it is `Dropped` and its block records the measurement rather than the fix.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2713-2747` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:18` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2775-2790` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2721` and `src/js/main.js:2730-2735` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2753-2773` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2414-2470`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2478-2547` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2376-2394`, manual issue add at `src/js/main.js:2549-2593` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1534-1554`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:301` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:853-893` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:2952-2986`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:2966-2968` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:1935` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2031-2037`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2092-2097` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2572` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3105-3111` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2123-2146` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:255-268` and `src/js/main.js:357-378` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3381-3401`. The work shipped since has
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3141-3163 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3066-3094 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Ready | PRODUCT_BACKLOG.md:3854-3857 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1620-1647 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Ready | src/js/main.js:3320-3368 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Ready | src/js/main.js:2905 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:357-378 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Ready | src/js/main.js:486-489 |
...
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Ready | src/js/main.js:2876 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:2952-2986 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:233 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:1012 |
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:86 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3381-3395 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3381-3395 |
...
that function is at `src/js/main.js:2627` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2725`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:2966-2968` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:573` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:261-268` rather
...
`src/js/main.js:2027-2037`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:499` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:511` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:1876-1885`, rather than on a list of the
...
`src/js/main.js:1866-1874`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:292` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:301-339` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:273`, and both success paths clear that
...
`src/js/main.js:2912`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1587-1590` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1610-1618` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:2912`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2880-2891`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:357`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:380-384` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:1633-1641`. The usual reason
...
`src/js/main.js:3089`, `src/js/main.js:3089` and `src/js/main.js:3105-3111`. Each replaces the whole
...
`src/js/main.js:309-320`. Moving a message into the modal makes it readable where the reader is
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1371-1425`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:2993-3011`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2559`, so the two kinds cannot be
...
`src/js/main.js:391-415` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:416-429` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:3115-3116`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2027-2037`, drawing on the labels at
`src/js/main.js:2092-2097`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1388-1391`, and `renderRows` asks it at `src/js/main.js:1935`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1383-1387` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:233` reads the pair back out of
...
`src/js/main.js:1841` and the reading rows at `src/js/main.js:2083`, and the reasoning is recorded at
`src/js/main.js:188-192` rather than only
...
`src/js/main.js:1243` carries a comment at `src/js/main.js:1253` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2859`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1086` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:2929` puts
...
`renderRail` at `src/js/main.js:853` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:827` and reaches `renderYours` through `renderHome` at
`src/js/main.js:829`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:3381` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:861`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1006` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2125`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1022`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:877` and `src/js/main.js:1203`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:3414`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:877` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1203`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2425`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1185`. That also retired a latent bug rather than fixing it. The old label said
...
comment, when it comes from the series name at `src/js/main.js:462-463`. Both are re-aimed, and the
...
destructive one: `src/js/main.js:135-147` confirms Start fresh replaces the unreadable data with an
...
that boot report, and the comment now standing in its place at `src/js/main.js:3459-3463` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:104-123`, and until BL-075 took its
...
the client and leaves the limiter alone, at `src/js/main.js:3128-3130`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2011` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:7336-7338` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:7355-7359`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:7170-7172` describes
...
`CHANGELOG.md:1485-1494` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:341-408`, `src/js/main.js:3066-3094`.
...
does not own ordinary writes. Evidence: `src/js/storage.js:290-335`, `src/js/main.js:65-79`.
...
attack. Evidence: `src/js/main.js:3066-3094`, `src/js/lib/model.js:782-891`.
...
`src/js/main.js:1012` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/main.js:1740` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2379` and `src/js/main.js:2395`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2488` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3320-3368`.
...
button at `src/js/main.js:3141-3163` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3148-3162` repaints the banner
...
it on. Evidence: `src/js/main.js:486-489`, `src/js/main.js:492-493`, `src/styles.css:436`.
...
over, for the hero backdrop alone: `src/js/main.js:1740` reads the setting before building the URL,
...
button. Evidence: `src/js/main.js:2876`, `src/js/lib/model.js:116`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:2902-2909`,
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1012`
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4675-4683`.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:2952-2986` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2696` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3381-3401` (renderAll rebuilds
every region), `src/js/main.js:73-78` (store.onChange is wired straight to renderAll),
`src/js/main.js:1907-2090` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2092-2097`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1620-1647` (undo), `src/js/main.js:3105-3111` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3381-3401`, `src/js/main.js:812-829` (showView switches views by
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3113-3133`, `src/js/lib/cachePolicy.js:16-20`,
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2696` and `src/js/main.js:2730-2735`. What is missing is data, in
