...
| 2.4 | BL-008 | P1 | orders and catalog generated by `scripts/vendor-orders.mjs:191-316`, consumed as data with no view change | Done |
...
`scripts/vendor-orders.mjs:47` and `scripts/build-event-order.mjs:207`, and none of them used the
...
takes each list's name from `curated-lists.json`, at `scripts/vendor-orders.mjs:141`, and
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:9476-9478` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:9495-9499`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:9310-9312` describes
...
`CHANGELOG.md:2171-2180` that says so and points at the backlog clause instead. That clause was
...
`scripts/vendor-orders.mjs:216-262`, `scripts/vendor-orders.mjs:315`.
...
`scripts/vendor-orders.mjs:247-253`.
...
two readings had to win: `scripts/vendor-orders.mjs:300-302` warns that a repeated id will collapse on
...
distinct descriptions and 89,558 characters remain recoverable from it. The distinct figures run
below the 798 and 151,840 because the same issue sits in more than one reading order. Rewriting
history would reach it, this repository has never been public and has no forks so the cost of doing
so will never be lower, and publication is the moment that window shuts. That is a decision for the
owner and it is deliberately not taken here.
...
89,558 characters are recoverable from them. The distinct figures sit below BL-130's 798 and 151,840
...
One thing to not misread on the way. `npm run publication` also scans history and reports it clean,
and that report is true of what it looks for, which is personal and credential-shaped content and
the ignore rules that keep session evidence local. It says nothing about third-party prose and was
never meant to. Its own header sets out both halves of its population at
`scripts/check-publication.mjs:7-13`. A green run there is not an answer to this item.
