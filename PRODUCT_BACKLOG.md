...
A hundred and thirteen items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3246-3302` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3330-3345` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3269` and `src/js/main.js:3287-3300` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3308-3328` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2960-3016`, `src/js/lib/markdown.js:36-115` | Done |
...
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2922-2940`, manual issue add at `src/js/main.js:3095-3139` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3512-3546`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3526-3528` | Done, shipped as BL-014 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3118` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3739-3745` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4089-4109`. The work shipped since has
  taken it further: the file is 4,197 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,110; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3788-3825 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3700-3728 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3903-3906 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1396-1397 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4062-4065 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4009-4072 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3469 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3512-3546 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3260-3264 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4089-4103 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4089-4103 |
...
that function is at `src/js/main.js:3173` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3293`, and with the selector corrected the
...
`src/js/main.js:3526-3528` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3472`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3472`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3441-3452`, leaving nothing behind and the offer still
...
`src/js/main.js:3799`, `src/js/main.js:3723` and `src/js/main.js:3739-3745`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3553-3571`.
...
The settings form keeps its own check and its own message at `src/js/main.js:3750-3751`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3414`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3489` puts
...
`src/js/main.js:4089` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4122`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4169-4173` records
...
file of 4,197 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3762-3764`, because the budget belongs to
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:10644-10646` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:10663-10667`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:10478-10480` describes
...
`CHANGELOG.md:2668-2677` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3700-3728`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3260-3264`,
...
attack. Evidence: `src/js/main.js:3700-3728`, `src/js/lib/model.js:965-1074`.
...
`src/index.html:125-126`, `src/js/main.js:4009-4072`.
...
size, at `PRODUCT_BACKLOG.md:2491`, and one of the four tests added to the counts suite pins the
...
button at `src/js/main.js:3788-3825` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3795-3824` repaints the banner
...
button. Evidence: `src/js/main.js:3431-3437`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3463-3469`,
...
here. Evidence: `src/js/main.js:3478`.
...
in place of that button while a copy is live at `src/js/main.js:3912-3918`, which is the state a
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4726-4734`.
...
`forgetDeleted()`, at `src/js/main.js:3799`, `src/js/main.js:3723` and `src/js/main.js:3744`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1396-1397`.
...
**Delivered.** The first citation now names `src/js/main.js:3799`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3788-3825`. The other two were already right and
are unchanged: `src/js/main.js:3723` is the call in the successful branch of the restore handler,
and `src/js/main.js:3739-3745` is the whole undo-restore handler, whose call is at
`src/js/main.js:3744`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3472` is its one call, `src/js/main.js:3441-3452` is the
...
at `PRODUCT_BACKLOG.md:4731-4732`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4726-4729` are the counts of items above
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4062-4065`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4044-4058` keeps
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3512-3546` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3294` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4089-4109` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1968-1995` (undo), `src/js/main.js:3739-3745` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4089-4109`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,197 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3747-3780`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1043-1074`, `src/js/main.js:3739-3745`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3308-3328` (attribution rendered in the UI before import). |
...
- Calculated: WSJF 3.67, rank 59 of 123.
...
- Calculated: WSJF 1.4, rank 117 of 123, below a hundred and thirteen unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3294` and `src/js/main.js:3287-3300`. What is missing is data, in
...
- BL-014, P1, rank 76 of 123. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 61 of 123. Mid-table.
- BL-017, P2, rank 122 of 123. The lowest-ranked scored story other than the one that cannot be
