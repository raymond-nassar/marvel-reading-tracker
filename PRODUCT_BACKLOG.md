...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3262-3318` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3346-3361` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3285` and `src/js/main.js:3303-3316` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3324-3344` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2960-3016`, `src/js/lib/markdown.js:61-140` | Done |
...
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2922-2940`, manual issue add at `src/js/main.js:3095-3155` | Done |
...
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3528-3562`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3542-3544` | Done, shipped as BL-014 |
...
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3125` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3755-3761` | Done |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4105-4125`. The work shipped since has
  taken it further: the file is 4,213 lines today. 1,566 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3804-3841 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3716-3744 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4078-4081 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4025-4088 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3485 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3528-3562 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3276-3280 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4105-4119 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4105-4119 |
...
that function is at `src/js/main.js:3189` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3309`, and with the selector corrected the
...
`src/js/main.js:3542-3544` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:3488`. The same order can be added from the landing page and imported from the
...
about to make at `src/js/main.js:3488`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3457-3468`, leaving nothing behind and the offer still
...
`src/js/main.js:3815`, `src/js/main.js:3739` and `src/js/main.js:3755-3761`. Each replaces the whole
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3569-3587`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:3105`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3766-3767`, unchanged.
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3430`, and disabling a focused
...
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3505` puts
...
`src/js/main.js:4105` calls both on every `store.update` with no navigation at all. Measured on that
...
sequence moved into an exported `boot()` at `src/js/main.js:4138`, and `src/js/app.js` is the entry
...
that boot report, and the comment now standing in its place at `src/js/main.js:4185-4189` records
...
file of 4,213 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3778-3780`, because the budget belongs to
...
`CHANGELOG.md:2692-2701` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3716-3744`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3276-3280`,
...
attack. Evidence: `src/js/main.js:3716-3744`, `src/js/lib/model.js:965-1074`.
...
`src/index.html:125-126`, `src/js/main.js:4025-4088`.
...
button at `src/js/main.js:3804-3841` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3811-3840` repaints the banner
...
button. Evidence: `src/js/main.js:3447-3453`, `src/js/lib/model.js:211`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3479-3485`,
...
here. Evidence: `src/js/main.js:3494`.
...
in place of that button while a copy is live at `src/js/main.js:3928-3934`, which is the state a
...
`forgetDeleted()`, at `src/js/main.js:3815`, `src/js/main.js:3739` and `src/js/main.js:3760`
...
**Delivered.** The first citation now names `src/js/main.js:3815`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3804-3841`. The other two were already right and
are unchanged: `src/js/main.js:3739` is the call in the successful branch of the restore handler,
and `src/js/main.js:3755-3761` is the whole undo-restore handler, whose call is at
`src/js/main.js:3760`. The range was left as the handler rather than narrowed to the call, because
...
`forgetDeletedFor`, `src/js/main.js:3488` is its one call, `src/js/main.js:3457-3468` is the
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4078-4081`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4060-4074` keeps
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3528-3562` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3310` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4105-4125` (renderAll rebuilds
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1968-1995` (undo), `src/js/main.js:3755-3761` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4105-4125`, `src/js/main.js:965-982` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,213 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3763-3796`, `src/js/lib/cachePolicy.js:16-20`,
...
| Data durability and export | No gap. Full JSON backup and restore, per-list Markdown export, validated and atomic restore with an undo. Evidence: `src/js/lib/model.js:1043-1074`, `src/js/main.js:3755-3761`. |
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3324-3344` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3310` and `src/js/main.js:3303-3316`. What is missing is data, in
