...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3525-3586` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3647-3662` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3549` and `src/js/main.js:3567-3584` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3625-3645` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:3026-3082`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3090-3159` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2988-3006`, manual issue add at `src/js/main.js:3275-3395` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1920-1940`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:433` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1048-1088` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3829-3863`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3843-3845` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2407` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2503-2504`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2559-2564` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3352` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:4056-4062` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2590-2613` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:337-350` and `src/js/main.js:467-488` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4406-4426`. The work shipped since has
  taken it further: the file is 4,514 lines today. 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 1,192; 224 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2348-2357 |
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4105-4142 |
...
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:4017-4045 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4379-4382 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:2023-2051 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4326-4389 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3786 |
...
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:467-488 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/main.js:663-667 |
...
| BL-146 | Put a way out of the capped landing grid where the reader runs out of it | Defect | EP-10 | Follows BL-145 | 3 | 1 | 2 | 2 | 3.0 | none | Measured | Shipped | `src/index.html:247`, `src/js/main.js:1245-1252` |
...
| BL-150 | Fill in what the metadata snapshot cannot, from the one catalogue a browser can reach | Feature | EP-03 | Follows BL-149 | 5 | 5 | 5 | 5 | 3.0 | none | Measured | Shipped | `src/js/lib/wikitext.js:38-42`, `src/js/main.js:3317-3329`, `absent: any record newer than 2025-10-29 upstream, measured by asking the metadata service for 2026 and being told the total is zero` |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3829-3863 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1216 |
...
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:594 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3539-3544 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3604-3619` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4406-4420 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4406-4420 |
...
that function is at `src/js/main.js:3429` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3573`, and with the selector corrected the
...
`src/js/main.js:3843-3845` instead of asserting "every list" whatever is being counted.
...
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:760` and
...
`src/js/main.js:2499-2504`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/js/main.js:682` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:698` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:2282-2297`, rather than on a list of the
...
`src/js/main.js:2272-2280`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3789`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1990-1993` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:2013-2021` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3789`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3758-3769`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:467`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:490-498` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:2036-2045`. The usual reason
...
`src/js/main.js:4116`, `src/js/main.js:4040` and `src/js/main.js:4056-4062`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1757-1811`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3870-3888`.
...
synthetic id from the clock, per `src/js/main.js:3286`, so the two kinds cannot be ordered against
...
`src/js/main.js:505-534` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:559-575` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:4067-4068`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2499-2504`, drawing on the labels at
`src/js/main.js:2559-2564`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1774-1777`, and `renderRows` asks it at `src/js/main.js:2407`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1769-1773` exists because
...
`src/js/main.js:2247` and the reading rows at `src/js/main.js:2550`, and the reasoning is recorded at
...
`src/js/main.js:1594` carries a comment at `src/js/main.js:1604` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3731`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1315` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3806` puts
...
`renderRail` at `src/js/main.js:1048` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:1014` and reaches `renderYours` through `renderHome` at
`src/js/main.js:1016`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4406` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1056`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1210` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2592`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1226`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1072` and `src/js/main.js:1549`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4439`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:1072` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1549`, joined an issue count to an invitation to open the
...
at `src/js/main.js:3037`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1496`. That also retired a latent bug rather than fixing it. The old label said
...
comment, when it comes from the series name at `src/js/main.js:636-637`. Both are re-aimed, and the
...
that boot report, and the comment now standing in its place at `src/js/main.js:4486-4490` records
...
file of 4,514 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:4079-4081`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2483` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11283-11285` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11302-11306`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:11117-11119` describes
...
`CHANGELOG.md:2808-2817` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:4017-4045`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3539-3544`,
...
attack. Evidence: `src/js/main.js:4017-4045`, `src/js/lib/model.js:968-1077`.
...
`src/js/main.js:1216` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/main.js:2145` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2991` and `src/js/main.js:3007`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3100` is named alongside the search it resembles, and the
...
`src/index.html:125-126`, `src/js/main.js:4326-4389`.
...
`src/js/main.js:467-477`, which routes to the announcer precisely when the container is not itself
...
The message policy is a pure function at `src/js/main.js:2646-2655` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2976` and then starts
the hydrator at `src/js/main.js:2983`, whose `start()` reports its running phase at
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1782-1783`
...
button at `src/js/main.js:4105-4142` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4112-4141` repaints the banner
...
it on. Evidence: `src/js/main.js:675-676`, `src/js/main.js:672`, `src/styles.css:453`.
...
over, for the hero backdrop alone: `src/js/main.js:2145` reads the setting before building the URL,
...
`src/js/main.js:2296`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3748-3754`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3780-3786`,
...
here. Evidence: `src/js/main.js:3795`.
...
in place of that button while a copy is live at `src/js/main.js:4229-4235`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1216`
...
`forgetDeleted()`, at `src/js/main.js:4116`, `src/js/main.js:4040` and `src/js/main.js:4061`
...
**Delivered.** The first citation now names `src/js/main.js:4116`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4105-4142`. The other two were already right and
are unchanged: `src/js/main.js:4040` is the call in the successful branch of the restore handler,
and `src/js/main.js:4056-4062` is the whole undo-restore handler, whose call is at
`src/js/main.js:4061`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1990-1993` is `forgetDeleted` itself, `src/js/main.js:2013-2021` is
`forgetDeletedFor`, `src/js/main.js:3789` is its one call, `src/js/main.js:3758-3769` is the
half-import rollback, `src/js/main.js:467` is `notify` with its `action` parameter,
`src/js/main.js:490-498` is `spoken`, `src/js/main.js:2036-2045` is the failed undo keeping its
...
`src/js/main.js:2016`, `src/js/main.js:2032` and `src/js/main.js:2048`. The sentence is right to
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4379-4382`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4361-4375` keeps
...
consumer at `src/js/main.js:2164`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2348-2357` returns a sentence saying no synopsis is
...
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:588` would have recorded
...
The disclaimer at `src/js/main.js:2695` is shown every time a run starts rather than once and
...
The one-time purge at `src/js/main.js:594` empties the response cache once, so the prose cached
...
All four messages under that key got the control, not just the offer. Three of them report something
that has already happened and offer nothing to do about it, so they are the ones a reader is most
likely to want gone, and they were the ones with no button at all. They are raised from a single
shared record rather than written out four times, and a test reads the source for every `notify` call
carrying that key and fails on any raised without it, because the defect was never in one message: it
was in the key, and a fifth message added later with no way out puts the reader back where they
started.
...
Ten tests were added, and each was checked against a tree without its fix. Six single-line reverts
were made in turn and every one turned a named test red: raising the offer without the control, and
raising the three withdrawals without it, each fail the source scan; drawing no control at all fails
four rendering tests; dropping `dismiss` from the spoken message fails two; restoring the old
layout rule leaves a dismiss-only notice without the flex row that makes space for a button; and
closing the message without spending the undo fails the test written for exactly that.
...
assertions, all passing, and they cover the two things the unit tests cannot see, which are that the
message really does survive a change of screen and that focus lands on the heading of the screen
being read rather than on `<body>`. Two mutations prove the scenario can fail. Stripping the second
control reproduces the notice exactly as it shipped and reddens three named assertions; making the
button hide the paragraph without spending the undo, which is the tempting shape of this fix, reddens
exactly one, the assertion that the closed message does not come back on the next screen.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3829-3863` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3574` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4406-4426` (renderAll rebuilds
every region), `src/js/main.js:77-82` (store.onChange is wired straight to renderAll),
`src/js/main.js:2379-2557` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2559-2564`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:2023-2051` (undo), `src/js/main.js:4056-4062` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4406-4426`, `src/js/main.js:999-1016` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,514 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4064-4097`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3625-3645` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3574` and `src/js/main.js:3567-3584`. What is missing is data, in
