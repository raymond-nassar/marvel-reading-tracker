...
Eighty-five items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-105, BL-106, BL-107, BL-108, BL-110, BL-111, BL-112, BL-115, BL-116, BL-121, BL-122 and BL-123.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:2822-2856` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:411-413`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:404-410`, search-as-you-type at `src/js/main.js:2884-2899` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:2830` and `src/js/main.js:2839-2844` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:2862-2882` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:2523-2579`, `src/js/lib/markdown.js:36-115` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:2587-2656` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:2485-2503`, manual issue add at `src/js/main.js:2658-2702` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1621-1641`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:352` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:940-980` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3060-3094`, with a scope control at `src/index.html:371-375` and a subtitle that stops claiming every list at `src/js/main.js:3074-3076` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-48`, applied without touching stored order at `src/js/main.js:2028` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2124-2130`, pending filter at `src/js/lib/readingFilters.js:46` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2185-2190` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:2681` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:3213-3219` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2216-2239` | Done, shipped as BL-026 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last
  line number when reading the file. Evidence: `src/js/main.js:3500-3520`. The work shipped since has
  taken it to 3,606; 1,566 is the figure as audited.
...
* The test count is 224 passing, not the 119 recorded in `.copilot-tracking/changes/`. Evidence:
  ...
  it to 827; 224 is the figure as audited.
...
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:3249-3271 |
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:3174-3202 |
...
| BL-107 | Date or re-derive the repeat figures BL-058 states as current | Debt | EP-12 | Extends BL-059 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:3870-3873 |
...
| BL-121 | Point the three erase-and-restore citations at three different lines | Debt | EP-12 | Extends BL-101 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | PRODUCT_BACKLOG.md:1372-1373 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Ready | src/js/main.js:3472-3474 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:1707-1734 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3455-3483 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3017 |
...
| BL-109 | Tell an issue upstream refused apart from one nobody has asked about | Defect | EP-05 | Extends BL-018 | 5 | 3 | 8 | 5 | 3.2 | none | Measured | Ready | src/js/main.js:2985 |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3060-3094 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:1099 |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:2825-2829 |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:3500-3514 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:3500-3514 |
...
that function is at `src/js/main.js:2736` today, with the rename-list handler occupying the lines the
...
written at `src/js/main.js:2834`, and with the selector corrected the same mutation fails it. It is
...
`src/js/main.js:3074-3076` instead of asserting "every list" whatever is being counted.
...
`src/js/main.js:2120-2130`, along with the pending badge's. A `title` is unreachable by touch and
...
Reuse is keyed on the whole item, at `src/js/main.js:1963-1978`, rather than on a list of the
...
`src/js/main.js:1953-1961`. Doing it the other way round leaves the stale node in front of the
...
`src/js/main.js:3020`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:1674-1677` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:1697-1705` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3020`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:2989-3000`, leaving nothing behind and the offer still
...
A failed undo keeps both the buffer and the button, at `src/js/main.js:1720-1728`. The usual reason
...
`src/js/main.js:3260`, `src/js/main.js:3197` and `src/js/main.js:3213-3219`. Each replaces the whole
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1458-1512`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3101-3119`.
...
gets a negative synthetic id from the clock, per `src/js/main.js:2668`, so the two kinds cannot be
...
The settings form keeps its own check and its own message at `src/js/main.js:3223-3224`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2120-2130`, drawing on the labels at
`src/js/main.js:2185-2190`, and the full sentence from `describe` sits in a visually-hidden span
...
`src/js/main.js:1475-1478`, and `renderRows` asks it at `src/js/main.js:2028`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1470-1474` exists because
...
`src/js/main.js:1928` and the reading rows at `src/js/main.js:2176`, and the reasoning is recorded at
...
`src/js/main.js:1330` carries a comment at `src/js/main.js:1340` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:2968`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1173` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3037` puts
...
`renderRail` at `src/js/main.js:940` is the other half, and it is the easier one: its mechanism is
...
`src/js/main.js:3500` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:948`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1093` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2218`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1109`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:964` and `src/js/main.js:1290`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:3533`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:964` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1290`, joined an issue count to an invitation to open the
...
at `src/js/main.js:2534`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1272`. That also retired a latent bug rather than fixing it. The old label said
...
that boot report, and the comment now standing in its place at `src/js/main.js:3578-3582` records
...
file of 3,606 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
...
the client and leaves the limiter alone, at `src/js/main.js:3236-3238`, because the budget belongs to
...
glyph that no one reads, since the button's label at `src/js/main.js:2104` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:8441-8443` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:8460-8464`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:8275-8277` describes
...
`CHANGELOG.md:1790-1799` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:3174-3202`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:2825-2829`,
...
attack. Evidence: `src/js/main.js:3174-3202`, `src/js/lib/model.js:833-942`.
...
`src/js/main.js:1099` and again on the rail, because neither render is bounded by anything. That is a
...
`src/js/main.js:1827` builds the reading hero's backdrop URL only when covers are on, and a computed
...
requests, from `src/js/main.js:2488` and `src/js/main.js:2504`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:2597` is named alongside the search it resembles, and the
...
`src/index.html:121-122`, `src/js/main.js:3455-3483`.
...
The message policy is a pure function at `src/js/main.js:2272-2281` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2473` and then starts
the hydrator at `src/js/main.js:2480`, whose `start()` reports its running phase at
...
size, at `PRODUCT_BACKLOG.md:2459`, and one of the four tests added to the counts suite pins the
...
button at `src/js/main.js:3249-3271` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:3256-3270` repaints the banner
...
over, for the hero backdrop alone: `src/js/main.js:1827` reads the setting before building the URL,
...
`src/js/main.js:1977`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:2985`, `src/js/lib/model.js:167`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3011-3017`,
...
here. Evidence: `src/js/main.js:3026`.
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1099`
...
wrong. Evidence: `PRODUCT_BACKLOG.md:4693-4701`.
...
`forgetDeleted()`, at `src/js/main.js:3260`, `src/js/main.js:3197` and `src/js/main.js:3218`
respectively, but the sentence cited the restore path twice and the erase path not at all. Evidence:
`PRODUCT_BACKLOG.md:1372-1373`.
...
**Delivered.** The first citation now names `src/js/main.js:3260`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:3249-3271`. The other two were already right and
are unchanged: `src/js/main.js:3197` is the call in the successful branch of the restore handler,
and `src/js/main.js:3213-3219` is the whole undo-restore handler, whose call is at
`src/js/main.js:3218`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:1674-1677` is `forgetDeleted` itself, `src/js/main.js:1697-1705` is
`forgetDeletedFor`, `src/js/main.js:3020` is its one call, `src/js/main.js:2989-3000` is the
half-import rollback, `src/js/main.js:431` is `notify` with its `action` parameter,
`src/js/main.js:454-458` is `spoken`, `src/js/main.js:1720-1728` is the failed undo keeping its
...
`src/js/main.js:1700`, `src/js/main.js:1716` and `src/js/main.js:1731`. The sentence is right to
...
at `PRODUCT_BACKLOG.md:4698-4699`.
...
the two shapes it named as unchecked at `PRODUCT_BACKLOG.md:4693-4696` are the counts of items above
...
faults are on one line at `src/js/main.js:3472-3474`.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3060-3094` (renderProgress, which now takes a scope), `src/index.html:369`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:2805` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:3500-3520` (renderAll rebuilds
every region), `src/js/main.js:74-79` (store.onChange is wired straight to renderAll),
`src/js/main.js:2000-2183` (renderRows, which then built every row with no virtualisation and no
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2185-2190`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:1707-1734` (undo), `src/js/main.js:3213-3219` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and
  ...
  Evidence: `src/js/main.js:3500-3520`, `src/js/main.js:899-916` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 3,606 lines now, so nearly every item shipped
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:3221-3241`, `src/js/lib/cachePolicy.js:16-20`,
...
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:2862-2882` (attribution rendered in the UI before import). |
...
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:2805` and `src/js/main.js:2839-2844`. What is missing is data, in
