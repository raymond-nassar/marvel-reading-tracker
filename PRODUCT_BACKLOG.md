...
A hundred and twenty items have since been delivered and are marked `Shipped` in the table below: BL-007,
...
BL-145, BL-146, BL-147, BL-149, BL-150, BL-152, BL-153, BL-154, BL-157 and BL-158.
...
|-------|-----|-------------------|-------------------|-------------|
| 1.1 | BL-001 | P0 | catalogRow renders name, type, count and description at `src/js/main.js:3543-3604` | Done |
| 1.2 | BL-002 | P0 | catalog filter fieldset at `src/index.html:425-427`, filterByFacet imported at `src/js/main.js:19` | Done |
| 1.3 | BL-003 | P1 | search form at `src/index.html:418-424`, search-as-you-type at `src/js/main.js:3665-3680` | Done |
| 1.4 | BL-004 | P1 | depth pill rendered before the Import button at `src/js/main.js:3567` and `src/js/main.js:3585-3602` | Done |
...
| 2.2 | BL-006 | P0 | attributionLine renders source and snapshot date at `src/js/main.js:3643-3663` | Done |
...
| 3.1 | BL-009 | P0 | parseChecklist and the import report at `src/js/main.js:3044-3100`, `src/js/lib/markdown.js:70-149` | Done |
| 3.2 | BL-010 | P1 | unresolvedRow offers search, auto-accepts a unique exact match, else lists candidates with series and date at `src/js/main.js:3108-3177` | Done |
| 3.3 | BL-011 | P1 | series and creator adds at `src/js/main.js:3006-3024`, manual issue add at `src/js/main.js:3293-3413` | Done |
| 3.4 | BL-012 | P2 | duplicate at `src/js/main.js:1929-1949`, with read progress deliberately shared rather than copied per `src/js/lib/model.js:433` | Done |
| 4.1 | BL-013 | P0 | renderRail marks the active list with `aria-current` and a progress bar at `src/js/main.js:1057-1097` | Done |
| 4.2 | BL-014 | P1 | the progress view is scoped to the active list at `src/js/main.js:3847-3881`, with a scope control at `src/index.html:385-389` and a subtitle that stops claiming every list at `src/js/main.js:3861-3863` | Done, shipped as BL-014 |
| 4.3 | BL-015 | P1 | all four named filters plus All at `src/js/lib/readingFilters.js:25-50`, applied without touching stored order at `src/js/main.js:2425` | Done |
...
| 5.1 | BL-018 | P0 | pending and by-hand badges at `src/js/main.js:2521-2522`, pending filter at `src/js/lib/readingFilters.js:48` | Done |
| 5.2 | BL-019 | P1 | five-state availability model at `src/js/lib/availability.js:17-23`, hedged short labels at `src/js/main.js:2577-2582` | Done |
| 5.3 | BL-020 | P1 | manual entries carry `source: 'manual'` at `src/js/main.js:3370` and render, reorder, export and back up like any other issue | Done |
...
| 6.2 | BL-023 | P1 | same backup file restores on another browser, validated and atomic, with undo at `src/js/main.js:4074-4080` | Done |
...
| 7.1 | BL-026 | P0 | focus order and visible focus pass across 45 measured tab stops in `docs/ux-artifacts/live-inspection.json`, and the shortcut handler now stands down for text entry and for an open dialog at `src/js/main.js:2608-2631` | Done, shipped as BL-026 |
| 7.2 | BL-027 | P1 | announcements fired into two live regions at once and the first-run heading was empty per `docs/ux-artifacts/pa11y-landing.json`; both are fixed, and `notify` now picks one channel at `src/js/main.js:337-350` and `src/js/main.js:476-497` | Done, shipped as BL-027 |
...
* `src/js/main.js` is 1,566 lines, not 1,543, by `(Get-Content).Count` and confirmed by the last <!-- sizes:frozen -->
  line number when reading the file. Evidence: `src/js/main.js:4424-4444`. The work shipped since has
  taken it further: the file is 4,532 lines today. 1,566 is the figure as audited.
...
| BL-130 | Shrink the open licence question before publication | Enabler | EP-12 | Extends BL-099 | 8 | 8 | 13 | 3 | 9.67 | none | Measured | Shipped | src/js/main.js:2366-2375 |
...
| BL-091 | Let catalog descriptions survive the WCAG text-spacing override | Defect | EP-07 | Leaves BL-028 alone | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Dropped | src/styles.css:860-863 |
| BL-101 | Withdraw the undo-restore offer when erasing everything | Defect | EP-06 | Follows BL-083 | 5 | 3 | 3 | 2 | 5.5 | none | Measured | Shipped | src/js/main.js:4123-4160 |
...
| BL-085 | Bound backup restore before parsing and persistence | Enabler | EP-06 | Extends BL-022 | 5 | 3 | 8 | 3 | 5.33 | none | Measured | Shipped | src/js/main.js:4035-4063 |
...
| BL-124 | Stop the rate-limit backoff saying the same wait twice, and say it grammatically | Defect | EP-07 | Extends BL-090 | 2 | 1 | 2 | 1 | 5.0 | none | Measured | Shipped | src/js/main.js:4397-4400 |
...
| BL-035 | Offer an undo after a list is deleted | Story | EP-11 | Leaves alone | 5 | 2 | 5 | 3 | 4.0 | none | Observed | Shipped | src/js/main.js:2040-2069 |
...
| BL-090 | Announce passive service, cache and hydration status changes once | Defect | EP-07 | Extends BL-027 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:4344-4407 |
...
| BL-110 | Count the issues an order imported empty, and say so | Defect | EP-05 | Extends BL-009 | 3 | 2 | 3 | 2 | 4.0 | none | Measured | Shipped | src/js/main.js:3804 |
...
| BL-097 | Publish contribution, conduct, support and maintainer governance | Chore | EP-12 | Extends BL-052 | 3 | 3 | 5 | 3 | 3.67 | none | Observed | Shipped | README.md:399-466 |
| BL-027 | Announce each change once, in a way a screen reader can use | Story | EP-07 | Leaves alone | 5 | 3 | 3 | 3 | 3.67 | P1 | Measured | Shipped | src/js/main.js:476-497 |
...
| BL-108 | Make the cover art switch stop the cover requests it hides | Defect | EP-05 | Follows BL-087 | 3 | 2 | 5 | 3 | 3.33 | none | Measured | Shipped | src/js/main.js:672-676 |
...
| BL-146 | Put a way out of the capped landing grid where the reader runs out of it | Defect | EP-10 | Follows BL-145 | 3 | 1 | 2 | 2 | 3.0 | none | Measured | Shipped | `src/index.html:247`, `src/js/main.js:1254-1261` |
...
| BL-150 | Fill in what the metadata snapshot cannot, from the one catalogue a browser can reach | Feature | EP-03 | Follows BL-149 | 5 | 5 | 5 | 5 | 3.0 | none | Measured | Shipped | `src/js/lib/wikitext.js:38-42`, `src/js/main.js:3335-3347`, `absent: any record newer than 2025-10-29 upstream, measured by asking the metadata service for 2026 and being told the total is zero` |
...
| BL-014 | Count series progress for the list being read | Story | EP-04 | Leaves alone | 5 | 2 | 2 | 3 | 3.0 | P1 | Observed | Shipped | src/js/main.js:3847-3881 |
...
| BL-072 | Give the recovery banner's two actions different weights | Story | EP-11 | Leaves alone | 3 | 3 | 2 | 2 | 4.0 | none | Measured | Shipped | src/styles.css:1013 |
...
| BL-054 | Put focus back where it was when the shelf and the full order rebuild | Debt | EP-07 | Leaves alone | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:255 |
...
| BL-114 | Bound what a single render paints, not just what a restore builds | Enabler | EP-06 | Follows BL-085 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Shipped | src/js/main.js:1225 |
...
| BL-137 | Close the window a tab left open across the one-time purge is writing into | Debt | EP-06 | Follows BL-134 | 3 | 2 | 3 | 3 | 2.67 | none | Measured | Ready | src/js/main.js:603 |
...
| BL-037 | Keep the chosen filter across a reload | Story | EP-10 | Leaves alone | 3 | 1 | 1 | 2 | 2.5 | none | Observed | Shipped | src/js/main.js:108 |
...
| BL-159 | Add reading orders compiled from Comic Book Reading Orders guides | Feature | EP-01 | Follows BL-142 | 8 | 2 | 2 | 5 | 2.4 | none | Observed | Ready | absent: any list from comicbookreadingorders.com in src/data/catalog.json, read of all 26 entries and their sourceOrigin fields |
...
| BL-119 | Show an order's gap on the catalog row, before it is imported | Story | EP-05 | Follows BL-110 | 3 | 1 | 2 | 3 | 2.0 | none | Measured | Ready | src/js/main.js:3557-3562 |
...
| BL-155 | Draw the reading path as a path, instead of a badge on each row of a list | Story | EP-01 | Follows BL-152 | 5 | 1 | 3 | 5 | 1.8 | none | Measured | Ready | `src/js/main.js:3622-3637` |
...
| BL-033 | Re-render only what changed when an issue is marked read | Debt | EP-09 | Leaves alone | 5 | 2 | 5 | 8 | 1.5 | none | Measured | Shipped | src/js/main.js:4424-4438 |
...
| BL-042 | Break the single view file into per-view modules | Debt | EP-12 | Leaves alone | 2 | 1 | 8 | 20 | 0.55 | none | Measured | Proposed | src/js/main.js:4424-4438 |
...
that function is at `src/js/main.js:3447` today, with the rename-list handler occupying the lines the
...
real title is `.result-title`, written at `src/js/main.js:3591`, and with the selector corrected the
...
`src/js/main.js:3861-3863` instead of asserting "every list" whatever is being counted.
...
The About view carries the shortcut reference at `src/index.html:658-665`, naming all three
bindings. Only two of them were advertised in the interface, on the hero's `kbd` hints; `Ctrl` +
`\` for the sidebar was written into the toggle button's tooltip at `src/js/main.js:769` and
...
Which channel to use is now decided by reading the container at `src/js/main.js:343-350` rather
...
`src/js/main.js:2517-2522`, along with the pending badge's. A `title` is unreachable by touch and
...
`src/index.html:788`, so resolving the theme in JavaScript would paint dark and then flip, and
...
`src/js/main.js:691` rewrites to "Cover art on" or "Cover art off" and `src/js/main.js:707` announces,
...
Reuse is keyed on the whole item, at `src/js/main.js:2300-2315`, rather than on a list of the
...
`src/js/main.js:2290-2298`. Doing it the other way round leaves the stale node in front of the
...
outstanding notice is kept as a record at `src/js/main.js:374` rather than only as a paragraph in
the page, and `placeNotices()` at `src/js/main.js:383-423` renders every outstanding record into
...
report. Both sites key it `catalog-load` at `src/js/main.js:355`, and both success paths clear that
...
`src/js/main.js:3807`. The same order can be added from the landing page and imported from the
...
making sense: `forgetDeleted()` at `src/js/main.js:2007-2010` runs when the whole state is replaced, by
a backup restore, by undoing one, or by erasing everything, and `forgetDeletedFor()` at
`src/js/main.js:2030-2038` runs when the deleted list's own order is imported again. An offer the reader
...
about to make at `src/js/main.js:3807`, because two announcements in one tick leave only the last.
...
rolls the half-import back at `src/js/main.js:3776-3787`, leaving nothing behind and the offer still
...
`notify` gained an `action` at `src/js/main.js:476`, which puts a button inside the notice rather
...
`spoken()` at `src/js/main.js:499-507` appends "Undo delete is available." A button that is never
spoken is a button a screen-reader user cannot know to look for.

A failed undo keeps both the buffer and the button, at `src/js/main.js:2053-2063`. The usual reason
...
`src/js/main.js:4134`, `src/js/main.js:4058` and `src/js/main.js:4074-4080`. Each replaces the whole
...
`src/js/main.js:391-404`. Moving a message into the modal makes it readable where the reader is
...
- **The address is synced from `renderAll`, not only from `showView`.** The active list is changed
  ...
  every change at `src/js/main.js:77-82`. Syncing anywhere else would have left the address naming
...
other settings" names, and is restored in `wireReading` at `src/js/main.js:1766-1820`, before the
...
in `src/js/lib/library.js:27-51` and rendered by a single function at `src/js/main.js:3888-3906`.
...
synthetic id from the clock, per `src/js/main.js:3304`, so the two kinds cannot be ordered against
...
`src/js/main.js:514-543` now applies the same rule on the way out of storage and falls back to the
...
hold the old value. `saveSettings` at `src/js/main.js:568-584` now writes the refused value back in
...
The settings form keeps its own check and its own message at `src/js/main.js:4085-4086`, unchanged.
...
is written out in words inside the pill at `src/js/main.js:2517-2522`, drawing on the labels at
`src/js/main.js:2577-2582`, and the full sentence from `describe` sits in a visually-hidden span
...
now three, at `README.md:688-699`, and the four sentences over 40 words are now none: the longest
...
`README.md:558-562`, as fetching a list once and committing what came back. `depth` gained a gloss
...
`src/js/main.js:1783-1786`, and `renderRows` asks it at `src/js/main.js:2425`. "In the markup but
...
it, naming the file to add the filter to. That guard at `src/js/main.js:1778-1782` exists because
...
control the two lists build, and `preservingFocus` at `src/js/main.js:255` reads the pair back out of
...
`src/js/main.js:2265` and the reading rows at `src/js/main.js:2568`, and the reasoning is recorded at
`src/js/main.js:210-214` rather than only
...
`src/js/main.js:1603` carries a comment at `src/js/main.js:1613` saying adding must not move the
...
drop in. `importCurated` sets `btn.disabled = true` at `src/js/main.js:3749`, and disabling a focused
control blurs it there and then, before any rebuild has run. By the time `renderHomeCatalog` reaches
`grid.replaceChildren` at `src/js/main.js:1324` there is nothing left to preserve, so a wrapper that
reads focus at rebuild time reads `BODY` and correctly declines. The grid is then rebuilt a second
time after a 1500 ms `setTimeout`, and the re-enable in its `finally` at `src/js/main.js:3824` puts
...
`renderRail` at `src/js/main.js:1057` is the other half, and it is the easier one: its mechanism is
...
`renderRail` at `src/js/main.js:1023` and reaches `renderYours` through `renderHome` at
`src/js/main.js:1025`, where the reader is being moved anyway, but `renderAll` at
`src/js/main.js:4424` calls both on every `store.update` with no navigation at all. Measured on that
...
`document.activeElement` at `BODY`. Only `#list-nav` is rebuilt, at `src/js/main.js:1065`, so the
loss is scoped to the per-order entries.

`renderYours` at `src/js/main.js:1219` is a different answer, and the reason is a guard rather than a
call site. The shortcut handler returns unless the read view is showing, at `src/js/main.js:2610`,
and `#home-yours` sits inside the home view, which is hidden exactly then. Its buttons navigate on
click at `src/js/main.js:1235`. There is no route that rebuilds it while it holds focus, so it is
...
`src/js/main.js:1081` and `src/js/main.js:1558`. Both predate this change and neither is part of its
...
sequence moved into an exported `boot()` at `src/js/main.js:4457`, and `src/js/app.js` is the entry
...
built at `src/js/main.js:1081` and joined the list's name to its progress with one; the preview button
on every catalog card, at `src/js/main.js:1558`, joined an issue count to an invitation to open the
...
at `src/js/main.js:3055`. A reader who imports one of this repository's own checklists therefore saw
...
`src/js/main.js:1505`. That also retired a latent bug rather than fixing it. The old label said
...
comment, when it comes from the series name at `src/js/main.js:645-646`. Both are re-aimed, and the
...
destructive one: `src/js/main.js:157-169` confirms Start fresh replaces the unreadable data with an
...
confirm dialog at `src/index.html:782`. The convention there is the same shape as this: the action
...
that boot report, and the comment now standing in its place at `src/js/main.js:4504-4508` records
...
`#blocked-why` is repainted on every render at `src/js/main.js:126-145`, and until BL-075 took its
...
file of 4,532 lines wires a store, a metadata client, a rate limiter, a response cache and a hydrator
together at `src/js/main.js:70-83`, and behind it sit sixteen library modules, none of which holds
...
the client and leaves the limiter alone, at `src/js/main.js:4097-4099`, because the budget belongs to
...
`src/js/storage.js:9-12`, two more belong to the view file at `src/js/main.js:41-42`, and the
...
the one change callback at `src/js/main.js:77-82`, so nothing left on screen said why saving was
paused.
...
background is `color-mix(in srgb, var(--warn) 12%, var(--panel))` at `src/styles.css:989`. The choice
...
glyph that no one reads, since the button's label at `src/js/main.js:2501` reads "Mark X as unread"
...
still. The maintainability gap at `PRODUCT_BACKLOG.md:11346-11348` uses that size as the argument for
...
`Resolved:` line rather than editing the bullet it resolves, at `PRODUCT_BACKLOG.md:11365-11369`.
Overwriting would have destroyed the audit trail these sections exist to keep.

So the audited figures stand and each now carries its drift. Two of the three statements were
treated as live and one was not. The outcome narrative at `PRODUCT_BACKLOG.md:11180-11182` describes
...
`CHANGELOG.md:2837-2846` that says so and points at the backlog clause instead. That clause was
...
`src/js/storage.js:495-571`, `src/js/main.js:4035-4063`.
...
proxy for it.
Evidence: `src/js/storage.js:426-450`, `src/js/main.js:90-101`.
...
already represents, which is why the size is 3 rather than 2. Evidence: `src/js/main.js:3557-3562`,
...
attack. Evidence: `src/js/main.js:4035-4063`, `src/js/lib/model.js:968-1077`.
...
`src/js/main.js:1225` and again on the rail, because neither render is bounded by anything. That is a
...
remain local or absent. Evidence: `README.md:95-114`, `src/index.html:529-587`,
...
`src/js/main.js:2163` builds the reading hero's backdrop URL only when covers are on, and a computed
...
at once. The About view's "Metadata and links only" card, at `src/index.html:632-636`, is the sixth,
...
requests, from `src/js/main.js:3009` and `src/js/main.js:3025`. Two of the three said in the same
...
line's "Find match" at `src/js/main.js:3118` is named alongside the search it resembles, and the
...
`test/ci-supply-chain.test.js:124-142`, `README.md:516-551`.
...
`package.json:27-37`, `package-lock.json:1-22`, `absent: .github/dependabot.yml, tracked-file inventory`.
...
`src/index.html:125-126`, `src/js/main.js:4344-4407`.
...
`src/js/main.js:476-486`, which routes to the announcer precisely when the container is not itself
...
rendered text at `src/js/main.js:318-325`, because both surfaces are rewritten far more often than
they change: `checkHealth` runs at boot and again on every API URL save, and `renderHydration` is
called once per issue fetched. Seeding the health key as reachable at `src/js/main.js:331-333` is
...
The message policy is a pure function at `src/js/main.js:2664-2673` so a whole run can be replayed
...
every add and every import: `addToActive` confirms the add at `src/js/main.js:2994` and then starts
the hydrator at `src/js/main.js:3001`, whose `start()` reports its running phase at
...
`src/js/main.js:273-288`, because two messages produced by one action are one thing to say. Four of
...
The full reading order sits inside a `<details>` that starts closed, and `src/js/main.js:1791-1792`
...
are valuable history and are not a concise public policy. Evidence: `README.md:399-754`,
...
button at `src/js/main.js:4123-4160` and the escape hatch out of the blocked state at
...
the answer is handed to the caller. The screen at `src/js/main.js:4130-4159` repaints the banner
...
it on. Evidence: `src/js/main.js:684-685`, `src/js/main.js:681`, `src/styles.css:453`.
...
over, for the hero backdrop alone: `src/js/main.js:2163` reads the setting before building the URL,
...
`src/js/main.js:2314`, and `test/render-rows.test.js` proves the two keys differ.
...
button. Evidence: `src/js/main.js:3766-3772`, `src/js/lib/model.js:212`,
...
report the one kind this repository does not currently have. Evidence: `src/js/main.js:3798-3804`,
...
here. Evidence: `src/js/main.js:3813`.
...
in place of that button while a copy is live at `src/js/main.js:4247-4253`, which is the state a
...
is that a state which arrives with 250,000 lists is painted one tile per list at `src/js/main.js:1225`
...
config that imports it needs it declared. Evidence: `package.json:33-36`, `eslint.config.mjs:13`.
...
`forgetDeleted()`, at `src/js/main.js:4134`, `src/js/main.js:4058` and `src/js/main.js:4079`
...
**Delivered.** The first citation now names `src/js/main.js:4134`, which is the `forgetDeleted()`
call inside the erase handler at `src/js/main.js:4123-4160`. The other two were already right and
are unchanged: `src/js/main.js:4058` is the call in the successful branch of the restore handler,
and `src/js/main.js:4074-4080` is the whole undo-restore handler, whose call is at
`src/js/main.js:4079`. The range was left as the handler rather than narrowed to the call, because
...
catalog claim, `src/js/main.js:2007-2010` is `forgetDeleted` itself, `src/js/main.js:2030-2038` is
`forgetDeletedFor`, `src/js/main.js:3807` is its one call, `src/js/main.js:3776-3787` is the
half-import rollback, `src/js/main.js:476` is `notify` with its `action` parameter,
`src/js/main.js:499-507` is `spoken`, `src/js/main.js:2053-2063` is the failed undo keeping its
buffer, `src/js/main.js:391-404` is the placement loop that leaves a notice carrying a control in
...
`src/js/main.js:2033`, `src/js/main.js:2049` and `src/js/main.js:2066`. The sentence is right to
...
faults were on one line, inside `onApiStatus` at `src/js/main.js:4397-4400`.
...
lets the repeat through in both directions. `backoffAnnouncer` at `src/js/main.js:4379-4393` keeps
...
consumer at `src/js/main.js:2182`, and the function behind that line already answers for its
absence: `synopsisFallback` at `src/js/main.js:2366-2375` returns a sentence saying no synopsis is
...
return nothing and swallow its errors, so `purgeStaleCache` at `src/js/main.js:597` would have recorded
...
The disclaimer at `src/js/main.js:2713` is shown every time a run starts rather than once and
...
The one-time purge at `src/js/main.js:603` empties the response cache once, so the prose cached
...
`package.json:29-31` asks for Node 20 or newer, which every current line satisfies, so the floor
...
this repository's own README says so at `README.md:173-175`, where the reader is already told their
...
than merely questioned. `README.md:174-175` already carries a workaround for the milder version of
...
**BL-159: Add reading orders compiled from Comic Book Reading Orders guides**

- [ ] Credit Comic Book Reading Orders on the card of every list built from one of its guides
- [ ] Link each card back to the exact order page the list follows
- [ ] Build nothing from the Marvel Master Reading Order, which the permission excludes by name
- [ ] Build nothing from an order that site keeps behind its Patreon, which it also excludes
- [ ] Record the origin the way the twelve Comic Book Herald lists already record theirs

Constraint gate: checked 1 to 11, none breached. Nothing here adds a runtime dependency, an account
or a stored image; the lists carry an order and a link, as the twelve before them do.

The permission is real and it is quoted verbatim in `docs/DATA_PROVENANCE.md`, which is where the
conditions live rather than here. This item exists because a grant nobody acts on is not worth having
and a grant somebody acts on carelessly is worse than none.

It is scored exactly as BL-142 was, because it is the same work against a different site. The one
difference is the order of events. BL-142 built twelve lists from a guide site and asked afterwards,
and that question is still unanswered. Here the answer came first, which is the better sequence and
is the only reason this can be started without a reviewer.

The two exclusions are the part most likely to be lost, since a later contributor adding one more
order will be reading a catalog file rather than this paragraph. The first needs no judgement: the
excluded order is named, so if the page is the Marvel Master Reading Order it is out. The second
needs a check rather than a memory, because a page that is public today can move behind that site's
Patreon later, and the permission is written in the present tense about what is behind it, not about
what was behind it on the day it was given.

What the permission does not do is worth stating in the same place as what it does. It reaches that
site's selection and arrangement of issues and stops there, so it says nothing about Marvel's
material and closes none of the four open questions in the provenance document. A card claiming a
list is licensed would be wrong even for the lists this item adds.
...
- Gap: series progress is computed across every list at once rather than for the list being read,
  ...
  Evidence: `src/js/main.js:3847-3881` (renderProgress, which now takes a scope), `src/index.html:383`
  (the view's subtitle).
...
- Gap: the catalog carries a variant grouping model, but only the Hickman creator run populates it.
  ...
  `src/js/main.js:3592` (groupCatalog renders variant rows only where a group exists).
...
Evidence: `docs/ux-artifacts/render-cost.json`, `src/js/main.js:4424-4444` (renderAll rebuilds
every region), `src/js/main.js:77-82` (store.onChange is wired straight to renderAll),
`src/js/main.js:2397-2575` (renderRows, which then built every row with no virtualisation and no
...
Evidence: `package.json:1-37` (no `dependencies` key at all, `engines.node >= 20`),
...
- Appropriateness recognisability and learnability: good. Labels are written in plain English and
  the availability wording is careful to hedge. Evidence: `src/js/main.js:2577-2582`.
...
- User error protection: no gap, closed by BL-034 and BL-035. Deleting a list is confirmed in the
  ...
  `src/js/main.js:2040-2069` (undo), `src/js/main.js:4074-4080` (undoRestore exists).
...
- Modularity gap: `src/js/main.js` is 1,566 lines and carries every view, every event handler and <!-- sizes:frozen -->
  ...
  Evidence: `src/js/main.js:4424-4444`, `src/js/main.js:1008-1025` (showView switches views by
  mutating a module-level variable).
  Still open, and wider than audited: the file is 4,532 lines now, so nearly every item shipped
...
- Installability: good, and suited to an app cloned and run by hand. `npm start` runs the server
  with no install step, because there is nothing to install. Evidence: `package.json:8-27`.
...
- Replaceability: good. The metadata API base URL is user-configurable and validated, the cache is
  ...
  Evidence: `src/js/main.js:4082-4115`, `src/js/lib/cachePolicy.js:16-20`,
...
| Observability | Partial gap, bounded by Repository Constraint 3. Product analytics are forbidden and are not proposed. What is missing is local and private: there is no way for the reader to see why hydration stalled beyond a queue-depth pill. Evidence: `src/js/main.js:4397-4400`. |
| Performance | Gap, measured. See characteristic 2. Evidence: `docs/ux-artifacts/render-cost.json`. |
| Security, OWASP Top 10 | Gap under A05 Security Misconfiguration: no CSP and no `x-frame-options` on the dev server. Evidence: `server.mjs:129-139`. Resolved: `BL-030` shipped both, assembled at `server.mjs:53-64` and sent at `server.mjs:134` and `server.mjs:137`. Partial gap under A10 Server-Side Request Forgery by analogy: `MarvelApi` accepted any base URL and only stripped trailing slashes, with the https-or-local check living in the settings form rather than in the client. Resolved: `BL-045` moved the rule into the constructor at `src/js/api.js:52-67` and onto the read out of storage at `src/js/main.js:514-543`, so a base the rule refuses cannot reach a fetch from any of the three call sites, and the form keeps its own message at `src/js/main.js:4085-4086`. A01, A02, A03, A07 and A09 are not applicable, because there is no server-side authorisation boundary, no credential store, no server-side query language, no account system and no central log to protect. |
| Privacy | No gap. Nothing is uploaded, there is no account and there is no telemetry, which is the product promise itself. Evidence: `package.json:1-37` (no dependency that could exfiltrate), `absent: analytics|telemetry|gtag|beacon, grep across src/ and scripts/`. |
...
| Release and versioning | Gap. Version is pinned at `0.1.0` with no tags and no changelog, so there is no way to say which build a backup or a bug report came from. Evidence: `package.json:3`, `absent: CHANGELOG.md and git tags, glob of repository root and git tag --list`. Resolved: `BL-043` set the version at `package.json:3`, to `1.0.0` as audited, added `CHANGELOG.md`, and wired a `version` script at `package.json:27` that syncs the version the app reports. |
| Dependency management | Not applicable, because runtime dependencies are zero by Repository Constraint 4, there are no `devDependencies`, and there is therefore no lockfile and no dependency graph to manage or audit. The repository invokes no package-fetching tool at all. Evidence: `package.json:1-37` (neither a `dependencies` nor a `devDependencies` key), `absent: npx, grep across the repository returning only this appendix's own text`. The absence of dev tooling is recorded as a maintainability and CI gap above rather than counted twice here. Changed since: the "not applicable" verdict no longer holds. `BL-040` added three `devDependencies` at `package.json:32-37` and a tracked `package-lock.json`, and `BL-115` took that to four, so there is now a dev dependency graph to audit even though runtime dependencies remain zero. |
| Licensing | No gap. The project is MIT, and every vendored order records its upstream source and licence rather than absorbing it silently. Evidence: `LICENSE`, `src/data/catalog.json` (`source` and `sourceLicense` per list), `src/js/main.js:3643-3663` (attribution rendered in the UI before import). |
...
### Case 1: BL-026 is labelled P0 but ranks sixty-seventh

- Stated: P0 Foundation, the first keyboard story in the original Epic 7.
- Calculated: WSJF 3.67, rank 67 of 134.
- Driver: job size, not value. Its Cost of Delay of 11 is the tenth highest figure in the backlog.
  It is outranked by sixty-six items, sixty-three of them sized 1, 2 or 3 whose Cost of Delay
...
- Calculated: WSJF 1.4, rank 128 of 134, below a hundred and twenty-four unlabelled items and five places above the
  single P2 story.
- Driver: both sides. Job size is 5, because the work is editorial rather than technical, and value
  ...
  `src/js/main.js:3592` and `src/js/main.js:3585-3602`. What is missing is data, in
...
- BL-014, P1, rank 85 of 134. Mid-table, which is where a P1 belongs.
- BL-027, P1, rank 69 of 134. Mid-table.
- BL-017, P2, rank 133 of 134. The lowest-ranked scored story other than the one that cannot be
