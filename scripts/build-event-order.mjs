...
//
// It also emits a second, shorter checklist per event where one can be derived honestly. See the
// note above `essentialOrder`: it is the main series alone, and it is written only for the events
// whose main series opens the order.
...
//
// A complete event order runs from 20 to 38 issues. A reader who wants the story and not the
// tie-ins is asking for a shorter path, and the catalog can already carry two variants of one
// order: the Hickman pair has shipped that way since the first release. The question is what
// "essential" can mean for an event without this script starting to make the editorial judgements
// the note at the top of the file says it does not make.
//
// It means the main series alone. `main` is already declared for every event and is already
// load-bearing, since `readingOrder` uses it to break same-day ties, so naming it as the spine
// asserts nothing new about the comics.
//
// The catch, and the reason this is a function with a refusal in it rather than a flag on each
// event: for two of the five the main series is not where the story starts. Annihilation is
// preceded by seventeen issues, a prologue and four mini-series that set it up, and King in Black
// by one. Main-only would drop a reader into the middle of a story it claimed was the essential
// path. That is a fact about the publication dates rather than an opinion about the comics, so the
// check is derived from the order itself: the variant is offered only when the main series opens
// the complete order. Where it does not, nothing is written and the script says which event and
// why, because an event quietly missing its short path looks identical to one that never had it.
export function essentialOrder(event, order) {
  if (!order.length) return null;
  if (order[0].seriesId !== event.main) return null;
  const kept = order.filter((item) => item.seriesId === event.main);
  // A one-issue "order" is a one-shot, not a path through a story, and offering it as a variant
  // alongside a complete list would be a choice between a reading order and a single comic.
  return kept.length > 1 ? kept : null;
}

export function essentialFileName(file) {
  return file.replace(/\.md$/, '-essential.md');
}
...
function blurb(event, essential) {
  if (essential) {
    return [
      `It covers ${event.name} itself and nothing else: the main series, without the branded`,
...
function render(event, order, { essential = false } = {}) {
  const lines = [
    `# ${event.name}: ${essential ? 'Main Series Reading Checklist' : 'Issue-by-Issue Reading Checklist'}`,
    '',
    'Generated by `scripts/build-event-order.mjs` from Marvel series metadata, in publication order.',
    ...blurb(event, essential),
    '',
  ];
  let month = null;
...
async function main() {
  ...
  for (const event of targets) {
    ...
    const essential = essentialOrder(event, order);
    ...
    if (dryRun) {
      ...
      reportEssential(event, order, essential, true);
      continue;
    }
    ...
    if (essential) {
      const essentialPath = join(ORDERS_DIR, essentialFileName(event.file));
      const essentialText = render(event, essential, { essential: true });
      let essentialBefore = null;
      try {
        essentialBefore = await readFile(essentialPath, 'utf8');
      } catch {
        /* first run for this variant */
      }
      await writeFile(essentialPath, essentialText, 'utf8');
      const essentialState =
        essentialBefore === null ? 'created' : essentialBefore === essentialText ? 'unchanged' : 'updated';
      console.log(
        `${event.id}: ${essential.length} of those are the main series -> ` +
          `src/data/orders/${essentialFileName(event.file)} (${essentialState})`,
      );
    } else {
      reportEssential(event, order, essential, false);
    }
  }
