...
export const PAIRS = [
  ...
  ['--muted', 'the ghost button on the unreadable-data banner', LARGE, 'the border that sets the start-fresh button apart, at `src/styles.css:944`'],
];
...
export const SURFACES = {
  ...
  'the unreadable-data banner': {
    ...
    fraction: 0.12,
    on: '--panel',
    css: 'color-mix(in srgb, var(--warn) 12%, var(--panel)), at `src/styles.css:920`',
  },
...
export const KNOWN = [
  ...
  // the button takes its accessible name from the `aria-label` at `src/js/main.js:2236`, which
  // replaces the glyph in the name computation, and `aria-pressed` at `src/js/main.js:2235`
