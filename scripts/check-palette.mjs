...
export const PAIRS = [
  ...
  // (`src/styles.css:709` and `src/styles.css:711`), which is 1.93:1 in the dark theme. It is
  ...
  // `.railed .rail-foot .pill` sets `color: transparent` at `src/styles.css:397-398`, so this is
  ...
  // sets its own background at `src/styles.css:336`, a tint over the rail, and the bar at
  // `src/styles.css:340` is its `::before`, so it can only ever land on the tint. It reads 3.44 and
  ...
  ['--muted', 'the ghost button on the unreadable-data banner', LARGE, 'the border that sets the start-fresh button apart, at `src/styles.css:1324`'],
];
...
export const SURFACES = {
  'the selected rail item': {
    ...
    fraction: 20 / 255,
    on: '--rail',
    css: 'rgb(var(--tint-base) / 8%) over the rail, at `src/styles.css:336`',
  },
  'the unreadable-data banner': {
    layer: '--warn',
    fraction: 0.12,
    on: '--panel',
    css: 'color-mix(in srgb, var(--warn) 12%, var(--panel)), at `src/styles.css:1300`',
  },
  ...
  'the ghost button on the unreadable-data banner': {
    ...
    css: 'rgb(var(--tint-base) / 5.882%) over the banner, at `src/styles.css:572`',
  },
};
...
// stated as text beside it, at `src/js/main.js:1296` in the rail and `src/js/main.js:5019` in the
// saved lists.
//
...
export const KNOWN = [
  ...
  // the button takes its accessible name from the `aria-label` at `src/js/main.js:2791`, which
  // replaces the glyph in the name computation, and `aria-pressed` at `src/js/main.js:2791`
