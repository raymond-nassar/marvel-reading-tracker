...
export const PAIRS = [
  ...
  // (`src/styles.css:594` and `src/styles.css:596`), which is 2.30:1 in the dark theme. It is
  ...
  ['--muted', 'the ghost button on the unreadable-data banner', LARGE, 'the border that sets the start-fresh button apart, at `src/styles.css:974`'],
];
...
export const SURFACES = {
  ...
  'the unreadable-data banner': {
    ...
    fraction: 0.12,
    on: '--panel',
    css: 'color-mix(in srgb, var(--warn) 12%, var(--panel)), at `src/styles.css:950`',
  },
...
// stated as text beside it, at `src/js/main.js:1041` in the rail and `src/js/main.js:1182` in the
// saved lists.
//
...
export const KNOWN = [
  ...
  // the button takes its accessible name from the `aria-label` at `src/js/main.js:2396`, which
  // replaces the glyph in the name computation, and `aria-pressed` at `src/js/main.js:2395`
