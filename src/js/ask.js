...
export function wireAsk() {
  ...
  dlg.addEventListener('close', () => {
    ...
    if (settle) settle({ ok: dlg.returnValue === OK, value: input.value });
  });
}
...
function open({ title, body, confirmLabel, prefill = null, inputLabel = '' }) {
  const { dlg, title: h, body: p, field, label, input, ok } = parts();
  if (!dlg || pending) return Promise.resolve({ ok: false, value: '' });
  ...
  const asksForText = prefill !== null;
